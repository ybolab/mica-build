...
- Deployment target: embedded Linux images (signed file deployments, independent kernel/support and root components) for the boards, which live in `ybolab/mica-boards` and whose `board.env` and `evidence.json` are mirrored under `boards/`
