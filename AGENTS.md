...
- Deployment target: embedded Linux images (signed file deployments, independent kernel/support and root components) for the boards, which live in `ybolab/mica-boards`; a board exists here as its pinned bundle (`deps/packages/mica-kernel-<board>.json`), read out of `_out/boards/<board>/` after `make board-fetch` -- there is no committed copy of a board file
