#!/usr/bin/env bash
# Render the virt-arm64 board's configuration files for the mos-board-virt-arm64 package.
#
#   bash render.sh <src-root> <out-dir>
#
#   <src-root>/boards/virt-arm64/board.env
#   <src-root>/boards/virt-arm64/overlay/etc/systemd/system/boot.mount.in
...
#
# Runs INSIDE the producer's packing stage, from boards/virt-arm64/deb/board-virt-arm64/Dockerfile,
...
# on this board, and that template names two partitions boards/virt-arm64/board.env
# does not create. build.sh deletes it wherever RAUC_BOOTLOADER is not uboot, for the same
...
# /boot is a mountpoint on the UEFI boards and not on cx3576 because RAUC's grub backend
