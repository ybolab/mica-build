#!/usr/bin/env bash
# Stage the virt-arm64 BSP's kernel output into ${MOS_DEB_STAGE}.
#
# This is the producer's PREPARE hook, named in
# boards/virt-arm64/deb/kernel-virt-arm64/producer.env and run by the driver -- it is not an
# entry point and it builds nothing:
#
#   bash build-env/deb/build.sh --producer kernel-virt-arm64 --arch arm64
#
...
if [ "${MOS_DEB_PREFLIGHT:-0}" = 0 ]; then
    [ -n "${STAGE}" ] ||
        die "MOS_DEB_STAGE is unset. This is a PREPARE hook: build-env/deb/build.sh exports the directory to stage into and passes it to the build as the 'bin' context. Run the producer through the driver -- bash build-env/deb/build.sh --producer kernel-${MOS_BOARD} --arch arm64"
...
cp "${OUT}/Image" "${OUT}/config" "${OUT}/modules.tar" "${OUT}/kernel.release" "${STAGE}/"
echo "stage.sh: staged kernel ${release} from ${OUT} ($(stat -c%s "${OUT}/Image") bytes of Image, $(stat -c%s "${OUT}/modules.tar") bytes of modules)"
