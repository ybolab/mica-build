...
# stage. _out/boards/x64/ is a build product, so a fresh worktree has none.
#
...
# take the opportunity to give this hook an override it does not need. x64's escape is simpler and needs no variable:
# _out/boards/x64/kernel/ is a build product, so copying a built kernel into it
# is the same act.
set -euo pipefail
