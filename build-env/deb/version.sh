...
#
# tests/deb-package-gate.sh requires ONE version across the whole pool, and
...
WORKSPACE="${REPO_ROOT}/pkgs/mosd"
for p in "${REPO_ROOT}/Makefile" "${WORKSPACE}/Cargo.toml"; do
    [ -e "${p}" ] || {
        echo "error: ${p} does not exist. build-env/deb/version.sh derives the repository as two levels above itself; if this file moved, that arithmetic moved with it" >&2
        exit 1
    }
done
...
for m in "${WORKSPACE}"/*/Cargo.toml; do
    ...
    if [ -z "${VERSION}" ]; then
        ...
        echo "error: the mosd workspace declares two versions -- ${DECLARED_BY} is ${VERSION} and ${name} is ${v}. The package pool carries ONE version across every producer, so there is no answer to give here until those agree; tests/deb-package-gate.sh asserts the same fact over the built archives" >&2
        exit 1
    fi
done
