...
#
#   bash build-env/from.sh MOS_IMAGE_UBUNTU_2404=IMAGE_UBUNTU_2404
...
#   bash build-env/from.sh --arch=arm64 --contexts=/some/dir LOCAL_MOS_BUILD_C
...
# what says which. build-env/images.env holds the repository name and no tag;
...
# `docker run` -- including build/'s toolbox, which opens the containers
...
# there are eight call sites -- pkgs/podman/build.sh, pkgs/rauc/build.sh,
# rootfs/build.sh, tests/handshake-test/run.sh,
# pkgs/mosd/hack/build-target.sh and four boards/cx3576/bsp make recipes -- and the check
...
# eventually stop being it. build-env/build.sh calls this too, with --check.
...
# Path arithmetic, proved rather than assumed -- build-env/build.sh derives
...
# This one is called from boards/cx3576/bsp/Makefile, whose cwd is four directories
...
# The same expression build-env/build.sh applied to IMAGE_ keys before this
...
# build-env/build.sh calls this before it builds anything, for the reason its
...
# tests/quadlet-doc-test.sh and build-env/build.sh -- so a registry
