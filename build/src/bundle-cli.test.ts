...
// only fire when the repository-root meta/rauc/ has been deleted is a guard nobody has
// run.
...
describe('CERT/KEY/KEYRING are resolved and checked before anything runs', () => {
  test('POSITIVE CONTROL: with no environment, the three meta/rauc/ paths are the defaults', () => {
  ...
  test('EACH of the three, missing from the meta/rauc/ directory, says `make os-devkeys`', () => {
...
// reader of meta/rauc/ that knows which four files it writes -- so what is decided
...
describe('a missing meta/rauc/ makes the build generate one, unless the caller named its own', () => {
  ...
  test('all three supplied: explicit material BEATS the convention, so generate nothing', () => {
    ...
    // build that named an HSM's material -- and leave meta/GENERATED behind to
  ...
  test('EACH of the three left unset: the rest still come from meta/rauc/, so generate', () => {
  ...
  test('an EMPTY value counts as supplied, because resolveSigningMaterial counts it', () => {
    // `CERT= bash build/run.sh --bundle` reaches process.env as the empty
    ...
    // files come from meta/rauc/; the refusal that follows names the empty path, which
...
describe('the HOST\'s architecture, not the board\'s', () => {
  ...
  test('anything else is refused, naming what pkgs/rauc/ actually builds', () => {
    for (const m of ['riscv64', 'armv7l', 'ppc64le', '']) {
      expect(() => hostArchFor(m)).toThrow(/pkgs\/rauc\/ builds amd64 and arm64/)
    }
  })
...
describe('the rauc that writes the bundle is the one this tree built', () => {
  ...
  test('its absence names the build, not "rauc is missing"', () => {
    ...
    // pkgs/rauc/versions.env records why -- the distribution build links
