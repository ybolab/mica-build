...
import {
  BOOT_DIGEST_ARTEFACTS, bootDigestBase, bootDigestEnv, checkBootAttempts, verityCmdlineFields,
  verityEnvBase,
} from './boot-cx3576.ts'
...
//
//   * The mcopy order. `Image rk3576-src.dtb boot.scr mos-boot-digest-a.env
//     mos-boot-digest-b.env mos-verity-a.env mos-verity-b.env` is the order the
//     shell hands them over and therefore the order they land in the FAT
//     directory. It is a written-out list in both places, not a glob, so it is
//     transcribed as a list.
...
export async function buildBundle(
  ...
  try {
    ...
    if (geometry.bootloader === 'uboot') {
      ...
      // does: these are the bytes mcopy is about to write.
      //
      // BOTH slots' files, exactly as with the two verity envs above and for
      // the same reason: this one payload is installed into whichever boot slot
      // is inactive, and the script loads the name that matches the slot it is
