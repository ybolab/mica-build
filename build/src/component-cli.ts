...
const USAGE = `Usage: bash build/run.sh --components COMMAND [OPTIONS]
  ...
  kernel      --input BSP_KERNEL --init MOS_INIT --public-key BASE64 (repeatable)
              --board x64|virt-arm64|cx3576 --out DIR --content-key FILE --content-cert FILE
              --boot-key FILE --boot-cert FILE
  firmware    --board x64|virt-arm64|cx3576 --out DIR --metadata-key FILE --generation N --version VERSION
              UEFI: --boot-key FILE --boot-cert FILE; cx3576: --input ROCKCHIP_LOADER
  ...
  image       --records FILE --public-key BASE64 (repeatable) --firmware DIR
              --board x64|virt-arm64|cx3576 --out DIR
...
async function main() {
  const options: Record<string, { type: 'string' | 'boolean', multiple?: boolean }> = Object.fromEntries([
    'input', 'arch', 'version', 'out', 'content-key', 'content-cert', 'init', 'board', 'boot-key', 'boot-cert',
  ...
  const kernelBoard = () => {
    ...
    if (board !== 'x64' && board !== 'virt-arm64' && board !== 'cx3576') throw new Error('Unsupported kernel board')
    return board
  }
  ...
  switch (positionals[0]) {
    ...
    case 'root': {
      ...
      const signed = spawnSync('bash', [join(REPO_ROOT, 'pkgs/mos-boot/verity-tool.sh'), 'sign', join(work, 'rootfs.roothash'), material.key, material.certificate, join(work, 'rootfs.roothash.p7s')], { encoding: 'utf8', timeout: 120000 })
    ...
    case 'kernel': {
      ...
      try {
        await packKernel({ board: kernelBoard(), kernelDirectory: input, init: path('init'), publicKeys: keys(), systemPartUuid: board.partitions[1]!.guid, dataPartUuid: board.partitions[2]!.guid,
    ...
    case 'firmware': {
      ...
      packBootFirmware(board === 'cx3576' ? { ...metadata, board, input: path('input') } : { ...metadata, board, bootSigning: bootSigning() })
      break
    }
    case 'deployment': {
