export function factoryImageFilename(board: string, builtAt: Date): string {
  if (!['x64', 'virt-arm64', 'cx3576', 's905x5m'].includes(board)) throw new Error('Invalid image board')
...
export function isFactoryImageFilename(filename: string, board: string): boolean {
  const match = /^mos-(x64|virt-arm64|cx3576|s905x5m)-(\d{4})(\d{2})(\d{2})-(\d{2})(\d{2})(\d{2})\.img$/.exec(filename)
