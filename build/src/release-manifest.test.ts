...
/** The image's baked update configuration, in the shape meta.example commits. */
const BAKED_META_MANIFEST = `${JSON.stringify({
  schema: 'mos/meta/v1',
  update: { source: null, channel: 'stable', policy: 'check', checkIntervalMinutes: 1440 },
  trust: { signingKeys: [], signingKeyIds: [] },
}, null, 2)}\n`

/** meta/GENERATED as pkgs/rauc/gen-dev-keys.sh writes it, DOMAINS line included. */
...
function fixture(overrides: Partial<AssembleInputs> = {}): AssembleInputs {
  ...
  // The image's baked /usr/share/mos/meta/, as extracted. PRODUCTION-shaped by
  // default -- the required member present, no GENERATED beside it -- because
  // that is the released state; a case that wants the development branch
  // writes the marker itself.
  mkdirSync(join(dir, 'in', 'meta', 'updates'), { recursive: true })
  writeFileSync(join(dir, 'in', 'meta', 'updates', 'manifest.json'), BAKED_META_MANIFEST)
  return {
    board: 'cx3576',
    profile: 'dev',
    channel: 'development',
...
function validManifest(): Record<string, unknown> {
  ...
  return {
    ...
    build: { builderImages: {} },
    bootAssurance: 'I1',
    trust: { grade: 'production', developmentDomains: [] },
    artifacts,
  }
}
...
describe('the gate refuses each gap by name, proven RED against an assembled fixture', () => {
  function assembled(overrides: Partial<AssembleInputs> = {}): { dir: string, evidence: string, meta: string } {
    const inputs = fixture(overrides)
    const result = assembleRelease(inputs)
    return { dir: result.outDir, evidence: inputs.evidencePath, meta: inputs.bakedMetaDir }
  }

  test('POSITIVE CONTROL: the pristine directory passes, with its counts', () => {
    const { dir, evidence, meta } = assembled()
  ...
  test('a DELETED artifact is named with its role', () => {
    ...
    expect(() => gateReleaseDir(dir, evidence, meta))
  ...
  test('forbidden wording INJECTED into the evidence after assembly is refused at the gate', () => {
    ...
    expect(() => gateReleaseDir(dir, evidence, meta)).toThrow(/"secure boot" while claiming boot-assurance I1/)
  })
  ...
  test('a channel edited in the manifest is caught by the schema re-validation', () => {
    ...
    expect(() => gateReleaseDir(dir, evidence, meta)).toThrow(/not one of development\/candidate\/stable/)
  })
  ...
  test('RESEALED empty notes reach the emptiness guard', () => {
    ...
    reseal(dir)
    expect(() => gateReleaseDir(dir, evidence, meta)).toThrow(/release notes release-notes\.md .* are empty/)
  })
  ...
  test('a RESEALED component-less SBOM reaches the vacuity guard', () => {
    ...
    reseal(dir)
    expect(() => gateReleaseDir(dir, evidence, meta)).toThrow(/listing at least one component/)
  })
  ...
  test('a RESEALED non-JSON SBOM is refused as such', () => {
    ...
    reseal(dir)
    expect(() => gateReleaseDir(dir, evidence, meta)).toThrow(/sbom\.cdx\.json is not JSON/)
  })
  ...
  test('SHA256SUMS drifting from the manifest is the generator-is-broken refusal', () => {
    ...
    expect(() => gateReleaseDir(dir, evidence, meta)).toThrow(/does not agree with manifest\.json/)
  })
})

// The trust grade: measured from the image's own baked meta/, recorded in the
// manifest, and refused where "published" means "to a customer" (PLAN-077 §4).

describe('readBakedTrust measures the grade, and refuses to guess it', () => {
  function metaDir(shape: { marker?: string, manifest?: string } = {}): string {
    caseN += 1
    const dir = join(WORK, `meta-${caseN}`)
    mkdirSync(join(dir, 'updates'), { recursive: true })
    writeFileSync(join(dir, 'updates', 'manifest.json'), shape.manifest ?? BAKED_META_MANIFEST)
    if (shape.marker !== undefined) writeFileSync(join(dir, 'GENERATED'), shape.marker)
    return dir
  }

  test('the required member present and no marker beside it is production', () => {
    const trust = readBakedTrust(metaDir())
    expect(trust.grade).toBe('production')
    expect(trust.developmentDomains).toEqual([])
    expect(trust.updateSource).toBeNull()
    expect(trust.signingKeyCount).toBe(0)
  })

  test('a marker is development, and its DOMAINS line says which half', () => {
    const trust = readBakedTrust(metaDir({
      marker: 'DEVELOPMENT-GRADE\n\nDOMAINS=rauc updates\n',
    }))
    expect(trust.grade).toBe('development')
    expect(trust.developmentDomains).toEqual(['rauc', 'updates'])
  })

  test('THE VACUITY GUARD: a directory without the required member THROWS, never "production"', () => {
    // The failure this whole function exists to prevent. Reading "no
    // GENERATED here" out of a directory nobody populated grades every image
    // ever built as production, on any host that never extracted one.
    caseN += 1
    const empty = join(WORK, `meta-empty-${caseN}`)
    mkdirSync(empty, { recursive: true })
    expect(() => readBakedTrust(empty)).toThrow(/is not the \/usr\/share\/mos\/meta\/ of a mos image/)
    expect(() => readBakedTrust(join(WORK, 'absent-meta-dir'))).toThrow(/does not exist/)
  })

  test('a baked manifest that is not JSON, or names an unreadable source, is refused', () => {
    expect(() => readBakedTrust(metaDir({ manifest: 'not json' }))).toThrow(/is not JSON/)
    expect(() => readBakedTrust(metaDir({
      manifest: JSON.stringify({ update: { source: 7 }, trust: { signingKeys: [] } }),
    }))).toThrow(/neither a string nor null/)
  })

  test('a baked manifest carrying no signingKeys array is refused, not read as zero keys', () => {
    expect(() => readBakedTrust(metaDir({
      manifest: JSON.stringify({ update: { source: null }, trust: {} }),
    }))).toThrow(/not an array/)
  })

  test('markerDomains takes the LAST DOMAINS line, and a marker naming none is still a marker', () => {
    expect(markerDomains('DOMAINS=rauc\nDOMAINS=rauc updates\n')).toEqual(['rauc', 'updates'])
    expect(markerDomains('no domains at all\n')).toEqual([])
    expect(markerDomains('DOMAINS=\n')).toEqual([])
  })
})
...
describe('the publication gate refuses a development-grade image on a customer channel', () => {
  ...
  { dir: string, evidence: string, meta: string } {
    const inputs = fixture(overrides)
    if (mutateMeta !== undefined) mutateMeta(inputs.bakedMetaDir)
    const result = assembleRelease(inputs)
    return { dir: result.outDir, evidence: inputs.evidencePath, meta: inputs.bakedMetaDir }
  }

  test('development-grade on EVERY customer channel is refused, naming the file and the domains', () => {
    // Assembly runs the gate over what it wrote, so this is where an operator
    // meets the refusal: the directory is never finished. Both customer
    // channels, from the exported list, so adding one cannot quietly escape.
    for (const channel of CUSTOMER_CHANNELS) {
      const inputs = fixture({ channel })
      writeFileSync(join(inputs.bakedMetaDir, 'GENERATED'), GENERATED_MARKER)
      expect(() => assembleRelease(inputs)).toThrow(new RegExp(`on the '${channel}' channel`))
      expect(() => assembleRelease(inputs)).toThrow(/carries \/usr\/share\/mos\/meta\/GENERATED/)
      expect(() => assembleRelease(inputs)).toThrow(/the rauc domain\(s\)/)
    }
  })

  test('the GATE refuses a finished directory whose manifest names a customer channel', () => {
    // The archive-restore path, and the reason the fact travels in the image
    // rather than on the build host: a directory assembled on `development`
    // and relabelled `stable` afterwards is re-measured against the image it
    // was built from and refused again.
    const { dir, evidence, meta } = assembledWith({ channel: 'development' },
      m => writeFileSync(join(m, 'GENERATED'), GENERATED_MARKER))
    const path = join(dir, 'manifest.json')
    const manifest = JSON.parse(readFileSync(path, 'utf8')) as { release: { channel: string } }
    manifest.release.channel = 'stable'
    writeFileSync(path, `${JSON.stringify(manifest, null, 2)}\n`)
    reseal(dir)
    expect(() => gateReleaseDir(dir, evidence, meta)).toThrow(/on the 'stable' channel/)
  })

  test('development-grade on DEVELOPMENT passes, and the manifest RECORDS the grade', () => {
    // The channel that carries no promise keeps the release path runnable --
    // and the directory still says what it is, rather than being
    // distinguishable only by having been let through.
    const { dir, evidence, meta } = assembledWith({ channel: 'development' },
      m => writeFileSync(join(m, 'GENERATED'), GENERATED_MARKER))
    const report = gateReleaseDir(dir, evidence, meta)
    expect(report.manifest.trust.grade).toBe('development')
    expect(report.manifest.trust.developmentDomains).toEqual(['rauc'])
    expect(report.trust.grade).toBe('development')
  })
  ...
  test('a manifest whose trust block disagrees with the image is refused as two builds', () => {
    const { dir, evidence, meta } = assembledWith({ channel: 'development' })
    writeFileSync(join(meta, 'GENERATED'), GENERATED_MARKER)
    expect(() => gateReleaseDir(dir, evidence, meta))
  ...
  test('assembly refuses before it writes anything when the meta directory is not one', () => {
    const inputs = fixture()
    rmSync(join(inputs.bakedMetaDir, 'updates', 'manifest.json'))
    expect(() => assembleRelease(inputs)).toThrow(/is not the \/usr\/share\/mos\/meta\/ of a mos image/)
    expect(statSync(join(inputs.outDir, 'manifest.json'), { throwIfNoEntry: false })).toBeUndefined()
  })
})
