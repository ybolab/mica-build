...
// out of the ENVIRONMENT. The cx3576 assembly contract's comment: "without it mkimage
...
export interface BootScriptSpec {
  ...
  /** `-n`. The cx3576 assembly contract passes "mos boot"; it lands in the legacy header. */
...
export function bootScriptArgs(spec: BootScriptSpec): string[] {
  // -T script -C none: the cx3576 assembly contract's shape, and no -A. A boot script is
