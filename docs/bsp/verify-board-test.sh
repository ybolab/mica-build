#!/usr/bin/env bash
# Negative tests for docs/bsp/verify-board.sh: each assertion is driven
# against a fixture in which its fact is false, and is required to fail with
# ITS OWN message.
#
#   bash docs/bsp/verify-board-test.sh          (or: make docs-verify-test)
#
# HOW. The REAL docs/bsp/verify-board.sh is run, once per case, over a COPY
...
# from ${BASH_SOURCE}, so a copy at ${case}/docs/bsp/verify-board.sh reads
# ${case}/docs/bsp, and unlike the link and status gates this one's inputs
# live entirely inside docs/bsp/, so the SHIPPED template and dossier are the
...
TEMPLATE="${HERE}/board-template.md"
DOSSIER="${HERE}/cx3576-example.md"
...
new_fixture() {
    local dir="$1"
    rm -rf "${dir}"
    mkdir -p "${dir}/docs/bsp"
    cp "${VERIFIER}" "${TEMPLATE}" "${DOSSIER}" "${dir}/docs/bsp/"
}
...
run_verifier() {
    RC=0
    bash "$1/docs/bsp/verify-board.sh" >"${WORK}/out" 2>&1 || RC=$?
}
...
expect_all_pass() {
    ...
    got_checks="$(sed -n 's|^docs/bsp/verify-board.sh: \([0-9]*\)/\1 PASS$|\1|p' "${WORK}/out")"
...
new_fixture "${FIX}"
replace_line "${FIX}/docs/bsp/cx3576-example.md" '## Console' ''
expect_fail "a dossier without the Console section" 1 \
    "cx3576-example.md is missing the required heading '## Console'"

# --- 2. an H2 the template does not name -------------------------------------
FIX="${WORK}/extra-heading"
new_fixture "${FIX}"
printf '\n## Bench notes\n\nScratch.\n' >>"${FIX}/docs/bsp/cx3576-example.md"
...
new_fixture "${FIX}"
replace_line "${FIX}/docs/bsp/cx3576-example.md" \
...
new_fixture "${FIX}"
sed -i '/^[0-9][0-9]*\. `## /d' "${FIX}/docs/bsp/board-template.md"
...
FIX="${WORK}/no-dossier"
new_fixture "${FIX}"
rm "${FIX}/docs/bsp/cx3576-example.md"
