#!/usr/bin/env bash
# Asserts that every board dossier under docs/bsp/ carries the section list
...
#
#   bash docs/bsp/verify-board.sh          (or: make docs-verify)
#
...
# ("Board dossier: <board>" as the H1 -- cx3576-example.md is the filled
# instance), so every docs/bsp/*.md whose H1 starts `# Board dossier:` is
...
if [ "${#dossiers[@]}" -eq 0 ]; then
    fail "no dossier instance found (no docs/bsp/*.md with an H1 starting '# Board dossier:'); nothing would be asserted"
else
    ok
fi

echo "docs/bsp/verify-board.sh: ${#dossiers[@]} dossier(s) against $TEMPLATE (${#required[@]} required headings)"
...
# is the verdict.
if [ "$FAIL" -ne 0 ]; then
    echo "docs/bsp/verify-board.sh: $FAIL FAILED, $CHECKS passed" >&2
    exit 1
fi
...
echo "docs/bsp/verify-board.sh: $CHECKS/$CHECKS PASS"
