...
> access.md 为准。
>
> **本文有三处已被英文版明确更正或取代，切勿依赖：**
> 1. 头部与 §2 声称 shell/SSH *“在生产镜像中彻底不存在”*。已推翻——prod 镜像
>    **携带** sshd（默认关闭，管理员可启用），编译期缺失不再是 prod 的防线；
>    完全无 shell 的 `sealed` 构建**未实现**（英文版 §5.3 [partial]）。
> 2. §2/§4 的“阶段一：一机一密默认密码”模型已被 `sshweb` 战役（2026-08-19）
>    整体取代：SSH 默认关闭、root 默认**无密码**、持久访问凭 SSH 公钥、
>    操作员设置的 root 密码为**瞬态**（下次启动即消失）。英文版 §4.2 将旧
>    模型记为 superseded。
> 3. 本文描述的 COSI/machined/webd/Go-sshd 架构属 Plan A 时代；PLAN-010 已
>    迁移到 systemd + mosd + OpenSSH，英文版通篇按新架构重写并带状态标记。
>
