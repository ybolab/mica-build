...
(`os/pkgs/mosd/apid/src/routes.rs:722`), whose body runs from
`const STYLE: &str = "\` (`os/pkgs/mosd/apid/src/routes.rs:723`) to
`os/pkgs/mosd/apid/src/routes.rs:730`, injected into the page head as
`style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:741`). The crate
...
`pub fn app(state: AppState) -> Router {` (`os/pkgs/mosd/apid/src/routes.rs:116`) and
runs to `.with_state(state)` (`os/pkgs/mosd/apid/src/routes.rs:191`). Its declaration
...
| `GET /healthz` | `:172` | `healthz` (`:715`) | neither — plain text | Returns the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:719`) | none |
...
| every method on `/api/` | `:184` (`.route("/api/", any(api_not_found))`) | `api_not_found` (`:196`) | JSON | 404 in §2.4's envelope. Not redundant with the nest: *"`nest` claims `/api`, `/api/x` and `/api/x/y`, and not `/api/`; the difference is a request that begins `/api/` reaching the asset router"* (`os/pkgs/mosd/apid/src/routes.rs:181-184`) | none |
...
**The `/api` subtree.** `api_router()` (`os/pkgs/mosd/apid/src/routes.rs:264-271`) is
nested under the prefix `const API: &str = "/api";`
(`os/pkgs/mosd/apid/src/routes.rs:215`). Each route is declared from the same constant
...
one string and cannot disagree"* (`os/pkgs/mosd/apid/src/routes.rs:261-263`).
...
(`os/pkgs/mosd/apid/src/routes.rs:227`) while OpenAPI's is
`const V1_SETTINGS_DOC: &str = "/v1/settings/{path}";`
(`os/pkgs/mosd/apid/src/routes.rs:228`), and the source states why: *"axum names a
...
(`os/pkgs/mosd/apid/src/routes.rs:221-224`). Every response under the prefix is built
by one helper that sets `no-store` on success as well as on failure
(`os/pkgs/mosd/apid/src/routes.rs:304-311`).

**Kinds, counted, at this commit.** The outer router declares **twenty-one**
`.route()` calls (`os/pkgs/mosd/apid/src/routes.rs:123`, `:148-154`, `:158-160`,
...
neither and returns a bare string (`os/pkgs/mosd/apid/src/routes.rs:718-720`), and
fourteen are POSTs.
...
(`os/pkgs/mosd/apid/src/routes.rs:308`), as does `api_not_found`. **No route accepts a
...
example `Form(form): Form<SetupForm>,` (`os/pkgs/mosd/apid/src/routes.rs:959`),
...
(`os/pkgs/mosd/apid/src/routes.rs:158-160`) — or for any of the four SSH mutations
(`os/pkgs/mosd/apid/src/routes.rs:164-166`), and the rule has since been extended to
the bundle deactivation control (`os/pkgs/mosd/apid/src/routes.rs:145-147`). And there
...
on power (`os/pkgs/mosd/apid/src/routes.rs:1676`) and on the transient password
(`os/pkgs/mosd/apid/src/routes.rs:2529`).

**The second router.** `redirect_app()` at `os/pkgs/mosd/apid/src/routes.rs:615-619` is
the router served on the **HTTP** listener. It declares no routes at all — only
a fallback, `.fallback(redirect_to_https)` (`os/pkgs/mosd/apid/src/routes.rs:617`) —
...
(`os/pkgs/mosd/apid/src/routes.rs:621-635`). It carries no state beyond that port
...
(`os/pkgs/mosd/apid/src/routes.rs:655`), built at `os/pkgs/mosd/apid/src/routes.rs:649-659`.
...
(`os/pkgs/mosd/apid/src/routes.rs:641-646`), typed as
...
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:1149`), set
at `os/pkgs/mosd/apid/src/routes.rs:1150-1154`. `POST /setup` does the same at the end
of the first-run wizard without a login step
(`os/pkgs/mosd/apid/src/routes.rs:1058-1063`). The store generates 16 random bytes from
...
through the `ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:589-611`), which
tests `session::cookie_from_headers(&parts.headers)`
(`os/pkgs/mosd/apid/src/routes.rs:598`) and nothing else.

**The gate.** One middleware, layered over the whole HTTPS router with
`.layer(middleware::from_fn_with_state(state.clone(), gate))`
(`os/pkgs/mosd/apid/src/routes.rs:190`), running from
`async fn gate(State(state): State<AppState>, request: Request, next: Next) -> Response {`
(`os/pkgs/mosd/apid/src/routes.rs:671`) to `os/pkgs/mosd/apid/src/routes.rs:716`. It implements
...
(`os/pkgs/mosd/apid/src/routes.rs:694-698`), so an authenticated request reaches its
handler without touching mosd, and the source states the reason and its
soundness argument at `os/pkgs/mosd/apid/src/routes.rs:677-693`: *"an authenticated
...
`state.api.get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:700`) is now paid
...
the SSH pane reads the whole key list (`os/pkgs/mosd/apid/src/routes.rs:1881-1883`),
edits it in memory (`os/pkgs/mosd/apid/src/routes.rs:2562-2566`), and writes the whole
list back through `write_key_list` (`os/pkgs/mosd/apid/src/routes.rs:1886-1899`).
...
(`os/pkgs/mosd/apid/src/routes.rs:509`) — which is the one place where
...
2. **A file-serving service is constructed, as the router's fallback.**
   ...
   now ends in `.fallback(serve::fallback)` (`os/pkgs/mosd/apid/src/routes.rs:189`),
...
3. **Nothing is embedded in the binary, and that is still deliberate.**
   ...
   `os/pkgs/mosd/apid/src/tests.rs:1757` reads the committed `openapi.json` so a test
...
4. **The crate still ships no asset of its own; the assets come from
   ...
   `bundles: Arc::new(Store::at_default()),` (`os/pkgs/mosd/apid/src/routes.rs:71`).
...
(`os/pkgs/mosd/apid/src/routes.rs:741`), from a constant described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:722`). There is still no favicon route, no font and
no image declared in `os/pkgs/mosd/apid/src/routes.rs:116-191`. A request for
`/favicon.ico` no longer ends at the gate, though: it is unauthenticated, so
the gate redirects it to `/login` (`os/pkgs/mosd/apid/src/routes.rs:715`), and
...
`std::fs::read_to_string("/proc/uptime")` (`os/pkgs/mosd/apid/src/routes.rs:1225`) and
...
control (`os/pkgs/mosd/apid/src/routes.rs:1347`) and the install path. The consequence
...
1. **Line numbers throughout its section 2.1 no longer resolve to the routes
   ...
   `f7cb5ba` the route is `os/pkgs/mosd/apid/src/routes.rs:123` and the handler is
   `serve::root`, at `os/pkgs/mosd/apid/src/assets/serve.rs:57`, with the built-in branch
   at `os/pkgs/mosd/apid/src/routes.rs:1263`.
2. **Its route table is missing thirteen routes.** The five SSH routes it
   ...
   `os/pkgs/mosd/apid/src/routes.rs:163-170`, and eight more have landed since: the
   four `/containers` and `/mqtt` pairs (`os/pkgs/mosd/apid/src/routes.rs:171-174`) and
   the four `/api` routes (`os/pkgs/mosd/apid/src/routes.rs:266-269`). Its own section
...
3. **"Six methods and one signal"** (`docs/research/mos-ui-inventory.md:367`) is
   ...
   `os/pkgs/mosd/apid/src/routes.rs:2539`.
...
renders **eight** links (`os/pkgs/mosd/apid/src/routes.rs:746-758`) plus the logout form
(`os/pkgs/mosd/apid/src/routes.rs:759-761`): Status, Network, Hostname, Power, SSH,
...
(`os/pkgs/mosd/apid/src/routes.rs:215`), nested as `.nest(API, api_router())`
(`os/pkgs/mosd/apid/src/routes.rs:186`), and the version is the segment immediately
after it — `const V1_META_PATH: &str = "/v1/meta";`
(`os/pkgs/mosd/apid/src/routes.rs:217`). `GET /api/versions` answers unauthenticated
from `const SERVED_VERSIONS: [&str; 1] = ["v1"];`
(`os/pkgs/mosd/apid/src/routes.rs:253`) and `const CURRENT_VERSION: &str = "v1";`
(`os/pkgs/mosd/apid/src/routes.rs:256`), and `GET /api/v1/meta` answers authenticated
with `ApiMeta` whose `settingsSchemaVersion` is read live from
`settings_schema_version: mosd_settings::SCHEMA_VERSION,`
(`os/pkgs/mosd/apid/src/routes.rs:438`) rather than copied. The served set is an array
...
(`os/pkgs/mosd/apid/src/routes.rs:187`) claims the one spelling the nest does not, and
...
(`os/pkgs/mosd/apid/src/routes.rs:178-180`).
...
1. The gate already dispatches on `let path = request.uri().path();`
   (`os/pkgs/mosd/apid/src/routes.rs:672`). A version check is the same string operation
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:190`). This is now measurable rather than
   predicted: the shipped gate tests membership with `is_declared_api_route`
   (`os/pkgs/mosd/apid/src/routes.rs:280-285`), a pure string operation over the same
   constants the router declares.
2. axum registers routes by path (`os/pkgs/mosd/apid/src/routes.rs:116-191`). A second
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:264-271`) is the first of those routers and shows
   the shape.
...
(`os/pkgs/mosd/apid/src/routes.rs:631`) — that is not the objection. The
...
Both rows ship. The first is declared at `os/pkgs/mosd/apid/src/routes.rs:266` and
...
exactly one source"* (`os/pkgs/mosd/apid/src/routes.rs:420-422`). The API version is the
...
(`os/pkgs/mosd/apid/src/routes.rs:704-709`). An authenticated probe cannot answer the
...
so"* (`os/pkgs/mosd/apid/src/routes.rs:395-398`); the gate hands the route off above its own
bus call at `os/pkgs/mosd/apid/src/routes.rs:673`. The cost is an unauthenticated
...
(`os/pkgs/mosd/apid/src/routes.rs:673`, `:715-717`), so this is one more bit on a
...
(`os/pkgs/mosd/apid/src/routes.rs:227`) declared at `os/pkgs/mosd/apid/src/routes.rs:268` and
handled by `api_v1_settings` (`:474`), whose whole body is
`resource_response(state.api.get_settings(&path).await, &path)`
(`os/pkgs/mosd/apid/src/routes.rs:482`) — the dot-path passthrough this section asks
for, with no second model beside it. The live-state root is the same shape,
`const V1_STATE_ROUTE: &str = "/v1/state/{*path}";`
(`os/pkgs/mosd/apid/src/routes.rs:230`) at `:266` and `:501`, ending in
`resource_response(state.api.get_state(&path).await, &path)`
(`os/pkgs/mosd/apid/src/routes.rs:509`). Both answer `ResourceValue`
(`os/pkgs/mosd/apid/src/routes.rs:456`), which carries a
`serde(transparent)` attribute (`os/pkgs/mosd/apid/src/routes.rs:455`) so the body is
...
(`os/pkgs/mosd/apid/src/routes.rs:451-453`). And `GET /api/v1/state/{path}` today
...
- **No array indexing.** The dot-path syntax has none; the model says a list is
  *"Written as a whole JSON array through the dot-path API"*
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:2562-2567`). Two clients doing that concurrently
...
- **A dot in a value collides with a dot in the path.** Verified at `86cd669`:
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:802-807` permits `.`, `_` and `-`), but `split_path`
...
nobody asked to delete"* (`os/pkgs/mosd/apid/src/routes.rs:2578-2582`). A `DELETE` whose
...
3. **Uptime is not in either tree.** the status pane reads `/proc/uptime` in apid
   (`os/pkgs/mosd/apid/src/routes.rs:1225`) — one of the three disk paths apid touches
...
prefix beyond a fallback (`os/pkgs/mosd/apid/src/routes.rs:264-271`). There is no
...
(`os/pkgs/mosd/apid/src/routes.rs:158-160`) or for any of the four SSH mutations
(`os/pkgs/mosd/apid/src/routes.rs:164-166`), *"so a browser prefetch, a crawler or a mis-clicked link cannot power
the appliance off"*.

The **confirmation token** that guards the three form posts today
(`os/pkgs/mosd/apid/src/routes.rs:1676`, and `TRANSIENT_CONFIRM_TOKEN` at `os/pkgs/mosd/apid/src/routes.rs:1768`) does
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:1696-1709`). 202
...
- **`POST /login` and `POST /logout`.** A session is a browser mechanism (§3.1).
  ...
  mislead.
- **`GET /healthz`** — declared at `os/pkgs/mosd/apid/src/routes.rs:175` and answering
  at `os/pkgs/mosd/apid/src/routes.rs:718-720`. It keeps its path, its unauthenticated exemption (`os/pkgs/mosd/apid/src/routes.rs:673`) and
  its literal `ok` body,
  ...
  **But it answers a narrower question than its name suggests, and the API must
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:673`), so it answers `ok` on an appliance whose
...
- **A password change.** `access.webAdmin.password_hash` is written by exactly
  ...
  `os/pkgs/mosd/apid/src/routes.rs:1027`, and
...
(`os/pkgs/mosd/apid/src/routes.rs:2599-2605`, via `ssh_error` at `os/pkgs/mosd/apid/src/routes.rs:1902-1911`). For a
...
(`os/pkgs/mosd/apid/src/routes.rs:314-316`, payload at `:316-334`), the schema records
...
(`os/pkgs/mosd/apid/src/routes.rs:249`), attached only when the status is 503
(`os/pkgs/mosd/apid/src/routes.rs:561-566`). The recommendation this section makes —
...
(`os/pkgs/mosd/apid/src/routes.rs:532-566`), which matches on the concrete
`zbus::Error::MethodError` before the conversion flattens it and maps the three
fdo names declared at `os/pkgs/mosd/apid/src/routes.rs:244-246`. The source states the
rule in the same words this section chose: *"The classification is translated
and the message is not"* (`os/pkgs/mosd/apid/src/routes.rs:523`).
...
(`os/pkgs/mosd/apid/src/routes.rs:341`), and `ApiError::mosd`, which passes
`Self::new(code, message, "mosd")` (`os/pkgs/mosd/apid/src/routes.rs:346`). The published contract does not say so, so a client generated from the document gets an unconstrained string and cannot exhaustively match on it |
| `path` | OPTIONAL, present only when the failure names a dot-path | `"type": [ "string", "null" ]` (`os/pkgs/mosd/apid/openapi.json:241-245`), **absent from `required`** | **a real difference, in the opposite direction.** The schema admits an explicit `null`; the wire never carries one, because the field carries a `serde(skip_serializing_if = "Option::is_none")` attribute (`os/pkgs/mosd/apid/src/routes.rs:334`) and is therefore omitted rather than nulled — which is the behaviour this section asks for and the source's own doc comment defends (`os/pkgs/mosd/apid/src/routes.rs:329-333`). The schema is wider than the implementation, so a client that handles `null` is handling a case the device does not produce |
...
| `code` | Ships? | Where |
|---|---|---|
| `not_authenticated` | **yes**, with a different credential | `os/pkgs/mosd/apid/src/routes.rs:606`. It is raised by the session-cookie extractor (`os/pkgs/mosd/apid/src/routes.rs:603-609`), not by a bearer token: §3.2's token does not exist, so the shipped 401 means *"no session cookie, or one that does not verify"* (`os/pkgs/mosd/apid/src/routes.rs:607`) |
| `not_found` | **yes** | `os/pkgs/mosd/apid/src/routes.rs:202`, from the reserved subtree's fallback |
...
| `settings_rejected` | **yes**, 422 | `os/pkgs/mosd/apid/src/routes.rs:542-545`, on fdo `InvalidArgs` (`os/pkgs/mosd/apid/src/routes.rs:244`) |
| `settings_io` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:546-549`, on fdo `IOError` (`os/pkgs/mosd/apid/src/routes.rs:245`) |
| `mosd_failed` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:550-553`, on fdo `Failed` (`os/pkgs/mosd/apid/src/routes.rs:246`) |
| `mosd_unreachable` | **yes**, 503 with `Retry-After` | `os/pkgs/mosd/apid/src/routes.rs:572-578`, and it is exhaustive over everything the three above do not name (`os/pkgs/mosd/apid/src/routes.rs:568-570`) |
...
(`os/pkgs/mosd/apid/src/routes.rs:649-659`, message at `os/pkgs/mosd/apid/src/routes.rs:655`). So a settings value mosd
...
(`os/pkgs/mosd/apid/src/routes.rs:1534-1540`), and `write_key_list` does the same
(`os/pkgs/mosd/apid/src/routes.rs:1895-1897`). The VLAN example above is exactly this — a rejection that reads
as an outage.
...
text in an error box (`os/pkgs/mosd/apid/src/routes.rs:782-784`, `:1223`, `:1243`) — without
widening it.
...
2. **Bus unavailable.** `mosd_unreachable`, **503**, with `Retry-After: 5`. 503
   ...
   `mosd_unreachable` answers 503 (`os/pkgs/mosd/apid/src/routes.rs:573-577`) with
   `Retry-After` attached (`os/pkgs/mosd/apid/src/routes.rs:561-566`). The cost is
   named and is now real rather than predicted: the HTML path still returns 502
   for the same underlying failure (`os/pkgs/mosd/apid/src/routes.rs:649-659`), so until §8 changes both, one appliance
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   `/healthz` returns before any check (`os/pkgs/mosd/apid/src/routes.rs:673`) and
   answers the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:719`), so **an appliance whose mosd is dead
   ...
   off above the bus call (`os/pkgs/mosd/apid/src/routes.rs:673`), and a request with a
   valid session cookie is handed off above it too
   (`os/pkgs/mosd/apid/src/routes.rs:694-698`), so no API route depends on the gate's
...
middleware, `pub(crate) struct ApiSession;` (`os/pkgs/mosd/apid/src/routes.rs:589`),
whose whole test is
`session::cookie_from_headers(&parts.headers)` verified against the store
(`os/pkgs/mosd/apid/src/routes.rs:598-599`); its rejection is §2.4's envelope with a
401 rather than the gate's HTML redirect
(`os/pkgs/mosd/apid/src/routes.rs:603-609`). The gate hands the declared `/api` routes
off to it with `is_declared_api_route(path)`
(`os/pkgs/mosd/apid/src/routes.rs:673`, predicate at `:277-282`). There is no bearer
...
exchange as success"* (`os/pkgs/mosd/apid/src/routes.rs:586-588`). **Unanswered:**
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   `Redirect::to("/login").into_response()` (`os/pkgs/mosd/apid/src/routes.rs:715`), a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:710-712`) and which returns **200 OK** with an HTML
   form (`os/pkgs/mosd/apid/src/routes.rs:1081-1098`). A script that checks the status
   ...
   instead (`os/pkgs/mosd/apid/src/routes.rs:603-609`). Every other path on the daemon
   still redirects.
2. **Obtaining the credential means emulating three browser behaviours.**
   `POST /login` takes `Form(form): Form<LoginForm>,`
   (`os/pkgs/mosd/apid/src/routes.rs:1103`) — axum's URL-encoded extractor, imported at
   `os/pkgs/mosd/apid/src/routes.rs:19`, not JSON — answers **302 to `/`**
   (`os/pkgs/mosd/apid/src/routes.rs:1152`), and delivers the credential in a
   `Set-Cookie` header (`os/pkgs/mosd/apid/src/routes.rs:1151`, value built at
...
6. **There is exactly one credential, and it identifies a human.** The only
   ...
   the admin hash, read through `password_hash` (`os/pkgs/mosd/apid/src/routes.rs:641-646`); there is
   no second credential, no user table, and no reference to `access.device` in
...
7. **A misconfigured script locks the operator out, repeatedly.** The login
   ...
   *"the appliance has one admin password, so per-client tracking buys nothing
   ...
   `os/pkgs/mosd/apid/src/routes.rs:1116-1126`). A script retrying with a
...
`ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:589-611`) against the same
...
4. **It costs zero extra bus round trips.** The gate already calls
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:707`) and `apiTokens` is a child of `access`, so
...
- Mint and revoke are read-modify-write of the whole array, because the dot-path
  ...
  the same pattern the SSH key pane uses (`os/pkgs/mosd/apid/src/routes.rs:2562-2567`).
  Two concurrent mints lose one token, silently.
- Identity is the `id`, never a list position, for the reason recorded at
  `os/pkgs/mosd/apid/src/routes.rs:2591-2595`: an index is meaningful only against the
...
- **Coexisting means two credentials at one privilege level** — and there is
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:194`). A token can do everything the operator can do
...
2. **§3.1 item 7 changes meaning.** The `auth.rs:53-54` comment's reasoning —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1123-1132`). Bearer verification is **not** rate
...
(`os/pkgs/mosd/apid/src/routes.rs:603-609`) and a redirect to `/login` everywhere else
(`os/pkgs/mosd/apid/src/routes.rs:715`) — and the CORS argument below, which depends on
...
2. **A stolen token is full management access, indefinitely.** There is no
   ...
   `os/pkgs/mosd/apid/src/routes.rs:1764`) and is then no longer needed. Detection is not
...
5. **A same-origin page mints a permanent token with the operator's cookie.**
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:1764`). But a minted token survives deactivating
...
pane is tested to carry, `os/pkgs/mosd/apid/src/routes.rs:1764`); apid runs as root
...
- **Under this proposal the API path has no CSRF exposure**, because a bearer
  ...
  **None of that is true of the shipped API**, whose credential *is* the cookie
  (`os/pkgs/mosd/apid/src/routes.rs:598-599`), so today the four `/api` routes sit
...
- **The per-action confirm tokens are not CSRF tokens and must not be counted as
  ...
  and `"poweroff"` (`os/pkgs/mosd/apid/src/routes.rs:1596-1600`) and
  `TRANSIENT_CONFIRM_TOKEN` is the literal `"set-transient-password"`
  (`os/pkgs/mosd/apid/src/routes.rs:1768`). They are not secret, not per-session and not
  ...
  the checkbox has to be ticked as well"* (`os/pkgs/mosd/apid/src/routes.rs:1594-1595`). They stop a mis-click and a prefetch. Against a
...
(`os/pkgs/mosd/apid/src/routes.rs:694-700`), so an authenticated flood no longer
...
(`os/pkgs/mosd/apid/src/routes.rs:673`) and its handler makes no mosd call
(`os/pkgs/mosd/apid/src/routes.rs:408-416`), so it is an unauthenticated route that
...
`/proc/uptime` (`os/pkgs/mosd/apid/src/routes.rs:1225`) and its own state directory
...
declaration at `os/pkgs/mosd/apid/src/routes.rs:159`, which is why that line is not
...
`0d4f3c6` the fifteen declarations are at `os/pkgs/mosd/apid/src/routes.rs:129-148`,
...
(`os/pkgs/mosd/apid/src/routes.rs:155-179`). A custom
...
`os/pkgs/mosd/apid/src/routes.rs:48` and `:578`) moves under section 6.3's reserved
...
(`os/pkgs/mosd/apid/src/routes.rs:179`). The immutable class is read per request from
...
- **`/var/lib/mos/settings.toml`** (`os/pkgs/mosd/mosd-settings/src/store.rs:67`) — the
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:645-649`) and every authorized SSH key.
...
§6.3's reserved prefix, `builtin_home` in `os/pkgs/mosd/apid/src/routes.rs:805`. Its
...
(`os/pkgs/mosd/apid/src/routes.rs:744-749`), described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:722`). Section 1.6 evidences the rest: no
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:649`), reached when a mosd call
  ...
  *before* dispatch — so at the moment that page is on screen, `/builtin/` is
  ...
  page (`os/pkgs/mosd/apid/src/routes.rs:633`), the navigation on every built-in pane
...
page *"The management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:655`,
...
1. **A signature and the upload credential authorise the same blast radius.** A
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:1764`). §3.2 states there are **no scopes in
...
(`os/pkgs/mosd/apid/src/routes.rs:32`, `:727-734`, `:745`; §1.1 constraint 1 and §1.6
...
Items (iv) and (v) are already shipped — `os/pkgs/mosd/apid/src/routes.rs:1572`, `:1728`,
...
the settings write fails (`os/pkgs/mosd/apid/src/routes.rs:1541-1547`), and `write_key_list`
...
(`os/pkgs/mosd/apid/src/routes.rs:534-535`) and mapping `FDO_INVALID_ARGS`,
...
(`os/pkgs/mosd/apid/src/routes.rs:707`) the whole appliance answers the 502 page *"The
management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:653-663`). **The
...
path answers 422 (`os/pkgs/mosd/apid/src/routes.rs:2599-2605`). The HTML path is not
changed.
...
setup mode (`os/pkgs/mosd/apid/src/routes.rs:711-716`) — and it returns a token (§2.3),
...
HTML path answers **502** (`os/pkgs/mosd/apid/src/routes.rs:653-663`), so one appliance
...
- **The VLAN dot-path limit.** `valid_iface_name` permits `.`
  (`os/pkgs/mosd/apid/src/routes.rs:809-814`) while `split_path` splits on it
...
- **Uptime.** §2.2 chose that mosd should publish it into the live-state tree
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:1225`). Adding it later is additive and cheap;
...
`os/pkgs/mosd/apid/src/routes.rs:194`), so a token can do everything the operator can do
...
(`os/pkgs/mosd/apid/src/routes.rs:707`), against the single lock mosd holds over both
