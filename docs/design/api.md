...
(`os/pkgs/mosd/apid/src/routes.rs:33`). The HTTP stack is axum
...
(`os/pkgs/mosd/apid/src/routes.rs:2426`), whose body runs from
`const STYLE: &str = "\` (`os/pkgs/mosd/apid/src/routes.rs:2427`) to
`os/pkgs/mosd/apid/src/routes.rs:2434`, injected into the page head as
`style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:2445`). The crate
...
`pub fn app(state: AppState) -> Router {` (`os/pkgs/mosd/apid/src/routes.rs:137`) and
runs to `.with_state(state)` (`os/pkgs/mosd/apid/src/routes.rs:228`). Its declaration
...
| `GET /healthz` | `:173` | `healthz` (`:729`) | neither — plain text | Returns the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:2423`) — a liveness probe only: it proves the apid process is listening and checks nothing else, not mosd, not the settings tree, not the appliance | none |
...
| every method on `/api/` | `:201` (`.route("/api/", any(api_not_found))`) | `api_not_found` (`:196`) | JSON | 404 in §2.4's envelope. Not redundant with the nest: *"`nest` claims `/api`, `/api/x` and `/api/x/y`, and not `/api/`; the difference is a request that begins `/api/` reaching the asset router"* (`os/pkgs/mosd/apid/src/routes.rs:218-221`) | none |
...
**The `/api` subtree.** `api_router()` (`os/pkgs/mosd/apid/src/routes.rs:377-423`) is
nested under the prefix `const API: &str = "/api";`
(`os/pkgs/mosd/apid/src/routes.rs:252`). Each route is declared from the same constant
...
one string and cannot disagree"* (`os/pkgs/mosd/apid/src/routes.rs:374-376`).
...
(`os/pkgs/mosd/apid/src/routes.rs:289`) while OpenAPI's is
`const V1_SETTINGS_DOC: &str = "/v1/settings/{path}";`
(`os/pkgs/mosd/apid/src/routes.rs:290`), and the source states why: *"axum names a
...
(`os/pkgs/mosd/apid/src/routes.rs:283-286`). Every response under the prefix is built
by one helper that sets `no-store` on success as well as on failure
(`os/pkgs/mosd/apid/src/routes.rs:556-563`).

**Kinds, counted, at this commit.** The outer router declares **twenty-one**
`.route()` calls (`os/pkgs/mosd/apid/src/routes.rs:144`, `:161-167`, `:188-190`,
`:181-189`, `:201`), the nested `/builtin` router declares **two** (`:148`,
`:152`) and `api_router()` declares **four** (`:280-283`) — **twenty-seven**
...
neither and returns a bare string (`os/pkgs/mosd/apid/src/routes.rs:2422-2424`), and
fourteen are POSTs.
...
(`os/pkgs/mosd/apid/src/routes.rs:560`), as does `api_not_found`. **No route accepts a
...
example `Form(form): Form<SetupForm>,` (`os/pkgs/mosd/apid/src/routes.rs:2924`),
...
(`os/pkgs/mosd/apid/src/routes.rs:195-197`) — or for any of the four SSH mutations
(`os/pkgs/mosd/apid/src/routes.rs:201-203`), and the rule has since been extended to
the bundle deactivation control (`os/pkgs/mosd/apid/src/routes.rs:166-168`). And there
...
on power (`os/pkgs/mosd/apid/src/routes.rs:4534`) and on the transient password
(`os/pkgs/mosd/apid/src/routes.rs:5387`).

**The second router.** `redirect_app()` at `os/pkgs/mosd/apid/src/routes.rs:2312-2316` is
the router served on the **HTTP** listener. It declares no routes at all — only
a fallback, `.fallback(redirect_to_https)` (`os/pkgs/mosd/apid/src/routes.rs:2314`) —
...
(`os/pkgs/mosd/apid/src/routes.rs:2318-2332`). It carries no state beyond that port
(`:668`), no auth gate, and no access to mosd. Both routers are wired in
...
(`os/pkgs/mosd/apid/src/routes.rs:2356`), built at `os/pkgs/mosd/apid/src/routes.rs:2349-2360`.
...
(`os/pkgs/mosd/apid/src/routes.rs:2338-2343`), typed as
...
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:3122`), set
at `os/pkgs/mosd/apid/src/routes.rs:3123-3127`. `POST /setup` does the same at the end
of the first-run wizard without a login step
(`os/pkgs/mosd/apid/src/routes.rs:3031-3036`). The store generates 16 random bytes from
...
through the `ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:2205-2228`), which
tests `session::cookie_from_headers(&parts.headers)`
(`os/pkgs/mosd/apid/src/routes.rs:2216`) and nothing else.

**The gate.** One middleware, layered over the whole HTTPS router with
`.layer(middleware::from_fn_with_state(state.clone(), gate))`
(`os/pkgs/mosd/apid/src/routes.rs:227`), running from
`async fn gate(State(state): State<AppState>, request: Request, next: Next) -> Response {`
(`os/pkgs/mosd/apid/src/routes.rs:2372`) to `os/pkgs/mosd/apid/src/routes.rs:2420`. It implements
...
(`os/pkgs/mosd/apid/src/routes.rs:2395-2399`), so an authenticated request reaches its
handler without touching mosd, and the source states the reason and its
soundness argument at `os/pkgs/mosd/apid/src/routes.rs:2378-2394`: *"an authenticated
...
`state.api.get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:2305`) is now paid
...
time (`os/pkgs/mosd/apid/src/routes.rs:694`) rather than copying it. It is **read-only
...
the SSH pane reads the whole key list (`os/pkgs/mosd/apid/src/routes.rs:4739-4741`),
edits it in memory (`os/pkgs/mosd/apid/src/routes.rs:5420-5424`), and writes the whole
list back through `write_key_list` (`os/pkgs/mosd/apid/src/routes.rs:4744-4757`).
...
(`os/pkgs/mosd/apid/src/routes.rs:1130`) — which is the one place where
...
2. **A file-serving service is constructed, as the router's fallback.**
   ...
   now ends in `.fallback(serve::fallback)` (`os/pkgs/mosd/apid/src/routes.rs:226`),
...
3. **Nothing is embedded in the binary, and that is still deliberate.**
   ...
   `os/pkgs/mosd/apid/src/tests.rs:1787` reads the committed `openapi.json` so a test
...
4. **The crate still ships no asset of its own; the assets come from
   ...
   `bundles: Arc::new(Store::at_default()),` (`os/pkgs/mosd/apid/src/routes.rs:84`).
...
(`os/pkgs/mosd/apid/src/routes.rs:2445`), from a constant described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:2426`). There is still no favicon route, no font and
no image declared in `os/pkgs/mosd/apid/src/routes.rs:137-228`. A request for
`/favicon.ico` no longer ends at the gate, though: it is unauthenticated, so
the gate redirects it to `/login` (`os/pkgs/mosd/apid/src/routes.rs:2419`), and
...
control (`os/pkgs/mosd/apid/src/routes.rs:3818`) and the install path. The consequence
...
1. **Line numbers throughout its section 2.1 no longer resolve to the routes
   ...
   `f7cb5ba` the route is `os/pkgs/mosd/apid/src/routes.rs:144` and the handler is
   `serve::root`, at `os/pkgs/mosd/apid/src/assets/serve.rs:57`, with the built-in branch
   at `os/pkgs/mosd/apid/src/routes.rs:3505`.
2. **Its route table is missing thirteen routes.** The five SSH routes it
   ...
   `os/pkgs/mosd/apid/src/routes.rs:200-207`, and eight more have landed since: the
   four `/containers` and `/mqtt` pairs (`os/pkgs/mosd/apid/src/routes.rs:208-211`) and
   the four `/api` routes (`os/pkgs/mosd/apid/src/routes.rs:379-386`). Its own section
...
renders **eight** links (`os/pkgs/mosd/apid/src/routes.rs:2450-2463`) plus the logout form
(`os/pkgs/mosd/apid/src/routes.rs:2464-2466`): Status, Network, Hostname, Power, SSH,
...
`"/api/v1/state/{path}"` (`os/pkgs/mosd/apid/openapi.json:714`) and
`"/api/versions"` (`os/pkgs/mosd/apid/openapi.json:1287`). Its
`components.schemas` are exactly five: `"ApiError"`
(`os/pkgs/mosd/apid/openapi.json:1353`), `"ApiErrorDetail"`
(`os/pkgs/mosd/apid/openapi.json:1365`), `"ApiMeta"` (`os/pkgs/mosd/apid/openapi.json:1429`),
`"ApiVersions"` (`os/pkgs/mosd/apid/openapi.json:1479`) and `"ResourceValue"`
(`os/pkgs/mosd/apid/openapi.json:1602`). The document is generated from the handlers
...
(`os/pkgs/mosd/apid/src/routes.rs:252`), nested as `.nest(API, api_router())`
(`os/pkgs/mosd/apid/src/routes.rs:223`), and the version is the segment immediately
after it — `const V1_META_PATH: &str = "/v1/meta";`
(`os/pkgs/mosd/apid/src/routes.rs:254`). `GET /api/versions` answers unauthenticated
from `const SERVED_VERSIONS: [&str; 1] = ["v1"];`
(`os/pkgs/mosd/apid/src/routes.rs:366`) and `const CURRENT_VERSION: &str = "v1";`
(`os/pkgs/mosd/apid/src/routes.rs:369`), and `GET /api/v1/meta` answers authenticated
with `ApiMeta` whose `settingsSchemaVersion` is read live from
`settings_schema_version: mosd_settings::SCHEMA_VERSION,`
(`os/pkgs/mosd/apid/src/routes.rs:694`) rather than copied. The served set is an array
on the wire, `"versions"` typed `"type": "array"`
(`os/pkgs/mosd/apid/openapi.json:1491-1492`), so the dual-major recommendation below has
...
(`os/pkgs/mosd/apid/src/routes.rs:224`) claims the one spelling the nest does not, and
...
(`os/pkgs/mosd/apid/src/routes.rs:215-217`).
...
1. The gate already dispatches on `let path = request.uri().path();`
   (`os/pkgs/mosd/apid/src/routes.rs:2373`). A version check is the same string operation
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:227`). This is now measurable rather than
   predicted: the shipped gate tests membership with `is_declared_api_route`
   (`os/pkgs/mosd/apid/src/routes.rs:467-504`), a pure string operation over the same
   constants the router declares.
2. axum registers routes by path (`os/pkgs/mosd/apid/src/routes.rs:137-228`). A second
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:377-423`) is the first of those routers and shows
   the shape.
...
(`os/pkgs/mosd/apid/src/routes.rs:2328`) — that is not the objection. The
...
Both rows ship. The first is declared at `os/pkgs/mosd/apid/src/routes.rs:379` and
handled at `:444`; the second at `:281` and `:483`. The `meta` row's
...
exactly one source"* (`os/pkgs/mosd/apid/src/routes.rs:675-677`). The API version is the
...
(`os/pkgs/mosd/apid/src/routes.rs:2408-2413`). An authenticated probe cannot answer the
...
so"* (`os/pkgs/mosd/apid/src/routes.rs:647-650`); the gate hands the route off above its own
bus call at `os/pkgs/mosd/apid/src/routes.rs:2374`. The cost is an unauthenticated
...
(`os/pkgs/mosd/apid/src/routes.rs:2374`, `:809-811`), so this is one more bit on a
...
(`os/pkgs/mosd/apid/src/routes.rs:289`) declared at `os/pkgs/mosd/apid/src/routes.rs:382-385` and
handled by `api_v1_settings` (`:474`), whose whole body is
`resource_response(state.api.get_settings(&path).await, &path)`
(`os/pkgs/mosd/apid/src/routes.rs:847`) — the dot-path passthrough this section asks
for, with no second model beside it. The live-state root is the same shape,
`const V1_STATE_ROUTE: &str = "/v1/state/{*path}";`
(`os/pkgs/mosd/apid/src/routes.rs:292`) at `:301` and `:573`, ending in
`resource_response(state.api.get_state(&path).await, &path)`
(`os/pkgs/mosd/apid/src/routes.rs:1130`). Both answer `ResourceValue`
(`os/pkgs/mosd/apid/src/routes.rs:819`), which carries a
`serde(transparent)` attribute (`os/pkgs/mosd/apid/src/routes.rs:818`) so the body is
...
to write"* (`os/pkgs/mosd/apid/src/routes.rs:1148`) — the key lives in a file on
...
`/api/versions` (`os/pkgs/mosd/apid/openapi.json:8-1318`). Neither collection
...
(`os/pkgs/mosd/apid/src/routes.rs:812-813`). And `GET /api/v1/state/{path}` today
...
- **No array indexing.** The dot-path syntax has none; the model says a list is
  *"Written as a whole JSON array through the dot-path API"*
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:5420-5425`). Two clients doing that concurrently
...
- **A key holding a dot is addressed with a quoted segment.** apid accepts an
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:2506-2512`) — and until PLAN-022 M2 the
...
nobody asked to delete"* (`os/pkgs/mosd/apid/src/routes.rs:5436-5440`). A `DELETE` whose
...
the file (`os/pkgs/mosd/apid/src/routes.rs:4745-4747`). So the difference is the quality
...
| SSH keys | the authorized-keys collection above | `access.ssh.authorizedKeys` | **every key is a root key** (`docs/design/access.md` §4.1, `os/pkgs/mosd/apid/src/routes.rs:4622`); the API response must carry that sentence in a `notice` field for the same reason the pane must carry it |
...
(`os/pkgs/mosd/apid/openapi.json:8-1318`) — `"/api/v1/meta"`
(`os/pkgs/mosd/apid/openapi.json:228`), `"/api/v1/settings/{path}"`
(`os/pkgs/mosd/apid/openapi.json:270`), `"/api/v1/state/{path}"`
(`os/pkgs/mosd/apid/openapi.json:714`) and `"/api/versions"`
(`os/pkgs/mosd/apid/openapi.json:1287`) — and the router declares nothing else under the
prefix beyond a fallback (`os/pkgs/mosd/apid/src/routes.rs:377-423`). There is no
...
| `GET /healthz` (`routes.rs:172`, `:768`) | — | **none, and it must not change.** See below | — |
...
(`os/pkgs/mosd/apid/src/routes.rs:195-197`) or for any of the four SSH mutations
(`os/pkgs/mosd/apid/src/routes.rs:201-203`), *"so a browser prefetch, a crawler or a mis-clicked link cannot power
the appliance off"*.

The **confirmation token** that guards the three form posts today
(`os/pkgs/mosd/apid/src/routes.rs:4534`, and `TRANSIENT_CONFIRM_TOKEN` at `os/pkgs/mosd/apid/src/routes.rs:4626`) does
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:4554-4567`). 202
...
- **`POST /login` and `POST /logout`.** A session is a browser mechanism (§3.1).
  ...
  mislead.
- **`GET /healthz`** — declared at `os/pkgs/mosd/apid/src/routes.rs:212` and answering
  at `os/pkgs/mosd/apid/src/routes.rs:2422-2424`. It keeps its path, its unauthenticated exemption (`os/pkgs/mosd/apid/src/routes.rs:2374`) and
  its literal `ok` body,
  ...
  **But it answers a narrower question than its name suggests, and the API must
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:2374`), so it answers `ok` on an appliance whose
...
- **A password change — [implemented], on both surfaces.** This bullet once
  ...
  `os/pkgs/mosd/apid/src/routes.rs:186`; the API route is
  `POST /api/v1/actions/change-password`, declared at
  `os/pkgs/mosd/apid/src/routes.rs:387` (handler at `:1903-1958`), `POST` only
  ...
  (`change_password`, `os/pkgs/mosd/apid/src/routes.rs:3198-3257`): the
...
(`os/pkgs/mosd/apid/src/routes.rs:5457-5463`, via `ssh_error` at `os/pkgs/mosd/apid/src/routes.rs:4760-4769`). For a
...
(`os/pkgs/mosd/apid/src/routes.rs:566-568`, payload at `:334-352`), the schema records
it as `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:1356-1358`), and every
failure under the prefix is built through it. `Retry-After` ships on exactly
one class: `const RETRY_AFTER_SECONDS: &str = "5";`
(`os/pkgs/mosd/apid/src/routes.rs:362`), attached only when the status is 503
(`os/pkgs/mosd/apid/src/routes.rs:2161-2166`). The recommendation this section makes —
...
(`os/pkgs/mosd/apid/src/routes.rs:2124-2166`), which matches on the concrete
`zbus::Error::MethodError` before the conversion flattens it and maps the five
error names declared at `os/pkgs/mosd/apid/src/routes.rs:357-359` — two
...
translated and the message is not"* (`os/pkgs/mosd/apid/src/routes.rs:2115`).
...
|---|---|---|---|
| `error` | the only top-level member | `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:1356-1358`) | none |
| `code` | *"stable machine token; an OPEN set"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:1374-1375`), required (`:237-241`) | none in shape. The **open set** is stated only in the description (`os/pkgs/mosd/apid/openapi.json:1376`); nothing in the schema expresses it, so a generated client learns the rule only if a human reads the prose |
| `message` | *"human-readable; not for matching on"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:1378-1380`), required | none |
| `source` | `"apid" \| "mosd"`, an enum of exactly two | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:1389-1390`), required, **no `enum`** | **a real difference.** The Rust type admits only two values, because `ApiError` is constructed by exactly two constructors, `ApiError::apid`, which passes `Self::new(code, message, "apid")`
(`os/pkgs/mosd/apid/src/routes.rs:593`), and `ApiError::mosd`, which passes
`Self::new(code, message, "mosd")` (`os/pkgs/mosd/apid/src/routes.rs:598`). The published contract does not say so, so a client generated from the document gets an unconstrained string and cannot exhaustively match on it |
| `path` | OPTIONAL, present only when the failure names a dot-path | `"type": [ "string", "null" ]` (`os/pkgs/mosd/apid/openapi.json:1382-1386`), **absent from `required`** | **a real difference, in the opposite direction.** The schema admits an explicit `null`; the wire never carries one, because the field carries a `serde(skip_serializing_if = "Option::is_none")` attribute (`os/pkgs/mosd/apid/src/routes.rs:586`) and is therefore omitted rather than nulled — which is the behaviour this section asks for and the source's own doc comment defends (`os/pkgs/mosd/apid/src/routes.rs:581-585`). The schema is wider than the implementation, so a client that handles `null` is handling a case the device does not produce |
...
| `code` | Ships? | Where |
|---|---|---|
| `settings_not_found` | **yes**, 404 | `os/pkgs/mosd/apid/src/routes.rs:2145-2148`, on `com.mos.mosd1.Error.NotFound` |
| `settings_read_only` | **yes**, 409 | `os/pkgs/mosd/apid/src/routes.rs:2149-2152`, on `com.mos.mosd1.Error.ReadOnly` |
| `not_authenticated` | **yes**, with a different credential | `os/pkgs/mosd/apid/src/routes.rs:2264`. It is raised by the session-cookie extractor (`os/pkgs/mosd/apid/src/routes.rs:2224-2226`), not by a bearer token: §3.2's token does not exist, so the shipped 401 means *"no session cookie, or one that does not verify"* (`os/pkgs/mosd/apid/src/routes.rs:2225`) |
| `not_found` | **yes** | `os/pkgs/mosd/apid/src/routes.rs:239`, from the reserved subtree's fallback |
...
| `settings_rejected` | **yes**, 422 | `os/pkgs/mosd/apid/src/routes.rs:2142-2145`, on fdo `InvalidArgs` (`os/pkgs/mosd/apid/src/routes.rs:357`) |
| `settings_io` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:2146-2149`, on fdo `IOError` (`os/pkgs/mosd/apid/src/routes.rs:358`) |
| `mosd_failed` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:2150-2153`, on fdo `Failed` (`os/pkgs/mosd/apid/src/routes.rs:359`) |
| `mosd_unreachable` | **yes**, 503 with `Retry-After` | `os/pkgs/mosd/apid/src/routes.rs:2172-2178`, and it is exhaustive over everything the three above do not name (`os/pkgs/mosd/apid/src/routes.rs:2168-2170`) |
...
(`os/pkgs/mosd/apid/src/routes.rs:2345-2360`, message at `os/pkgs/mosd/apid/src/routes.rs:2356`). So a settings value mosd
...
(`os/pkgs/mosd/apid/src/routes.rs:4311-4398`), and `write_key_list` does the same
(`os/pkgs/mosd/apid/src/routes.rs:4753-4755`). The VLAN example above is exactly this — a rejection that reads
as an outage.
...
text in an error box (`os/pkgs/mosd/apid/src/routes.rs:2487-2489`, `:1737`, `:1757`) — without
widening it.
...
2. **Bus unavailable.** `mosd_unreachable`, **503**, with `Retry-After: 5`. 503
   ...
   `mosd_unreachable` answers 503 (`os/pkgs/mosd/apid/src/routes.rs:2173-2177`) with
   `Retry-After` attached (`os/pkgs/mosd/apid/src/routes.rs:2161-2166`). The cost this
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:2345-2360`), so one appliance reports one
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   `/healthz` returns before any check (`os/pkgs/mosd/apid/src/routes.rs:2374`) and
   answers the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:2423`), so **an appliance whose mosd is dead
   ...
   *"whole seconds since boot, a bare JSON number"* everywhere else it appears.
   ...
   off above the bus call (`os/pkgs/mosd/apid/src/routes.rs:2374`), and a request with a
   valid session cookie is handed off above it too
   (`os/pkgs/mosd/apid/src/routes.rs:2395-2399`), so no API route depends on the gate's
...
middleware, `pub(crate) struct ApiSession;` (`os/pkgs/mosd/apid/src/routes.rs:2205`),
whose whole test is
`session::cookie_from_headers(&parts.headers)` verified against the store
(`os/pkgs/mosd/apid/src/routes.rs:2216-2217`); its rejection is §2.4's envelope with a
401 rather than the gate's HTML redirect
(`os/pkgs/mosd/apid/src/routes.rs:2224-2226`). The gate hands the declared `/api` routes
off to it with `is_declared_api_route(path)`
(`os/pkgs/mosd/apid/src/routes.rs:2374`, predicate at `:325-330`). There is no bearer
...
exchange as success"* (`os/pkgs/mosd/apid/src/routes.rs:2202-2204`). **Unanswered:**
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   `Redirect::to("/login").into_response()` (`os/pkgs/mosd/apid/src/routes.rs:2419`), a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:2414-2416`) and which returns **200 OK** with an HTML
   form (`os/pkgs/mosd/apid/src/routes.rs:3054-3071`). A script that checks the status
   ...
   instead (`os/pkgs/mosd/apid/src/routes.rs:2224-2226`). Every other path on the daemon
   still redirects.
2. **Obtaining the credential means emulating three browser behaviours.**
   `POST /login` takes `Form(form): Form<LoginForm>,`
   (`os/pkgs/mosd/apid/src/routes.rs:3076`) — axum's URL-encoded extractor, imported at
   `os/pkgs/mosd/apid/src/routes.rs:19`, not JSON — answers **302 to `/`**
   (`os/pkgs/mosd/apid/src/routes.rs:3125`), and delivers the credential in a
   `Set-Cookie` header (`os/pkgs/mosd/apid/src/routes.rs:3124`, value built at
...
6. **There is exactly one credential, and it identifies a human.** The only
   ...
   the admin hash, read through `password_hash` (`os/pkgs/mosd/apid/src/routes.rs:2338-2343`); there is
   no second credential, no user table, and no reference to `access.device` in
...
7. **A misconfigured script locks the operator out, repeatedly.** The login
   ...
   *"the appliance has one admin password, so per-client tracking buys nothing
   ...
   `os/pkgs/mosd/apid/src/routes.rs:3089-3099`). A script retrying with a
...
(`os/pkgs/mosd/apid/openapi.json:8-1318`).
...
`ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:2205-2228`) against the same
...
4. **It costs zero extra bus round trips.** The gate already calls
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:2411`) and `apiTokens` is a child of `access`, so
...
- Mint and revoke are read-modify-write of the whole array, because the dot-path
  ...
  the same pattern the SSH key pane uses (`os/pkgs/mosd/apid/src/routes.rs:5420-5425`).
  Two concurrent mints lose one token, silently.
- Identity is the `id`, never a list position, for the reason recorded at
  `os/pkgs/mosd/apid/src/routes.rs:5449-5453`: an index is meaningful only against the
...
- **Coexisting means two credentials at one privilege level** — and there is
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:231`). A token can do everything the operator can do
...
2. **§3.1 item 7 changes meaning.** The `auth.rs:53-54` comment's reasoning —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3096-3105`). Bearer verification is **not** rate
...
(`os/pkgs/mosd/apid/src/routes.rs:2224-2226`) and a redirect to `/login` everywhere else
(`os/pkgs/mosd/apid/src/routes.rs:2419`) — and the CORS argument below, which depends on
...
2. **A stolen token is full management access, indefinitely.** There is no
   ...
   `os/pkgs/mosd/apid/src/routes.rs:4622`) and is then no longer needed. Detection is not
...
5. **A same-origin page mints a permanent token with the operator's cookie.**
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:4622`). But a minted token survives deactivating
...
pane is tested to carry, `os/pkgs/mosd/apid/src/routes.rs:4622`); apid runs as root
...
- **Under this proposal the API path has no CSRF exposure**, because a bearer
  ...
  **None of that is true of the shipped API**, whose credential *is* the cookie
  (`os/pkgs/mosd/apid/src/routes.rs:2216-2217`), so today the four `/api` routes sit
...
- **The per-action confirm tokens are not CSRF tokens and must not be counted as
  ...
  and `"poweroff"` (`os/pkgs/mosd/apid/src/routes.rs:4454-4458`) and
  `TRANSIENT_CONFIRM_TOKEN` is the literal `"set-transient-password"`
  (`os/pkgs/mosd/apid/src/routes.rs:4626`). They are not secret, not per-session and not
  ...
  the checkbox has to be ticked as well"* (`os/pkgs/mosd/apid/src/routes.rs:4452-4453`). They stop a mis-click and a prefetch. Against a
...
(`os/pkgs/mosd/apid/src/routes.rs:2395-2401`), so an authenticated flood no longer
...
(`os/pkgs/mosd/apid/src/routes.rs:2374`) and its handler makes no mosd call
(`os/pkgs/mosd/apid/src/routes.rs:663-671`), so it is an unauthenticated route that
...
declaration at `os/pkgs/mosd/apid/src/routes.rs:196`, which is why that line is not
...
`0d4f3c6` the fifteen declarations are at `os/pkgs/mosd/apid/src/routes.rs:150-169`,
...
(`os/pkgs/mosd/apid/src/routes.rs:187-216`). A custom
...
`os/pkgs/mosd/apid/src/routes.rs:57` and `:658`) moves under section 6.3's reserved
...
allowlist (`.wasm` and `.webmanifest` in it from the start, `:65-66`), the
`octet-stream` fallback, `nosniff`, and the three cache classes with their
header values (`:90-139`). The headers are attached to every response the asset
...
(`os/pkgs/mosd/apid/src/routes.rs:216`). The immutable class is read per request from
...
- **`/var/lib/mos/settings.toml`** (`os/pkgs/mosd/mosd-settings/src/store.rs:67`) — the
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:2342-2349`) and every authorized SSH key.
...
§6.3's reserved prefix, `builtin_home` in `os/pkgs/mosd/apid/src/routes.rs:2510`. Its
...
(`os/pkgs/mosd/apid/src/routes.rs:2448-2454`), described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:2426`). Section 1.6 evidences the rest: no
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:2349`), reached when a mosd call
  ...
  *before* dispatch — so at the moment that page is on screen, `/builtin/` is
  ...
  page (`os/pkgs/mosd/apid/src/routes.rs:2330`), the navigation on every built-in pane
  (`:341`), and the reserved subtree's own 404 (`:1317`). The cost that must not be glossed: **(A) is a way *in*, not a way
...
page *"The management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:2356`,
...
1. **A signature and the upload credential authorise the same blast radius.** A
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:4622`). §3.2 states there are **no scopes in
...
(`os/pkgs/mosd/apid/src/routes.rs:39`, `:821-828`, `:839`; §1.1 constraint 1 and §1.6
...
Items (iv) and (v) are already shipped — `os/pkgs/mosd/apid/src/routes.rs:4430`, `:2245`,
...
rendering `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:2345-2360`) for a value mosd
...
the settings write fails (`os/pkgs/mosd/apid/src/routes.rs:4399-4405`), and `write_key_list`
does the same (`:1544-1546`), so a rejected value is reported to the operator as
...
(`os/pkgs/mosd/apid/src/routes.rs:2126-2127`) and mapping `FDO_INVALID_ARGS`,
`FDO_IO_ERROR` and `FDO_FAILED` onto three distinct API error codes
(`:594-605`). Second: §2.1's breaking-change list makes *"changing
...
(`os/pkgs/mosd/apid/src/routes.rs:2411`) the whole appliance answers the 502 page *"The
management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:2354-2364`). **The
...
path answers 422 (`os/pkgs/mosd/apid/src/routes.rs:5457-5463`). The HTML path is not
changed.
...
setup mode (`os/pkgs/mosd/apid/src/routes.rs:2415-2420`) — and it returns a token (§2.3),
...
HTML path answers **502** (`os/pkgs/mosd/apid/src/routes.rs:2354-2364`), so one appliance
...
- **The VLAN dot-path limit — since closed, and closed additively.**
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:2506-2512`) — while `split_path` split on it
...
`os/pkgs/mosd/apid/src/routes.rs:231`), so a token can do everything the operator can do
