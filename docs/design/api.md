...
`os/pkgs/mosd/apid/src/routes.rs:3326`, injected into the page head as
...
neither and returns a bare string (`os/pkgs/mosd/apid/src/routes.rs:3314-3316`), and
fourteen are POSTs.
...
on power (`os/pkgs/mosd/apid/src/routes.rs:5426`) and on the transient password
(`os/pkgs/mosd/apid/src/routes.rs:6361`).
...
(`os/pkgs/mosd/apid/src/routes.rs:3210-3224`). It carries no state beyond that port
...
(`os/pkgs/mosd/apid/src/routes.rs:3248`), built at `os/pkgs/mosd/apid/src/routes.rs:3241-3252`.
...
at `os/pkgs/mosd/apid/src/routes.rs:4015-4019`. `POST /setup` does the same at the end
of the first-run wizard without a login step
(`os/pkgs/mosd/apid/src/routes.rs:3923-3928`). The store generates 16 random bytes from
...
through the `ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:3097-3120`), which
...
(`os/pkgs/mosd/apid/src/routes.rs:3264`) to `os/pkgs/mosd/apid/src/routes.rs:3312`. It implements
...
the SSH pane reads the whole key list (`os/pkgs/mosd/apid/src/routes.rs:5713-5715`),
edits it in memory (`os/pkgs/mosd/apid/src/routes.rs:6479-6483`), and writes the whole
...
through with `let value = state.api.get_state(&path).await;`
(`os/pkgs/mosd/apid/src/routes.rs:1227`) — which is the one place where
...
(`os/pkgs/mosd/apid/src/routes.rs:3337`), from a constant described in the source as
*"Inline stylesheet shared by every page; no external assets"*
...
control (`os/pkgs/mosd/apid/src/routes.rs:4710`) and the install path. The consequence
...
1. **Line numbers throughout its section 2.1 no longer resolve to the routes
   ...
   at `os/pkgs/mosd/apid/src/routes.rs:4397`.
...
renders **eight** links (`os/pkgs/mosd/apid/src/routes.rs:3342-3355`) plus the logout form
(`os/pkgs/mosd/apid/src/routes.rs:3356-3358`): Status, Network, Hostname, Power, SSH,
...
(`os/pkgs/mosd/apid/src/routes.rs:3266`, `:809-811`), so this is one more bit on a
...
`/api/versions` (`os/pkgs/mosd/apid/openapi.json:8-2083`). Neither collection
...
- **No array indexing.** The dot-path syntax has none; the model says a list is
  *"Written as a whole JSON array through the dot-path API"*
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:6479-6484`). Two clients doing that concurrently
...
the file (`os/pkgs/mosd/apid/src/routes.rs:5719-5721`). So the difference is the quality
...
| SSH keys | the authorized-keys collection above | `access.ssh.authorizedKeys` | **every key is a root key** (`docs/design/access.md` §4.1, `os/pkgs/mosd/apid/src/routes.rs:5596`); the API response must carry that sentence in a `notice` field for the same reason the pane must carry it |
...
(`os/pkgs/mosd/apid/openapi.json:8-2083`) — `"/api/v1/meta"`
...
(`os/pkgs/mosd/apid/src/routes.rs:5426`, and `TRANSIENT_CONFIRM_TOKEN` at `os/pkgs/mosd/apid/src/routes.rs:5600`) does
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:5511-5517`). 202
...
- **`GET /healthz`** — declared at `os/pkgs/mosd/apid/src/routes.rs:213` and answering
  at `os/pkgs/mosd/apid/src/routes.rs:3314-3316`. It keeps its path, its unauthenticated exemption (`os/pkgs/mosd/apid/src/routes.rs:3266`) and
  its literal `ok` body,
  ...
  **But it answers a narrower question than its name suggests, and the API must
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3266`), so it answers `ok` on an appliance whose
...
- **A password change — [implemented], on both surfaces.** This bullet once
  ...
  `os/pkgs/mosd/apid/src/routes.rs:415` (handler at `:1903-1958`), `POST` only
  ...
  (`change_password`, `os/pkgs/mosd/apid/src/routes.rs:4090-4149`): the
...
(`os/pkgs/mosd/apid/src/routes.rs:6516-6522`, via `ssh_error` at `os/pkgs/mosd/apid/src/routes.rs:5734-5743`). For a
...
(`os/pkgs/mosd/apid/src/routes.rs:662-664`, payload at `:334-352`), the schema records
...
(`os/pkgs/mosd/apid/src/routes.rs:3053-3058`). The recommendation this section makes —
...
(`os/pkgs/mosd/apid/src/routes.rs:3009-3058`), which matches on the concrete
...
| `source` | `"apid" \| "mosd"`, an enum of exactly two | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:2154-2155`), required, **no `enum`** | **a real difference.** The Rust type admits only two values, because `ApiError` is constructed by exactly two constructors, `ApiError::apid`, which passes `Self::new(code, message, "apid")`
...
| `code` | Ships? | Where |
|---|---|---|
| `settings_not_found` | **yes**, 404 | `os/pkgs/mosd/apid/src/routes.rs:3030-3033`, on `com.mos.mosd1.Error.NotFound` |
| `settings_read_only` | **yes**, 409 | `os/pkgs/mosd/apid/src/routes.rs:3034-3037`, on `com.mos.mosd1.Error.ReadOnly` |
| `not_authenticated` | **yes**, with a different credential | `os/pkgs/mosd/apid/src/routes.rs:3156`. It is raised by the session-cookie extractor (`os/pkgs/mosd/apid/src/routes.rs:3116-3118`), not by a bearer token: §3.2's token does not exist, so the shipped 401 means *"no session cookie, or one that does not verify"* (`os/pkgs/mosd/apid/src/routes.rs:3117`) |
...
| `settings_rejected` | **yes**, 422 | `os/pkgs/mosd/apid/src/routes.rs:3027-3030`, on fdo `InvalidArgs` (`os/pkgs/mosd/apid/src/routes.rs:385`) |
| `settings_io` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:3031-3034`, on fdo `IOError` (`os/pkgs/mosd/apid/src/routes.rs:386`) |
| `mosd_failed` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:3035-3038`, on fdo `Failed` (`os/pkgs/mosd/apid/src/routes.rs:387`) |
| `mosd_unreachable` | **yes**, 503 with `Retry-After` | `os/pkgs/mosd/apid/src/routes.rs:3064-3070`, and it is exhaustive over everything the three above do not name (`os/pkgs/mosd/apid/src/routes.rs:3060-3062`) |
...
(`os/pkgs/mosd/apid/src/routes.rs:3237-3252`, message at `os/pkgs/mosd/apid/src/routes.rs:3248`). So a settings value mosd
...
(`os/pkgs/mosd/apid/src/routes.rs:5203-5290`), and `write_key_list` does the same
(`os/pkgs/mosd/apid/src/routes.rs:5727-5729`). The VLAN example above is exactly this — a rejection that reads
as an outage.
...
text in an error box (`os/pkgs/mosd/apid/src/routes.rs:3379-3381`, `:1737`, `:1757`) — without
widening it.
...
2. **Bus unavailable.** `mosd_unreachable`, **503**, with `Retry-After: 5`. 503
   ...
   `mosd_unreachable` answers 503 (`os/pkgs/mosd/apid/src/routes.rs:3065-3069`) with
   `Retry-After` attached (`os/pkgs/mosd/apid/src/routes.rs:3053-3058`). The cost this
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3237-3252`), so one appliance reports one
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   `/healthz` returns before any check (`os/pkgs/mosd/apid/src/routes.rs:3266`) and
   ...
   *"whole seconds since boot, a bare JSON number"* everywhere else it appears.
   ...
   off above the bus call (`os/pkgs/mosd/apid/src/routes.rs:3266`), and a request with a
   valid session cookie is handed off above it too
   (`os/pkgs/mosd/apid/src/routes.rs:3287-3291`), so no API route depends on the gate's
...
(`os/pkgs/mosd/apid/src/routes.rs:3266`, predicate at `:325-330`). There is no bearer
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3306-3308`) and which returns **200 OK** with an HTML
   form (`os/pkgs/mosd/apid/src/routes.rs:3946-3963`). A script that checks the status
   ...
   instead (`os/pkgs/mosd/apid/src/routes.rs:3116-3118`). Every other path on the daemon
   still redirects.
2. **Obtaining the credential means emulating three browser behaviours.**
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:4017`), and delivers the credential in a
   `Set-Cookie` header (`os/pkgs/mosd/apid/src/routes.rs:4016`, value built at
...
7. **A misconfigured script locks the operator out, repeatedly.** The login
   ...
   *"the appliance has one admin password, so per-client tracking buys nothing
   ...
   `os/pkgs/mosd/apid/src/routes.rs:3981-3991`). A script retrying with a
...
(`os/pkgs/mosd/apid/openapi.json:8-2083`).
...
`ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:3097-3120`) against the same
...
4. **It costs zero extra bus round trips.** The gate already calls
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3303`) and `apiTokens` is a child of `access`, so
...
- Mint and revoke are read-modify-write of the whole array, because the dot-path
  ...
  the same pattern the SSH key pane uses (`os/pkgs/mosd/apid/src/routes.rs:6479-6484`).
  Two concurrent mints lose one token, silently.
- Identity is the `id`, never a list position, for the reason recorded at
  `os/pkgs/mosd/apid/src/routes.rs:6508-6512`: an index is meaningful only against the
...
2. **§3.1 item 7 changes meaning.** The `auth.rs:53-54` comment's reasoning —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3988-3997`). Bearer verification is **not** rate
...
(`os/pkgs/mosd/apid/src/routes.rs:3116-3118`) and a redirect to `/login` everywhere else
(`os/pkgs/mosd/apid/src/routes.rs:3311`) — and the CORS argument below, which depends on
...
2. **A stolen token is full management access, indefinitely.** There is no
   ...
   `os/pkgs/mosd/apid/src/routes.rs:5596`) and is then no longer needed. Detection is not
...
5. **A same-origin page mints a permanent token with the operator's cookie.**
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:5596`). But a minted token survives deactivating
...
pane is tested to carry, `os/pkgs/mosd/apid/src/routes.rs:5596`); apid runs as root
...
(`os/pkgs/mosd/apid/src/routes.rs:3287-3293`), so an authenticated flood no longer
...
(`os/pkgs/mosd/apid/src/routes.rs:3266`) and its handler makes no mosd call
...
- **`/var/lib/mos/settings.toml`** (`os/pkgs/mosd/mosd-settings/src/store.rs:67`) — the
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3234-3241`) and every authorized SSH key.
...
§6.3's reserved prefix, `builtin_home` in `os/pkgs/mosd/apid/src/routes.rs:3402`. Its
...
(`os/pkgs/mosd/apid/src/routes.rs:3340-3346`), described in the source as
*"Inline stylesheet shared by every page; no external assets"*
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  *before* dispatch — so at the moment that page is on screen, `/builtin/` is
  ...
  page (`os/pkgs/mosd/apid/src/routes.rs:3222`), the navigation on every built-in pane
...
1. **A signature and the upload credential authorise the same blast radius.** A
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:5596`). §3.2 states there are **no scopes in
...
Items (iv) and (v) are already shipped — `os/pkgs/mosd/apid/src/routes.rs:5322`, `:2245`,
...
the settings write fails (`os/pkgs/mosd/apid/src/routes.rs:5291-5297`), and `write_key_list`
...
(`os/pkgs/mosd/apid/src/routes.rs:3303`) the whole appliance answers the 502 page *"The
...
path answers 422 (`os/pkgs/mosd/apid/src/routes.rs:6516-6522`). The HTML path is not
changed.
...
setup mode (`os/pkgs/mosd/apid/src/routes.rs:3307-3312`) — and it returns a token (§2.3),
...
HTML path answers **502** (`os/pkgs/mosd/apid/src/routes.rs:3246-3256`), so one appliance
