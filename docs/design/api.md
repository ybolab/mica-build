...
(`os/pkgs/mosd/apid/src/routes.rs:3290`), whose body runs from
`const STYLE: &str = "\` (`os/pkgs/mosd/apid/src/routes.rs:3291`) to
`os/pkgs/mosd/apid/src/routes.rs:3298`, injected into the page head as
`style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:3309`). The crate
...
| `GET /healthz` | `:173` | `healthz` (`:729`) | neither — plain text | Returns the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:3287`) — a liveness probe only: it proves the apid process is listening and checks nothing else, not mosd, not the settings tree, not the appliance | none |
...
example `Form(form): Form<SetupForm>,` (`os/pkgs/mosd/apid/src/routes.rs:3788`),
...
a fallback, `.fallback(redirect_to_https)` (`os/pkgs/mosd/apid/src/routes.rs:3178`) —
...
(`os/pkgs/mosd/apid/src/routes.rs:3220`), built at `os/pkgs/mosd/apid/src/routes.rs:3213-3224`.
...
(`os/pkgs/mosd/apid/src/routes.rs:3202-3207`), typed as
...
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:3986`), set
...
(`os/pkgs/mosd/apid/src/routes.rs:3080`) and nothing else.
...
(`os/pkgs/mosd/apid/src/routes.rs:3236`) to `os/pkgs/mosd/apid/src/routes.rs:3284`. It implements
...
1. `/healthz` and the declared `/api/` routes always pass —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3238`). The API routes answer for themselves in
...
4. **Normal mode** — `/login` and `/setup` pass and everything else redirects
   to `/login` (`os/pkgs/mosd/apid/src/routes.rs:3278-3283`).
...
`state.api.get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:3169`) is now paid
...
(`os/pkgs/mosd/apid/src/routes.rs:1226`) — which is the one place where
...
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:3290`). There is still no favicon route, no font and
...
the gate redirects it to `/login` (`os/pkgs/mosd/apid/src/routes.rs:3283`), and
...
`"/api/versions"` (`os/pkgs/mosd/apid/openapi.json:2042`). Its
`components.schemas` are exactly five: `"ApiError"`
(`os/pkgs/mosd/apid/openapi.json:2108`), `"ApiErrorDetail"`
(`os/pkgs/mosd/apid/openapi.json:2120`), `"ApiMeta"` (`os/pkgs/mosd/apid/openapi.json:2184`),
`"ApiVersions"` (`os/pkgs/mosd/apid/openapi.json:2234`) and `"ResourceValue"`
(`os/pkgs/mosd/apid/openapi.json:2435`). The document is generated from the handlers
...
(`os/pkgs/mosd/apid/openapi.json:2246-2247`), so the dual-major recommendation below has
...
1. The gate already dispatches on `let path = request.uri().path();`
   (`os/pkgs/mosd/apid/src/routes.rs:3237`). A version check is the same string operation
...
(`os/pkgs/mosd/apid/src/routes.rs:3192`) — that is not the objection. The
...
(`os/pkgs/mosd/apid/src/routes.rs:3272-3277`). An authenticated probe cannot answer the
...
(`os/pkgs/mosd/apid/src/routes.rs:3238`, `:809-811`), so this is one more bit on a
...
(`os/pkgs/mosd/apid/src/routes.rs:1226`). Both answer `ResourceValue`
...
to write"* (`os/pkgs/mosd/apid/src/routes.rs:1244`) — the key lives in a file on
...
- **A key holding a dot is addressed with a quoted segment.** apid accepts an
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3370-3376`) — and until PLAN-022 M2 the
...
(`os/pkgs/mosd/apid/openapi.json:2042`) — and the router declares nothing else under the
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:5483-5489`). 202
...
it as `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:2111-2113`), and every
...
(`os/pkgs/mosd/apid/src/routes.rs:3025-3030`). The recommendation this section makes —
...
(`os/pkgs/mosd/apid/src/routes.rs:2981-3030`), which matches on the concrete
...
translated and the message is not"* (`os/pkgs/mosd/apid/src/routes.rs:2966`).
...
|---|---|---|---|
| `error` | the only top-level member | `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:2111-2113`) | none |
| `code` | *"stable machine token; an OPEN set"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:2129-2130`), required (`:237-241`) | none in shape. The **open set** is stated only in the description (`os/pkgs/mosd/apid/openapi.json:2131`); nothing in the schema expresses it, so a generated client learns the rule only if a human reads the prose |
| `message` | *"human-readable; not for matching on"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:2133-2135`), required | none |
...
| `code` | Ships? | Where |
|---|---|---|
| `settings_not_found` | **yes**, 404 | `os/pkgs/mosd/apid/src/routes.rs:3002-3005`, on `com.mos.mosd1.Error.NotFound` |
| `settings_read_only` | **yes**, 409 | `os/pkgs/mosd/apid/src/routes.rs:3006-3009`, on `com.mos.mosd1.Error.ReadOnly` |
| `not_authenticated` | **yes**, with a different credential | `os/pkgs/mosd/apid/src/routes.rs:3128`. It is raised by the session-cookie extractor (`os/pkgs/mosd/apid/src/routes.rs:3088-3090`), not by a bearer token: §3.2's token does not exist, so the shipped 401 means *"no session cookie, or one that does not verify"* (`os/pkgs/mosd/apid/src/routes.rs:3089`) |
...
| `settings_rejected` | **yes**, 422 | `os/pkgs/mosd/apid/src/routes.rs:2999-3002`, on fdo `InvalidArgs` (`os/pkgs/mosd/apid/src/routes.rs:385`) |
| `settings_io` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:3003-3006`, on fdo `IOError` (`os/pkgs/mosd/apid/src/routes.rs:386`) |
| `mosd_failed` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:3007-3010`, on fdo `Failed` (`os/pkgs/mosd/apid/src/routes.rs:387`) |
| `mosd_unreachable` | **yes**, 503 with `Retry-After` | `os/pkgs/mosd/apid/src/routes.rs:3036-3042`, and it is exhaustive over everything the three above do not name (`os/pkgs/mosd/apid/src/routes.rs:3032-3034`) |
...
(`os/pkgs/mosd/apid/src/routes.rs:3209-3224`, message at `os/pkgs/mosd/apid/src/routes.rs:3220`). So a settings value mosd
...
2. **Bus unavailable.** `mosd_unreachable`, **503**, with `Retry-After: 5`. 503
   ...
   `mosd_unreachable` answers 503 (`os/pkgs/mosd/apid/src/routes.rs:3037-3041`) with
   `Retry-After` attached (`os/pkgs/mosd/apid/src/routes.rs:3025-3030`). The cost this
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   answers the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:3287`), so **an appliance whose mosd is dead
...
middleware, `pub(crate) struct ApiSession;` (`os/pkgs/mosd/apid/src/routes.rs:3069`),
whose whole test is
...
(`os/pkgs/mosd/apid/src/routes.rs:3238`, predicate at `:325-330`). There is no bearer
...
exchange as success"* (`os/pkgs/mosd/apid/src/routes.rs:3066-3068`). **Unanswered:**
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   `Redirect::to("/login").into_response()` (`os/pkgs/mosd/apid/src/routes.rs:3283`), a
...
2. **Obtaining the credential means emulating three browser behaviours.**
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3940`) — axum's URL-encoded extractor, imported at
...
6. **There is exactly one credential, and it identifies a human.** The only
   ...
   the admin hash, read through `password_hash` (`os/pkgs/mosd/apid/src/routes.rs:3202-3207`); there is
   no second credential, no user table, and no reference to `access.device` in
...
- **The per-action confirm tokens are not CSRF tokens and must not be counted as
  ...
  and `"poweroff"` (`os/pkgs/mosd/apid/src/routes.rs:5318-5322`) and
  `TRANSIENT_CONFIRM_TOKEN` is the literal `"set-transient-password"`
  (`os/pkgs/mosd/apid/src/routes.rs:5572`). They are not secret, not per-session and not
  ...
  the checkbox has to be ticked as well"* (`os/pkgs/mosd/apid/src/routes.rs:5316-5317`). They stop a mis-click and a prefetch. Against a
...
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:3290`). Section 1.6 evidences the rest: no
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:3213`), reached when a mosd call
...
page *"The management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:3220`,
...
rendering `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:3209-3224`) for a value mosd
...
(`os/pkgs/mosd/apid/src/routes.rs:2983-2984`) and mapping `FDO_INVALID_ARGS`,
...
management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:3218-3228`). **The
...
- **The VLAN dot-path limit — since closed, and closed additively.**
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3370-3376`) — while `split_path` split on it
