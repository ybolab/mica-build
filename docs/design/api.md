...
(`os/pkgs/mosd/apid/src/routes.rs:881`), whose body runs from
`const STYLE: &str = "\` (`os/pkgs/mosd/apid/src/routes.rs:882`) to
`os/pkgs/mosd/apid/src/routes.rs:889`, injected into the page head as
`style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:900`). The crate
...
| `GET /healthz` | `:173` | `healthz` (`:729`) | neither — plain text | Returns the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:878`) — a liveness probe only: it proves the apid process is listening and checks nothing else, not mosd, not the settings tree, not the appliance | none |
...
neither and returns a bare string (`os/pkgs/mosd/apid/src/routes.rs:877-879`), and
fourteen are POSTs.
...
example `Form(form): Form<SetupForm>,` (`os/pkgs/mosd/apid/src/routes.rs:1379`),
...
on power (`os/pkgs/mosd/apid/src/routes.rs:2745`) and on the transient password
(`os/pkgs/mosd/apid/src/routes.rs:3598`).

**The second router.** `redirect_app()` at `os/pkgs/mosd/apid/src/routes.rs:756-760` is
the router served on the **HTTP** listener. It declares no routes at all — only
a fallback, `.fallback(redirect_to_https)` (`os/pkgs/mosd/apid/src/routes.rs:758`) —
...
(`os/pkgs/mosd/apid/src/routes.rs:762-776`). It carries no state beyond that port
...
(`os/pkgs/mosd/apid/src/routes.rs:800`), built at `os/pkgs/mosd/apid/src/routes.rs:793-804`.
...
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:1577`), set
at `os/pkgs/mosd/apid/src/routes.rs:1578-1582`. `POST /setup` does the same at the end
of the first-run wizard without a login step
(`os/pkgs/mosd/apid/src/routes.rs:1486-1491`). The store generates 16 random bytes from
...
through the `ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:730-752`), which
tests `session::cookie_from_headers(&parts.headers)`
(`os/pkgs/mosd/apid/src/routes.rs:739`) and nothing else.
...
(`os/pkgs/mosd/apid/src/routes.rs:816`) to `os/pkgs/mosd/apid/src/routes.rs:875`. It implements
...
1. `/healthz` and the declared `/api/` routes always pass —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:818`). The API routes answer for themselves in
...
2. A request carrying a **valid session cookie** passes without any bus call at
   all (`os/pkgs/mosd/apid/src/routes.rs:839-843`).
3. **Setup mode** — no admin password hash present — only `/setup` passes and
   everything else redirects there (`os/pkgs/mosd/apid/src/routes.rs:863-868`).
4. **Normal mode** — `/login` and `/setup` pass and everything else redirects
   to `/login` (`os/pkgs/mosd/apid/src/routes.rs:869-874`).
...
(`os/pkgs/mosd/apid/src/routes.rs:839-843`), so an authenticated request reaches its
handler without touching mosd, and the source states the reason and its
soundness argument at `os/pkgs/mosd/apid/src/routes.rs:822-838`: *"an authenticated
...
`state.api.get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:854`) is now paid
...
(`os/pkgs/mosd/apid/src/routes.rs:586`) — which is the one place where
...
(`os/pkgs/mosd/apid/src/routes.rs:900`), from a constant described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:881`). There is still no favicon route, no font and
...
the gate redirects it to `/login` (`os/pkgs/mosd/apid/src/routes.rs:874`), and
...
control (`os/pkgs/mosd/apid/src/routes.rs:2029`) and the install path. The consequence
...
1. **Line numbers throughout its section 2.1 no longer resolve to the routes
   ...
   at `os/pkgs/mosd/apid/src/routes.rs:1945`.
...
3. **"Six methods and one signal"** (`docs/research/mos-ui-inventory.md:367`) is
   ...
   `os/pkgs/mosd/apid/src/routes.rs:3608`.
...
renders **eight** links (`os/pkgs/mosd/apid/src/routes.rs:905-918`) plus the logout form
(`os/pkgs/mosd/apid/src/routes.rs:919-921`): Status, Network, Hostname, Power, SSH,
...
`"/api/versions"` (`os/pkgs/mosd/apid/openapi.json:354`). Its
`components.schemas` are exactly five: `"ApiError"`
(`os/pkgs/mosd/apid/openapi.json:379`), `"ApiErrorDetail"`
(`os/pkgs/mosd/apid/openapi.json:391`), `"ApiMeta"` (`os/pkgs/mosd/apid/openapi.json:421`),
`"ApiVersions"` (`os/pkgs/mosd/apid/openapi.json:446`) and `"ResourceValue"`
(`os/pkgs/mosd/apid/openapi.json:485`). The document is generated from the handlers
...
(`os/pkgs/mosd/apid/openapi.json:458-459`), so the dual-major recommendation below has
...
1. The gate already dispatches on `let path = request.uri().path();`
   (`os/pkgs/mosd/apid/src/routes.rs:817`). A version check is the same string operation
...
(`os/pkgs/mosd/apid/src/routes.rs:772`) — that is not the objection. The
...
(`os/pkgs/mosd/apid/src/routes.rs:863-868`). An authenticated probe cannot answer the
...
bus call at `os/pkgs/mosd/apid/src/routes.rs:818`. The cost is an unauthenticated
...
(`os/pkgs/mosd/apid/src/routes.rs:586`). Both answer `ResourceValue`
...
to write"* (`os/pkgs/mosd/apid/src/routes.rs:604`) — the key lives in a file on
...
`/api/versions` (`os/pkgs/mosd/apid/openapi.json:8-375`). Neither collection
...
- **A key holding a dot is addressed with a quoted segment.** apid accepts an
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:961-967`) — and until PLAN-022 M2 the
...
nobody asked to delete"* (`os/pkgs/mosd/apid/src/routes.rs:3647-3651`). A `DELETE` whose
...
the file (`os/pkgs/mosd/apid/src/routes.rs:2956-2958`). So the difference is the quality
...
| SSH keys | the authorized-keys collection above | `access.ssh.authorizedKeys` | **every key is a root key** (`docs/design/access.md` §4.1, `os/pkgs/mosd/apid/src/routes.rs:2833`); the API response must carry that sentence in a `notice` field for the same reason the pane must carry it |
...
(`os/pkgs/mosd/apid/openapi.json:8-375`) — `"/api/v1/meta"`
...
(`os/pkgs/mosd/apid/openapi.json:354`) — and the router declares nothing else under the
...
(`os/pkgs/mosd/apid/src/routes.rs:2745`, and `TRANSIENT_CONFIRM_TOKEN` at `os/pkgs/mosd/apid/src/routes.rs:2837`) does
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:2765-2778`). 202
...
- **`GET /healthz`** — declared at `os/pkgs/mosd/apid/src/routes.rs:195` and answering
  at `os/pkgs/mosd/apid/src/routes.rs:877-879`. It keeps its path, its unauthenticated exemption (`os/pkgs/mosd/apid/src/routes.rs:818`) and
  its literal `ok` body,
  ...
  **But it answers a narrower question than its name suggests, and the API must
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:818`), so it answers `ok` on an appliance whose
...
- **A password change — [implemented], on both surfaces.** This bullet once
  ...
  (`change_password`, `os/pkgs/mosd/apid/src/routes.rs:1653-1712`): the
...
(`os/pkgs/mosd/apid/src/routes.rs:3668-3674`, via `ssh_error` at `os/pkgs/mosd/apid/src/routes.rs:2971-2980`). For a
...
it as `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:382-384`), and every
...
(`os/pkgs/mosd/apid/src/routes.rs:702-707`). The recommendation this section makes —
...
(`os/pkgs/mosd/apid/src/routes.rs:665-707`), which matches on the concrete
...
translated and the message is not"* (`os/pkgs/mosd/apid/src/routes.rs:656`).
...
|---|---|---|---|
| `error` | the only top-level member | `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:382-384`) | none |
| `code` | *"stable machine token; an OPEN set"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:400-401`), required (`:237-241`) | none in shape. The **open set** is stated only in the description (`os/pkgs/mosd/apid/openapi.json:402`); nothing in the schema expresses it, so a generated client learns the rule only if a human reads the prose |
| `message` | *"human-readable; not for matching on"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:404-406`), required | none |
| `source` | `"apid" \| "mosd"`, an enum of exactly two | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:415-416`), required, **no `enum`** | **a real difference.** The Rust type admits only two values, because `ApiError` is constructed by exactly two constructors, `ApiError::apid`, which passes `Self::new(code, message, "apid")`
...
| `code` | Ships? | Where |
|---|---|---|
| `settings_not_found` | **yes**, 404 | `os/pkgs/mosd/apid/src/routes.rs:686-689`, on `com.mos.mosd1.Error.NotFound` |
| `settings_read_only` | **yes**, 409 | `os/pkgs/mosd/apid/src/routes.rs:690-693`, on `com.mos.mosd1.Error.ReadOnly` |
| `not_authenticated` | **yes**, with a different credential | `os/pkgs/mosd/apid/src/routes.rs:747`. It is raised by the session-cookie extractor (`os/pkgs/mosd/apid/src/routes.rs:744-750`), not by a bearer token: §3.2's token does not exist, so the shipped 401 means *"no session cookie, or one that does not verify"* (`os/pkgs/mosd/apid/src/routes.rs:748`) |
...
| `settings_rejected` | **yes**, 422 | `os/pkgs/mosd/apid/src/routes.rs:683-686`, on fdo `InvalidArgs` (`os/pkgs/mosd/apid/src/routes.rs:287`) |
...
(`os/pkgs/mosd/apid/src/routes.rs:789-804`, message at `os/pkgs/mosd/apid/src/routes.rs:800`). So a settings value mosd
...
(`os/pkgs/mosd/apid/src/routes.rs:2522-2609`), and `write_key_list` does the same
(`os/pkgs/mosd/apid/src/routes.rs:2964-2966`). The VLAN example above is exactly this — a rejection that reads
as an outage.
...
text in an error box (`os/pkgs/mosd/apid/src/routes.rs:942-944`, `:1268`, `:1288`) — without
widening it.
...
2. **Bus unavailable.** `mosd_unreachable`, **503**, with `Retry-After: 5`. 503
   ...
   `mosd_unreachable` answers 503 (`os/pkgs/mosd/apid/src/routes.rs:714-718`) with
   `Retry-After` attached (`os/pkgs/mosd/apid/src/routes.rs:702-707`). The cost this
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:789-804`), so one appliance reports one
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   `/healthz` returns before any check (`os/pkgs/mosd/apid/src/routes.rs:818`) and
   answers the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:878`), so **an appliance whose mosd is dead
   ...
   off above the bus call (`os/pkgs/mosd/apid/src/routes.rs:818`), and a request with a
   valid session cookie is handed off above it too
   (`os/pkgs/mosd/apid/src/routes.rs:839-843`), so no API route depends on the gate's
...
middleware, `pub(crate) struct ApiSession;` (`os/pkgs/mosd/apid/src/routes.rs:730`),
whose whole test is
`session::cookie_from_headers(&parts.headers)` verified against the store
(`os/pkgs/mosd/apid/src/routes.rs:739-740`); its rejection is §2.4's envelope with a
401 rather than the gate's HTML redirect
(`os/pkgs/mosd/apid/src/routes.rs:744-750`). The gate hands the declared `/api` routes
off to it with `is_declared_api_route(path)`
(`os/pkgs/mosd/apid/src/routes.rs:818`, predicate at `:295-300`). There is no bearer
...
exchange as success"* (`os/pkgs/mosd/apid/src/routes.rs:727-729`). **Unanswered:**
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   `Redirect::to("/login").into_response()` (`os/pkgs/mosd/apid/src/routes.rs:874`), a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:869-871`) and which returns **200 OK** with an HTML
   form (`os/pkgs/mosd/apid/src/routes.rs:1509-1526`). A script that checks the status
   ...
   instead (`os/pkgs/mosd/apid/src/routes.rs:744-750`). Every other path on the daemon
   still redirects.
2. **Obtaining the credential means emulating three browser behaviours.**
   `POST /login` takes `Form(form): Form<LoginForm>,`
   (`os/pkgs/mosd/apid/src/routes.rs:1531`) — axum's URL-encoded extractor, imported at
   `os/pkgs/mosd/apid/src/routes.rs:19`, not JSON — answers **302 to `/`**
   (`os/pkgs/mosd/apid/src/routes.rs:1580`), and delivers the credential in a
   `Set-Cookie` header (`os/pkgs/mosd/apid/src/routes.rs:1579`, value built at
...
6. **There is exactly one credential, and it identifies a human.** The only
   ...
   the admin hash, read through `password_hash` (`os/pkgs/mosd/apid/src/routes.rs:782-787`); there is
   no second credential, no user table, and no reference to `access.device` in
...
7. **A misconfigured script locks the operator out, repeatedly.** The login
   ...
   *"the appliance has one admin password, so per-client tracking buys nothing
   ...
   `os/pkgs/mosd/apid/src/routes.rs:1544-1554`). A script retrying with a
...
(`os/pkgs/mosd/apid/openapi.json:8-375`).
...
`ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:730-752`) against the same
...
4. **It costs zero extra bus round trips.** The gate already calls
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:866`) and `apiTokens` is a child of `access`, so
...
- Identity is the `id`, never a list position, for the reason recorded at
  `os/pkgs/mosd/apid/src/routes.rs:3660-3664`: an index is meaningful only against the
...
2. **§3.1 item 7 changes meaning.** The `auth.rs:53-54` comment's reasoning —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1551-1560`). Bearer verification is **not** rate
...
(`os/pkgs/mosd/apid/src/routes.rs:744-750`) and a redirect to `/login` everywhere else
(`os/pkgs/mosd/apid/src/routes.rs:874`) — and the CORS argument below, which depends on
...
2. **A stolen token is full management access, indefinitely.** There is no
   ...
   `os/pkgs/mosd/apid/src/routes.rs:2833`) and is then no longer needed. Detection is not
...
5. **A same-origin page mints a permanent token with the operator's cookie.**
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:2833`). But a minted token survives deactivating
...
pane is tested to carry, `os/pkgs/mosd/apid/src/routes.rs:2833`); apid runs as root
...
- **Under this proposal the API path has no CSRF exposure**, because a bearer
  ...
  **None of that is true of the shipped API**, whose credential *is* the cookie
  (`os/pkgs/mosd/apid/src/routes.rs:739-740`), so today the four `/api` routes sit
...
- **The per-action confirm tokens are not CSRF tokens and must not be counted as
  ...
  and `"poweroff"` (`os/pkgs/mosd/apid/src/routes.rs:2665-2669`) and
  `TRANSIENT_CONFIRM_TOKEN` is the literal `"set-transient-password"`
  (`os/pkgs/mosd/apid/src/routes.rs:2837`). They are not secret, not per-session and not
  ...
  the checkbox has to be ticked as well"* (`os/pkgs/mosd/apid/src/routes.rs:2663-2664`). They stop a mis-click and a prefetch. Against a
...
(`os/pkgs/mosd/apid/src/routes.rs:839-845`), so an authenticated flood no longer
...
(`os/pkgs/mosd/apid/src/routes.rs:818`) and its handler makes no mosd call
...
- **`/var/lib/mos/settings.toml`** (`os/pkgs/mosd/mosd-settings/src/store.rs:67`) — the
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:786-793`) and every authorized SSH key.
...
§6.3's reserved prefix, `builtin_home` in `os/pkgs/mosd/apid/src/routes.rs:965`. Its
...
(`os/pkgs/mosd/apid/src/routes.rs:903-909`), described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:881`). Section 1.6 evidences the rest: no
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:793`), reached when a mosd call
  ...
  *before* dispatch — so at the moment that page is on screen, `/builtin/` is
  ...
  page (`os/pkgs/mosd/apid/src/routes.rs:774`), the navigation on every built-in pane
...
page *"The management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:800`,
...
1. **A signature and the upload credential authorise the same blast radius.** A
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:2833`). §3.2 states there are **no scopes in
...
Items (iv) and (v) are already shipped — `os/pkgs/mosd/apid/src/routes.rs:2641`, `:1776`,
...
rendering `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:789-804`) for a value mosd
...
the settings write fails (`os/pkgs/mosd/apid/src/routes.rs:2610-2616`), and `write_key_list`
...
(`os/pkgs/mosd/apid/src/routes.rs:667-668`) and mapping `FDO_INVALID_ARGS`,
...
(`os/pkgs/mosd/apid/src/routes.rs:866`) the whole appliance answers the 502 page *"The
management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:798-808`). **The
...
path answers 422 (`os/pkgs/mosd/apid/src/routes.rs:3668-3674`). The HTML path is not
changed.
...
setup mode (`os/pkgs/mosd/apid/src/routes.rs:870-875`) — and it returns a token (§2.3),
...
HTML path answers **502** (`os/pkgs/mosd/apid/src/routes.rs:798-808`), so one appliance
...
- **The VLAN dot-path limit — since closed, and closed additively.**
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:961-967`) — while `split_path` split on it
