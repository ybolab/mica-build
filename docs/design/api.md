...
(`os/pkgs/mosd/apid/src/routes.rs:1056`), whose body runs from
`const STYLE: &str = "\` (`os/pkgs/mosd/apid/src/routes.rs:1057`) to
`os/pkgs/mosd/apid/src/routes.rs:1064`, injected into the page head as
`style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:1075`). The crate
...
`pub fn app(state: AppState) -> Router {` (`os/pkgs/mosd/apid/src/routes.rs:130`) and
runs to `.with_state(state)` (`os/pkgs/mosd/apid/src/routes.rs:211`). Its declaration
...
| `POST /power/reboot` | `:158` | `power_reboot` (`:1703`) → `power_submit` (`:1666`) | HTML form POST | 422 when the confirm token does not match (`:1667-1678`); otherwise **202 Accepted** (`:1696-1700`) with the D-Bus call spawned on a detached task so the response goes out first (`:1687-1695`) | a write to the `/Actions/reboot` item over the `com.mos.Item1` proxy (`os/pkgs/mosd/apid/src/bus_client.rs:48`, path at `:54`, call at `:260-262`) |
...
| `GET /healthz` | `:173` | `healthz` (`:729`) | neither — plain text | Returns the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:1053`) — a liveness probe only: it proves the apid process is listening and checks nothing else, not mosd, not the settings tree, not the appliance | none |
...
| every method on `/api/` | `:184` (`.route("/api/", any(api_not_found))`) | `api_not_found` (`:196`) | JSON | 404 in §2.4's envelope. Not redundant with the nest: *"`nest` claims `/api`, `/api/x` and `/api/x/y`, and not `/api/`; the difference is a request that begins `/api/` reaching the asset router"* (`os/pkgs/mosd/apid/src/routes.rs:201-204`) | none |
...
**The `/api` subtree.** `api_router()` (`os/pkgs/mosd/apid/src/routes.rs:327-383`) is
nested under the prefix `const API: &str = "/api";`
(`os/pkgs/mosd/apid/src/routes.rs:235`). Each route is declared from the same constant
...
one string and cannot disagree"* (`os/pkgs/mosd/apid/src/routes.rs:324-326`).
...
(`os/pkgs/mosd/apid/src/routes.rs:272`) while OpenAPI's is
`const V1_SETTINGS_DOC: &str = "/v1/settings/{path}";`
(`os/pkgs/mosd/apid/src/routes.rs:273`), and the source states why: *"axum names a
...
(`os/pkgs/mosd/apid/src/routes.rs:266-269`). Every response under the prefix is built
by one helper that sets `no-store` on success as well as on failure
(`os/pkgs/mosd/apid/src/routes.rs:442-449`).

**Kinds, counted, at this commit.** The outer router declares **twenty-one**
`.route()` calls (`os/pkgs/mosd/apid/src/routes.rs:137`, `:161-167`, `:171-173`,
...
neither and returns a bare string (`os/pkgs/mosd/apid/src/routes.rs:1052-1054`), and
fourteen are POSTs.
...
(`os/pkgs/mosd/apid/src/routes.rs:446`), as does `api_not_found`. **No route accepts a
...
example `Form(form): Form<SetupForm>,` (`os/pkgs/mosd/apid/src/routes.rs:1554`),
...
(`os/pkgs/mosd/apid/src/routes.rs:178-180`) — or for any of the four SSH mutations
(`os/pkgs/mosd/apid/src/routes.rs:184-186`), and the rule has since been extended to
the bundle deactivation control (`os/pkgs/mosd/apid/src/routes.rs:159-161`). And there
...
on power (`os/pkgs/mosd/apid/src/routes.rs:2921`) and on the transient password
(`os/pkgs/mosd/apid/src/routes.rs:3774`).

**The second router.** `redirect_app()` at `os/pkgs/mosd/apid/src/routes.rs:931-935` is
the router served on the **HTTP** listener. It declares no routes at all — only
a fallback, `.fallback(redirect_to_https)` (`os/pkgs/mosd/apid/src/routes.rs:933`) —
...
(`os/pkgs/mosd/apid/src/routes.rs:937-951`). It carries no state beyond that port
...
`com.mos.mosd1` trait (`os/pkgs/mosd/apid/src/bus_client.rs:27-40`) declares four; the
power actions are the fifth, `set_value` on the `com.mos.Item1` trait
(`os/pkgs/mosd/apid/src/bus_client.rs:48-51`), written to one action item per verb and
so listed once per verb below:

| Proxy method | Line | mosd's implementation | Called from |
|---|---|---|---|
| `get_settings(path) -> String` | `bus_client.rs:21` | `os/pkgs/mosd/mosd/src/bus.rs:610` | the gate (`routes.rs:697`) and the `/`, `/setup`, `/login`, `/network`, `/hostname`, `/ssh`, `/containers`, `/mqtt` handlers |
| `set_settings(path, value_json)` | `bus_client.rs:22` | `os/pkgs/mosd/mosd/src/bus.rs:619` | `/setup`, `/network`, `/hostname`, `/ssh/enable`, `/ssh/keys/*`, `/containers/enable`, `/mqtt/enable` |
| `get_state(path) -> String` | `bus_client.rs:23` | `os/pkgs/mosd/mosd/src/bus.rs:649` | four paths: `GetState("network")` (`routes.rs:1215`), `GetState("sshd")` (`routes.rs:1856`), `GetState("container")` (`routes.rs:2079`) and `GetState("mqtt")` (`routes.rs:2337`) |
...
| `set_transient_root_password(password)` | `bus_client.rs:24` | `os/pkgs/mosd/mosd/src/bus.rs:826` | `POST /ssh/password` (`routes.rs:2530`) |
...
(`os/pkgs/mosd/mosd/src/bus.rs:673`) — and `ForgetService` (`:711`), `Reboot` (`:725`),
...
(`os/pkgs/mosd/mosd/src/bus.rs:633-635`, declared at `:817-818`), and the proxy declares
...
`os/pkgs/mosd/apid/src/bus_client.rs:166-178` and dropped at `:187-190`. A failed call
renders a 502 page reading
*"The management daemon is unavailable."*
(`os/pkgs/mosd/apid/src/routes.rs:975`), built at `os/pkgs/mosd/apid/src/routes.rs:968-979`.
...
(`os/pkgs/mosd/apid/src/routes.rs:957-962`), typed as
...
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:1752`), set
at `os/pkgs/mosd/apid/src/routes.rs:1753-1757`. `POST /setup` does the same at the end
of the first-run wizard without a login step
(`os/pkgs/mosd/apid/src/routes.rs:1661-1666`). The store generates 16 random bytes from
...
through the `ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:905-927`), which
tests `session::cookie_from_headers(&parts.headers)`
(`os/pkgs/mosd/apid/src/routes.rs:914`) and nothing else.

**The gate.** One middleware, layered over the whole HTTPS router with
`.layer(middleware::from_fn_with_state(state.clone(), gate))`
(`os/pkgs/mosd/apid/src/routes.rs:210`), running from
`async fn gate(State(state): State<AppState>, request: Request, next: Next) -> Response {`
(`os/pkgs/mosd/apid/src/routes.rs:991`) to `os/pkgs/mosd/apid/src/routes.rs:1050`. It implements
...
(`os/pkgs/mosd/apid/src/routes.rs:1014-1018`), so an authenticated request reaches its
handler without touching mosd, and the source states the reason and its
soundness argument at `os/pkgs/mosd/apid/src/routes.rs:997-1013`: *"an authenticated
...
`state.api.get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:1029`) is now paid
...
time (`os/pkgs/mosd/apid/src/routes.rs:580`) rather than copying it. It is **read-only
...
the SSH pane reads the whole key list (`os/pkgs/mosd/apid/src/routes.rs:3126-3128`),
edits it in memory (`os/pkgs/mosd/apid/src/routes.rs:3807-3811`), and writes the whole
list back through `write_key_list` (`os/pkgs/mosd/apid/src/routes.rs:3131-3144`).
...
(`os/pkgs/mosd/apid/src/routes.rs:760`) — which is the one place where
...
2. **A file-serving service is constructed, as the router's fallback.**
   ...
   now ends in `.fallback(serve::fallback)` (`os/pkgs/mosd/apid/src/routes.rs:209`),
...
3. **Nothing is embedded in the binary, and that is still deliberate.**
   ...
   `os/pkgs/mosd/apid/src/tests.rs:1779` reads the committed `openapi.json` so a test
...
4. **The crate still ships no asset of its own; the assets come from
   ...
   `bundles: Arc::new(Store::at_default()),` (`os/pkgs/mosd/apid/src/routes.rs:77`).
...
(`os/pkgs/mosd/apid/src/routes.rs:1075`), from a constant described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:1056`). There is still no favicon route, no font and
no image declared in `os/pkgs/mosd/apid/src/routes.rs:130-211`. A request for
`/favicon.ico` no longer ends at the gate, though: it is unauthenticated, so
the gate redirects it to `/login` (`os/pkgs/mosd/apid/src/routes.rs:1049`), and
...
control (`os/pkgs/mosd/apid/src/routes.rs:2205`) and the install path. The consequence
...
1. **Line numbers throughout its section 2.1 no longer resolve to the routes
   ...
   `f7cb5ba` the route is `os/pkgs/mosd/apid/src/routes.rs:137` and the handler is
   `serve::root`, at `os/pkgs/mosd/apid/src/assets/serve.rs:57`, with the built-in branch
   at `os/pkgs/mosd/apid/src/routes.rs:2121`.
2. **Its route table is missing thirteen routes.** The five SSH routes it
   ...
   `os/pkgs/mosd/apid/src/routes.rs:183-190`, and eight more have landed since: the
   four `/containers` and `/mqtt` pairs (`os/pkgs/mosd/apid/src/routes.rs:191-194`) and
   the four `/api` routes (`os/pkgs/mosd/apid/src/routes.rs:329-333`). Its own section
...
renders **eight** links (`os/pkgs/mosd/apid/src/routes.rs:1080-1093`) plus the logout form
(`os/pkgs/mosd/apid/src/routes.rs:1094-1096`): Status, Network, Hostname, Power, SSH,
...
paths**, all `GET`: `"/api/v1/meta"` (`os/pkgs/mosd/apid/openapi.json:228`),
`"/api/v1/settings/{path}"` (`os/pkgs/mosd/apid/openapi.json:270`),
`"/api/v1/state/{path}"` (`os/pkgs/mosd/apid/openapi.json:363`) and
`"/api/versions"` (`os/pkgs/mosd/apid/openapi.json:446`). Its
`components.schemas` are exactly five: `"ApiError"`
(`os/pkgs/mosd/apid/openapi.json:481`), `"ApiErrorDetail"`
(`os/pkgs/mosd/apid/openapi.json:493`), `"ApiMeta"` (`os/pkgs/mosd/apid/openapi.json:557`),
`"ApiVersions"` (`os/pkgs/mosd/apid/openapi.json:582`) and `"ResourceValue"`
(`os/pkgs/mosd/apid/openapi.json:621`). The document is generated from the handlers
...
(`os/pkgs/mosd/apid/src/routes.rs:235`), nested as `.nest(API, api_router())`
(`os/pkgs/mosd/apid/src/routes.rs:206`), and the version is the segment immediately
after it — `const V1_META_PATH: &str = "/v1/meta";`
(`os/pkgs/mosd/apid/src/routes.rs:237`). `GET /api/versions` answers unauthenticated
from `const SERVED_VERSIONS: [&str; 1] = ["v1"];`
(`os/pkgs/mosd/apid/src/routes.rs:316`) and `const CURRENT_VERSION: &str = "v1";`
(`os/pkgs/mosd/apid/src/routes.rs:319`), and `GET /api/v1/meta` answers authenticated
with `ApiMeta` whose `settingsSchemaVersion` is read live from
`settings_schema_version: mosd_settings::SCHEMA_VERSION,`
(`os/pkgs/mosd/apid/src/routes.rs:580`) rather than copied. The served set is an array
on the wire, `"versions"` typed `"type": "array"`
(`os/pkgs/mosd/apid/openapi.json:594-595`), so the dual-major recommendation below has
...
(`os/pkgs/mosd/apid/src/routes.rs:207`) claims the one spelling the nest does not, and
...
(`os/pkgs/mosd/apid/src/routes.rs:198-200`).
...
1. The gate already dispatches on `let path = request.uri().path();`
   (`os/pkgs/mosd/apid/src/routes.rs:992`). A version check is the same string operation
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:210`). This is now measurable rather than
   predicted: the shipped gate tests membership with `is_declared_api_route`
   (`os/pkgs/mosd/apid/src/routes.rs:392-423`), a pure string operation over the same
   constants the router declares.
2. axum registers routes by path (`os/pkgs/mosd/apid/src/routes.rs:130-211`). A second
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:327-383`) is the first of those routers and shows
   the shape.
...
(`os/pkgs/mosd/apid/src/routes.rs:947`) — that is not the objection. The
...
Both rows ship. The first is declared at `os/pkgs/mosd/apid/src/routes.rs:329` and
...
exactly one source"* (`os/pkgs/mosd/apid/src/routes.rs:561-563`). The API version is the
...
(`os/pkgs/mosd/apid/src/routes.rs:1038-1043`). An authenticated probe cannot answer the
...
so"* (`os/pkgs/mosd/apid/src/routes.rs:533-536`); the gate hands the route off above its own
bus call at `os/pkgs/mosd/apid/src/routes.rs:993`. The cost is an unauthenticated
...
(`os/pkgs/mosd/apid/src/routes.rs:993`, `:756-758`), so this is one more bit on a
...
(`os/pkgs/mosd/apid/src/routes.rs:272`) declared at `os/pkgs/mosd/apid/src/routes.rs:332` and
handled by `api_v1_settings` (`:474`), whose whole body is
`resource_response(state.api.get_settings(&path).await, &path)`
(`os/pkgs/mosd/apid/src/routes.rs:732`) — the dot-path passthrough this section asks
for, with no second model beside it. The live-state root is the same shape,
`const V1_STATE_ROUTE: &str = "/v1/state/{*path}";`
(`os/pkgs/mosd/apid/src/routes.rs:275`) at `:284` and `:520`, ending in
`resource_response(state.api.get_state(&path).await, &path)`
(`os/pkgs/mosd/apid/src/routes.rs:760`). Both answer `ResourceValue`
(`os/pkgs/mosd/apid/src/routes.rs:704`), which carries a
`serde(transparent)` attribute (`os/pkgs/mosd/apid/src/routes.rs:703`) so the body is
...
(`os/pkgs/mosd/apid/openapi.json:103`), a `POST` that draws a WireGuard interface
...
to write"* (`os/pkgs/mosd/apid/src/routes.rs:778`) — the key lives in a file on
...
`/api/versions` (`os/pkgs/mosd/apid/openapi.json:8-477`). Neither collection
...
(`os/pkgs/mosd/apid/src/routes.rs:697-699`). And `GET /api/v1/state/{path}` today
...
impl, `os/pkgs/mosd/mosd/src/bus.rs:607-892`), and different lifetimes (the settings tree
...
- **No array indexing.** The dot-path syntax has none; the model says a list is
  *"Written as a whole JSON array through the dot-path API"*
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3807-3812`). Two clients doing that concurrently
...
- **A key holding a dot is addressed with a quoted segment.** apid accepts an
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:1136-1142`) — and until PLAN-022 M2 the
...
nobody asked to delete"* (`os/pkgs/mosd/apid/src/routes.rs:3823-3827`). A `DELETE` whose
...
the file (`os/pkgs/mosd/apid/src/routes.rs:3132-3134`). So the difference is the quality
...
| SSH keys | the authorized-keys collection above | `access.ssh.authorizedKeys` | **every key is a root key** (`docs/design/access.md` §4.1, `os/pkgs/mosd/apid/src/routes.rs:3009`); the API response must carry that sentence in a `notice` field for the same reason the pane must carry it |
...
(`os/pkgs/mosd/apid/openapi.json:8-477`) — `"/api/v1/meta"`
(`os/pkgs/mosd/apid/openapi.json:228`), `"/api/v1/settings/{path}"`
(`os/pkgs/mosd/apid/openapi.json:270`), `"/api/v1/state/{path}"`
(`os/pkgs/mosd/apid/openapi.json:363`) and `"/api/versions"`
(`os/pkgs/mosd/apid/openapi.json:446`) — and the router declares nothing else under the
prefix beyond a fallback (`os/pkgs/mosd/apid/src/routes.rs:327-383`). There is no
...
(`os/pkgs/mosd/apid/src/routes.rs:178-180`) or for any of the four SSH mutations
(`os/pkgs/mosd/apid/src/routes.rs:184-186`), *"so a browser prefetch, a crawler or a mis-clicked link cannot power
the appliance off"*.

The **confirmation token** that guards the three form posts today
(`os/pkgs/mosd/apid/src/routes.rs:2921`, and `TRANSIENT_CONFIRM_TOKEN` at `os/pkgs/mosd/apid/src/routes.rs:3013`) does
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:2941-2954`). 202
...
- **`POST /login` and `POST /logout`.** A session is a browser mechanism (§3.1).
  ...
  mislead.
- **`GET /healthz`** — declared at `os/pkgs/mosd/apid/src/routes.rs:195` and answering
  at `os/pkgs/mosd/apid/src/routes.rs:1052-1054`. It keeps its path, its unauthenticated exemption (`os/pkgs/mosd/apid/src/routes.rs:993`) and
  its literal `ok` body,
  ...
  **But it answers a narrower question than its name suggests, and the API must
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:993`), so it answers `ok` on an appliance whose
...
- **A password change — [implemented], on both surfaces.** This bullet once
  ...
  `os/pkgs/mosd/apid/src/routes.rs:169`; the API route is
  `POST /api/v1/actions/change-password`, declared at
  `os/pkgs/mosd/apid/src/routes.rs:334` (handler at `:1434-1489`), `POST` only
  ...
  (`change_password`, `os/pkgs/mosd/apid/src/routes.rs:1828-1887`): the
...
(`os/pkgs/mosd/apid/src/routes.rs:3844-3850`, via `ssh_error` at `os/pkgs/mosd/apid/src/routes.rs:3147-3156`). For a
...
(`os/pkgs/mosd/apid/src/routes.rs:452-454`, payload at `:334-352`), the schema records
it as `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:484-486`), and every
failure under the prefix is built through it. `Retry-After` ships on exactly
one class: `const RETRY_AFTER_SECONDS: &str = "5";`
(`os/pkgs/mosd/apid/src/routes.rs:312`), attached only when the status is 503
(`os/pkgs/mosd/apid/src/routes.rs:877-882`). The recommendation this section makes —
...
(`os/pkgs/mosd/apid/src/routes.rs:840-882`), which matches on the concrete
`zbus::Error::MethodError` before the conversion flattens it and maps the five
error names declared at `os/pkgs/mosd/apid/src/routes.rs:307-309` — two
...
translated and the message is not"* (`os/pkgs/mosd/apid/src/routes.rs:831`).
...
|---|---|---|---|
| `error` | the only top-level member | `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:484-486`) | none |
| `code` | *"stable machine token; an OPEN set"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:502-503`), required (`:237-241`) | none in shape. The **open set** is stated only in the description (`os/pkgs/mosd/apid/openapi.json:504`); nothing in the schema expresses it, so a generated client learns the rule only if a human reads the prose |
| `message` | *"human-readable; not for matching on"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:506-508`), required | none |
| `source` | `"apid" \| "mosd"`, an enum of exactly two | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:517-518`), required, **no `enum`** | **a real difference.** The Rust type admits only two values, because `ApiError` is constructed by exactly two constructors, `ApiError::apid`, which passes `Self::new(code, message, "apid")`
(`os/pkgs/mosd/apid/src/routes.rs:479`), and `ApiError::mosd`, which passes
`Self::new(code, message, "mosd")` (`os/pkgs/mosd/apid/src/routes.rs:484`). The published contract does not say so, so a client generated from the document gets an unconstrained string and cannot exhaustively match on it |
| `path` | OPTIONAL, present only when the failure names a dot-path | `"type": [ "string", "null" ]` (`os/pkgs/mosd/apid/openapi.json:510-514`), **absent from `required`** | **a real difference, in the opposite direction.** The schema admits an explicit `null`; the wire never carries one, because the field carries a `serde(skip_serializing_if = "Option::is_none")` attribute (`os/pkgs/mosd/apid/src/routes.rs:472`) and is therefore omitted rather than nulled — which is the behaviour this section asks for and the source's own doc comment defends (`os/pkgs/mosd/apid/src/routes.rs:467-471`). The schema is wider than the implementation, so a client that handles `null` is handling a case the device does not produce |
...
| `code` | Ships? | Where |
|---|---|---|
| `settings_not_found` | **yes**, 404 | `os/pkgs/mosd/apid/src/routes.rs:861-864`, on `com.mos.mosd1.Error.NotFound` |
| `settings_read_only` | **yes**, 409 | `os/pkgs/mosd/apid/src/routes.rs:865-868`, on `com.mos.mosd1.Error.ReadOnly` |
| `not_authenticated` | **yes**, with a different credential | `os/pkgs/mosd/apid/src/routes.rs:922`. It is raised by the session-cookie extractor (`os/pkgs/mosd/apid/src/routes.rs:919-925`), not by a bearer token: §3.2's token does not exist, so the shipped 401 means *"no session cookie, or one that does not verify"* (`os/pkgs/mosd/apid/src/routes.rs:923`) |
| `not_found` | **yes** | `os/pkgs/mosd/apid/src/routes.rs:222`, from the reserved subtree's fallback |
...
| `settings_rejected` | **yes**, 422 | `os/pkgs/mosd/apid/src/routes.rs:858-861`, on fdo `InvalidArgs` (`os/pkgs/mosd/apid/src/routes.rs:307`) |
| `settings_io` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:862-865`, on fdo `IOError` (`os/pkgs/mosd/apid/src/routes.rs:308`) |
| `mosd_failed` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:866-869`, on fdo `Failed` (`os/pkgs/mosd/apid/src/routes.rs:309`) |
| `mosd_unreachable` | **yes**, 503 with `Retry-After` | `os/pkgs/mosd/apid/src/routes.rs:888-894`, and it is exhaustive over everything the three above do not name (`os/pkgs/mosd/apid/src/routes.rs:884-886`) |
...
(`os/pkgs/mosd/apid/src/routes.rs:964-979`, message at `os/pkgs/mosd/apid/src/routes.rs:975`). So a settings value mosd
...
(`os/pkgs/mosd/apid/src/routes.rs:2698-2785`), and `write_key_list` does the same
(`os/pkgs/mosd/apid/src/routes.rs:3140-3142`). The VLAN example above is exactly this — a rejection that reads
as an outage.
...
text in an error box (`os/pkgs/mosd/apid/src/routes.rs:1117-1119`, `:1268`, `:1288`) — without
widening it.
...
2. **Bus unavailable.** `mosd_unreachable`, **503**, with `Retry-After: 5`. 503
   ...
   `mosd_unreachable` answers 503 (`os/pkgs/mosd/apid/src/routes.rs:889-893`) with
   `Retry-After` attached (`os/pkgs/mosd/apid/src/routes.rs:877-882`). The cost this
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:964-979`), so one appliance reports one
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   `/healthz` returns before any check (`os/pkgs/mosd/apid/src/routes.rs:993`) and
   answers the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:1053`), so **an appliance whose mosd is dead
   ...
   *"whole seconds since boot, a bare JSON number"* everywhere else it appears.
   ...
   off above the bus call (`os/pkgs/mosd/apid/src/routes.rs:993`), and a request with a
   valid session cookie is handed off above it too
   (`os/pkgs/mosd/apid/src/routes.rs:1014-1018`), so no API route depends on the gate's
...
middleware, `pub(crate) struct ApiSession;` (`os/pkgs/mosd/apid/src/routes.rs:905`),
whose whole test is
`session::cookie_from_headers(&parts.headers)` verified against the store
(`os/pkgs/mosd/apid/src/routes.rs:914-915`); its rejection is §2.4's envelope with a
401 rather than the gate's HTML redirect
(`os/pkgs/mosd/apid/src/routes.rs:919-925`). The gate hands the declared `/api` routes
off to it with `is_declared_api_route(path)`
(`os/pkgs/mosd/apid/src/routes.rs:993`, predicate at `:295-300`). There is no bearer
...
exchange as success"* (`os/pkgs/mosd/apid/src/routes.rs:902-904`). **Unanswered:**
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   `Redirect::to("/login").into_response()` (`os/pkgs/mosd/apid/src/routes.rs:1049`), a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1044-1046`) and which returns **200 OK** with an HTML
   form (`os/pkgs/mosd/apid/src/routes.rs:1684-1701`). A script that checks the status
   ...
   instead (`os/pkgs/mosd/apid/src/routes.rs:919-925`). Every other path on the daemon
   still redirects.
2. **Obtaining the credential means emulating three browser behaviours.**
   `POST /login` takes `Form(form): Form<LoginForm>,`
   (`os/pkgs/mosd/apid/src/routes.rs:1706`) — axum's URL-encoded extractor, imported at
   `os/pkgs/mosd/apid/src/routes.rs:19`, not JSON — answers **302 to `/`**
   (`os/pkgs/mosd/apid/src/routes.rs:1755`), and delivers the credential in a
   `Set-Cookie` header (`os/pkgs/mosd/apid/src/routes.rs:1754`, value built at
...
6. **There is exactly one credential, and it identifies a human.** The only
   ...
   the admin hash, read through `password_hash` (`os/pkgs/mosd/apid/src/routes.rs:957-962`); there is
   no second credential, no user table, and no reference to `access.device` in
...
7. **A misconfigured script locks the operator out, repeatedly.** The login
   ...
   *"the appliance has one admin password, so per-client tracking buys nothing
   ...
   `os/pkgs/mosd/apid/src/routes.rs:1719-1729`). A script retrying with a
...
(`os/pkgs/mosd/apid/openapi.json:8-477`).
...
`ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:905-927`) against the same
...
4. **It costs zero extra bus round trips.** The gate already calls
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1041`) and `apiTokens` is a child of `access`, so
...
- Mint and revoke are read-modify-write of the whole array, because the dot-path
  ...
  the same pattern the SSH key pane uses (`os/pkgs/mosd/apid/src/routes.rs:3807-3812`).
  Two concurrent mints lose one token, silently.
- Identity is the `id`, never a list position, for the reason recorded at
  `os/pkgs/mosd/apid/src/routes.rs:3836-3840`: an index is meaningful only against the
...
- **Coexisting means two credentials at one privilege level** — and there is
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:214`). A token can do everything the operator can do
...
2. **§3.1 item 7 changes meaning.** The `auth.rs:53-54` comment's reasoning —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1726-1735`). Bearer verification is **not** rate
...
(`os/pkgs/mosd/apid/src/routes.rs:919-925`) and a redirect to `/login` everywhere else
(`os/pkgs/mosd/apid/src/routes.rs:1049`) — and the CORS argument below, which depends on
...
(`os/pkgs/mosd/Cargo.toml:70`, installed explicitly at `os/pkgs/mosd/apid/src/main.rs:147-148`),
...
2. **A stolen token is full management access, indefinitely.** There is no
   ...
   `os/pkgs/mosd/apid/src/routes.rs:3009`) and is then no longer needed. Detection is not
...
5. **A same-origin page mints a permanent token with the operator's cookie.**
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:3009`). But a minted token survives deactivating
...
pane is tested to carry, `os/pkgs/mosd/apid/src/routes.rs:3009`); apid runs as root
...
- **Under this proposal the API path has no CSRF exposure**, because a bearer
  ...
  **None of that is true of the shipped API**, whose credential *is* the cookie
  (`os/pkgs/mosd/apid/src/routes.rs:914-915`), so today the four `/api` routes sit
...
- **The per-action confirm tokens are not CSRF tokens and must not be counted as
  ...
  and `"poweroff"` (`os/pkgs/mosd/apid/src/routes.rs:2841-2845`) and
  `TRANSIENT_CONFIRM_TOKEN` is the literal `"set-transient-password"`
  (`os/pkgs/mosd/apid/src/routes.rs:3013`). They are not secret, not per-session and not
  ...
  the checkbox has to be ticked as well"* (`os/pkgs/mosd/apid/src/routes.rs:2839-2840`). They stop a mis-click and a prefetch. Against a
...
(`os/pkgs/mosd/apid/src/routes.rs:1014-1020`), so an authenticated flood no longer
...
(`os/pkgs/mosd/apid/src/routes.rs:993`) and its handler makes no mosd call
(`os/pkgs/mosd/apid/src/routes.rs:549-557`), so it is an unauthenticated route that
...
declaration at `os/pkgs/mosd/apid/src/routes.rs:179`, which is why that line is not
...
`0d4f3c6` the fifteen declarations are at `os/pkgs/mosd/apid/src/routes.rs:143-162`,
...
(`os/pkgs/mosd/apid/src/routes.rs:170-199`). A custom
...
`os/pkgs/mosd/apid/src/routes.rs:50` and `:605`) moves under section 6.3's reserved
...
(`os/pkgs/mosd/apid/src/routes.rs:199`). The immutable class is read per request from
...
- **`/var/lib/mos/settings.toml`** (`os/pkgs/mosd/mosd-settings/src/store.rs:67`) — the
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:961-968`) and every authorized SSH key.
...
§6.3's reserved prefix, `builtin_home` in `os/pkgs/mosd/apid/src/routes.rs:1140`. Its
...
(`os/pkgs/mosd/apid/src/routes.rs:1078-1084`), described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:1056`). Section 1.6 evidences the rest: no
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:968`), reached when a mosd call
  ...
  *before* dispatch — so at the moment that page is on screen, `/builtin/` is
  ...
  page (`os/pkgs/mosd/apid/src/routes.rs:949`), the navigation on every built-in pane
...
page *"The management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:975`,
...
1. **A signature and the upload credential authorise the same blast radius.** A
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:3009`). §3.2 states there are **no scopes in
...
(`os/pkgs/mosd/apid/src/routes.rs:33`, `:768-775`, `:786`; §1.1 constraint 1 and §1.6
...
Items (iv) and (v) are already shipped — `os/pkgs/mosd/apid/src/routes.rs:2817`, `:1776`,
...
rendering `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:964-979`) for a value mosd
...
the settings write fails (`os/pkgs/mosd/apid/src/routes.rs:2786-2792`), and `write_key_list`
...
(`os/pkgs/mosd/apid/src/routes.rs:842-843`) and mapping `FDO_INVALID_ARGS`,
...
(`os/pkgs/mosd/apid/src/routes.rs:1041`) the whole appliance answers the 502 page *"The
management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:973-983`). **The
...
path answers 422 (`os/pkgs/mosd/apid/src/routes.rs:3844-3850`). The HTML path is not
changed.
...
setup mode (`os/pkgs/mosd/apid/src/routes.rs:1045-1050`) — and it returns a token (§2.3),
...
HTML path answers **502** (`os/pkgs/mosd/apid/src/routes.rs:973-983`), so one appliance
...
- **The VLAN dot-path limit — since closed, and closed additively.**
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:1136-1142`) — while `split_path` split on it
...
`os/pkgs/mosd/apid/src/routes.rs:214`), so a token can do everything the operator can do
