...
JavaScript, `os/pkgs/mosd/apid/src/routes.rs:5427`. The substantive claim is
...
(`os/pkgs/mosd/apid/src/routes.rs:3369`), whose body runs from
`const STYLE: &str = "\` (`os/pkgs/mosd/apid/src/routes.rs:3370`) to the line that closes it,
`.saved{background:#dfd;border:1px solid #080;padding:.5rem 1rem;margin-bottom:1rem}";`
(`os/pkgs/mosd/apid/src/routes.rs:3377`), injected into the page head as
`style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:3388`). The crate
...
|---|---|---|---|---|---|
| `GET /` | `get(serve::root)` (`os/pkgs/mosd/apid/src/routes.rs:145`) | `pub async fn root` (`os/pkgs/mosd/apid/src/assets/serve.rs:57`) | GET page, conditional | The active bundle's `index.html` when a bundle is active and its index is readable, and the built-in status pane otherwise — `None => built_in(&state).await,` (`os/pkgs/mosd/apid/src/assets/serve.rs:57-65`); the built-in branch is `fn home` (`os/pkgs/mosd/apid/src/routes.rs:4803`) over `fn status_body` (`os/pkgs/mosd/apid/src/routes.rs:4759`) | on the built-in branch only, three now and not two: `get_settings("hostname")` (`os/pkgs/mosd/apid/src/routes.rs:4760`), `get_state("network")` (`os/pkgs/mosd/apid/src/routes.rs:4761`) and `get_state("uptime")` (`os/pkgs/mosd/apid/src/routes.rs:4762-4767`) |
| `GET /builtin` | `get(builtin_home)` (`os/pkgs/mosd/apid/src/routes.rs:166`, inside the nest opened with `.nest(` (`os/pkgs/mosd/apid/src/routes.rs:163`)) | `fn builtin_home` (`os/pkgs/mosd/apid/src/routes.rs:4848`) | GET page | §6.3's escape: the status pane, §3.2's token section (`fn tokens_section` (`os/pkgs/mosd/apid/src/routes.rs:4886`)) and the deactivate control (`fn escape_section` (`os/pkgs/mosd/apid/src/routes.rs:5084`)), reachable whatever state a bundle is in | `status_body`'s three, plus, for the token list, `.get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:4872-4877`) |
| `POST /builtin/deactivate` | `post(builtin_deactivate)` (`os/pkgs/mosd/apid/src/routes.rs:170`) | `fn builtin_deactivate` (`os/pkgs/mosd/apid/src/routes.rs:5115`) | HTML form POST, no body | Removes `/srv/ui/current` through `deactivate()` (`os/pkgs/mosd/apid/src/routes.rs:5116`); "a pointer was there" and "there was none" are both success — `Ok(removed) => {` (`os/pkgs/mosd/apid/src/routes.rs:5117-5144`) — and 500 only when the removal itself failed, `Err(err) => {` (`os/pkgs/mosd/apid/src/routes.rs:5150-5166`) | none |
| `POST /builtin/tokens` | `post(builtin_tokens_mint)` (`os/pkgs/mosd/apid/src/routes.rs:179`) | `fn builtin_tokens_mint` (`os/pkgs/mosd/apid/src/routes.rs:4955`) | HTML form POST | §3.2's bootstrap, and the one place a browser holding only the session cookie can obtain a bearer token: mints, writes the list, and renders the secret once, `fn minted_page` (`os/pkgs/mosd/apid/src/routes.rs:5045`). `POST` only, and the comment says why: *"a GET mint would be a permanent-credential factory reachable from any link an operator clicks"* (`os/pkgs/mosd/apid/src/routes.rs:172-178`) | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:4872-4877`), `set_settings(API_TOKENS_PATH, &value)` (`os/pkgs/mosd/apid/src/routes.rs:1653`) |
| `POST /builtin/tokens/revoke` | `post(builtin_tokens_revoke)` (`os/pkgs/mosd/apid/src/routes.rs:180`) | `fn builtin_tokens_revoke` (`os/pkgs/mosd/apid/src/routes.rs:5007`) | HTML form POST | Drops one token by id and rewrites the list; an id matching nothing is **422** here and 404 on `DELETE /api/v1/tokens/{id}`, a split argued from *"this response body is a re-rendered pane, no consumer on this surface reads the status"* (`os/pkgs/mosd/apid/src/routes.rs:4998-5006`) | as the mint |
| any other `/builtin/…` | `.fallback(builtin_not_found)` (`os/pkgs/mosd/apid/src/routes.rs:181`) | `fn builtin_not_found` (`os/pkgs/mosd/apid/src/routes.rs:5179`) | GET page | 404 — `StatusCode::NOT_FOUND,` (`os/pkgs/mosd/apid/src/routes.rs:5180-5181`) — naming the escape, so the reserved prefix answers for its whole subtree rather than falling through to a bundle | none |
| `GET /builtin/` | `.route(BUILTIN_PATH, get(builtin_home))` (`os/pkgs/mosd/apid/src/routes.rs:183`) | `fn builtin_home` (`os/pkgs/mosd/apid/src/routes.rs:4848`) | GET page | The trailing-slash spelling, declared outside the nest because `nest` claims `/builtin` and not `/builtin/` | as `GET /builtin` |
| `GET /setup` | `get(setup_form)` (`os/pkgs/mosd/apid/src/routes.rs:184`) | `fn setup_form` (`os/pkgs/mosd/apid/src/routes.rs:3863`) | GET page | First-run wizard: admin password, optional hostname, optional single interface; bounces to `/login` once `if password_hash(&access).is_some() {` (`os/pkgs/mosd/apid/src/routes.rs:3868-3870`) | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:3864`), `get_settings("hostname")` (`os/pkgs/mosd/apid/src/routes.rs:3871`) |
| `POST /setup` | `.post(setup_submit)` (`os/pkgs/mosd/apid/src/routes.rs:184`) | `fn setup_submit` (`os/pkgs/mosd/apid/src/routes.rs:3905`) | HTML form POST | 409 when already configured, `if password_hash(&access).is_some() {` (`os/pkgs/mosd/apid/src/routes.rs:3914-3923`); 400 on `if password_under_floor(&form.password) {` (`os/pkgs/mosd/apid/src/routes.rs:3924-3940`); 422 on `!valid_hostname(hostname)` (`os/pkgs/mosd/apid/src/routes.rs:3943-3961`); 500 on *"password hashing failed"* (`os/pkgs/mosd/apid/src/routes.rs:3970-3973`); on success writes, invalidates the gate's access cache with `state.access_cache.invalidate();` (`os/pkgs/mosd/apid/src/routes.rs:3982`) and issues a session cookie, `let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:4015-4019`), then redirects to `/` | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:3910`), `set_settings("access.webAdmin", &value)` (`os/pkgs/mosd/apid/src/routes.rs:3976`), `get_settings("hostname")` (`os/pkgs/mosd/apid/src/routes.rs:3988`), `set_settings("hostname", &Value::String(hostname.to_string()))` (`os/pkgs/mosd/apid/src/routes.rs:3995`), `set_settings(&iface_settings_path(iface), &value)` (`os/pkgs/mosd/apid/src/routes.rs:4009`) |
| `GET /login` | `get(login_form)` (`os/pkgs/mosd/apid/src/routes.rs:185`) | `fn login_form` (`os/pkgs/mosd/apid/src/routes.rs:4352`) | GET page | One password field, and a link to §6.3's prefix | none |
| `POST /login` | `.post(login_submit)` (`os/pkgs/mosd/apid/src/routes.rs:185`) | `fn login_submit` (`os/pkgs/mosd/apid/src/routes.rs:4371`) | HTML form POST | 429 while the login guard is locked, `if !state.guard.begin_attempt() {` (`os/pkgs/mosd/apid/src/routes.rs:4387-4397`); 401 on *"Wrong password."* (`os/pkgs/mosd/apid/src/routes.rs:4426-4436`); on success sets the session cookie, `session::session_cookie(&cookie)` (`os/pkgs/mosd/apid/src/routes.rs:4417-4425`) | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:4398`) |
| `POST /logout` | `post(logout)` (`os/pkgs/mosd/apid/src/routes.rs:186`) | `fn logout` (`os/pkgs/mosd/apid/src/routes.rs:4439`) | HTML form POST, no body | Drops the server-side session and clears the cookie — `state.sessions.remove(&value);` (`os/pkgs/mosd/apid/src/routes.rs:4444-4452`) | none |
| `GET /password` | `get(password_form)` (`os/pkgs/mosd/apid/src/routes.rs:187`) | `fn password_form` (`os/pkgs/mosd/apid/src/routes.rs:4593`) | GET page | The change-password pane, carrying §3.2's ratified token sentence verbatim in `PASSWORD_TOKEN_NOTICE` (`os/pkgs/mosd/apid/src/routes.rs:4568`) | none |
| `POST /password` | `.post(password_submit)` (`os/pkgs/mosd/apid/src/routes.rs:187`) | `fn password_submit` (`os/pkgs/mosd/apid/src/routes.rs:4597`) | HTML form POST | 400 on a mismatch or an under-floor password, 401 on a wrong current password, 500 on a hashing failure — the arms from `Ok(()) => Redirect::to("/password?saved=1").into_response(),` (`os/pkgs/mosd/apid/src/routes.rs:4620-4636`); on success every session but the acting one is dropped, in `fn change_password` (`os/pkgs/mosd/apid/src/routes.rs:4496`) | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:4506`), `set_settings("access.webAdmin", &value)` (`os/pkgs/mosd/apid/src/routes.rs:4540`) |
| `GET /network` | `get(network_form)` (`os/pkgs/mosd/apid/src/routes.rs:188`) | `fn network_form` (`os/pkgs/mosd/apid/src/routes.rs:5570`) | GET page | One form per configured interface, per kind, plus an add-interface form; `?saved=1` renders a banner, `query.saved.is_some().then(saved_banner)` (`os/pkgs/mosd/apid/src/routes.rs:5573`) | `get_settings("network")` (`os/pkgs/mosd/apid/src/routes.rs:5354`), `get_state("network")` (`os/pkgs/mosd/apid/src/routes.rs:5357`) |
| `POST /network` | `.post(network_submit)` (`os/pkgs/mosd/apid/src/routes.rs:188`) | `fn network_submit` (`os/pkgs/mosd/apid/src/routes.rs:5580`) | HTML form POST | The whole candidate map is validated, not the one entry: `validate_entries(&candidate)` (`os/pkgs/mosd/apid/src/routes.rs:5603`), and the peers the form does not carry are preserved by `let peers = view` (`os/pkgs/mosd/apid/src/routes.rs:5589-5594`); on success redirects to `/network?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:5616`) | `get_settings("network")` (`os/pkgs/mosd/apid/src/routes.rs:5354`), `set_settings(&iface_settings_path(&iface), &value)` (`os/pkgs/mosd/apid/src/routes.rs:5611`) |
| `POST /network/peers/add` | `post(network_peer_add)` (`os/pkgs/mosd/apid/src/routes.rs:192`) | `fn network_peer_add` (`os/pkgs/mosd/apid/src/routes.rs:5619`) | HTML form POST | Appends one WireGuard peer, refusing a public key the tunnel already carries — *"That public key is already a peer of this tunnel."* (`os/pkgs/mosd/apid/src/routes.rs:5643-5652`) — then rewrites the whole peer list through `fn write_peers` (`os/pkgs/mosd/apid/src/routes.rs:5684`) | `get_settings("network")` (`os/pkgs/mosd/apid/src/routes.rs:5378`), `set_settings(&peers_settings_path(iface), &value)` (`os/pkgs/mosd/apid/src/routes.rs:5692`) |
| `POST /network/peers/remove` | `post(network_peer_remove)` (`os/pkgs/mosd/apid/src/routes.rs:193`) | `fn network_peer_remove` (`os/pkgs/mosd/apid/src/routes.rs:5657`) | HTML form POST | Removes one peer by public key, never by index, and errors when nothing matched, from `let before = peers.len();` (`os/pkgs/mosd/apid/src/routes.rs:5667-5675`); then rewrites the whole list | as add |
| `GET /hostname` | `get(hostname_form)` (`os/pkgs/mosd/apid/src/routes.rs:194`) | `fn hostname_form` (`os/pkgs/mosd/apid/src/routes.rs:5721`) | GET page | One text input pre-filled with the current hostname | `get_settings("hostname")` (`os/pkgs/mosd/apid/src/routes.rs:5722`) |
| `POST /hostname` | `.post(hostname_submit)` (`os/pkgs/mosd/apid/src/routes.rs:194`) | `fn hostname_submit` (`os/pkgs/mosd/apid/src/routes.rs:5968`) | HTML form POST | 422 on `if !valid_hostname(hostname) {` (`os/pkgs/mosd/apid/src/routes.rs:5973-5979`); on success redirects to `/hostname?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:5987`) | `set_settings("hostname", &Value::String(hostname.to_string()))` (`os/pkgs/mosd/apid/src/routes.rs:5982`) |
| `GET /power` | `get(power_form)` (`os/pkgs/mosd/apid/src/routes.rs:195`) | `fn power_form` (`os/pkgs/mosd/apid/src/routes.rs:5815`) | GET page | Two confirmation forms, each with a required checkbox carrying an action-specific token, in `fn power_form_markup` (`os/pkgs/mosd/apid/src/routes.rs:5788`) | none |
| `POST /power/reboot` | `post(power_reboot)` (`os/pkgs/mosd/apid/src/routes.rs:199`) | `fn power_reboot` (`os/pkgs/mosd/apid/src/routes.rs:5881`) → `fn power_submit` (`os/pkgs/mosd/apid/src/routes.rs:5831`) | HTML form POST | 422 on `if confirm != action.confirm_token() {` (`os/pkgs/mosd/apid/src/routes.rs:5832-5843`); otherwise **202 Accepted**, `StatusCode::ACCEPTED,` (`os/pkgs/mosd/apid/src/routes.rs:5845-5849`), with the D-Bus call spawned on a detached task so the response goes out first — `fn dispatch_power_action` (`os/pkgs/mosd/apid/src/routes.rs:5865`) | `api.reboot()` (`os/pkgs/mosd/apid/src/routes.rs:5872`), which is a write to the `/Actions/reboot` item over the `com.mos.Item1` proxy `trait Item` (`os/pkgs/mosd/apid/src/bus_client.rs:48-51`), whose path is `const REBOOT_ACTION: &str = "/Actions/reboot";` (`os/pkgs/mosd/apid/src/bus_client.rs:55`) and whose call is `async fn reboot` (`os/pkgs/mosd/apid/src/bus_client.rs:261-263`) |
| `POST /power/poweroff` | `post(power_poweroff)` (`os/pkgs/mosd/apid/src/routes.rs:200`) | `fn power_poweroff` (`os/pkgs/mosd/apid/src/routes.rs:5889`) → `fn power_submit` (`os/pkgs/mosd/apid/src/routes.rs:5831`) | HTML form POST | Same shape | `api.power_off()` (`os/pkgs/mosd/apid/src/routes.rs:5873`), the `const POWER_OFF_ACTION: &str = "/Actions/poweroff";` item (`os/pkgs/mosd/apid/src/bus_client.rs:56`), called at `async fn power_off` (`os/pkgs/mosd/apid/src/bus_client.rs:265-267`) |
| `GET /ssh` | `get(ssh_form)` (`os/pkgs/mosd/apid/src/routes.rs:201`) | `fn ssh_form` (`os/pkgs/mosd/apid/src/routes.rs:6258`) → `fn load_ssh_view` (`os/pkgs/mosd/apid/src/routes.rs:6092`) | GET page | SSH pane: stored `access.ssh` settings, the authorized-key list, and the sshd reconciler's live state | `get_settings("access.ssh")` (`os/pkgs/mosd/apid/src/routes.rs:6093`), `get_state("sshd")` (`os/pkgs/mosd/apid/src/routes.rs:6103`) |
| `POST /ssh/enable` | `post(ssh_enable)` (`os/pkgs/mosd/apid/src/routes.rs:205`) | `fn ssh_enable` (`os/pkgs/mosd/apid/src/routes.rs:6274`) | HTML form POST | Writes the checkbox state and redirects to `/ssh?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:6283`) | `set_settings("access.ssh.enabled", &Value::Bool(enabled))` (`os/pkgs/mosd/apid/src/routes.rs:6278`) |
| `POST /ssh/password` | `post(ssh_password)` (`os/pkgs/mosd/apid/src/routes.rs:206`) | `fn ssh_password` (`os/pkgs/mosd/apid/src/routes.rs:6762`) | HTML form POST | Sets a **transient** root password after `if form.confirm != TRANSIENT_CONFIRM_TOKEN {` (`os/pkgs/mosd/apid/src/routes.rs:6767-6773`) and the shared byte bounds of `validate_transient_password` (`os/pkgs/mosd/apid/src/routes.rs:6774`) | `set_transient_root_password(&form.password)` (`os/pkgs/mosd/apid/src/routes.rs:6777`) |
| `POST /ssh/keys/add` | `post(ssh_key_add)` (`os/pkgs/mosd/apid/src/routes.rs:207`) | `fn ssh_key_add` (`os/pkgs/mosd/apid/src/routes.rs:6877`) | HTML form POST | Parses and validates one public key with `parse_authorized_key(&form.key)` (`os/pkgs/mosd/apid/src/routes.rs:6881`), appends it in memory — `keys.push(parsed);` (`os/pkgs/mosd/apid/src/routes.rs:6889`) — then rewrites the whole list through `fn write_key_list` (`os/pkgs/mosd/apid/src/routes.rs:6124`); 422 on `validate_authorized_keys(keys)` (`os/pkgs/mosd/apid/src/routes.rs:6127-6129`) | `get_settings("access.ssh")` (`os/pkgs/mosd/apid/src/routes.rs:6120`), `set_settings(SSH_KEYS_PATH, &value)` (`os/pkgs/mosd/apid/src/routes.rs:6133`) |
| `POST /ssh/keys/remove` | `post(ssh_key_remove)` (`os/pkgs/mosd/apid/src/routes.rs:208`) | `fn ssh_key_remove` (`os/pkgs/mosd/apid/src/routes.rs:6910`) | HTML form POST | Removes one key by fingerprint or canonical key text — `keys.iter().position(` (`os/pkgs/mosd/apid/src/routes.rs:6918-6921`) — never by index, and errors on `let Some(index) = found else {` (`os/pkgs/mosd/apid/src/routes.rs:6922-6928`); then rewrites the whole list | same as add |
| `GET /containers` | `get(containers_form)` (`os/pkgs/mosd/apid/src/routes.rs:209`) | `fn containers_form` (`os/pkgs/mosd/apid/src/routes.rs:6412`) → `fn load_container_view` (`os/pkgs/mosd/apid/src/routes.rs:6319`) | GET page | Container pane: the `container.enabled` switch, the root-capability notice, and the reconciler's published Quadlet files and generated units | `get_settings("container")` (`os/pkgs/mosd/apid/src/routes.rs:6320`), `get_state("container")` (`os/pkgs/mosd/apid/src/routes.rs:6326`) |
| `POST /containers/enable` | `post(containers_enable)` (`os/pkgs/mosd/apid/src/routes.rs:210`) | `fn containers_enable` (`os/pkgs/mosd/apid/src/routes.rs:6428`) | HTML form POST | Writes the switch and redirects to `/containers?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:6440`) | `set_settings("container.enabled", &Value::Bool(enabled))` (`os/pkgs/mosd/apid/src/routes.rs:6435`) |
| `GET /mqtt` | `get(mqtt_form)` (`os/pkgs/mosd/apid/src/routes.rs:211`) | `fn mqtt_form` (`os/pkgs/mosd/apid/src/routes.rs:6698`) → `fn load_mqtt_view` (`os/pkgs/mosd/apid/src/routes.rs:6577`) | GET page | MQTT pane: the `mqtt.enabled` switch and the broker/bridge unit state published by the mqtt reconciler | `get_settings("mqtt")` (`os/pkgs/mosd/apid/src/routes.rs:6578`), `get_state("mqtt")` (`os/pkgs/mosd/apid/src/routes.rs:6584`) |
| `POST /mqtt/enable` | `post(mqtt_enable)` (`os/pkgs/mosd/apid/src/routes.rs:212`) | `fn mqtt_enable` (`os/pkgs/mosd/apid/src/routes.rs:6714`) | HTML form POST | Writes one dot-path and nothing else, so saving the switch cannot rewrite the listen or auth settings: *"the switch writes nothing about the listener or about authentication"* (`os/pkgs/mosd/apid/src/routes.rs:6716-6718`); redirects to `/mqtt?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:6726`) | `set_settings("mqtt.enabled", &Value::Bool(enabled))` (`os/pkgs/mosd/apid/src/routes.rs:6721`) |
| `GET /healthz` | `get(healthz)` (`os/pkgs/mosd/apid/src/routes.rs:213`) | `fn healthz` (`os/pkgs/mosd/apid/src/routes.rs:3365`) | neither — plain text | Returns the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:3366`) — a liveness probe only: it proves the apid process is listening and checks nothing else, not mosd, not the settings tree, not the appliance. `GET /api/v1/health` is the other question and the two are not interchangeable | none |
...
| `POST /api/v1/actions/change-password` | `post(api_v1_change_password)` (`os/pkgs/mosd/apid/src/routes.rs:424`) | `fn api_v1_change_password` (`os/pkgs/mosd/apid/src/routes.rs:4669`) | bearer | 204, through the same `fn change_password` (`os/pkgs/mosd/apid/src/routes.rs:4496`) the pane calls; a wrong current password is 403 `wrong_password` (`os/pkgs/mosd/apid/src/routes.rs:4704`) |
...
| `POST /api/v1/actions/reboot` | `post(api_v1_reboot)` (`os/pkgs/mosd/apid/src/routes.rs:474`) | `fn api_v1_reboot` (`os/pkgs/mosd/apid/src/routes.rs:5938`) | bearer | **202** with an empty body, through the same `fn dispatch_power_action` (`os/pkgs/mosd/apid/src/routes.rs:5865`) the pane uses, and with no confirmation token: *"There is no mis-click on a `POST` a script constructed"* (`os/pkgs/mosd/apid/src/routes.rs:5908-5913`) |
| `POST /api/v1/actions/poweroff` | `post(api_v1_poweroff)` (`os/pkgs/mosd/apid/src/routes.rs:475`) | `fn api_v1_poweroff` (`os/pkgs/mosd/apid/src/routes.rs:5958`) | bearer | Same shape |
| `POST /api/v1/actions/transient-root-password` | `post(api_v1_transient_root_password)` (`os/pkgs/mosd/apid/src/routes.rs:476-479`) | `fn api_v1_transient_root_password` (`os/pkgs/mosd/apid/src/routes.rs:6832`) | bearer | 204, the same byte bounds the pane runs, and no echo of the password |
| `POST /api/v1/setup` | `post(api_v1_setup)` (`os/pkgs/mosd/apid/src/routes.rs:487`) | `fn api_v1_setup` (`os/pkgs/mosd/apid/src/routes.rs:4132`) | **none** | The device's one unauthenticated write, and the only handler under this prefix naming no credential extractor at all — *"the only handler under this prefix that takes no credential extractor"* (`os/pkgs/mosd/apid/src/routes.rs:480-486`): 201 carrying a first bearer token, 409 `"already_configured",` (`os/pkgs/mosd/apid/src/routes.rs:4155`), 422 with nothing written |
...
neither and returns a bare string (`os/pkgs/mosd/apid/src/routes.rs:3365-3367`), and
...
`os/pkgs/mosd/apid/src/routes.rs:2177`, `os/pkgs/mosd/apid/src/routes.rs:2633`,
`os/pkgs/mosd/apid/src/routes.rs:2704`, `os/pkgs/mosd/apid/src/routes.rs:2949`,
`os/pkgs/mosd/apid/src/routes.rs:4135`, `os/pkgs/mosd/apid/src/routes.rs:4674` and
`os/pkgs/mosd/apid/src/routes.rs:6836`; four of them share the reader
`fn json_body` (`os/pkgs/mosd/apid/src/routes.rs:2576`). The HTML surface is
...
example `Form(form): Form<NetworkForm>` (`os/pkgs/mosd/apid/src/routes.rs:5580`) and
`Form(form): Form<SshKeyAddForm>` (`os/pkgs/mosd/apid/src/routes.rs:6877`). The two POSTs
that are in neither set — `/logout`, at `fn logout`
(`os/pkgs/mosd/apid/src/routes.rs:4439`), and `/builtin/deactivate`, at
`fn builtin_deactivate` (`os/pkgs/mosd/apid/src/routes.rs:5115`) — take no request
...
(`os/pkgs/mosd/apid/src/routes.rs:5832`) and on the transient password
(`os/pkgs/mosd/apid/src/routes.rs:6767`) — neither of which the `/api/v1/` actions
carry, and §2.4's own note says why: there is no mis-click on a `POST` a script
constructed (`os/pkgs/mosd/apid/src/routes.rs:5908-5913`).
...
| `fn get_state` | `os/pkgs/mosd/apid/src/bus_client.rs:30` | `async fn get_state` (`os/pkgs/mosd/mosd/src/bus.rs:649`) | five literal live-state paths: `get_state("network")` (`os/pkgs/mosd/apid/src/routes.rs:4761`), `.get_state("uptime")` (`os/pkgs/mosd/apid/src/routes.rs:4764`), `get_state("sshd")` (`os/pkgs/mosd/apid/src/routes.rs:6103`), `get_state("container")` (`os/pkgs/mosd/apid/src/routes.rs:6326`) and `get_state("mqtt")` (`os/pkgs/mosd/apid/src/routes.rs:6584`), plus the health probe `get_state(HEALTH_PROBE_PATH)` (`os/pkgs/mosd/apid/src/routes.rs:890`) and the passthrough `get_state(&path)` (`os/pkgs/mosd/apid/src/routes.rs:1246`), which serves any dot-path a client asks for |
| `fn set_value` on `/Actions/reboot` | `os/pkgs/mosd/apid/src/bus_client.rs:50`, path at `os/pkgs/mosd/apid/src/bus_client.rs:55` | `os/pkgs/mosd/mosd/src/tree.rs:491` → `os/pkgs/mosd/mosd/src/actions.rs:101` | `POST /power/reboot` and `POST /api/v1/actions/reboot`, both through `PowerAction::Reboot => api.reboot().await,` (`os/pkgs/mosd/apid/src/routes.rs:5872`) |
| `fn set_value` on `/Actions/poweroff` | `os/pkgs/mosd/apid/src/bus_client.rs:50`, path at `os/pkgs/mosd/apid/src/bus_client.rs:56` | `os/pkgs/mosd/mosd/src/tree.rs:491` → `os/pkgs/mosd/mosd/src/actions.rs:102` | `POST /power/poweroff` and `POST /api/v1/actions/poweroff`, both through `PowerAction::PowerOff => api.power_off().await,` (`os/pkgs/mosd/apid/src/routes.rs:5873`) |
| `fn set_transient_root_password` | `os/pkgs/mosd/apid/src/bus_client.rs:31` | `async fn set_transient_root_password` (`os/pkgs/mosd/mosd/src/bus.rs:826`) | `POST /ssh/password`, at `app.api.set_transient_root_password(&form.password)` (`os/pkgs/mosd/apid/src/routes.rs:6777`), and `POST /api/v1/actions/transient-root-password`, at `app.api.set_transient_root_password(&request.password)` (`os/pkgs/mosd/apid/src/routes.rs:6856`) |
...
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:4420`), set
on the response by `session::session_cookie(&cookie)`
(`os/pkgs/mosd/apid/src/routes.rs:4421-4425`). `POST /setup` does the same at the end
of the first-run wizard without a login step, at its own
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:4015-4019`). The store generates 16 random bytes from
...
(`os/pkgs/mosd/apid/src/routes.rs:3315`) to `Redirect::to("/login").into_response()`
(`os/pkgs/mosd/apid/src/routes.rs:3362-3363`). It implements
...
1. `/healthz` and the declared `/api/` routes always pass —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3317`). The API routes answer for themselves in
...
2. A request carrying a **valid session cookie** passes without any bus call at
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3338-3342`).
3. **Setup mode** — no admin password hash present — only `/setup` passes and
   everything else redirects there (`os/pkgs/mosd/apid/src/routes.rs:3351-3356`).
4. **Normal mode** — `/login` and `/setup` pass and everything else redirects
   to `/login` (`os/pkgs/mosd/apid/src/routes.rs:3357-3362`).
...
(`os/pkgs/mosd/apid/src/routes.rs:3338-3342`) — so a cookie-authenticated request reaches its
handler without touching mosd, and the source states the reason and its
soundness argument at `os/pkgs/mosd/apid/src/routes.rs:3321-3337`: *"an authenticated
...
(`os/pkgs/mosd/apid/src/routes.rs:6119-6121`) — edits it in memory with `keys.push(parsed);`
(`os/pkgs/mosd/apid/src/routes.rs:6889`) or `keys.remove(index);` (`os/pkgs/mosd/apid/src/routes.rs:6929`), and writes the whole
list back through `async fn write_key_list` (`os/pkgs/mosd/apid/src/routes.rs:6124-6137`). Every
...
reads it at `.get_state("uptime")` (`os/pkgs/mosd/apid/src/routes.rs:4764`). A sixth reader
...
3. **Nothing is embedded in the binary, and that is still deliberate.**
   ...
   `include_str!("../openapi.json"),` (`os/pkgs/mosd/apid/src/tests.rs:1929`) reads the
...
by `style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:3388`), from a constant described in
the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:3369`). There is still no favicon route, no font and
...
the gate redirects it to `/login` (`os/pkgs/mosd/apid/src/routes.rs:3362`), and
...
(`os/pkgs/mosd/apid/src/routes.rs:5116`) — and the install path. The consequence
...
1. **Line numbers throughout its section 2.1 no longer resolve to the routes
   ...
   `pub(crate) async fn home` (`os/pkgs/mosd/apid/src/routes.rs:4803`).
...
3. **"Six methods and one signal"** (`docs/research/mos-ui-inventory.md:367`) is
   ...
   `app.api.set_transient_root_password(&form.password)` (`os/pkgs/mosd/apid/src/routes.rs:6777`)
   and from M7's action route at
   `app.api.set_transient_root_password(&request.password)` (`os/pkgs/mosd/apid/src/routes.rs:6856`).
4. **"the sole call site is `GetState("network")`"**
   (`docs/research/mos-ui-inventory.md:373`) is now eight call sites:
   `get_state("network")` (`os/pkgs/mosd/apid/src/routes.rs:4761`),
   `.get_state("uptime")` (`os/pkgs/mosd/apid/src/routes.rs:4764`),
   `get_state("network")` again in the network pane (`os/pkgs/mosd/apid/src/routes.rs:5357`),
   `get_state("sshd")` (`os/pkgs/mosd/apid/src/routes.rs:6103`),
   `get_state("container")` (`os/pkgs/mosd/apid/src/routes.rs:6326`),
   `get_state("mqtt")` (`os/pkgs/mosd/apid/src/routes.rs:6584`),
...
(`os/pkgs/mosd/apid/src/routes.rs:3380`) now renders **nine** links, from
`a href="/" { "Status" }` (`os/pkgs/mosd/apid/src/routes.rs:3393-3406`), plus the logout
form `form method="post" action="/logout" {` (`os/pkgs/mosd/apid/src/routes.rs:3407-3409`):
...
1. The gate already dispatches on `let path = request.uri().path();`
   (`os/pkgs/mosd/apid/src/routes.rs:3316`). A version check is the same string operation
...
(`os/pkgs/mosd/apid/src/routes.rs:3351-3356`). An authenticated probe cannot answer the
...
bus call at `os/pkgs/mosd/apid/src/routes.rs:3317`. The cost is an unauthenticated
...
(`os/pkgs/mosd/apid/src/routes.rs:3365-3367`, released by the gate at
`if path == "/healthz" || is_declared_api_route(path) {` (`os/pkgs/mosd/apid/src/routes.rs:3317`)),
...
- **No array indexing.** The dot-path syntax has none; the model says a list is
  *"Written as a whole JSON array through the dot-path API"*
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:6772-6777`). Two clients doing that concurrently
...
- **A key holding a dot is addressed with a quoted segment.** apid accepts an
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3473-3479`) — and until PLAN-022 M2 the
...
nobody asked to delete"* (`os/pkgs/mosd/apid/src/routes.rs:6901-6905`). A `DELETE` whose
...
the file (`os/pkgs/mosd/apid/src/routes.rs:6097-6099`). So the difference is the quality
...
| SSH keys | the authorized-keys collection above | `access.ssh.authorizedKeys` | **every key is a root key** (`docs/design/access.md` §4.1, `os/pkgs/mosd/apid/src/routes.rs:5974`); the API response must carry that sentence in a `notice` field for the same reason the pane must carry it |
...
(`os/pkgs/mosd/apid/src/routes.rs:5804`, and `TRANSIENT_CONFIRM_TOKEN` at `os/pkgs/mosd/apid/src/routes.rs:5978`) does
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:5824-5837`). 202
...
- **`GET /healthz`** — declared at `os/pkgs/mosd/apid/src/routes.rs:213` and answering
  at `os/pkgs/mosd/apid/src/routes.rs:3337-3339`. It keeps its path, its unauthenticated exemption (`os/pkgs/mosd/apid/src/routes.rs:3289`) and
  its literal `ok` body,
...
- **A password change — [implemented], on both surfaces.** This bullet once
  ...
  (`change_password`, `os/pkgs/mosd/apid/src/routes.rs:4468-4527`): the
...
(`os/pkgs/mosd/apid/src/routes.rs:6894-6900`, via `ssh_error` at `os/pkgs/mosd/apid/src/routes.rs:6112-6121`). For a
...
- **Five codes ship that this section never proposed**, each added by a
  ...
  `os/pkgs/mosd/apid/src/routes.rs:2469`, `os/pkgs/mosd/apid/src/routes.rs:4247`), `already_configured`
  (`os/pkgs/mosd/apid/src/routes.rs:4155`), `mint_failed` (`os/pkgs/mosd/apid/src/routes.rs:1489`,
  `os/pkgs/mosd/apid/src/routes.rs:4258`), `hash_failed` (`os/pkgs/mosd/apid/src/routes.rs:4281`) and
  `hashing_failed` (`os/pkgs/mosd/apid/src/routes.rs:4719`). They are additive under §2.1 — a
...
| `request_invalid` | **yes**, 400 — this snapshot said **no** | seven sites, one per JSON-body route that reports a rejected body itself, plus the shared reader `fn json_body`, at `at(ApiError::apid("request_invalid", rejection.body_text())),` (`os/pkgs/mosd/apid/src/routes.rs:2587`): `os/pkgs/mosd/apid/src/routes.rs:1173`, `os/pkgs/mosd/apid/src/routes.rs:1458`, `os/pkgs/mosd/apid/src/routes.rs:1924`, `os/pkgs/mosd/apid/src/routes.rs:2184`, `os/pkgs/mosd/apid/src/routes.rs:4682`, `os/pkgs/mosd/apid/src/routes.rs:6846`. It could not ship when this row was written because no route took a body; ten do now (section 1.2) |
| `validation_failed` | **yes**, 422 — this snapshot said **no** | twenty-two sites, the most-used code in the crate, from `"validation_failed",` (`os/pkgs/mosd/apid/src/routes.rs:1090`) to `os/pkgs/mosd/apid/src/routes.rs:6853`. The four validators the row below names are all reachable from `/api/v1/` now: `valid_hostname` through the settings write, `validate_iface` through `POST /api/v1/setup`, at `if let Err(message) = validate_iface(iface, cfg.dhcp, address) {` (`os/pkgs/mosd/apid/src/routes.rs:4216`), `validate_transient_password` and `parse_authorized_key` through their action and collection routes |
| `wrong_password` | **yes**, 403 | `"wrong_password",` (`os/pkgs/mosd/apid/src/routes.rs:4704`), on `POST /api/v1/actions/change-password` |
...
(`os/pkgs/mosd/apid/src/routes.rs:5581-5668`), and `write_key_list` does the same
(`os/pkgs/mosd/apid/src/routes.rs:6105-6107`). The VLAN example above is exactly this — a rejection that reads
as an outage.
...
text in an error box (`os/pkgs/mosd/apid/src/routes.rs:3402-3404`, `:1737`, `:1757`) — without
widening it.
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   `/healthz` returns before any check (`os/pkgs/mosd/apid/src/routes.rs:3289`) and
   answers the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:3366`), so **an appliance whose mosd is dead
   ...
   *"whole seconds since boot, a bare JSON number"* everywhere else it appears.
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3310-3314`), so no API route depends on the gate's
...
(`os/pkgs/mosd/apid/src/routes.rs:3317`, predicate at `:503-520`). There is no bearer
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   `Redirect::to("/login").into_response()` (`os/pkgs/mosd/apid/src/routes.rs:3362`), a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3329-3331`) and which returns **200 OK** with an HTML
   form (`os/pkgs/mosd/apid/src/routes.rs:4010-4341`). A script that checks the status
...
2. **Obtaining the credential means emulating three browser behaviours.**
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:4374`) — axum's URL-encoded extractor, imported at
   `os/pkgs/mosd/apid/src/routes.rs:19`, not JSON — answers **302 to `/`**
   (`os/pkgs/mosd/apid/src/routes.rs:4395`), and delivers the credential in a
   `Set-Cookie` header (`os/pkgs/mosd/apid/src/routes.rs:4394`, value built at
...
7. **A misconfigured script locks the operator out, repeatedly.** The login
   ...
   *"the appliance has one admin password, so per-client tracking buys nothing
   ...
   `os/pkgs/mosd/apid/src/routes.rs:4359-4369`). A script retrying with a
...
4. **It costs zero extra bus round trips.** The gate already calls
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3326`) and `apiTokens` is a child of `access`, so
...
- Mint and revoke are read-modify-write of the whole array, because the dot-path
  ...
  the same pattern the SSH key pane uses (`os/pkgs/mosd/apid/src/routes.rs:6885-6890`).
  Two concurrent mints lose one token, silently.
- Identity is the `id`, never a list position, for the reason recorded at
  `os/pkgs/mosd/apid/src/routes.rs:6886-6890`: an index is meaningful only against the
...
2. **§3.1 item 7 changes meaning.** The `auth.rs:53-54` comment's reasoning —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:4366-4375`). Bearer verification is **not** rate
...
(`os/pkgs/mosd/apid/src/routes.rs:3174-3176`) and a redirect to `/login` everywhere else
(`os/pkgs/mosd/apid/src/routes.rs:3334`) — and the CORS argument below, which depends on
...
2. **A stolen token is full management access, indefinitely.** There is no
   ...
   `os/pkgs/mosd/apid/src/routes.rs:5974`) and is then no longer needed. Detection is not
...
5. **A same-origin page mints a permanent token with the operator's cookie.**
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:5974`). But a minted token survives deactivating
...
pane is tested to carry, `os/pkgs/mosd/apid/src/routes.rs:5974`); apid runs as root
...
- **The per-action confirm tokens are not CSRF tokens and must not be counted as
  ...
  and `"poweroff"` (`os/pkgs/mosd/apid/src/routes.rs:5752-5756`) and
  `TRANSIENT_CONFIRM_TOKEN` is the literal `"set-transient-password"`
  (`os/pkgs/mosd/apid/src/routes.rs:6006`). They are not secret, not per-session and not
  ...
  the checkbox has to be ticked as well"* (`os/pkgs/mosd/apid/src/routes.rs:5750-5751`). They stop a mis-click and a prefetch. Against a
...
(`os/pkgs/mosd/apid/src/routes.rs:3310-3316`), so an authenticated flood no longer
...
§6.3's reserved prefix, `builtin_home` in `os/pkgs/mosd/apid/src/routes.rs:3425`. Its
...
(`os/pkgs/mosd/apid/src/routes.rs:3363-3369`), described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:3369`). Section 1.6 evidences the rest: no
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  *before* dispatch — so at the moment that page is on screen, `/builtin/` is
  ...
  page (`os/pkgs/mosd/apid/src/routes.rs:3245`), the navigation on every built-in pane
...
1. **A signature and the upload credential authorise the same blast radius.** A
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:5974`). §3.2 states there are **no scopes in
...
Items (iv) and (v) are already shipped — `os/pkgs/mosd/apid/src/routes.rs:5700`, `:2245`,
...
the settings write fails (`os/pkgs/mosd/apid/src/routes.rs:5669-5675`), and `write_key_list`
...
(`os/pkgs/mosd/apid/src/routes.rs:3326`) the whole appliance answers the 502 page *"The
management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:3297-3307`). **The
...
path answers 422 (`os/pkgs/mosd/apid/src/routes.rs:6894-6900`). The HTML path is not
changed.
...
setup mode (`os/pkgs/mosd/apid/src/routes.rs:3330-3335`) — and it returns a token (§2.3),
...
- **The VLAN dot-path limit — since closed, and closed additively.**
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3473-3479`) — while `split_path` split on it
