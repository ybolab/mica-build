...
(`os/pkgs/mosd/apid/src/routes.rs:828`), whose body runs from
`const STYLE: &str = "\` (`os/pkgs/mosd/apid/src/routes.rs:829`) to
`os/pkgs/mosd/apid/src/routes.rs:836`, injected into the page head as
`style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:847`). The crate
...
`pub fn app(state: AppState) -> Router {` (`os/pkgs/mosd/apid/src/routes.rs:117`) and
runs to `.with_state(state)` (`os/pkgs/mosd/apid/src/routes.rs:197`). Its declaration
...
| `POST /power/reboot` | `:158` | `power_reboot` (`:1703`) → `power_submit` (`:1666`) | HTML form POST | 422 when the confirm token does not match (`:1667-1678`); otherwise **202 Accepted** (`:1696-1700`) with the D-Bus call spawned on a detached task so the response goes out first (`:1687-1695`) | a write to the `/Actions/reboot` item over the `com.mos.Item1` proxy (`os/pkgs/mosd/apid/src/bus_client.rs:34`, path at `:40`, call at `:189-191`) |
| `POST /power/poweroff` | `:159` | `power_poweroff` (`:1711`) → `power_submit` (`:1666`) | HTML form POST | Same shape | a write to the `/Actions/poweroff` item over the `com.mos.Item1` proxy (`os/pkgs/mosd/apid/src/bus_client.rs:34`, path at `:41`, call at `:193-195`) |
...
| `GET /healthz` | `:172` | `healthz` (`:715`) | neither — plain text | Returns the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:825`) | none |
...
| every method on `/api/` | `:184` (`.route("/api/", any(api_not_found))`) | `api_not_found` (`:196`) | JSON | 404 in §2.4's envelope. Not redundant with the nest: *"`nest` claims `/api`, `/api/x` and `/api/x/y`, and not `/api/`; the difference is a request that begins `/api/` reaching the asset router"* (`os/pkgs/mosd/apid/src/routes.rs:187-190`) | none |
...
**The `/api` subtree.** `api_router()` (`os/pkgs/mosd/apid/src/routes.rs:283-294`) is
nested under the prefix `const API: &str = "/api";`
(`os/pkgs/mosd/apid/src/routes.rs:221`). Each route is declared from the same constant
...
one string and cannot disagree"* (`os/pkgs/mosd/apid/src/routes.rs:280-282`).
...
(`os/pkgs/mosd/apid/src/routes.rs:233`) while OpenAPI's is
`const V1_SETTINGS_DOC: &str = "/v1/settings/{path}";`
(`os/pkgs/mosd/apid/src/routes.rs:234`), and the source states why: *"axum names a
...
(`os/pkgs/mosd/apid/src/routes.rs:227-230`). Every response under the prefix is built
by one helper that sets `no-store` on success as well as on failure
(`os/pkgs/mosd/apid/src/routes.rs:351-358`).

**Kinds, counted, at this commit.** The outer router declares **twenty-one**
`.route()` calls (`os/pkgs/mosd/apid/src/routes.rs:124`, `:148-154`, `:158-160`,
...
neither and returns a bare string (`os/pkgs/mosd/apid/src/routes.rs:824-826`), and
fourteen are POSTs.
...
(`os/pkgs/mosd/apid/src/routes.rs:355`), as does `api_not_found`. **No route accepts a
...
example `Form(form): Form<SetupForm>,` (`os/pkgs/mosd/apid/src/routes.rs:1325`),
...
(`os/pkgs/mosd/apid/src/routes.rs:164-166`) — or for any of the four SSH mutations
(`os/pkgs/mosd/apid/src/routes.rs:170-172`), and the rule has since been extended to
the bundle deactivation control (`os/pkgs/mosd/apid/src/routes.rs:146-148`). And there
...
on power (`os/pkgs/mosd/apid/src/routes.rs:2433`) and on the transient password
(`os/pkgs/mosd/apid/src/routes.rs:3286`).

**The second router.** `redirect_app()` at `os/pkgs/mosd/apid/src/routes.rs:721-725` is
the router served on the **HTTP** listener. It declares no routes at all — only
a fallback, `.fallback(redirect_to_https)` (`os/pkgs/mosd/apid/src/routes.rs:723`) —
...
(`os/pkgs/mosd/apid/src/routes.rs:727-741`). It carries no state beyond that port
...
`com.mos.mosd1` trait (`os/pkgs/mosd/apid/src/bus_client.rs:20-26`) declares four; the
power actions are the fifth, `set_value` on the `com.mos.Item1` trait
(`os/pkgs/mosd/apid/src/bus_client.rs:34-37`), written to one action item per verb and
...
`os/pkgs/mosd/apid/src/bus_client.rs:15-37` is six method declarations and two proxy
...
(`os/pkgs/mosd/apid/src/bus_client.rs:77-79`), with the cache read at
`os/pkgs/mosd/apid/src/bus_client.rs:95-107` and dropped at `:116-119`. A failed call
renders a 502 page reading
*"The management daemon is unavailable."*
(`os/pkgs/mosd/apid/src/routes.rs:761`), built at `os/pkgs/mosd/apid/src/routes.rs:755-765`.
...
(`os/pkgs/mosd/apid/src/routes.rs:747-752`), typed as
...
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:1519`), set
at `os/pkgs/mosd/apid/src/routes.rs:1520-1524`. `POST /setup` does the same at the end
of the first-run wizard without a login step
(`os/pkgs/mosd/apid/src/routes.rs:1428-1433`). The store generates 16 random bytes from
...
through the `ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:695-717`), which
tests `session::cookie_from_headers(&parts.headers)`
(`os/pkgs/mosd/apid/src/routes.rs:704`) and nothing else.

**The gate.** One middleware, layered over the whole HTTPS router with
`.layer(middleware::from_fn_with_state(state.clone(), gate))`
(`os/pkgs/mosd/apid/src/routes.rs:196`), running from
`async fn gate(State(state): State<AppState>, request: Request, next: Next) -> Response {`
(`os/pkgs/mosd/apid/src/routes.rs:777`) to `os/pkgs/mosd/apid/src/routes.rs:822`. It implements
...
(`os/pkgs/mosd/apid/src/routes.rs:800-804`), so an authenticated request reaches its
handler without touching mosd, and the source states the reason and its
soundness argument at `os/pkgs/mosd/apid/src/routes.rs:783-799`: *"an authenticated
...
`state.api.get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:806`) is now paid
...
time (`os/pkgs/mosd/apid/src/routes.rs:485`) rather than copying it. It is **read-only
...
the SSH pane reads the whole key list (`os/pkgs/mosd/apid/src/routes.rs:2638-2640`),
edits it in memory (`os/pkgs/mosd/apid/src/routes.rs:3319-3323`), and writes the whole
list back through `write_key_list` (`os/pkgs/mosd/apid/src/routes.rs:2643-2656`).
...
(`os/pkgs/mosd/apid/src/routes.rs:559`) — which is the one place where
...
2. **A file-serving service is constructed, as the router's fallback.**
   ...
   now ends in `.fallback(serve::fallback)` (`os/pkgs/mosd/apid/src/routes.rs:195`),
...
3. **Nothing is embedded in the binary, and that is still deliberate.**
   ...
   `os/pkgs/mosd/apid/src/tests.rs:1758` reads the committed `openapi.json` so a test
...
4. **The crate still ships no asset of its own; the assets come from
   ...
   `bundles: Arc::new(Store::at_default()),` (`os/pkgs/mosd/apid/src/routes.rs:72`).
...
(`os/pkgs/mosd/apid/src/routes.rs:847`), from a constant described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:828`). There is still no favicon route, no font and
no image declared in `os/pkgs/mosd/apid/src/routes.rs:117-197`. A request for
`/favicon.ico` no longer ends at the gate, though: it is unauthenticated, so
the gate redirects it to `/login` (`os/pkgs/mosd/apid/src/routes.rs:821`), and
...
`std::fs::read_to_string("/proc/uptime")` (`os/pkgs/mosd/apid/src/routes.rs:1595`) and
...
control (`os/pkgs/mosd/apid/src/routes.rs:1717`) and the install path. The consequence
...
1. **Line numbers throughout its section 2.1 no longer resolve to the routes
   ...
   `f7cb5ba` the route is `os/pkgs/mosd/apid/src/routes.rs:124` and the handler is
   `serve::root`, at `os/pkgs/mosd/apid/src/assets/serve.rs:57`, with the built-in branch
   at `os/pkgs/mosd/apid/src/routes.rs:1633`.
2. **Its route table is missing thirteen routes.** The five SSH routes it
   ...
   `os/pkgs/mosd/apid/src/routes.rs:169-176`, and eight more have landed since: the
   four `/containers` and `/mqtt` pairs (`os/pkgs/mosd/apid/src/routes.rs:177-180`) and
   the four `/api` routes (`os/pkgs/mosd/apid/src/routes.rs:285-288`). Its own section
...
renders **eight** links (`os/pkgs/mosd/apid/src/routes.rs:852-864`) plus the logout form
(`os/pkgs/mosd/apid/src/routes.rs:865-867`): Status, Network, Hostname, Power, SSH,
...
paths**, all `GET`: `"/api/v1/meta"` (`os/pkgs/mosd/apid/openapi.json:82`),
`"/api/v1/settings/{path}"` (`os/pkgs/mosd/apid/openapi.json:114`),
`"/api/v1/state/{path}"` (`os/pkgs/mosd/apid/openapi.json:187`) and
`"/api/versions"` (`os/pkgs/mosd/apid/openapi.json:260`). Its
`components.schemas` are exactly five: `"ApiError"`
(`os/pkgs/mosd/apid/openapi.json:285`), `"ApiErrorDetail"`
(`os/pkgs/mosd/apid/openapi.json:297`), `"ApiMeta"` (`os/pkgs/mosd/apid/openapi.json:327`),
`"ApiVersions"` (`os/pkgs/mosd/apid/openapi.json:352`) and `"ResourceValue"`
(`os/pkgs/mosd/apid/openapi.json:373`). The document is generated from the handlers
...
(`os/pkgs/mosd/apid/src/routes.rs:221`), nested as `.nest(API, api_router())`
(`os/pkgs/mosd/apid/src/routes.rs:192`), and the version is the segment immediately
after it — `const V1_META_PATH: &str = "/v1/meta";`
(`os/pkgs/mosd/apid/src/routes.rs:223`). `GET /api/versions` answers unauthenticated
from `const SERVED_VERSIONS: [&str; 1] = ["v1"];`
(`os/pkgs/mosd/apid/src/routes.rs:272`) and `const CURRENT_VERSION: &str = "v1";`
(`os/pkgs/mosd/apid/src/routes.rs:275`), and `GET /api/v1/meta` answers authenticated
with `ApiMeta` whose `settingsSchemaVersion` is read live from
`settings_schema_version: mosd_settings::SCHEMA_VERSION,`
(`os/pkgs/mosd/apid/src/routes.rs:485`) rather than copied. The served set is an array
on the wire, `"versions"` typed `"type": "array"`
(`os/pkgs/mosd/apid/openapi.json:364-365`), so the dual-major recommendation below has
...
(`os/pkgs/mosd/apid/src/routes.rs:193`) claims the one spelling the nest does not, and
...
(`os/pkgs/mosd/apid/src/routes.rs:184-186`).
...
1. The gate already dispatches on `let path = request.uri().path();`
   (`os/pkgs/mosd/apid/src/routes.rs:778`). A version check is the same string operation
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:196`). This is now measurable rather than
   predicted: the shipped gate tests membership with `is_declared_api_route`
   (`os/pkgs/mosd/apid/src/routes.rs:303-332`), a pure string operation over the same
   constants the router declares.
2. axum registers routes by path (`os/pkgs/mosd/apid/src/routes.rs:117-197`). A second
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:283-294`) is the first of those routers and shows
   the shape.
...
(`os/pkgs/mosd/apid/src/routes.rs:737`) — that is not the objection. The
...
Both rows ship. The first is declared at `os/pkgs/mosd/apid/src/routes.rs:285` and
...
exactly one source"* (`os/pkgs/mosd/apid/src/routes.rs:467-469`). The API version is the
...
(`os/pkgs/mosd/apid/src/routes.rs:810-815`). An authenticated probe cannot answer the
...
so"* (`os/pkgs/mosd/apid/src/routes.rs:442-445`); the gate hands the route off above its own
bus call at `os/pkgs/mosd/apid/src/routes.rs:779`. The cost is an unauthenticated
...
(`os/pkgs/mosd/apid/src/routes.rs:779`, `:715-717`), so this is one more bit on a
...
(`os/pkgs/mosd/apid/src/routes.rs:233`) declared at `os/pkgs/mosd/apid/src/routes.rs:287` and
handled by `api_v1_settings` (`:474`), whose whole body is
`resource_response(state.api.get_settings(&path).await, &path)`
(`os/pkgs/mosd/apid/src/routes.rs:532`) — the dot-path passthrough this section asks
for, with no second model beside it. The live-state root is the same shape,
`const V1_STATE_ROUTE: &str = "/v1/state/{*path}";`
(`os/pkgs/mosd/apid/src/routes.rs:236`) at `:266` and `:501`, ending in
`resource_response(state.api.get_state(&path).await, &path)`
(`os/pkgs/mosd/apid/src/routes.rs:559`). Both answer `ResourceValue`
(`os/pkgs/mosd/apid/src/routes.rs:506`), which carries a
`serde(transparent)` attribute (`os/pkgs/mosd/apid/src/routes.rs:505`) so the body is
...
the proxy trait, `os/pkgs/mosd/apid/src/bus_client.rs:20-26`, nor in mosd's interface
...
- **No array indexing.** The dot-path syntax has none; the model says a list is
  *"Written as a whole JSON array through the dot-path API"*
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3319-3324`). Two clients doing that concurrently
...
nobody asked to delete"* (`os/pkgs/mosd/apid/src/routes.rs:3335-3339`). A `DELETE` whose
...
the file (`os/pkgs/mosd/apid/src/routes.rs:2644-2646`). So the difference is the quality
...
| SSH keys | the authorized-keys collection above | `access.ssh.authorizedKeys` | **every key is a root key** (`docs/design/access.md` §4.1, `os/pkgs/mosd/apid/src/routes.rs:2521`); the API response must carry that sentence in a `notice` field for the same reason the pane must carry it |
...
3. **Uptime is not in either tree.** the status pane reads `/proc/uptime` in apid
   (`os/pkgs/mosd/apid/src/routes.rs:1595`) — one of the three disk paths apid touches
...
(`os/pkgs/mosd/apid/openapi.json:8-281`) — `"/api/v1/meta"`
(`os/pkgs/mosd/apid/openapi.json:82`), `"/api/v1/settings/{path}"`
(`os/pkgs/mosd/apid/openapi.json:114`), `"/api/v1/state/{path}"`
(`os/pkgs/mosd/apid/openapi.json:187`) and `"/api/versions"`
(`os/pkgs/mosd/apid/openapi.json:260`) — and the router declares nothing else under the
prefix beyond a fallback (`os/pkgs/mosd/apid/src/routes.rs:283-294`). There is no
...
(`os/pkgs/mosd/apid/src/routes.rs:164-166`) or for any of the four SSH mutations
(`os/pkgs/mosd/apid/src/routes.rs:170-172`), *"so a browser prefetch, a crawler or a mis-clicked link cannot power
the appliance off"*.

The **confirmation token** that guards the three form posts today
(`os/pkgs/mosd/apid/src/routes.rs:2433`, and `TRANSIENT_CONFIRM_TOKEN` at `os/pkgs/mosd/apid/src/routes.rs:2525`) does
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:2453-2466`). 202
...
- **`POST /login` and `POST /logout`.** A session is a browser mechanism (§3.1).
  ...
  mislead.
- **`GET /healthz`** — declared at `os/pkgs/mosd/apid/src/routes.rs:181` and answering
  at `os/pkgs/mosd/apid/src/routes.rs:824-826`. It keeps its path, its unauthenticated exemption (`os/pkgs/mosd/apid/src/routes.rs:779`) and
  its literal `ok` body,
  ...
  **But it answers a narrower question than its name suggests, and the API must
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:779`), so it answers `ok` on an appliance whose
...
- **A password change.** `access.webAdmin.password_hash` is written by exactly
  ...
  `os/pkgs/mosd/apid/src/routes.rs:1393`, and
...
- **`ReportHealth`.** mosd exposes it (`os/pkgs/mosd/mosd/src/bus.rs:550`) and apid does
  not declare it on the proxy (`os/pkgs/mosd/apid/src/bus_client.rs:20-26`). The API does
...
(`os/pkgs/mosd/apid/src/routes.rs:3356-3362`, via `ssh_error` at `os/pkgs/mosd/apid/src/routes.rs:2659-2668`). For a
...
(`os/pkgs/mosd/apid/src/routes.rs:361-363`, payload at `:316-334`), the schema records
it as `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:288-290`), and every
failure under the prefix is built through it. `Retry-After` ships on exactly
one class: `const RETRY_AFTER_SECONDS: &str = "5";`
(`os/pkgs/mosd/apid/src/routes.rs:268`), attached only when the status is 503
(`os/pkgs/mosd/apid/src/routes.rs:667-672`). The recommendation this section makes —
...
(`os/pkgs/mosd/apid/src/routes.rs:638-672`), which matches on the concrete
`zbus::Error::MethodError` before the conversion flattens it and maps the three
fdo names declared at `os/pkgs/mosd/apid/src/routes.rs:263-265`. The source states the
rule in the same words this section chose: *"The classification is translated
and the message is not"* (`os/pkgs/mosd/apid/src/routes.rs:629`).
...
|---|---|---|---|
| `error` | the only top-level member | `"required": [ "error" ]` (`os/pkgs/mosd/apid/openapi.json:288-290`) | none |
| `code` | *"stable machine token; an OPEN set"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:306-307`), required (`:227-231`) | none in shape. The **open set** is stated only in the description (`os/pkgs/mosd/apid/openapi.json:308`); nothing in the schema expresses it, so a generated client learns the rule only if a human reads the prose |
| `message` | *"human-readable; not for matching on"* | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:310-312`), required | none |
| `source` | `"apid" \| "mosd"`, an enum of exactly two | `"type": "string"` (`os/pkgs/mosd/apid/openapi.json:321-322`), required, **no `enum`** | **a real difference.** The Rust type admits only two values, because `ApiError` is constructed by exactly two constructors, `ApiError::apid`, which passes `Self::new(code, message, "apid")`
(`os/pkgs/mosd/apid/src/routes.rs:388`), and `ApiError::mosd`, which passes
`Self::new(code, message, "mosd")` (`os/pkgs/mosd/apid/src/routes.rs:393`). The published contract does not say so, so a client generated from the document gets an unconstrained string and cannot exhaustively match on it |
| `path` | OPTIONAL, present only when the failure names a dot-path | `"type": [ "string", "null" ]` (`os/pkgs/mosd/apid/openapi.json:314-318`), **absent from `required`** | **a real difference, in the opposite direction.** The schema admits an explicit `null`; the wire never carries one, because the field carries a `serde(skip_serializing_if = "Option::is_none")` attribute (`os/pkgs/mosd/apid/src/routes.rs:381`) and is therefore omitted rather than nulled — which is the behaviour this section asks for and the source's own doc comment defends (`os/pkgs/mosd/apid/src/routes.rs:376-380`). The schema is wider than the implementation, so a client that handles `null` is handling a case the device does not produce |
...
| `code` | Ships? | Where |
|---|---|---|
| `not_authenticated` | **yes**, with a different credential | `os/pkgs/mosd/apid/src/routes.rs:712`. It is raised by the session-cookie extractor (`os/pkgs/mosd/apid/src/routes.rs:709-715`), not by a bearer token: §3.2's token does not exist, so the shipped 401 means *"no session cookie, or one that does not verify"* (`os/pkgs/mosd/apid/src/routes.rs:713`) |
| `not_found` | **yes** | `os/pkgs/mosd/apid/src/routes.rs:208`, from the reserved subtree's fallback |
...
| `settings_rejected` | **yes**, 422 | `os/pkgs/mosd/apid/src/routes.rs:648-651`, on fdo `InvalidArgs` (`os/pkgs/mosd/apid/src/routes.rs:263`) |
| `settings_io` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:652-655`, on fdo `IOError` (`os/pkgs/mosd/apid/src/routes.rs:264`) |
| `mosd_failed` | **yes**, 500 | `os/pkgs/mosd/apid/src/routes.rs:656-659`, on fdo `Failed` (`os/pkgs/mosd/apid/src/routes.rs:265`) |
| `mosd_unreachable` | **yes**, 503 with `Retry-After` | `os/pkgs/mosd/apid/src/routes.rs:678-684`, and it is exhaustive over everything the three above do not name (`os/pkgs/mosd/apid/src/routes.rs:674-676`) |

**What does not ship.** `GET /api/v1/health` does not exist: it is not among
the four paths in `os/pkgs/mosd/apid/openapi.json:8-281`. The concern its last
...
(`os/pkgs/mosd/apid/src/bus_client.rs:154-163`, and the same shape at `:164-173`, `:175-187`,
...
(`os/pkgs/mosd/apid/src/routes.rs:755-765`, message at `os/pkgs/mosd/apid/src/routes.rs:761`). So a settings value mosd
...
(`os/pkgs/mosd/apid/src/routes.rs:2210-2297`), and `write_key_list` does the same
(`os/pkgs/mosd/apid/src/routes.rs:2652-2654`). The VLAN example above is exactly this — a rejection that reads
as an outage.
...
text in an error box (`os/pkgs/mosd/apid/src/routes.rs:888-890`, `:1223`, `:1243`) — without
widening it.
...
2. **Bus unavailable.** `mosd_unreachable`, **503**, with `Retry-After: 5`. 503
   ...
   reconnects (`os/pkgs/mosd/apid/src/bus_client.rs:76-79`, `:116-119`). **This shipped**:
   `mosd_unreachable` answers 503 (`os/pkgs/mosd/apid/src/routes.rs:679-683`) with
   `Retry-After` attached (`os/pkgs/mosd/apid/src/routes.rs:667-672`). The cost is
   named and is now real rather than predicted: the HTML path still returns 502
   for the same underlying failure (`os/pkgs/mosd/apid/src/routes.rs:755-765`), so until §8 changes both, one appliance
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   crash"* (`os/pkgs/mosd/apid/src/bus_client.rs:77-79`). The trap is already in the tree:
   `/healthz` returns before any check (`os/pkgs/mosd/apid/src/routes.rs:779`) and
   answers the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:825`), so **an appliance whose mosd is dead
   ...
   off above the bus call (`os/pkgs/mosd/apid/src/routes.rs:779`), and a request with a
   valid session cookie is handed off above it too
   (`os/pkgs/mosd/apid/src/routes.rs:800-804`), so no API route depends on the gate's
...
middleware, `pub(crate) struct ApiSession;` (`os/pkgs/mosd/apid/src/routes.rs:695`),
whose whole test is
`session::cookie_from_headers(&parts.headers)` verified against the store
(`os/pkgs/mosd/apid/src/routes.rs:704-705`); its rejection is §2.4's envelope with a
401 rather than the gate's HTML redirect
(`os/pkgs/mosd/apid/src/routes.rs:709-715`). The gate hands the declared `/api` routes
off to it with `is_declared_api_route(path)`
(`os/pkgs/mosd/apid/src/routes.rs:779`, predicate at `:277-282`). There is no bearer
...
exchange as success"* (`os/pkgs/mosd/apid/src/routes.rs:692-694`). **Unanswered:**
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   `Redirect::to("/login").into_response()` (`os/pkgs/mosd/apid/src/routes.rs:821`), a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:816-818`) and which returns **200 OK** with an HTML
   form (`os/pkgs/mosd/apid/src/routes.rs:1451-1468`). A script that checks the status
   ...
   instead (`os/pkgs/mosd/apid/src/routes.rs:709-715`). Every other path on the daemon
   still redirects.
2. **Obtaining the credential means emulating three browser behaviours.**
   `POST /login` takes `Form(form): Form<LoginForm>,`
   (`os/pkgs/mosd/apid/src/routes.rs:1473`) — axum's URL-encoded extractor, imported at
   `os/pkgs/mosd/apid/src/routes.rs:19`, not JSON — answers **302 to `/`**
   (`os/pkgs/mosd/apid/src/routes.rs:1522`), and delivers the credential in a
   `Set-Cookie` header (`os/pkgs/mosd/apid/src/routes.rs:1521`, value built at
...
6. **There is exactly one credential, and it identifies a human.** The only
   ...
   the admin hash, read through `password_hash` (`os/pkgs/mosd/apid/src/routes.rs:747-752`); there is
   no second credential, no user table, and no reference to `access.device` in
...
7. **A misconfigured script locks the operator out, repeatedly.** The login
   ...
   *"the appliance has one admin password, so per-client tracking buys nothing
   ...
   `os/pkgs/mosd/apid/src/routes.rs:1486-1496`). A script retrying with a
...
(`os/pkgs/mosd/apid/openapi.json:8-281`).
...
`ApiSession` extractor (`os/pkgs/mosd/apid/src/routes.rs:695-717`) against the same
...
4. **It costs zero extra bus round trips.** The gate already calls
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:813`) and `apiTokens` is a child of `access`, so
...
- Mint and revoke are read-modify-write of the whole array, because the dot-path
  ...
  the same pattern the SSH key pane uses (`os/pkgs/mosd/apid/src/routes.rs:3319-3324`).
  Two concurrent mints lose one token, silently.
- Identity is the `id`, never a list position, for the reason recorded at
  `os/pkgs/mosd/apid/src/routes.rs:3348-3352`: an index is meaningful only against the
...
- **Coexisting means two credentials at one privilege level** — and there is
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:200`). A token can do everything the operator can do
...
2. **§3.1 item 7 changes meaning.** The `auth.rs:53-54` comment's reasoning —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1493-1502`). Bearer verification is **not** rate
...
(`os/pkgs/mosd/apid/src/routes.rs:709-715`) and a redirect to `/login` everywhere else
(`os/pkgs/mosd/apid/src/routes.rs:821`) — and the CORS argument below, which depends on
...
2. **A stolen token is full management access, indefinitely.** There is no
   ...
   `os/pkgs/mosd/apid/src/routes.rs:2521`) and is then no longer needed. Detection is not
...
5. **A same-origin page mints a permanent token with the operator's cookie.**
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:2521`). But a minted token survives deactivating
...
pane is tested to carry, `os/pkgs/mosd/apid/src/routes.rs:2521`); apid runs as root
...
- **Under this proposal the API path has no CSRF exposure**, because a bearer
  ...
  **None of that is true of the shipped API**, whose credential *is* the cookie
  (`os/pkgs/mosd/apid/src/routes.rs:704-705`), so today the four `/api` routes sit
...
- **The per-action confirm tokens are not CSRF tokens and must not be counted as
  ...
  and `"poweroff"` (`os/pkgs/mosd/apid/src/routes.rs:2353-2357`) and
  `TRANSIENT_CONFIRM_TOKEN` is the literal `"set-transient-password"`
  (`os/pkgs/mosd/apid/src/routes.rs:2525`). They are not secret, not per-session and not
  ...
  the checkbox has to be ticked as well"* (`os/pkgs/mosd/apid/src/routes.rs:2351-2352`). They stop a mis-click and a prefetch. Against a
...
(`os/pkgs/mosd/apid/src/routes.rs:800-806`), so an authenticated flood no longer
...
(`os/pkgs/mosd/apid/src/routes.rs:779`) and its handler makes no mosd call
(`os/pkgs/mosd/apid/src/routes.rs:455-463`), so it is an unauthenticated route that
...
`/proc/uptime` (`os/pkgs/mosd/apid/src/routes.rs:1595`) and its own state directory
...
declaration at `os/pkgs/mosd/apid/src/routes.rs:165`, which is why that line is not
...
`0d4f3c6` the fifteen declarations are at `os/pkgs/mosd/apid/src/routes.rs:130-149`,
...
(`os/pkgs/mosd/apid/src/routes.rs:156-185`). A custom
...
`os/pkgs/mosd/apid/src/routes.rs:49` and `:578`) moves under section 6.3's reserved
...
(`os/pkgs/mosd/apid/src/routes.rs:185`). The immutable class is read per request from
...
- **`/var/lib/mos/settings.toml`** (`os/pkgs/mosd/mosd-settings/src/store.rs:67`) — the
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:751-755`) and every authorized SSH key.
...
§6.3's reserved prefix, `builtin_home` in `os/pkgs/mosd/apid/src/routes.rs:911`. Its
...
(`os/pkgs/mosd/apid/src/routes.rs:850-855`), described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:828`). Section 1.6 evidences the rest: no
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:755`), reached when a mosd call
  ...
  *before* dispatch — so at the moment that page is on screen, `/builtin/` is
  ...
  page (`os/pkgs/mosd/apid/src/routes.rs:739`), the navigation on every built-in pane
...
page *"The management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:761`,
reachable because the bus client connects lazily and drops its cache on error,
`os/pkgs/mosd/apid/src/bus_client.rs:23-27`, `:42-59`).
...
1. **A signature and the upload credential authorise the same blast radius.** A
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:2521`). §3.2 states there are **no scopes in
...
(`os/pkgs/mosd/apid/src/routes.rs:33`, `:727-734`, `:745`; §1.1 constraint 1 and §1.6
...
Items (iv) and (v) are already shipped — `os/pkgs/mosd/apid/src/routes.rs:2329`, `:1728`,
...
(`os/pkgs/mosd/apid/src/bus_client.rs:160`, `:170`, `:181`, `:206`) and `zbus` 5.19.0
...
rendering `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:754-765`) for a value mosd
...
the settings write fails (`os/pkgs/mosd/apid/src/routes.rs:2298-2304`), and `write_key_list`
...
(`os/pkgs/mosd/apid/src/routes.rs:640-641`) and mapping `FDO_INVALID_ARGS`,
...
(`os/pkgs/mosd/apid/src/routes.rs:813`) the whole appliance answers the 502 page *"The
management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:759-769`). **The
...
path answers 422 (`os/pkgs/mosd/apid/src/routes.rs:3356-3362`). The HTML path is not
changed.
...
setup mode (`os/pkgs/mosd/apid/src/routes.rs:817-822`) — and it returns a token (§2.3),
...
HTML path answers **502** (`os/pkgs/mosd/apid/src/routes.rs:759-769`), so one appliance
...
2. **A `SettingsChanged` subscription, and any change-stream API.** mosd emits
   ...
   declares no `#[zbus(signal)]` member (`os/pkgs/mosd/apid/src/bus_client.rs:20-26`).
...
`os/pkgs/mosd/apid/src/routes.rs:200`), so a token can do everything the operator can do
...
(`os/pkgs/mosd/apid/src/routes.rs:813`), against the single lock mosd holds over both
