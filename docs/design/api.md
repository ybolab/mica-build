...
(`os/pkgs/mosd/apid/src/routes.rs:3346`), whose body runs from
`const STYLE: &str = "\` (`os/pkgs/mosd/apid/src/routes.rs:3347`) to
`os/pkgs/mosd/apid/src/routes.rs:3326`, injected into the page head as
`style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:3365`). The crate
...
| `GET /healthz` | `:173` | `healthz` (`:729`) | neither — plain text | Returns the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:3343`) — a liveness probe only: it proves the apid process is listening and checks nothing else, not mosd, not the settings tree, not the appliance | none |
...
**The `/api` subtree.** `api_router()` (`os/pkgs/mosd/apid/src/routes.rs:414-496`) is
...
(`os/pkgs/mosd/apid/src/routes.rs:670-677`).
...
(`os/pkgs/mosd/apid/src/routes.rs:674`), as does `api_not_found`. **No route accepts a
...
example `Form(form): Form<SetupForm>,` (`os/pkgs/mosd/apid/src/routes.rs:3868`),
...
on power (`os/pkgs/mosd/apid/src/routes.rs:5742`) and on the transient password
(`os/pkgs/mosd/apid/src/routes.rs:6677`).
...
(`os/pkgs/mosd/apid/src/routes.rs:3276`), built at `os/pkgs/mosd/apid/src/routes.rs:3241-3252`.
...
(`os/pkgs/mosd/apid/src/routes.rs:3258-3263`), typed as
...
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:4358`), set
at `os/pkgs/mosd/apid/src/routes.rs:4331-4335`. `POST /setup` does the same at the end
of the first-run wizard without a login step
(`os/pkgs/mosd/apid/src/routes.rs:3947-3952`). The store generates 16 random bytes from
...
(`os/pkgs/mosd/apid/src/routes.rs:3292`) to `os/pkgs/mosd/apid/src/routes.rs:3312`. It implements
...
1. `/healthz` and the declared `/api/` routes always pass —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3294`). The API routes answer for themselves in
   §2.4's envelope rather than in the gate's HTML redirect, and which paths
   qualify is decided by `is_declared_api_route`
   (`os/pkgs/mosd/apid/src/routes.rs:540-618`) rather than by a prefix test.
...
`state.api.get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:3225`) is now paid
...
time (`os/pkgs/mosd/apid/src/routes.rs:808`) rather than copying it. It is **read-only
...
the SSH pane reads the whole key list (`os/pkgs/mosd/apid/src/routes.rs:6029-6031`),
edits it in memory (`os/pkgs/mosd/apid/src/routes.rs:6710-6714`), and writes the whole
list back through `write_key_list` (`os/pkgs/mosd/apid/src/routes.rs:6062-6075`).
...
(`os/pkgs/mosd/apid/src/routes.rs:1271`) — which is the one place where
...
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:3346`). There is still no favicon route, no font and
...
the gate redirects it to `/login` (`os/pkgs/mosd/apid/src/routes.rs:3339`), and
...
control (`os/pkgs/mosd/apid/src/routes.rs:5026`) and the install path. The consequence
...
1. **Line numbers throughout its section 2.1 no longer resolve to the routes
   ...
   at `os/pkgs/mosd/apid/src/routes.rs:4713`.
2. **Its route table is missing thirteen routes.** The five SSH routes it
   ...
   the four `/api` routes (`os/pkgs/mosd/apid/src/routes.rs:416-423`). Its own section
...
`"/api/versions"` (`os/pkgs/mosd/apid/openapi.json:2144`). Its
`components.schemas` are exactly five: `"ApiError"`
(`os/pkgs/mosd/apid/openapi.json:2210`), `"ApiErrorDetail"`
(`os/pkgs/mosd/apid/openapi.json:2222`), `"ApiMeta"` (`os/pkgs/mosd/apid/openapi.json:2286`),
`"ApiVersions"` (`os/pkgs/mosd/apid/openapi.json:2336`) and `"ResourceValue"`
(`os/pkgs/mosd/apid/openapi.json:2537`). The document is generated from the handlers
...
(`os/pkgs/mosd/apid/src/routes.rs:808`) rather than copied. The served set is an array
...
1. The gate already dispatches on `let path = request.uri().path();`
   (`os/pkgs/mosd/apid/src/routes.rs:3293`). A version check is the same string operation
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:540-618`), a pure string operation over the same
...
2. axum registers routes by path (`os/pkgs/mosd/apid/src/routes.rs:138-229`). A second
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:414-496`) is the first of those routers and shows
   the shape.
...
(`os/pkgs/mosd/apid/src/routes.rs:3248`) — that is not the objection. The
...
Both rows ship. The first is declared at `os/pkgs/mosd/apid/src/routes.rs:416` and
...
exactly one source"* (`os/pkgs/mosd/apid/src/routes.rs:789-791`). The API version is the
...
(`os/pkgs/mosd/apid/src/routes.rs:3328-3333`). An authenticated probe cannot answer the
...
(`os/pkgs/mosd/apid/src/routes.rs:3266`, `:3278-3280`), so this is one more bit on a
...
(`os/pkgs/mosd/apid/src/routes.rs:961`) — the dot-path passthrough this section asks
...
(`os/pkgs/mosd/apid/src/routes.rs:1271`). Both answer `ResourceValue`
(`os/pkgs/mosd/apid/src/routes.rs:933`), which carries a
`serde(transparent)` attribute (`os/pkgs/mosd/apid/src/routes.rs:932`) so the body is
...
to write"* (`os/pkgs/mosd/apid/src/routes.rs:1290`) — the key lives in a file on
...
(`os/pkgs/mosd/apid/src/routes.rs:926-927`). And `GET /api/v1/state/{path}` today
...
- **No array indexing.** The dot-path syntax has none; the model says a list is
  *"Written as a whole JSON array through the dot-path API"*
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:6710-6715`). Two clients doing that concurrently
...
- **A key holding a dot is addressed with a quoted segment.** apid accepts an
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3450-3456`) — and until PLAN-022 M2 the
...
nobody asked to delete"* (`os/pkgs/mosd/apid/src/routes.rs:6839-6843`). A `DELETE` whose
...
the file (`os/pkgs/mosd/apid/src/routes.rs:6035-6037`). So the difference is the quality
...
| SSH keys | the authorized-keys collection above | `access.ssh.authorizedKeys` | **every key is a root key** (`docs/design/access.md` §4.1, `os/pkgs/mosd/apid/src/routes.rs:5912`); the API response must carry that sentence in a `notice` field for the same reason the pane must carry it |
...
prefix beyond a fallback (`os/pkgs/mosd/apid/src/routes.rs:414-496`). There is no
...
(`os/pkgs/mosd/apid/src/routes.rs:5742`, and `TRANSIENT_CONFIRM_TOKEN` at `os/pkgs/mosd/apid/src/routes.rs:5916`) does
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:5762-5775`). 202
...
- **A password change — [implemented], on both surfaces.** This bullet once
  ...
  (`change_password`, `os/pkgs/mosd/apid/src/routes.rs:4406-4465`): the
...
(`os/pkgs/mosd/apid/src/routes.rs:6832-6838`, via `ssh_error` at `os/pkgs/mosd/apid/src/routes.rs:6050-6059`). For a
...
(`os/pkgs/mosd/apid/src/routes.rs:680-682`, payload at `:646-661`), the schema records
...
error names declared at `os/pkgs/mosd/apid/src/routes.rs:394-396` — two
...
translated and the message is not"* (`os/pkgs/mosd/apid/src/routes.rs:3022`).
...
(`os/pkgs/mosd/apid/src/routes.rs:707`), and `ApiError::mosd`, which passes
`Self::new(code, message, "mosd")` (`os/pkgs/mosd/apid/src/routes.rs:712`). The published contract does not say so, so a client generated from the document gets an unconstrained string and cannot exhaustively match on it |
| `path` | OPTIONAL, present only when the failure names a dot-path | `"type": [ "string", "null" ]` (`os/pkgs/mosd/apid/openapi.json:2239-2243`), **absent from `required`** | **a real difference, in the opposite direction.** The schema admits an explicit `null`; the wire never carries one, because the field carries a `serde(skip_serializing_if = "Option::is_none")` attribute (`os/pkgs/mosd/apid/src/routes.rs:700`) and is therefore omitted rather than nulled — which is the behaviour this section asks for and the source's own doc comment defends (`os/pkgs/mosd/apid/src/routes.rs:695-699`). The schema is wider than the implementation, so a client that handles `null` is handling a case the device does not produce |
...
| `code` | Ships? | Where |
|---|---|---|
| `settings_not_found` | **yes**, 404 | `os/pkgs/mosd/apid/src/routes.rs:3024-3027`, on `com.mos.mosd1.Error.NotFound` |
| `settings_read_only` | **yes**, 409 | `os/pkgs/mosd/apid/src/routes.rs:3028-3037`, on `com.mos.mosd1.Error.ReadOnly` |
...
(`os/pkgs/mosd/apid/src/routes.rs:3265-3280`, message at `os/pkgs/mosd/apid/src/routes.rs:3248`). So a settings value mosd
...
(`os/pkgs/mosd/apid/src/routes.rs:5519-5606`), and `write_key_list` does the same
(`os/pkgs/mosd/apid/src/routes.rs:6043-6045`). The VLAN example above is exactly this — a rejection that reads
as an outage.
...
2. **Bus unavailable.** `mosd_unreachable`, **503**, with `Retry-After: 5`. 503
   ...
   `mosd_unreachable` answers 503 (`os/pkgs/mosd/apid/src/routes.rs:3058-3062`) with
   `Retry-After` attached (`os/pkgs/mosd/apid/src/routes.rs:3046-3051`). The cost this
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   answers the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:3343`), so **an appliance whose mosd is dead
...
exchange as success"* (`os/pkgs/mosd/apid/src/routes.rs:3122-3124`). **Unanswered:**
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   `Redirect::to("/login").into_response()` (`os/pkgs/mosd/apid/src/routes.rs:3339`), a
   ...
   form (`os/pkgs/mosd/apid/src/routes.rs:3970-4279`). A script that checks the status
...
2. **Obtaining the credential means emulating three browser behaviours.**
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:4312`) — axum's URL-encoded extractor, imported at
   `os/pkgs/mosd/apid/src/routes.rs:19`, not JSON — answers **302 to `/`**
   (`os/pkgs/mosd/apid/src/routes.rs:4333`), and delivers the credential in a
   `Set-Cookie` header (`os/pkgs/mosd/apid/src/routes.rs:4332`, value built at
...
6. **There is exactly one credential, and it identifies a human.** The only
   ...
   the admin hash, read through `password_hash` (`os/pkgs/mosd/apid/src/routes.rs:3258-3263`); there is
   no second credential, no user table, and no reference to `access.device` in
...
7. **A misconfigured script locks the operator out, repeatedly.** The login
   ...
   *"the appliance has one admin password, so per-client tracking buys nothing
   ...
   `os/pkgs/mosd/apid/src/routes.rs:4297-4307`). A script retrying with a
...
- Mint and revoke are read-modify-write of the whole array, because the dot-path
  ...
  the same pattern the SSH key pane uses (`os/pkgs/mosd/apid/src/routes.rs:6710-6715`).
  Two concurrent mints lose one token, silently.
- Identity is the `id`, never a list position, for the reason recorded at
  `os/pkgs/mosd/apid/src/routes.rs:6824-6828`: an index is meaningful only against the
...
2. **§3.1 item 7 changes meaning.** The `auth.rs:53-54` comment's reasoning —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:4304-4313`). Bearer verification is **not** rate
...
2. **A stolen token is full management access, indefinitely.** There is no
   ...
   `os/pkgs/mosd/apid/src/routes.rs:5912`) and is then no longer needed. Detection is not
...
5. **A same-origin page mints a permanent token with the operator's cookie.**
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:5912`). But a minted token survives deactivating
...
pane is tested to carry, `os/pkgs/mosd/apid/src/routes.rs:5912`); apid runs as root
...
- **The per-action confirm tokens are not CSRF tokens and must not be counted as
  ...
  and `"poweroff"` (`os/pkgs/mosd/apid/src/routes.rs:5690-5694`) and
  `TRANSIENT_CONFIRM_TOKEN` is the literal `"set-transient-password"`
  (`os/pkgs/mosd/apid/src/routes.rs:5944`). They are not secret, not per-session and not
  ...
  the checkbox has to be ticked as well"* (`os/pkgs/mosd/apid/src/routes.rs:5688-5689`). They stop a mis-click and a prefetch. Against a
...
(`os/pkgs/mosd/apid/src/routes.rs:777-785`), so it is an unauthenticated route that
...
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:3346`). Section 1.6 evidences the rest: no
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:3269`), reached when a mosd call
...
page *"The management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:3276`,
...
1. **A signature and the upload credential authorise the same blast radius.** A
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:5912`). §3.2 states there are **no scopes in
...
Items (iv) and (v) are already shipped — `os/pkgs/mosd/apid/src/routes.rs:5638`, `:2245`,
...
rendering `bus_error` (`os/pkgs/mosd/apid/src/routes.rs:3265-3280`) for a value mosd
...
the settings write fails (`os/pkgs/mosd/apid/src/routes.rs:5607-5613`), and `write_key_list`
...
(`os/pkgs/mosd/apid/src/routes.rs:3039-3040`) and mapping `FDO_INVALID_ARGS`,
...
management daemon is unavailable."* (`os/pkgs/mosd/apid/src/routes.rs:3274-3284`). **The
...
path answers 422 (`os/pkgs/mosd/apid/src/routes.rs:6832-6838`). The HTML path is not
changed.
...
- **The VLAN dot-path limit — since closed, and closed additively.**
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3450-3456`) — while `split_path` split on it
