...
JavaScript, `os/pkgs/mosd/apid/src/routes.rs:5402`. The substantive claim is
...
(`os/pkgs/mosd/apid/src/routes.rs:3361`), whose body runs from
`const STYLE: &str = "\` (`os/pkgs/mosd/apid/src/routes.rs:3362`) to the line that closes it,
`.saved{background:#dfd;border:1px solid #080;padding:.5rem 1rem;margin-bottom:1rem}";`
(`os/pkgs/mosd/apid/src/routes.rs:3369`), injected into the page head as
`style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:3380`). The crate
...
|---|---|---|---|---|---|
| `GET /` | `get(serve::root)` (`os/pkgs/mosd/apid/src/routes.rs:145`) | `pub async fn root` (`os/pkgs/mosd/apid/src/assets/serve.rs:57`) | GET page, conditional | The active bundle's `index.html` when a bundle is active and its index is readable, and the built-in status pane otherwise — `None => built_in(&state).await,` (`os/pkgs/mosd/apid/src/assets/serve.rs:57-65`); the built-in branch is `fn home` (`os/pkgs/mosd/apid/src/routes.rs:4778`) over `fn status_body` (`os/pkgs/mosd/apid/src/routes.rs:4734`) | on the built-in branch only, three now and not two: `get_settings("hostname")` (`os/pkgs/mosd/apid/src/routes.rs:4735`), `get_state("network")` (`os/pkgs/mosd/apid/src/routes.rs:4736`) and `get_state("uptime")` (`os/pkgs/mosd/apid/src/routes.rs:4737-4742`) |
| `GET /builtin` | `get(builtin_home)` (`os/pkgs/mosd/apid/src/routes.rs:166`, inside the nest opened with `.nest(` (`os/pkgs/mosd/apid/src/routes.rs:163`)) | `fn builtin_home` (`os/pkgs/mosd/apid/src/routes.rs:4823`) | GET page | §6.3's escape: the status pane, §3.2's token section (`fn tokens_section` (`os/pkgs/mosd/apid/src/routes.rs:4861`)) and the deactivate control (`fn escape_section` (`os/pkgs/mosd/apid/src/routes.rs:5059`)), reachable whatever state a bundle is in | `status_body`'s three, plus, for the token list, `.get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:4847-4852`) |
| `POST /builtin/deactivate` | `post(builtin_deactivate)` (`os/pkgs/mosd/apid/src/routes.rs:170`) | `fn builtin_deactivate` (`os/pkgs/mosd/apid/src/routes.rs:5090`) | HTML form POST, no body | Removes `/srv/ui/current` through `deactivate()` (`os/pkgs/mosd/apid/src/routes.rs:5091`); "a pointer was there" and "there was none" are both success — `Ok(removed) => {` (`os/pkgs/mosd/apid/src/routes.rs:5092-5119`) — and 500 only when the removal itself failed, `Err(err) => {` (`os/pkgs/mosd/apid/src/routes.rs:5125-5141`) | none |
| `POST /builtin/tokens` | `post(builtin_tokens_mint)` (`os/pkgs/mosd/apid/src/routes.rs:179`) | `fn builtin_tokens_mint` (`os/pkgs/mosd/apid/src/routes.rs:4930`) | HTML form POST | §3.2's bootstrap, and the one place a browser holding only the session cookie can obtain a bearer token: mints, writes the list, and renders the secret once, `fn minted_page` (`os/pkgs/mosd/apid/src/routes.rs:5020`). `POST` only, and the comment says why: *"a GET mint would be a permanent-credential factory reachable from any link an operator clicks"* (`os/pkgs/mosd/apid/src/routes.rs:172-178`) | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:4847-4852`), `set_settings(API_TOKENS_PATH, &value)` (`os/pkgs/mosd/apid/src/routes.rs:1653`) |
| `POST /builtin/tokens/revoke` | `post(builtin_tokens_revoke)` (`os/pkgs/mosd/apid/src/routes.rs:180`) | `fn builtin_tokens_revoke` (`os/pkgs/mosd/apid/src/routes.rs:4982`) | HTML form POST | Drops one token by id and rewrites the list; an id matching nothing is **422** here and 404 on `DELETE /api/v1/tokens/{id}`, a split argued from *"this response body is a re-rendered pane, no consumer on this surface reads the status"* (`os/pkgs/mosd/apid/src/routes.rs:4973-4981`) | as the mint |
| any other `/builtin/…` | `.fallback(builtin_not_found)` (`os/pkgs/mosd/apid/src/routes.rs:181`) | `fn builtin_not_found` (`os/pkgs/mosd/apid/src/routes.rs:5154`) | GET page | 404 — `StatusCode::NOT_FOUND,` (`os/pkgs/mosd/apid/src/routes.rs:5155-5156`) — naming the escape, so the reserved prefix answers for its whole subtree rather than falling through to a bundle | none |
| `GET /builtin/` | `.route(BUILTIN_PATH, get(builtin_home))` (`os/pkgs/mosd/apid/src/routes.rs:183`) | `fn builtin_home` (`os/pkgs/mosd/apid/src/routes.rs:4823`) | GET page | The trailing-slash spelling, declared outside the nest because `nest` claims `/builtin` and not `/builtin/` | as `GET /builtin` |
| `GET /setup` | `get(setup_form)` (`os/pkgs/mosd/apid/src/routes.rs:184`) | `fn setup_form` (`os/pkgs/mosd/apid/src/routes.rs:3838`) | GET page | First-run wizard: admin password, optional hostname, optional single interface; bounces to `/login` once `if password_hash(&access).is_some() {` (`os/pkgs/mosd/apid/src/routes.rs:3843-3845`) | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:3839`), `get_settings("hostname")` (`os/pkgs/mosd/apid/src/routes.rs:3846`) |
| `POST /setup` | `.post(setup_submit)` (`os/pkgs/mosd/apid/src/routes.rs:184`) | `fn setup_submit` (`os/pkgs/mosd/apid/src/routes.rs:3880`) | HTML form POST | 409 when already configured, `if password_hash(&access).is_some() {` (`os/pkgs/mosd/apid/src/routes.rs:3889-3898`); 400 on `if password_under_floor(&form.password) {` (`os/pkgs/mosd/apid/src/routes.rs:3899-3915`); 422 on `!valid_hostname(hostname)` (`os/pkgs/mosd/apid/src/routes.rs:3918-3936`); 500 on *"password hashing failed"* (`os/pkgs/mosd/apid/src/routes.rs:3945-3948`); on success writes, invalidates the gate's access cache with `state.access_cache.invalidate();` (`os/pkgs/mosd/apid/src/routes.rs:3957`) and issues a session cookie, `let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:3990-3994`), then redirects to `/` | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:3885`), `set_settings("access.webAdmin", &value)` (`os/pkgs/mosd/apid/src/routes.rs:3951`), `get_settings("hostname")` (`os/pkgs/mosd/apid/src/routes.rs:3963`), `set_settings("hostname", &Value::String(hostname.to_string()))` (`os/pkgs/mosd/apid/src/routes.rs:3970`), `set_settings(&iface_settings_path(iface), &value)` (`os/pkgs/mosd/apid/src/routes.rs:3984`) |
| `GET /login` | `get(login_form)` (`os/pkgs/mosd/apid/src/routes.rs:185`) | `fn login_form` (`os/pkgs/mosd/apid/src/routes.rs:4327`) | GET page | One password field, and a link to §6.3's prefix | none |
| `POST /login` | `.post(login_submit)` (`os/pkgs/mosd/apid/src/routes.rs:185`) | `fn login_submit` (`os/pkgs/mosd/apid/src/routes.rs:4346`) | HTML form POST | 429 while the login guard is locked, `if !state.guard.begin_attempt() {` (`os/pkgs/mosd/apid/src/routes.rs:4362-4372`); 401 on *"Wrong password."* (`os/pkgs/mosd/apid/src/routes.rs:4401-4411`); on success sets the session cookie, `session::session_cookie(&cookie)` (`os/pkgs/mosd/apid/src/routes.rs:4392-4400`) | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:4373`) |
| `POST /logout` | `post(logout)` (`os/pkgs/mosd/apid/src/routes.rs:186`) | `fn logout` (`os/pkgs/mosd/apid/src/routes.rs:4414`) | HTML form POST, no body | Drops the server-side session and clears the cookie — `state.sessions.remove(&value);` (`os/pkgs/mosd/apid/src/routes.rs:4419-4427`) | none |
| `GET /password` | `get(password_form)` (`os/pkgs/mosd/apid/src/routes.rs:187`) | `fn password_form` (`os/pkgs/mosd/apid/src/routes.rs:4568`) | GET page | The change-password pane, carrying §3.2's ratified token sentence verbatim in `PASSWORD_TOKEN_NOTICE` (`os/pkgs/mosd/apid/src/routes.rs:4543`) | none |
| `POST /password` | `.post(password_submit)` (`os/pkgs/mosd/apid/src/routes.rs:187`) | `fn password_submit` (`os/pkgs/mosd/apid/src/routes.rs:4572`) | HTML form POST | 400 on a mismatch or an under-floor password, 401 on a wrong current password, 500 on a hashing failure — the arms from `Ok(()) => Redirect::to("/password?saved=1").into_response(),` (`os/pkgs/mosd/apid/src/routes.rs:4595-4611`); on success every session but the acting one is dropped, in `fn change_password` (`os/pkgs/mosd/apid/src/routes.rs:4471`) | `get_settings("access")` (`os/pkgs/mosd/apid/src/routes.rs:4481`), `set_settings("access.webAdmin", &value)` (`os/pkgs/mosd/apid/src/routes.rs:4515`) |
| `GET /network` | `get(network_form)` (`os/pkgs/mosd/apid/src/routes.rs:188`) | `fn network_form` (`os/pkgs/mosd/apid/src/routes.rs:5545`) | GET page | One form per configured interface, per kind, plus an add-interface form; `?saved=1` renders a banner, `query.saved.is_some().then(saved_banner)` (`os/pkgs/mosd/apid/src/routes.rs:5548`) | `get_settings("network")` (`os/pkgs/mosd/apid/src/routes.rs:5329`), `get_state("network")` (`os/pkgs/mosd/apid/src/routes.rs:5332`) |
| `POST /network` | `.post(network_submit)` (`os/pkgs/mosd/apid/src/routes.rs:188`) | `fn network_submit` (`os/pkgs/mosd/apid/src/routes.rs:5555`) | HTML form POST | The whole candidate map is validated, not the one entry: `validate_entries(&candidate)` (`os/pkgs/mosd/apid/src/routes.rs:5578`), and the peers the form does not carry are preserved by `let peers = view` (`os/pkgs/mosd/apid/src/routes.rs:5564-5569`); on success redirects to `/network?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:5591`) | `get_settings("network")` (`os/pkgs/mosd/apid/src/routes.rs:5329`), `set_settings(&iface_settings_path(&iface), &value)` (`os/pkgs/mosd/apid/src/routes.rs:5586`) |
| `POST /network/peers/add` | `post(network_peer_add)` (`os/pkgs/mosd/apid/src/routes.rs:192`) | `fn network_peer_add` (`os/pkgs/mosd/apid/src/routes.rs:5594`) | HTML form POST | Appends one WireGuard peer, refusing a public key the tunnel already carries — *"That public key is already a peer of this tunnel."* (`os/pkgs/mosd/apid/src/routes.rs:5618-5627`) — then rewrites the whole peer list through `fn write_peers` (`os/pkgs/mosd/apid/src/routes.rs:5659`) | `get_settings("network")` (`os/pkgs/mosd/apid/src/routes.rs:5353`), `set_settings(&peers_settings_path(iface), &value)` (`os/pkgs/mosd/apid/src/routes.rs:5667`) |
| `POST /network/peers/remove` | `post(network_peer_remove)` (`os/pkgs/mosd/apid/src/routes.rs:193`) | `fn network_peer_remove` (`os/pkgs/mosd/apid/src/routes.rs:5632`) | HTML form POST | Removes one peer by public key, never by index, and errors when nothing matched, from `let before = peers.len();` (`os/pkgs/mosd/apid/src/routes.rs:5642-5650`); then rewrites the whole list | as add |
| `GET /hostname` | `get(hostname_form)` (`os/pkgs/mosd/apid/src/routes.rs:194`) | `fn hostname_form` (`os/pkgs/mosd/apid/src/routes.rs:5696`) | GET page | One text input pre-filled with the current hostname | `get_settings("hostname")` (`os/pkgs/mosd/apid/src/routes.rs:5697`) |
| `POST /hostname` | `.post(hostname_submit)` (`os/pkgs/mosd/apid/src/routes.rs:194`) | `fn hostname_submit` (`os/pkgs/mosd/apid/src/routes.rs:5943`) | HTML form POST | 422 on `if !valid_hostname(hostname) {` (`os/pkgs/mosd/apid/src/routes.rs:5948-5954`); on success redirects to `/hostname?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:5962`) | `set_settings("hostname", &Value::String(hostname.to_string()))` (`os/pkgs/mosd/apid/src/routes.rs:5957`) |
| `GET /power` | `get(power_form)` (`os/pkgs/mosd/apid/src/routes.rs:195`) | `fn power_form` (`os/pkgs/mosd/apid/src/routes.rs:5790`) | GET page | Two confirmation forms, each with a required checkbox carrying an action-specific token, in `fn power_form_markup` (`os/pkgs/mosd/apid/src/routes.rs:5763`) | none |
| `POST /power/reboot` | `post(power_reboot)` (`os/pkgs/mosd/apid/src/routes.rs:199`) | `fn power_reboot` (`os/pkgs/mosd/apid/src/routes.rs:5856`) → `fn power_submit` (`os/pkgs/mosd/apid/src/routes.rs:5806`) | HTML form POST | 422 on `if confirm != action.confirm_token() {` (`os/pkgs/mosd/apid/src/routes.rs:5807-5818`); otherwise **202 Accepted**, `StatusCode::ACCEPTED,` (`os/pkgs/mosd/apid/src/routes.rs:5820-5824`), with the D-Bus call spawned on a detached task so the response goes out first — `fn dispatch_power_action` (`os/pkgs/mosd/apid/src/routes.rs:5840`) | `api.reboot()` (`os/pkgs/mosd/apid/src/routes.rs:5847`), which is a write to the `/Actions/reboot` item over the `com.mos.Item1` proxy `trait Item` (`os/pkgs/mosd/apid/src/bus_client.rs:48-51`), whose path is `const REBOOT_ACTION: &str = "/Actions/reboot";` (`os/pkgs/mosd/apid/src/bus_client.rs:55`) and whose call is `async fn reboot` (`os/pkgs/mosd/apid/src/bus_client.rs:261-263`) |
| `POST /power/poweroff` | `post(power_poweroff)` (`os/pkgs/mosd/apid/src/routes.rs:200`) | `fn power_poweroff` (`os/pkgs/mosd/apid/src/routes.rs:5864`) → `fn power_submit` (`os/pkgs/mosd/apid/src/routes.rs:5806`) | HTML form POST | Same shape | `api.power_off()` (`os/pkgs/mosd/apid/src/routes.rs:5848`), the `const POWER_OFF_ACTION: &str = "/Actions/poweroff";` item (`os/pkgs/mosd/apid/src/bus_client.rs:56`), called at `async fn power_off` (`os/pkgs/mosd/apid/src/bus_client.rs:265-267`) |
| `GET /ssh` | `get(ssh_form)` (`os/pkgs/mosd/apid/src/routes.rs:201`) | `fn ssh_form` (`os/pkgs/mosd/apid/src/routes.rs:6233`) → `fn load_ssh_view` (`os/pkgs/mosd/apid/src/routes.rs:6067`) | GET page | SSH pane: stored `access.ssh` settings, the authorized-key list, and the sshd reconciler's live state | `get_settings("access.ssh")` (`os/pkgs/mosd/apid/src/routes.rs:6068`), `get_state("sshd")` (`os/pkgs/mosd/apid/src/routes.rs:6078`) |
| `POST /ssh/enable` | `post(ssh_enable)` (`os/pkgs/mosd/apid/src/routes.rs:205`) | `fn ssh_enable` (`os/pkgs/mosd/apid/src/routes.rs:6249`) | HTML form POST | Writes the checkbox state and redirects to `/ssh?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:6258`) | `set_settings("access.ssh.enabled", &Value::Bool(enabled))` (`os/pkgs/mosd/apid/src/routes.rs:6253`) |
| `POST /ssh/password` | `post(ssh_password)` (`os/pkgs/mosd/apid/src/routes.rs:206`) | `fn ssh_password` (`os/pkgs/mosd/apid/src/routes.rs:6737`) | HTML form POST | Sets a **transient** root password after `if form.confirm != TRANSIENT_CONFIRM_TOKEN {` (`os/pkgs/mosd/apid/src/routes.rs:6742-6748`) and the shared byte bounds of `validate_transient_password` (`os/pkgs/mosd/apid/src/routes.rs:6749`) | `set_transient_root_password(&form.password)` (`os/pkgs/mosd/apid/src/routes.rs:6752`) |
| `POST /ssh/keys/add` | `post(ssh_key_add)` (`os/pkgs/mosd/apid/src/routes.rs:207`) | `fn ssh_key_add` (`os/pkgs/mosd/apid/src/routes.rs:6852`) | HTML form POST | Parses and validates one public key with `parse_authorized_key(&form.key)` (`os/pkgs/mosd/apid/src/routes.rs:6856`), appends it in memory — `keys.push(parsed);` (`os/pkgs/mosd/apid/src/routes.rs:6864`) — then rewrites the whole list through `fn write_key_list` (`os/pkgs/mosd/apid/src/routes.rs:6099`); 422 on `validate_authorized_keys(keys)` (`os/pkgs/mosd/apid/src/routes.rs:6102-6104`) | `get_settings("access.ssh")` (`os/pkgs/mosd/apid/src/routes.rs:6095`), `set_settings(SSH_KEYS_PATH, &value)` (`os/pkgs/mosd/apid/src/routes.rs:6108`) |
| `POST /ssh/keys/remove` | `post(ssh_key_remove)` (`os/pkgs/mosd/apid/src/routes.rs:208`) | `fn ssh_key_remove` (`os/pkgs/mosd/apid/src/routes.rs:6885`) | HTML form POST | Removes one key by fingerprint or canonical key text — `keys.iter().position(` (`os/pkgs/mosd/apid/src/routes.rs:6893-6896`) — never by index, and errors on `let Some(index) = found else {` (`os/pkgs/mosd/apid/src/routes.rs:6897-6903`); then rewrites the whole list | same as add |
| `GET /containers` | `get(containers_form)` (`os/pkgs/mosd/apid/src/routes.rs:209`) | `fn containers_form` (`os/pkgs/mosd/apid/src/routes.rs:6387`) → `fn load_container_view` (`os/pkgs/mosd/apid/src/routes.rs:6294`) | GET page | Container pane: the `container.enabled` switch, the root-capability notice, and the reconciler's published Quadlet files and generated units | `get_settings("container")` (`os/pkgs/mosd/apid/src/routes.rs:6295`), `get_state("container")` (`os/pkgs/mosd/apid/src/routes.rs:6301`) |
| `POST /containers/enable` | `post(containers_enable)` (`os/pkgs/mosd/apid/src/routes.rs:210`) | `fn containers_enable` (`os/pkgs/mosd/apid/src/routes.rs:6403`) | HTML form POST | Writes the switch and redirects to `/containers?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:6415`) | `set_settings("container.enabled", &Value::Bool(enabled))` (`os/pkgs/mosd/apid/src/routes.rs:6410`) |
| `GET /mqtt` | `get(mqtt_form)` (`os/pkgs/mosd/apid/src/routes.rs:211`) | `fn mqtt_form` (`os/pkgs/mosd/apid/src/routes.rs:6673`) → `fn load_mqtt_view` (`os/pkgs/mosd/apid/src/routes.rs:6552`) | GET page | MQTT pane: the `mqtt.enabled` switch and the broker/bridge unit state published by the mqtt reconciler | `get_settings("mqtt")` (`os/pkgs/mosd/apid/src/routes.rs:6553`), `get_state("mqtt")` (`os/pkgs/mosd/apid/src/routes.rs:6559`) |
| `POST /mqtt/enable` | `post(mqtt_enable)` (`os/pkgs/mosd/apid/src/routes.rs:212`) | `fn mqtt_enable` (`os/pkgs/mosd/apid/src/routes.rs:6689`) | HTML form POST | Writes one dot-path and nothing else, so saving the switch cannot rewrite the listen or auth settings: *"the switch writes nothing about the listener or about authentication"* (`os/pkgs/mosd/apid/src/routes.rs:6691-6693`); redirects to `/mqtt?saved=1` (`os/pkgs/mosd/apid/src/routes.rs:6701`) | `set_settings("mqtt.enabled", &Value::Bool(enabled))` (`os/pkgs/mosd/apid/src/routes.rs:6696`) |
| `GET /healthz` | `get(healthz)` (`os/pkgs/mosd/apid/src/routes.rs:213`) | `fn healthz` (`os/pkgs/mosd/apid/src/routes.rs:3357`) | neither — plain text | Returns the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:3358`) — a liveness probe only: it proves the apid process is listening and checks nothing else, not mosd, not the settings tree, not the appliance. `GET /api/v1/health` is the other question and the two are not interchangeable | none |
...
| `POST /api/v1/actions/change-password` | `post(api_v1_change_password)` (`os/pkgs/mosd/apid/src/routes.rs:424`) | `fn api_v1_change_password` (`os/pkgs/mosd/apid/src/routes.rs:4644`) | bearer | 204, through the same `fn change_password` (`os/pkgs/mosd/apid/src/routes.rs:4471`) the pane calls; a wrong current password is 403 `wrong_password` (`os/pkgs/mosd/apid/src/routes.rs:4679`) |
...
| `POST /api/v1/actions/reboot` | `post(api_v1_reboot)` (`os/pkgs/mosd/apid/src/routes.rs:474`) | `fn api_v1_reboot` (`os/pkgs/mosd/apid/src/routes.rs:5913`) | bearer | **202** with an empty body, through the same `fn dispatch_power_action` (`os/pkgs/mosd/apid/src/routes.rs:5840`) the pane uses, and with no confirmation token: *"There is no mis-click on a `POST` a script constructed"* (`os/pkgs/mosd/apid/src/routes.rs:5883-5888`) |
| `POST /api/v1/actions/poweroff` | `post(api_v1_poweroff)` (`os/pkgs/mosd/apid/src/routes.rs:475`) | `fn api_v1_poweroff` (`os/pkgs/mosd/apid/src/routes.rs:5933`) | bearer | Same shape |
| `POST /api/v1/actions/transient-root-password` | `post(api_v1_transient_root_password)` (`os/pkgs/mosd/apid/src/routes.rs:476-479`) | `fn api_v1_transient_root_password` (`os/pkgs/mosd/apid/src/routes.rs:6807`) | bearer | 204, the same byte bounds the pane runs, and no echo of the password |
| `POST /api/v1/setup` | `post(api_v1_setup)` (`os/pkgs/mosd/apid/src/routes.rs:487`) | `fn api_v1_setup` (`os/pkgs/mosd/apid/src/routes.rs:4107`) | **none** | The device's one unauthenticated write, and the only handler under this prefix naming no credential extractor at all — *"the only handler under this prefix that takes no credential extractor"* (`os/pkgs/mosd/apid/src/routes.rs:480-486`): 201 carrying a first bearer token, 409 `"already_configured",` (`os/pkgs/mosd/apid/src/routes.rs:4130`), 422 with nothing written |
...
neither and returns a bare string (`os/pkgs/mosd/apid/src/routes.rs:3357-3359`), and
...
`os/pkgs/mosd/apid/src/routes.rs:4110`, `os/pkgs/mosd/apid/src/routes.rs:4649` and
`os/pkgs/mosd/apid/src/routes.rs:6811`; four of them share the reader
...
example `Form(form): Form<NetworkForm>` (`os/pkgs/mosd/apid/src/routes.rs:5555`) and
`Form(form): Form<SshKeyAddForm>` (`os/pkgs/mosd/apid/src/routes.rs:6852`). The two POSTs
that are in neither set — `/logout`, at `fn logout`
(`os/pkgs/mosd/apid/src/routes.rs:4414`), and `/builtin/deactivate`, at
`fn builtin_deactivate` (`os/pkgs/mosd/apid/src/routes.rs:5090`) — take no request
...
(`os/pkgs/mosd/apid/src/routes.rs:5807`) and on the transient password
(`os/pkgs/mosd/apid/src/routes.rs:6742`) — neither of which the `/api/v1/` actions
carry, and §2.4's own note says why: there is no mis-click on a `POST` a script
constructed (`os/pkgs/mosd/apid/src/routes.rs:5883-5888`).
...
| `fn get_state` | `os/pkgs/mosd/apid/src/bus_client.rs:30` | `async fn get_state` (`os/pkgs/mosd/mosd/src/bus.rs:649`) | five literal live-state paths: `get_state("network")` (`os/pkgs/mosd/apid/src/routes.rs:4736`), `.get_state("uptime")` (`os/pkgs/mosd/apid/src/routes.rs:4739`), `get_state("sshd")` (`os/pkgs/mosd/apid/src/routes.rs:6078`), `get_state("container")` (`os/pkgs/mosd/apid/src/routes.rs:6301`) and `get_state("mqtt")` (`os/pkgs/mosd/apid/src/routes.rs:6559`), plus the health probe `get_state(HEALTH_PROBE_PATH)` (`os/pkgs/mosd/apid/src/routes.rs:890`) and the passthrough `get_state(&path)` (`os/pkgs/mosd/apid/src/routes.rs:1246`), which serves any dot-path a client asks for |
| `fn set_value` on `/Actions/reboot` | `os/pkgs/mosd/apid/src/bus_client.rs:50`, path at `os/pkgs/mosd/apid/src/bus_client.rs:55` | `os/pkgs/mosd/mosd/src/tree.rs:491` → `os/pkgs/mosd/mosd/src/actions.rs:101` | `POST /power/reboot` and `POST /api/v1/actions/reboot`, both through `PowerAction::Reboot => api.reboot().await,` (`os/pkgs/mosd/apid/src/routes.rs:5847`) |
| `fn set_value` on `/Actions/poweroff` | `os/pkgs/mosd/apid/src/bus_client.rs:50`, path at `os/pkgs/mosd/apid/src/bus_client.rs:56` | `os/pkgs/mosd/mosd/src/tree.rs:491` → `os/pkgs/mosd/mosd/src/actions.rs:102` | `POST /power/poweroff` and `POST /api/v1/actions/poweroff`, both through `PowerAction::PowerOff => api.power_off().await,` (`os/pkgs/mosd/apid/src/routes.rs:5848`) |
| `fn set_transient_root_password` | `os/pkgs/mosd/apid/src/bus_client.rs:31` | `async fn set_transient_root_password` (`os/pkgs/mosd/mosd/src/bus.rs:826`) | `POST /ssh/password`, at `app.api.set_transient_root_password(&form.password)` (`os/pkgs/mosd/apid/src/routes.rs:6752`), and `POST /api/v1/actions/transient-root-password`, at `app.api.set_transient_root_password(&request.password)` (`os/pkgs/mosd/apid/src/routes.rs:6831`) |
...
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:4395`), set
on the response by `session::session_cookie(&cookie)`
(`os/pkgs/mosd/apid/src/routes.rs:4396-4400`). `POST /setup` does the same at the end
of the first-run wizard without a login step, at its own
`let cookie = state.sessions.create();` (`os/pkgs/mosd/apid/src/routes.rs:3990-3994`). The store generates 16 random bytes from
...
(`os/pkgs/mosd/apid/src/routes.rs:3330-3334`) — so a cookie-authenticated request reaches its
handler without touching mosd, and the source states the reason and its
soundness argument at `os/pkgs/mosd/apid/src/routes.rs:3313-3329`: *"an authenticated
...
(`os/pkgs/mosd/apid/src/routes.rs:6094-6096`) — edits it in memory with `keys.push(parsed);`
(`os/pkgs/mosd/apid/src/routes.rs:6864`) or `keys.remove(index);` (`os/pkgs/mosd/apid/src/routes.rs:6904`), and writes the whole
list back through `async fn write_key_list` (`os/pkgs/mosd/apid/src/routes.rs:6099-6112`). Every
...
reads it at `.get_state("uptime")` (`os/pkgs/mosd/apid/src/routes.rs:4739`). A sixth reader
...
3. **Nothing is embedded in the binary, and that is still deliberate.**
   ...
   `include_str!("../openapi.json"),` (`os/pkgs/mosd/apid/src/tests.rs:1831`) reads the
...
by `style { (PreEscaped(STYLE)) }` (`os/pkgs/mosd/apid/src/routes.rs:3380`), from a constant described in
the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:3361`). There is still no favicon route, no font and
...
the gate redirects it to `/login` (`os/pkgs/mosd/apid/src/routes.rs:3354`), and
...
(`os/pkgs/mosd/apid/src/routes.rs:5091`) — and the install path. The consequence
...
1. **Line numbers throughout its section 2.1 no longer resolve to the routes
   ...
   `pub(crate) async fn home` (`os/pkgs/mosd/apid/src/routes.rs:4778`).
...
3. **"Six methods and one signal"** (`docs/research/mos-ui-inventory.md:367`) is
   ...
   `app.api.set_transient_root_password(&form.password)` (`os/pkgs/mosd/apid/src/routes.rs:6752`)
   and from M7's action route at
   `app.api.set_transient_root_password(&request.password)` (`os/pkgs/mosd/apid/src/routes.rs:6831`).
4. **"the sole call site is `GetState("network")`"**
   (`docs/research/mos-ui-inventory.md:373`) is now eight call sites:
   `get_state("network")` (`os/pkgs/mosd/apid/src/routes.rs:4736`),
   `.get_state("uptime")` (`os/pkgs/mosd/apid/src/routes.rs:4739`),
   `get_state("network")` again in the network pane (`os/pkgs/mosd/apid/src/routes.rs:5332`),
   `get_state("sshd")` (`os/pkgs/mosd/apid/src/routes.rs:6078`),
   `get_state("container")` (`os/pkgs/mosd/apid/src/routes.rs:6301`),
   `get_state("mqtt")` (`os/pkgs/mosd/apid/src/routes.rs:6559`),
...
(`os/pkgs/mosd/apid/src/routes.rs:3372`) now renders **nine** links, from
`a href="/" { "Status" }` (`os/pkgs/mosd/apid/src/routes.rs:3385-3398`), plus the logout
form `form method="post" action="/logout" {` (`os/pkgs/mosd/apid/src/routes.rs:3399-3401`):
...
1. The gate already dispatches on `let path = request.uri().path();`
   (`os/pkgs/mosd/apid/src/routes.rs:3276`). A version check is the same string operation
...
(`os/pkgs/mosd/apid/src/routes.rs:3343-3348`). An authenticated probe cannot answer the
...
bus call at `os/pkgs/mosd/apid/src/routes.rs:3277`. The cost is an unauthenticated
...
(`os/pkgs/mosd/apid/src/routes.rs:3357-3359`, released by the gate at
`if path == "/healthz" || is_declared_api_route(path) {` (`os/pkgs/mosd/apid/src/routes.rs:3277`)),
...
- **No array indexing.** The dot-path syntax has none; the model says a list is
  *"Written as a whole JSON array through the dot-path API"*
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:6747-6752`). Two clients doing that concurrently
...
- **A key holding a dot is addressed with a quoted segment.** apid accepts an
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3465-3471`) — and until PLAN-022 M2 the
...
nobody asked to delete"* (`os/pkgs/mosd/apid/src/routes.rs:6876-6880`). A `DELETE` whose
...
the file (`os/pkgs/mosd/apid/src/routes.rs:6072-6074`). So the difference is the quality
...
| SSH keys | the authorized-keys collection above | `access.ssh.authorizedKeys` | **every key is a root key** (`docs/design/access.md` §4.1, `os/pkgs/mosd/apid/src/routes.rs:5949`); the API response must carry that sentence in a `notice` field for the same reason the pane must carry it |
...
| `GET /healthz` (`routes.rs:172`, `:3314`) | — | **none, and it must not change.** See below | — |
...
(`os/pkgs/mosd/apid/src/routes.rs:5779`, and `TRANSIENT_CONFIRM_TOKEN` at `os/pkgs/mosd/apid/src/routes.rs:5953`) does
...
goes out before the machine goes down (`os/pkgs/mosd/apid/src/routes.rs:5799-5812`). 202
...
- **`GET /healthz`** — declared at `os/pkgs/mosd/apid/src/routes.rs:213` and answering
  at `os/pkgs/mosd/apid/src/routes.rs:3329-3331`. It keeps its path, its unauthenticated exemption (`os/pkgs/mosd/apid/src/routes.rs:3245`) and
  its literal `ok` body,
...
- **A password change — [implemented], on both surfaces.** This bullet once
  ...
  `os/pkgs/mosd/apid/src/routes.rs:424` (handler at `:4263-4318`), `POST` only
  ...
  (`change_password`, `os/pkgs/mosd/apid/src/routes.rs:4443-4502`): the
...
(`os/pkgs/mosd/apid/src/routes.rs:6869-6875`, via `ssh_error` at `os/pkgs/mosd/apid/src/routes.rs:6087-6096`). For a
...
- **Five codes ship that this section never proposed**, each added by a
  ...
  `os/pkgs/mosd/apid/src/routes.rs:2469`, `os/pkgs/mosd/apid/src/routes.rs:4222`), `already_configured`
  (`os/pkgs/mosd/apid/src/routes.rs:4130`), `mint_failed` (`os/pkgs/mosd/apid/src/routes.rs:1489`,
  `os/pkgs/mosd/apid/src/routes.rs:4233`), `hash_failed` (`os/pkgs/mosd/apid/src/routes.rs:4256`) and
  `hashing_failed` (`os/pkgs/mosd/apid/src/routes.rs:4694`). They are additive under §2.1 — a
...
| `request_invalid` | **yes**, 400 — this snapshot said **no** | seven sites, one per JSON-body route that reports a rejected body itself, plus the shared reader `fn json_body`, at `at(ApiError::apid("request_invalid", rejection.body_text())),` (`os/pkgs/mosd/apid/src/routes.rs:2558`): `os/pkgs/mosd/apid/src/routes.rs:1173`, `os/pkgs/mosd/apid/src/routes.rs:1458`, `os/pkgs/mosd/apid/src/routes.rs:1924`, `os/pkgs/mosd/apid/src/routes.rs:2184`, `os/pkgs/mosd/apid/src/routes.rs:4657`, `os/pkgs/mosd/apid/src/routes.rs:6821`. It could not ship when this row was written because no route took a body; ten do now (section 1.2) |
| `validation_failed` | **yes**, 422 — this snapshot said **no** | twenty-two sites, the most-used code in the crate, from `"validation_failed",` (`os/pkgs/mosd/apid/src/routes.rs:1090`) to `os/pkgs/mosd/apid/src/routes.rs:6828`. The four validators the row below names are all reachable from `/api/v1/` now: `valid_hostname` through the settings write, `validate_iface` through `POST /api/v1/setup`, at `if let Err(message) = validate_iface(iface, cfg.dhcp, address) {` (`os/pkgs/mosd/apid/src/routes.rs:4191`), `validate_transient_password` and `parse_authorized_key` through their action and collection routes |
| `wrong_password` | **yes**, 403 | `"wrong_password",` (`os/pkgs/mosd/apid/src/routes.rs:4679`), on `POST /api/v1/actions/change-password` |
...
(`os/pkgs/mosd/apid/src/routes.rs:5556-5643`), and `write_key_list` does the same
(`os/pkgs/mosd/apid/src/routes.rs:6080-6082`). The VLAN example above is exactly this — a rejection that reads
as an outage.
...
text in an error box (`os/pkgs/mosd/apid/src/routes.rs:3394-3396`, `:1737`, `:1757`) — without
widening it.
...
3. **mosd down entirely, distinguished from "everything is fine".** This is the
   ...
   answers the literal `"ok"` (`os/pkgs/mosd/apid/src/routes.rs:3358`), so **an appliance whose mosd is dead
   ...
   *"whole seconds since boot, a bare JSON number"* everywhere else it appears.
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3270-3274`), so no API route depends on the gate's
...
(`os/pkgs/mosd/apid/src/routes.rs:3277`, predicate at `:503-520`). There is no bearer
...
1. **Authentication failure is a redirect, not a 401 — so the default client
   ...
   `Redirect::to("/login").into_response()` (`os/pkgs/mosd/apid/src/routes.rs:3354`), a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3321-3323`) and which returns **200 OK** with an HTML
   form (`os/pkgs/mosd/apid/src/routes.rs:3985-4316`). A script that checks the status
...
2. **Obtaining the credential means emulating three browser behaviours.**
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:4349`) — axum's URL-encoded extractor, imported at
   `os/pkgs/mosd/apid/src/routes.rs:19`, not JSON — answers **302 to `/`**
   (`os/pkgs/mosd/apid/src/routes.rs:4370`), and delivers the credential in a
   `Set-Cookie` header (`os/pkgs/mosd/apid/src/routes.rs:4369`, value built at
...
7. **A misconfigured script locks the operator out, repeatedly.** The login
   ...
   *"the appliance has one admin password, so per-client tracking buys nothing
   ...
   `os/pkgs/mosd/apid/src/routes.rs:4334-4344`). A script retrying with a
...
4. **It costs zero extra bus round trips.** The gate already calls
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3318`) and `apiTokens` is a child of `access`, so
...
- Mint and revoke are read-modify-write of the whole array, because the dot-path
  ...
  the same pattern the SSH key pane uses (`os/pkgs/mosd/apid/src/routes.rs:6860-6865`).
  Two concurrent mints lose one token, silently.
- Identity is the `id`, never a list position, for the reason recorded at
  `os/pkgs/mosd/apid/src/routes.rs:6861-6865`: an index is meaningful only against the
...
2. **§3.1 item 7 changes meaning.** The `auth.rs:53-54` comment's reasoning —
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:4341-4350`). Bearer verification is **not** rate
...
(`os/pkgs/mosd/apid/src/routes.rs:3326`) — and the CORS argument below, which depends on
...
2. **A stolen token is full management access, indefinitely.** There is no
   ...
   `os/pkgs/mosd/apid/src/routes.rs:5949`) and is then no longer needed. Detection is not
...
5. **A same-origin page mints a permanent token with the operator's cookie.**
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:5949`). But a minted token survives deactivating
...
pane is tested to carry, `os/pkgs/mosd/apid/src/routes.rs:5949`); apid runs as root
...
- **The per-action confirm tokens are not CSRF tokens and must not be counted as
  ...
  and `"poweroff"` (`os/pkgs/mosd/apid/src/routes.rs:5727-5731`) and
  `TRANSIENT_CONFIRM_TOKEN` is the literal `"set-transient-password"`
  (`os/pkgs/mosd/apid/src/routes.rs:5981`). They are not secret, not per-session and not
  ...
  the checkbox has to be ticked as well"* (`os/pkgs/mosd/apid/src/routes.rs:5725-5726`). They stop a mis-click and a prefetch. Against a
...
(`os/pkgs/mosd/apid/src/routes.rs:3270-3276`), so an authenticated flood no longer
...
§6.3's reserved prefix, `builtin_home` in `os/pkgs/mosd/apid/src/routes.rs:3417`. Its
...
(`os/pkgs/mosd/apid/src/routes.rs:3355-3361`), described in the source as
*"Inline stylesheet shared by every page; no external assets"*
(`os/pkgs/mosd/apid/src/routes.rs:3361`). Section 1.6 evidences the rest: no
...
- **Costs.** It burns a path prefix permanently, and it only helps an operator
  ...
  *before* dispatch — so at the moment that page is on screen, `/builtin/` is
  ...
  (`:341`), and the reserved subtree's own 404 (`:4775`). The cost that must not be glossed: **(A) is a way *in*, not a way
...
1. **A signature and the upload credential authorise the same blast radius.** A
   ...
   key (`os/pkgs/mosd/apid/src/routes.rs:5949`). §3.2 states there are **no scopes in
...
(`os/pkgs/mosd/apid/src/routes.rs:34`, `:3319-3326`, `:3337`; §1.1 constraint 1 and §1.6
...
Items (iv) and (v) are already shipped — `os/pkgs/mosd/apid/src/routes.rs:5675`, `:2245`,
`:1975`, `:1985`, `:2501`, `:3274`. Items (i), (ii) and (iii) are new panes, and
...
the settings write fails (`os/pkgs/mosd/apid/src/routes.rs:5644-5650`), and `write_key_list`
does the same (`:5645-5647`), so a rejected value is reported to the operator as
...
(`os/pkgs/mosd/apid/src/routes.rs:3318`) the whole appliance answers the 502 page *"The
...
path answers 422 (`os/pkgs/mosd/apid/src/routes.rs:6869-6875`). The HTML path is not
changed.
...
setup mode (`os/pkgs/mosd/apid/src/routes.rs:3322-3327`) — and it returns a token (§2.3),
...
- **The VLAN dot-path limit — since closed, and closed additively.**
  ...
  (`os/pkgs/mosd/apid/src/routes.rs:3465-3471`) — while `split_path` split on it
