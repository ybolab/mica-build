...
(`os/boards/x64/board.env:201-205`) — Debian's `linux-image-amd64`, installed by
...
is "Debian trixie + systemd" (`os/rootfs/stages/10-base.Dockerfile:15`). A board
...
1. Create `os/boards/<name>/` with a `board.env`, plus a `bsp/` only if the
   ...
   "the board-definition schema lint" (`os/verify/run.sh:7`).
...
|---|---|---|---|
| cx3576 (CX3576-Z, RK3576) | arm64 ("MOS_ARCH=arm64", `os/boards/cx3576/board.env:398`) | U-Boot at eMMC sector 64 -> `boot.scr` -> `booti` on `Image` + `rk3576-src.dtb` (`os/boards/cx3576/boot.cmd:127-129`) | BSP builds `uboot-mos` and the kernel; the §4 assertion set is enforced in the kernel build, and the RAUC `BOOT_ORDER` handshake is implemented in `os/boards/cx3576/boot.cmd`. `CONFIG_FIT_SIGNATURE` (§5) is configured nowhere in the tree |
| x64 (generic UEFI) | amd64 ("MOS_ARCH=amd64", `os/boards/x64/board.env:273`) | UEFI firmware -> GRUB from one static ESP -> the slot's own boot partition (`os/boards/x64/grub.cfg`) | QEMU/CI baseline. No `bsp/`, "by design, not by omission" (`os/boards/x64/board.env:204-205`) |
