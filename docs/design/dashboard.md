...
`GetState("network")` at `os/pkgs/mosd/apid/src/routes.rs:2964`, whose result is rendered
...
- **Feed:** `GetSettings("hostname")` — already called at
  `os/pkgs/mosd/apid/src/routes.rs:2963`; `GetSettings("provisioning")`, which returns
...
- **Feed:** mosd's live-state `uptime` key, read over the bus via `get_state`
  (`os/pkgs/mosd/apid/src/routes.rs:4250-4270`); mosd itself reads
...
`shell()` helper (`os/pkgs/mosd/apid/src/routes.rs:3236-3249`) on pages that opt in.
