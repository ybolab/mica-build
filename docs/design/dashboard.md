...
- **No power buttons.** They are one click from the nav bar already
  ...
  test at `os/pkgs/mosd/apid/src/tests.rs:690`), plus a required confirmation token.
...
- The power routes are POST-only with no GET handler (`routes.rs:49-53`, pinned
  by `os/pkgs/mosd/apid/src/tests.rs:690`). A `meta refresh` issues a GET and therefore
