...
`GetState("network")` at `os/pkgs/mosd/apid/src/routes.rs:2999`, whose result is rendered
...
- **Feed:** `GetSettings("hostname")` — already called at
  `os/pkgs/mosd/apid/src/routes.rs:2998`; `GetSettings("provisioning")`, which returns
...
`shell()` helper (`os/pkgs/mosd/apid/src/routes.rs:3271-3284`) on pages that opt in.
