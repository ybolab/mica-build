...
`GetState("network")` at `os/pkgs/mosd/apid/src/routes.rs:1270`, whose result is rendered
...
- **Feed:** `GetSettings("hostname")` — already called at
  `os/pkgs/mosd/apid/src/routes.rs:1269`; `GetSettings("provisioning")`, which returns
...
- **Feed:** mosd's live-state `uptime` key, read over the bus via `get_state`
  (`os/pkgs/mosd/apid/src/routes.rs:2556-2576`); mosd itself reads
...
`shell()` helper (`os/pkgs/mosd/apid/src/routes.rs:1542-1555`) on pages that opt in.
