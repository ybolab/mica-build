...
`GetState("network")` at `os/pkgs/mosd/apid/src/routes.rs:887`, whose result is rendered
...
- **Feed:** `GetSettings("hostname")` — already called at
  `os/pkgs/mosd/apid/src/routes.rs:886`; `GetSettings("provisioning")`, which returns
...
- **Feed:** mosd's live-state `uptime` key, read over the bus via `get_state`
  (`os/pkgs/mosd/apid/src/routes.rs:2073-2093`); mosd itself reads
...
`shell()` helper (`os/pkgs/mosd/apid/src/routes.rs:1073-1086`) on pages that opt in.
