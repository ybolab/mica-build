...
a single function (`os/pkgs/mosd/apid/src/routes.rs:52-73`), plus a catch-all 308
...
`Power` (`/power`) — rendered by `shell()` at `os/pkgs/mosd/apid/src/routes.rs:208-218`
...
`GetState("network")` at `os/pkgs/mosd/apid/src/routes.rs:1529`, whose result is rendered
...
- **Feed:** `GetSettings("hostname")` — already called at
  `os/pkgs/mosd/apid/src/routes.rs:1528`; `GetSettings("provisioning")`, which returns
...
- **Feed:** mosd's live-state `uptime` key, read over the bus via `get_state`
  (`os/pkgs/mosd/apid/src/routes.rs:2815-2835`); mosd itself reads
...
The single stylesheet is a `const STYLE` at `os/pkgs/mosd/apid/src/routes.rs:181-194`,
...
2. **All of `mosd`'s settings and live state sit behind one `tokio::sync::Mutex`.**
   ...
   `GetSettings("access")` on **every** request (`os/pkgs/mosd/apid/src/routes.rs:144`),
...
- The HTTP-listener redirect is 308 (`os/pkgs/mosd/apid/src/routes.rs:77-81` and its doc
...
`shell()` helper (`os/pkgs/mosd/apid/src/routes.rs:1801-1814`) on pages that opt in.
