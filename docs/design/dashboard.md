...
a single function (`os/pkgs/mosd/apid/src/routes.rs:44-61`), plus a catch-all 308
...
`Power` (`/power`) — rendered by `shell()` at `os/pkgs/mosd/apid/src/routes.rs:177-187`
...
`GetState("network")` at `os/pkgs/mosd/apid/src/routes.rs:677`, whose result is rendered
...
- **Feed:** `GetSettings("hostname")` — already called at
  `os/pkgs/mosd/apid/src/routes.rs:676`; `GetSettings("provisioning")`, which returns
...
- **Venus comparison, running both ways.** Venus surfaces config-versus-reality
  ...
  `os/pkgs/mosd/mosd/src/bus.rs:461-471` already records failures durably in a place the
...
- **Feed:** `/proc/uptime`, read by `apid` itself at
  `os/pkgs/mosd/apid/src/routes.rs:678-680`, parsed at `:539-546`, formatted at
...
- **Feed:** `GetState("sshd")`, which publishes `enabled`, `port`,
  ...
  exactly this purpose (`os/pkgs/mosd/mosd/src/reconciler/wifi_ap.rs:792-807`, and the
...
- **No power buttons.** They are one click from the nav bar already
  ...
  test at `os/pkgs/mosd/apid/src/tests.rs:573`), plus a required confirmation token.
...
| **Diagnostics** | what happened, and what do I send to support | — | the whole tree via `GetState("")` / `GetSettings("")` | **(a)** mechanically — both already accept `""` for the whole tree (`os/pkgs/mosd/mosd/src/bus.rs:160-164`, `:197-202`) — but **(b)** for the redaction rule that must precede any export |
...
distinct bus methods** — `GetSettings` at `os/pkgs/mosd/mosd/src/bus.rs:160-164` and
...
(`os/pkgs/mosd/mosd/src/bus.rs:169-194`, reconciler loop at `:183-188`), recording each
...
(`os/pkgs/mosd/mosd/src/bus.rs:461-471`), and gap row 18 is precisely about surfacing
...
3. **The settings/live-state boundary, and stored-versus-applied.** Both are
   ...
   (`os/pkgs/mosd/mosd/src/bus.rs:169-194`) where Venus needs `SettingSync` and a
...
The single stylesheet is a `const STYLE` at `os/pkgs/mosd/apid/src/routes.rs:151-163`,
...
2. **All of `mosd`'s settings and live state sit behind one `tokio::sync::Mutex`.**
   ...
   `GetSettings("access")` on **every** request (`os/pkgs/mosd/apid/src/routes.rs:124`),
...
- The HTTP-listener redirect is 308 (`os/pkgs/mosd/apid/src/routes.rs:65-69` and its doc
...
| **C5** | **`mosd` down** | Today: lazy connect, cache dropped on error, per-request 502 pages, never a `apid` crash (`os/pkgs/mosd/apid/src/bus_client.rs:22-26`, `:41-58`; `bus_error` at `routes.rs:95-105`). What does the option do to that? |
...
`shell()` helper (`os/pkgs/mosd/apid/src/routes.rs:845-857`) on pages that opt in.
...
- The power routes are POST-only with no GET handler (`routes.rs:49-53`, pinned
  by `os/pkgs/mosd/apid/src/tests.rs:573`). A `meta refresh` issues a GET and therefore
...
1. **The outcome is not in the return value.** `set_settings` returns
   ...
   (`os/pkgs/mosd/mosd/src/bus.rs:183-188`); `record` writes `Ok(value)` or
...
the outcome (`os/pkgs/mosd/mosd/src/bus.rs:437-442`). It must land before any generic
