...
`GetState("network")` at `os/pkgs/mosd/apid/src/routes.rs:3027`, whose result is rendered
...
- **Feed:** `GetSettings("hostname")` — already called at
  `os/pkgs/mosd/apid/src/routes.rs:3026`; `GetSettings("provisioning")`, which returns
...
`shell()` helper (`os/pkgs/mosd/apid/src/routes.rs:3299-3312`) on pages that opt in.
