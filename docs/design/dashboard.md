...
`GetState("network")` at `os/pkgs/mosd/apid/src/routes.rs:3031`, whose result is rendered
...
- **Feed:** `GetSettings("hostname")` — already called at
  `os/pkgs/mosd/apid/src/routes.rs:3030`; `GetSettings("provisioning")`, which returns
...
- **Feed:** mosd's live-state `uptime` key, read over the bus via `get_state`
  (`os/pkgs/mosd/apid/src/routes.rs:4668-4688`); mosd itself reads
...
- The HTTP-listener redirect is 308 (`os/pkgs/mosd/apid/src/routes.rs:3185-3189` and its doc
...
`shell()` helper (`os/pkgs/mosd/apid/src/routes.rs:3288-3301`) on pages that opt in.
