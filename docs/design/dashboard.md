...
`GetState("network")` at `os/pkgs/mosd/apid/src/routes.rs:3035`, whose result is rendered
...
- **Feed:** `GetSettings("hostname")` — already called at
  `os/pkgs/mosd/apid/src/routes.rs:3034`; `GetSettings("provisioning")`, which returns
...
- **Feed:** mosd's live-state `uptime` key, read over the bus via `get_state`
  (`os/pkgs/mosd/apid/src/routes.rs:4321-4341`); mosd itself reads
