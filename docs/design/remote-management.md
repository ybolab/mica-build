...
(`os/pkgs/mosd/apid/src/routes.rs:118`) and its outermost layer is the auth gate
(`os/pkgs/mosd/apid/src/routes.rs:201`), which *"routes every request into setup mode,
login, or through"* (`os/pkgs/mosd/apid/src/routes.rs:699`) on a session cookie minted
at first-run setup or at login. `/healthz` and the declared API routes are the
only exemptions; they *"answer for themselves"*
(`os/pkgs/mosd/apid/src/routes.rs:702-703`).

**The JSON API under `/api` is read-only.** The router reserves the prefix and
*"every other path under it 404s"* (`os/pkgs/mosd/apid/src/routes.rs:189-190`); every
route inside is a GET — `get(api_v1_settings)` (`os/pkgs/mosd/apid/src/routes.rs:289`)
and `get(api_v1_state)` (`os/pkgs/mosd/apid/src/routes.rs:290`), beside version
...
(`os/pkgs/mosd/apid/src/routes.rs:126-202`), so it offers no local check/apply button
