...
(`os/pkgs/mosd/apid/src/routes.rs:188`), which *"routes every request into setup mode,
login, or through"* (`os/pkgs/mosd/apid/src/routes.rs:672`) on a session cookie minted
at first-run setup or at login. `/healthz` and the declared API routes are the
only exemptions; they *"answer for themselves"*
(`os/pkgs/mosd/apid/src/routes.rs:675-676`).

**The JSON API under `/api` is read-only.** The router reserves the prefix and
*"every other path under it 404s"* (`os/pkgs/mosd/apid/src/routes.rs:176-177`); every
route inside is a GET — `get(api_v1_settings)` (`os/pkgs/mosd/apid/src/routes.rs:271`)
and `get(api_v1_state)` (`os/pkgs/mosd/apid/src/routes.rs:272`), beside version
...
(`os/pkgs/mosd/apid/src/routes.rs:113-189`), so it offers no local check/apply button
