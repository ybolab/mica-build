...
(`os/pkgs/mosd/apid/src/routes.rs:109`) and its outermost layer is the auth gate
(`os/pkgs/mosd/apid/src/routes.rs:196`), which *"routes every request into setup mode,
login, or through"* (`os/pkgs/mosd/apid/src/routes.rs:767`) on a session cookie minted
at first-run setup or at login. `/healthz` and the declared API routes are the
only exemptions; they *"answer for themselves"*
(`os/pkgs/mosd/apid/src/routes.rs:770-771`).

**The JSON API under `/api` is read-only.** The router reserves the prefix and
*"every other path under it 404s"* (`os/pkgs/mosd/apid/src/routes.rs:184-185`); every
route inside is a GET — `get(api_v1_settings)` (`os/pkgs/mosd/apid/src/routes.rs:287`)
and `get(api_v1_state)` (`os/pkgs/mosd/apid/src/routes.rs:288`), beside version
...
(`os/pkgs/mosd/apid/src/routes.rs:117-197`), so it offers no local check/apply button
