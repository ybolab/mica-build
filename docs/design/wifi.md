...
> Wi-Fi station and access point on the systemd/mosd base. Companion to
...
Both subtrees live in the mosd settings tree (`docs/design/mosd.md` §1) and are
persisted in `/mos/config/wifi.json` on DATA, at that
document's own schema version (`docs/design/mosd.md` §2.1a). Both Wi-Fi
...
- no secret enters the image. Every key in a rendered file comes from the
  settings tree on DATA/state or from `/var/lib/mos/secrets/ap-psk`, both born at
  runtime.
