...
- Boot level on x64 as well, against two x64 roots this branch composed: the
  shipped one (`3620e9da...`) and a second composition with rauc declined
  (`d871a7e7...`). Same kernel, both roots, `switch_root` onto each.
  `rootfs/build.sh` ends the second compose non-zero -- its smoke step requires
  the full feature set and says so by name -- after the root and its verity
  tree are written, which is what that fixture needed;
  `tests/signed-boot-lab/second-root.sh` is what keeps that composition from
  overwriting the shipped root's companions.
...
- Proof 1's whole matrix against this branch's own x64 root
  (`3620e9dabd7d...`, `_out/x64/rootfs-verity.img`), same verdicts and the same
...
- `verify/run.sh --verify --board x64`: **PASS (326/326 checks, 26 skipped)** on
  `_out/x64/x64-mos-1788900305.img`, and again unchanged after the second root
  was composed -- which is the point of composing it through
  `tests/signed-boot-lab/second-root.sh`. One more check than virt-arm64's 325
  ...
  board.
- Every one of these was driven by the committed lab, from a root and an image
  composed at this branch's HEAD with the amd64 pool built at the same commit.
