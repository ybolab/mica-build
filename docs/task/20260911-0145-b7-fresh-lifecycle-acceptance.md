...
The first ed723 guest refused SYSTEM discovery and powered off during partial
startup; the original API deadline ended with exit 1 at 06:40:00 UTC. Direct
regression work now corrects the native GPT geometry check for a factory image
written to a larger medium, preserving fixed partition identities and strict
CRC/range checks. Full guest/lifecycle/API acceptance remains pending.

## Dependencies

- **blocked by**: current guest executor/resource prerequisites and actual acceptance outcomes
