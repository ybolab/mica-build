...
1. The dot-path write of a VLAN name fails exactly as RFCT-135 and
   `docs/design/api.md:1419-1448` describe.
...
of alphanumerics plus `.`, `_`, `-` (`os/pkgs/mosd/apid/src/routes.rs:3439-3468`),
and the pane's error text advertises the dot
(`os/pkgs/mosd/apid/src/routes.rs:3479`). `network_submit` builds the value
(`os/pkgs/mosd/apid/src/routes.rs:3497-3513`) and writes it as a dot-path,
...
Form input (`NetworkForm`, `os/pkgs/mosd/apid/src/routes.rs:5138-5195`) →
apid validation (`:5189-5199`) → D-Bus `SetSettings("network.<iface>", json)`
(`:5205`, `os/pkgs/mosd/apid/src/bus_client.rs:29`) → `write_setting`
...
`GET /api/v1/state/network` (`docs/design/api.md:1578`).
...
(`os/pkgs/mosd/apid/src/routes.rs:3529`); paths the daemon
...
(`os/pkgs/mosd/apid/src/routes.rs:3411-3416`), the reconciler rejects it
...
Rules applied, quoted from `docs/design/api.md:1178-1184`: breaking is
...
|---|---|---|
| `kind`, `vlan`, `bridge`, `wireguard` fields in `IfaceSettings` bodies | additive | new *optional* request field; new response field (`docs/design/api.md:1182-1184`) |
| `kind` values as an enum in responses | additive | open-enum rule; clients must ignore unknowns (`docs/design/api.md:1186-1189`) |
...
also discharges the debt api.md records at `docs/design/api.md:4658-4668`
