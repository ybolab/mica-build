...
1. The dot-path write of a VLAN name fails exactly as RFCT-135 and
   `docs/design/api.md:1398-1427` describe.
...
of alphanumerics plus `.`, `_`, `-` (`os/pkgs/mosd/apid/src/routes.rs:3377-3406`),
and the pane's error text advertises the dot
(`os/pkgs/mosd/apid/src/routes.rs:3417`). `network_submit` builds the value
(`os/pkgs/mosd/apid/src/routes.rs:3435-3451`) and writes it as a dot-path,
...
Form input (`NetworkForm`, `os/pkgs/mosd/apid/src/routes.rs:5076-5133`) →
...
`GET /api/v1/state/network` (`docs/design/api.md:1557`).
...
(`os/pkgs/mosd/apid/src/routes.rs:3467`); paths the daemon
...
(`os/pkgs/mosd/apid/src/routes.rs:3349-3354`), the reconciler rejects it
...
Rules applied, quoted from `docs/design/api.md:1157-1163`: breaking is
...
|---|---|---|
| `kind`, `vlan`, `bridge`, `wireguard` fields in `IfaceSettings` bodies | additive | new *optional* request field; new response field (`docs/design/api.md:1161-1163`) |
| `kind` values as an enum in responses | additive | open-enum rule; clients must ignore unknowns (`docs/design/api.md:1165-1168`) |
...
also discharges the debt api.md records at `docs/design/api.md:4627-4637`
