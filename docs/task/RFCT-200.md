...
1. The dot-path write of a VLAN name fails exactly as RFCT-135 and
   `docs/design/api.md:1420-1449` describe.
...
of alphanumerics plus `.`, `_`, `-` (`os/pkgs/mosd/apid/src/routes.rs:3457-3486`),
and the pane's error text advertises the dot
(`os/pkgs/mosd/apid/src/routes.rs:3497`). `network_submit` builds the value
(`os/pkgs/mosd/apid/src/routes.rs:3515-3531`) and writes it as a dot-path,
...
Form input (`NetworkForm`, `os/pkgs/mosd/apid/src/routes.rs:5173-5230`) →
apid validation (`:5163-5173`) → D-Bus `SetSettings("network.<iface>", json)`
(`:5179`, `os/pkgs/mosd/apid/src/bus_client.rs:29`) → `write_setting`
...
`GET /api/v1/state/network` (`docs/design/api.md:1588`).
...
(`os/pkgs/mosd/apid/src/routes.rs:3564`); paths the daemon
...
(`os/pkgs/mosd/apid/src/routes.rs:3429-3434`), the reconciler rejects it
...
Rules applied, quoted from `docs/design/api.md:1179-1185`: breaking is
...
|---|---|---|
| `kind`, `vlan`, `bridge`, `wireguard` fields in `IfaceSettings` bodies | additive | new *optional* request field; new response field (`docs/design/api.md:1183-1185`) |
| `kind` values as an enum in responses | additive | open-enum rule; clients must ignore unknowns (`docs/design/api.md:1187-1190`) |
...
also discharges the debt api.md records at `docs/design/api.md:4659-4669`
