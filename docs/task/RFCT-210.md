...
Section 2.3's table (`docs/design/api.md:1532`) states its own provenance:
...
commit's."* (`docs/design/api.md:1551-1553`) Two things have happened to it since.
...
`os/pkgs/mosd/apid/src/routes.rs:3879`. `POST /ssh/password` is cited at
`routes.rs:2515`; it is at `os/pkgs/mosd/apid/src/routes.rs:6736`.
...
HEAD it is `os/pkgs/mosd/apid/src/routes.rs:3328-3333`. The confirmation-token
paragraph cites `os/pkgs/mosd/apid/src/routes.rs:5806`, which at HEAD is inside
...
index."* (`docs/design/api.md:1438`) — a collection is needed exactly when an
...
idempotency to promise"* (`docs/design/api.md:1578-1579`).
...
(`os/pkgs/mosd/apid/src/routes.rs:5631-5658`) and answers
*"No peer of this tunnel has that public key; the list may have changed since the page was loaded."*
(`os/pkgs/mosd/apid/src/routes.rs:5646`) rather than succeeding silently.
...
(`os/pkgs/mosd/apid/src/routes.rs:5575-5579`), rejecting the submission with a
...
two entries at once."* (`os/pkgs/mosd/apid/src/routes.rs:5573-5574`) The rules
`validate_entries` enforces (`os/pkgs/mosd/apid/src/routes.rs:3734-3772`) are a
...
(`docs/design/api.md:1457-1459`), and the departure is argued, not assumed:
...
`os/pkgs/mosd/apid/src/routes.rs:3947-3986` — and it is the only route the
gate lets through in setup mode
(`os/pkgs/mosd/apid/src/routes.rs:3328-3333`). It is not a settings write (three
...
(`os/pkgs/mosd/apid/src/routes.rs:3950-3986`), so a mosd failure halfway leaves
...
`docs/design/api.md:1673-1677`: it records that `ssh_key_remove` answers 422
...
(`docs/design/api.md:1676-1677`) That paragraph's own two citations are accurate
...
|---|---|---|---|
| `POST /ssh/keys/remove` | fingerprint **or** exact canonical key text | 422, HTML pane | `os/pkgs/mosd/apid/src/routes.rs:6896-6901` |
| `POST /network/peers/remove` | peer public key | 422, HTML pane | `os/pkgs/mosd/apid/src/routes.rs:5643-5649` |
| `POST /api/v1/actions/wireguard/{iface}/rotate-key` | `iface`, a path segment | **422 `settings_rejected`**, API envelope | `os/pkgs/mosd/mosd/src/bus.rs:898-902` → `os/pkgs/mosd/apid/src/routes.rs:3072-3075` |
| `POST /network/peers/add` | `iface` | **neither — it succeeds**; see below | `os/pkgs/mosd/apid/src/routes.rs:5354-5358` |
| `GET /api/v1/settings/{path}` | the dot-path | **404 `settings_not_found`** | `os/pkgs/mosd/apid/src/routes.rs:3064-3067` |
| `GET /api/v1/state/{path}` | the dot-path | **404 `settings_not_found`** | `os/pkgs/mosd/apid/src/routes.rs:3064-3067` |
...
(`os/pkgs/mosd/apid/src/routes.rs:5361`), and `ssh_error`
(`os/pkgs/mosd/apid/src/routes.rs:6114-6123`) is the same function for the SSH
...
document."* (`docs/design/api.md:1677`) This ratifies that, for three reasons it
does not give:
...
2. **The HTML condition really is different.** `ssh_key_remove` accepts a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:6892-6895`), so a submitted string that
   ...
   it and try again."* (`os/pkgs/mosd/apid/src/routes.rs:6899`) That is a
...
`os/pkgs/mosd/apid/src/routes.rs:5354-5358`); `write_peers` then validates and
writes straight to the peer list's own dot-path
(`os/pkgs/mosd/apid/src/routes.rs:5664-5667`); and the settings setter creates
...
(`os/pkgs/mosd/apid/src/routes.rs:3725-3748`) checks each peer's public key,
...
(`docs/design/api.md:1475-1478`) and a write-refusal list. Acceptance: a `PUT`
...
422 the HTML path gives (`os/pkgs/mosd/apid/src/routes.rs:6896-6901`), a
...
(`os/pkgs/mosd/apid/src/routes.rs:5976`) is present on both the `GET` and the
...
path segment (`os/pkgs/mosd/apid/src/routes.rs:3527-3529`).
...
(`os/pkgs/mosd/apid/src/routes.rs:5855-5940`); no `GET` handler is declared for
any of the three; the transient-password route enforces the same byte bounds as the form path's validator at
`os/pkgs/mosd/apid/src/routes.rs:6715-6730`, and
...
there is nothing to revoke."* (`docs/design/api.md:1650-1651`)
...
is deliberate"* (`docs/design/api.md:2376-2377`), with the reason
*"a human rotating their own password must not break every script"*
(`docs/design/api.md:2377-2378`) and the obligation
*"The UI's password pane has to say so."* (`docs/design/api.md:2378-2379`) Section 9
treats it as settled and reasons from it
(`docs/design/api.md:4733`).
...
(`os/pkgs/mosd/apid/src/routes.rs:4526`), and `SessionStore` documents the
...
(`os/pkgs/mosd/apid/src/routes.rs:4549`)
...
`docs/design/api.md:2374-2375`). Whichever way this resolves, the asymmetry has
...
`docs/design/api.md:2327`). An operator doing routine hygiene would silently
...
1. Change the admin password. This drops every other **session**
   (`os/pkgs/mosd/apid/src/routes.rs:4524-4526`) and no token.
2. `GET /api/v1/tokens`, then `DELETE /api/v1/tokens/{id}` **for every id
   ...
   Revocation takes effect on the next request (`docs/design/api.md:2328-2330`).
...
3. `GET /api/v1/settings/access.ssh` and remove every authorized key that is not
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:5976`), so a password rotation that skips
...
(`os/pkgs/mosd/apid/src/routes.rs:4549`). It needs a second, and this is the
...
bundles in phase 1 (`docs/design/api.md:3888-3891`), and this memo does not
...
the `docs/design/api.md:1673-1677` anchor was re-derived rather than trusted.
