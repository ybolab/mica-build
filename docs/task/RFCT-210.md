...
`os/pkgs/mosd/apid/src/routes.rs:1376`. `POST /ssh/password` is cited at
`routes.rs:2515`; it is at `os/pkgs/mosd/apid/src/routes.rs:3593`.
...
`os/pkgs/mosd/apid/src/routes.rs:307-319` and the HTTP-listener router
(`os/pkgs/mosd/apid/src/routes.rs:756-759`); resolve every path constant
(`os/pkgs/mosd/apid/src/routes.rs:235-269`, `:1959-1968`); one row per
...
(`os/pkgs/mosd/apid/openapi.json:93`), and one `GET` each on
`"/api/v1/meta"` (`os/pkgs/mosd/apid/openapi.json:166`),
`"/api/v1/settings/{path}"` (`os/pkgs/mosd/apid/openapi.json:198`),
`"/api/v1/state/{path}"` (`os/pkgs/mosd/apid/openapi.json:281`) and
`"/api/versions"` (`os/pkgs/mosd/apid/openapi.json:354`).
...
HEAD it is `os/pkgs/mosd/apid/src/routes.rs:866-871`. The confirmation-token
paragraph cites `os/pkgs/mosd/apid/src/routes.rs:2745`, which at HEAD is inside
...
(`os/pkgs/mosd/apid/src/routes.rs:2570-2597`) and answers
*"No peer of this tunnel has that public key; the list may have changed since the page was loaded."*
(`os/pkgs/mosd/apid/src/routes.rs:2585`) rather than succeeding silently.
...
(`os/pkgs/mosd/apid/src/routes.rs:2514-2518`), rejecting the submission with a
...
two entries at once."* (`os/pkgs/mosd/apid/src/routes.rs:2512-2513`) The rules
`validate_entries` enforces (`os/pkgs/mosd/apid/src/routes.rs:1259-1297`) are a
...
`os/pkgs/mosd/apid/src/routes.rs:1444-1483` — and it is the only route the
gate lets through in setup mode
(`os/pkgs/mosd/apid/src/routes.rs:866-871`). It is not a settings write (three
...
(`os/pkgs/mosd/apid/src/routes.rs:1447-1483`), so a mosd failure halfway leaves
...
(`os/pkgs/mosd/apid/src/routes.rs:2781-2797`); no `GET` handler is declared for
any of the three; the transient-password route enforces the same byte bounds as the form path's validator at
`os/pkgs/mosd/apid/src/routes.rs:3572-3587`, and
...
is deliberate"* (`docs/design/api.md:2056-2057`), with the reason
*"a human rotating their own password must not break every script"*
(`docs/design/api.md:2057-2058`) and the obligation
*"The UI's password pane has to say so."* (`docs/design/api.md:2058-2059`) Section 9
treats it as settled and reasons from it
(`docs/design/api.md:4413`).
...
(`os/pkgs/mosd/apid/src/routes.rs:1709`), and `SessionStore` documents the
...
(`os/pkgs/mosd/apid/src/routes.rs:1719`)
...
`docs/design/api.md:2054-2055`). Whichever way this resolves, the asymmetry has
...
`docs/design/api.md:2007`). An operator doing routine hygiene would silently
...
1. Change the admin password. This drops every other **session**
   (`os/pkgs/mosd/apid/src/routes.rs:1707-1709`) and no token.
2. `GET /api/v1/tokens`, then `DELETE /api/v1/tokens/{id}` **for every id
   ...
   Revocation takes effect on the next request (`docs/design/api.md:2008-2010`).
...
3. `GET /api/v1/settings/access.ssh` and remove every authorized key that is not
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:2833`), so a password rotation that skips
...
(`os/pkgs/mosd/apid/src/routes.rs:1719`). It needs a second, and this is the
...
bundles in phase 1 (`docs/design/api.md:3568-3571`), and this memo does not
