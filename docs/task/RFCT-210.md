...
`os/pkgs/mosd/apid/src/routes.rs:3714`. `POST /ssh/password` is cited at
`routes.rs:2515`; it is at `os/pkgs/mosd/apid/src/routes.rs:6175`.
...
`os/pkgs/mosd/apid/src/routes.rs:397-459` and the HTTP-listener router
(`os/pkgs/mosd/apid/src/routes.rs:3105-3108`); resolve every path constant
...
(`os/pkgs/mosd/apid/openapi.json:103`), and one `GET` each on
`"/api/v1/meta"` (`os/pkgs/mosd/apid/openapi.json:238`),
`"/api/v1/settings/{path}"` (`os/pkgs/mosd/apid/openapi.json:862`),
`"/api/v1/state/{path}"` (`os/pkgs/mosd/apid/openapi.json:1306`) and
`"/api/versions"` (`os/pkgs/mosd/apid/openapi.json:1879`).
...
HEAD it is `os/pkgs/mosd/apid/src/routes.rs:3204-3209`. The confirmation-token
paragraph cites `os/pkgs/mosd/apid/src/routes.rs:5327`, which at HEAD is inside
...
describe."* (`os/pkgs/mosd/apid/src/routes.rs:1221-1224`)
...
(`os/pkgs/mosd/apid/src/routes.rs:5152-5179`) and answers
*"No peer of this tunnel has that public key; the list may have changed since the page was loaded."*
(`os/pkgs/mosd/apid/src/routes.rs:5167`) rather than succeeding silently.
...
(`os/pkgs/mosd/apid/src/routes.rs:5096-5100`), rejecting the submission with a
...
two entries at once."* (`os/pkgs/mosd/apid/src/routes.rs:5094-5095`) The rules
`validate_entries` enforces (`os/pkgs/mosd/apid/src/routes.rs:3597-3635`) are a
...
`os/pkgs/mosd/apid/src/routes.rs:3782-3821` — and it is the only route the
gate lets through in setup mode
(`os/pkgs/mosd/apid/src/routes.rs:3204-3209`). It is not a settings write (three
...
(`os/pkgs/mosd/apid/src/routes.rs:3785-3821`), so a mosd failure halfway leaves
...
|---|---|---|---|
| `POST /ssh/keys/remove` | fingerprint **or** exact canonical key text | 422, HTML pane | `os/pkgs/mosd/apid/src/routes.rs:6250-6255` |
| `POST /network/peers/remove` | peer public key | 422, HTML pane | `os/pkgs/mosd/apid/src/routes.rs:5164-5170` |
| `POST /api/v1/actions/wireguard/{iface}/rotate-key` | `iface`, a path segment | **422 `settings_rejected`**, API envelope | `os/pkgs/mosd/mosd/src/bus.rs:884-888` → `os/pkgs/mosd/apid/src/routes.rs:2933-2936` |
| `POST /network/peers/add` | `iface` | **neither — it succeeds**; see below | `os/pkgs/mosd/apid/src/routes.rs:4875-4879` |
| `GET /api/v1/settings/{path}` | the dot-path | **404 `settings_not_found`** | `os/pkgs/mosd/apid/src/routes.rs:2925-2928` |
| `GET /api/v1/state/{path}` | the dot-path | **404 `settings_not_found`** | `os/pkgs/mosd/apid/src/routes.rs:2925-2928` |
...
(`os/pkgs/mosd/apid/src/routes.rs:4882`), and `ssh_error`
(`os/pkgs/mosd/apid/src/routes.rs:5553-5562`) is the same function for the SSH
...
`StatusCode::UNPROCESSABLE_ENTITY` (`os/pkgs/mosd/apid/src/routes.rs:2933-2936`)
...
(`os/pkgs/mosd/apid/openapi.json:103-195`). So the 422-where-404-is-meant pattern
...
- `POST /api/v1/actions/wireguard/{iface}/rotate-key` — **a correction to a
  ...
  respectively (`os/pkgs/mosd/apid/src/routes.rs:2925-2936`) with no apid change
...
2. **The HTML condition really is different.** `ssh_key_remove` accepts a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:6246-6249`), so a submitted string that
   ...
   it and try again."* (`os/pkgs/mosd/apid/src/routes.rs:6253`) That is a
...
`os/pkgs/mosd/apid/src/routes.rs:4875-4879`); `write_peers` then validates and
writes straight to the peer list's own dot-path
(`os/pkgs/mosd/apid/src/routes.rs:5185-5188`); and the settings setter creates
...
(`os/pkgs/mosd/apid/src/routes.rs:3560-3583`) checks each peer's public key,
...
422 the HTML path gives (`os/pkgs/mosd/apid/src/routes.rs:6250-6255`), a
...
(`os/pkgs/mosd/apid/src/routes.rs:5415`) is present on both the `GET` and the
...
path segment (`os/pkgs/mosd/apid/src/routes.rs:3362-3364`).
...
(`os/pkgs/mosd/apid/src/routes.rs:5363-5379`); no `GET` handler is declared for
any of the three; the transient-password route enforces the same byte bounds as the form path's validator at
`os/pkgs/mosd/apid/src/routes.rs:6154-6169`, and
...
is deliberate"* (`docs/design/api.md:2095-2096`), with the reason
*"a human rotating their own password must not break every script"*
(`docs/design/api.md:2096-2097`) and the obligation
*"The UI's password pane has to say so."* (`docs/design/api.md:2097-2098`) Section 9
treats it as settled and reasons from it
(`docs/design/api.md:4452`).
...
(`os/pkgs/mosd/apid/src/routes.rs:4047`), and `SessionStore` documents the
...
(`os/pkgs/mosd/apid/src/routes.rs:4070`)
...
`docs/design/api.md:2093-2094`). Whichever way this resolves, the asymmetry has
...
`docs/design/api.md:2046`). An operator doing routine hygiene would silently
...
1. Change the admin password. This drops every other **session**
   (`os/pkgs/mosd/apid/src/routes.rs:4045-4047`) and no token.
...
3. `GET /api/v1/settings/access.ssh` and remove every authorized key that is not
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:5415`), so a password rotation that skips
...
(`os/pkgs/mosd/apid/src/routes.rs:4070`). It needs a second, and this is the
...
bundles in phase 1 (`docs/design/api.md:3607-3610`), and this memo does not
