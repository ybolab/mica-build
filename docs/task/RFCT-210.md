...
`os/pkgs/mosd/apid/src/routes.rs:3880`. `POST /ssh/password` is cited at
`routes.rs:2515`; it is at `os/pkgs/mosd/apid/src/routes.rs:6737`.
...
| 26 | `POST /ssh/keys/remove` | 190 | 3656 | yes (cited `:3326`) | **proposed**: `DELETE /api/v1/ssh/authorized-keys/{fingerprint}` |
...
HEAD it is `os/pkgs/mosd/apid/src/routes.rs:3346-3351`. The confirmation-token
paragraph cites `os/pkgs/mosd/apid/src/routes.rs:5807`, which at HEAD is inside
...
(`os/pkgs/mosd/apid/src/routes.rs:5632-5659`) and answers
*"No peer of this tunnel has that public key; the list may have changed since the page was loaded."*
(`os/pkgs/mosd/apid/src/routes.rs:5647`) rather than succeeding silently.
...
(`os/pkgs/mosd/apid/src/routes.rs:5576-5580`), rejecting the submission with a
...
two entries at once."* (`os/pkgs/mosd/apid/src/routes.rs:5574-5575`) The rules
`validate_entries` enforces (`os/pkgs/mosd/apid/src/routes.rs:3735-3773`) are a
...
`os/pkgs/mosd/apid/src/routes.rs:3948-3987` — and it is the only route the
gate lets through in setup mode
(`os/pkgs/mosd/apid/src/routes.rs:3346-3351`). It is not a settings write (three
...
(`os/pkgs/mosd/apid/src/routes.rs:3951-3987`), so a mosd failure halfway leaves
...
|---|---|---|---|
| `POST /ssh/keys/remove` | fingerprint **or** exact canonical key text | 422, HTML pane | `os/pkgs/mosd/apid/src/routes.rs:6897-6902` |
| `POST /network/peers/remove` | peer public key | 422, HTML pane | `os/pkgs/mosd/apid/src/routes.rs:5644-5650` |
...
| `POST /network/peers/add` | `iface` | **neither — it succeeds**; see below | `os/pkgs/mosd/apid/src/routes.rs:5355-5359` |
...
(`os/pkgs/mosd/apid/src/routes.rs:5362`), and `ssh_error`
(`os/pkgs/mosd/apid/src/routes.rs:6115-6124`) is the same function for the SSH
...
2. **The HTML condition really is different.** `ssh_key_remove` accepts a
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:6893-6896`), so a submitted string that
   ...
   it and try again."* (`os/pkgs/mosd/apid/src/routes.rs:6900`) That is a
...
`os/pkgs/mosd/apid/src/routes.rs:5355-5359`); `write_peers` then validates and
writes straight to the peer list's own dot-path
(`os/pkgs/mosd/apid/src/routes.rs:5665-5668`); and the settings setter creates
...
(`os/pkgs/mosd/apid/src/routes.rs:3726-3749`) checks each peer's public key,
...
422 the HTML path gives (`os/pkgs/mosd/apid/src/routes.rs:6897-6902`), a
...
(`os/pkgs/mosd/apid/src/routes.rs:5977`) is present on both the `GET` and the
...
path segment (`os/pkgs/mosd/apid/src/routes.rs:3528-3530`).
...
(`os/pkgs/mosd/apid/src/routes.rs:5856-5941`); no `GET` handler is declared for
any of the three; the transient-password route enforces the same byte bounds as the form path's validator at
`os/pkgs/mosd/apid/src/routes.rs:6716-6731`, and
...
(`os/pkgs/mosd/apid/src/routes.rs:4527`), and `SessionStore` documents the
...
(`os/pkgs/mosd/apid/src/routes.rs:4550`)
...
1. Change the admin password. This drops every other **session**
   (`os/pkgs/mosd/apid/src/routes.rs:4525-4527`) and no token.
...
3. `GET /api/v1/settings/access.ssh` and remove every authorized key that is not
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:5977`), so a password rotation that skips
...
(`os/pkgs/mosd/apid/src/routes.rs:4550`). It needs a second, and this is the
