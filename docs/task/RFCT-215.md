...
| §1.1 the `STYLE` body was said to run from `:3326` to a line holding `Err(err) => return bus_error(&err),` (`os/pkgs/mosd/apid/src/routes.rs:3341`) | it ends at line 3333; the line it named is inside `gate` |
...
| §1.4 the gate was said to run from `:3275` to a line holding `static asset still serves while mosd is down` (`os/pkgs/mosd/apid/src/routes.rs:3327`) | it ends at line 3319; the line it named is inside the ordering comment |
| §1.4 decision 2 cited at `:3270-3274` | the session check is at `:3330-3334`; `:3270-3274` is the doc comment |
...
`` `fn ssh_key_add` (`os/pkgs/mosd/apid/src/routes.rs:6852`) ``, which the gate
...
1. **The CIDR gap — `PUT /api/v1/network/{iface}` accepts an invalid CIDR and
   answers 204.** `fn valid_cidr` (`os/pkgs/mosd/apid/src/routes.rs:3485`) has
   exactly one caller, `fn validate_iface` (`os/pkgs/mosd/apid/src/routes.rs:3516`), which
   runs it at `if !dhcp && !address.is_empty() && !valid_cidr(address) {`
   (`os/pkgs/mosd/apid/src/routes.rs:3520`).
   `validate_iface` has three callers: `iface_settings_from_form`, at
   `validate_iface(form.iface.trim(), dhcp, address)?;`
   (`os/pkgs/mosd/apid/src/routes.rs:3615`), and `setup_submit`, at
   `&& let Err(message) = validate_iface(iface, dhcp, address)`
   (`os/pkgs/mosd/apid/src/routes.rs:3929`) — both HTML — and, since M8,
   `api_v1_setup`, at
   `if let Err(message) = validate_iface(iface, cfg.dhcp, address) {`
   (`os/pkgs/mosd/apid/src/routes.rs:4191`). **No route of M6's
   typed network cluster calls it.** Pinned by
   `the_setup_route_runs_the_wizards_cidr_bound_where_the_network_routes_do_not`
   (`os/pkgs/mosd/apid/src/tests.rs:10425`).
2. **The gate asymmetry — a bearer-only client asking for an *undeclared* path
   ...
   routes (`os/pkgs/mosd/apid/src/routes.rs:3277`) and its whole credential test
   is the cookie (`os/pkgs/mosd/apid/src/routes.rs:3330-3334`); a bearer does not
   satisfy it. Predates M2. Asserted by
   `an_absent_token_id_is_404_and_a_malformed_one_is_422`
   (`os/pkgs/mosd/apid/src/tests.rs:6434`), whose bearer arm asserts the 303.
...
4. **The gate-list/router agreement gap.** `fn is_declared_api_route`
   ...
   test that names it (`os/pkgs/mosd/apid/src/tests.rs:5881`) is about path
