...
(`os/pkgs/mosd/mosd/src/bus.rs:654`) has two failure paths and no others. The
first is the uptime read,
`fdo::Error::Failed("read /proc/uptime".to_string())`
(`os/pkgs/mosd/mosd/src/bus.rs:657`), which is `Failed` and reaches apid as 500
...
`SettingsFault::NotFound` on the line below it. The whole-tree read (`""`) and
the `uptime` graft return early and cannot fail with a name at all.
*(2026-08-29, appended: the second path was `fdo::Error::InvalidArgs` when this
was measured, and the signature was `fdo::Result<String>`; RFCT-251 renamed it
to `NotFound` so that apid would stop reading the name on the state route. The
count of failure paths, which is what this section was measuring, is
unchanged.)*
...
(`docs/design/api.md:1844`) as the 404 row and the fix agrees with it; that file
...
`FDO_INVALID_ARGS => (`
(`os/pkgs/mosd/apid/src/routes.rs:3072`) to 422 `settings_rejected`, and every
...
(`os/pkgs/mosd/apid/openapi.json:1602`), and the eleventh-and-deletion pair is
...
(`os/pkgs/mosd/apid/src/tests.rs:4405`) is the settings route's existing
...
(`os/pkgs/mosd/apid/src/tests.rs:4325`) failed with *"left: 404"* against
...
ends in, `resource_response(value, &path)`
(`os/pkgs/mosd/apid/src/routes.rs:1247`), which is the smallest true restatement
