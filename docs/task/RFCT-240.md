...
|---|---|---|
| `hostname` | a JSON string accepted by `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3515`) | `204` |
...
(`os/pkgs/mosd/apid/src/routes.rs:5982`) -- because a browser sends whatever
...
(`os/pkgs/mosd/apid/src/routes.rs:6288`). The token routes invalidate by
...
(`os/pkgs/mosd/apid/src/routes.rs:1073`) gives one of three, in this order:
...
(`os/pkgs/mosd/apid/src/routes.rs:555`), so the gate already handed a `PUT`
...
5. `docs/design/api.md` section 2.3's *"There is no
   ...
   (`docs/design/api.md:1568-1569`). The `PUT` half is now false. Section 2.3
