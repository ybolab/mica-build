...
(`os/pkgs/mosd/apid/src/routes.rs:364`) and authenticated by `ApiSession` —
bearer **or** cookie.

| Dot-path | Body | Success |
|---|---|---|
| `hostname` | a JSON string accepted by `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:1896`) | `204` |
...
(`os/pkgs/mosd/apid/src/routes.rs:3950`) -- because a browser sends whatever
...
(`os/pkgs/mosd/apid/src/routes.rs:4256`). The token routes invalidate by
...
(`os/pkgs/mosd/apid/src/routes.rs:1435`), and for the token id,
*"a token id is 1 to 64 lowercase hex characters"*
(`os/pkgs/mosd/apid/src/routes.rs:1338`) — and that is what this is: apid
...
(`os/pkgs/mosd/mosd-settings/src/model.rs:656-657`), proved by the committed
...
(`os/pkgs/mosd/apid/src/routes.rs:900`) gives one of three, in this order:
...
2. **404 `settings_not_found`** — the path is well formed and names nothing.
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1373`), so the rule is inherited by
...
(`os/pkgs/mosd/apid/src/routes.rs:1563`) after M2. PLAN-023 Amendment 1's
...
(`os/pkgs/mosd/apid/src/routes.rs:440`), so the gate already handed a `PUT`
