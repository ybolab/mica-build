...
|---|---|---|
| `hostname` | a JSON string accepted by `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3479`) | `204` |
...
(`os/pkgs/mosd/apid/src/routes.rs:5946`) -- because a browser sends whatever
...
(`os/pkgs/mosd/apid/src/routes.rs:6252`). The token routes invalidate by
