...
|---|---|---|
| `hostname` | a JSON string accepted by `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3402`) | `204` |
...
(`os/pkgs/mosd/apid/src/routes.rs:5538`) -- because a browser sends whatever
...
(`os/pkgs/mosd/apid/src/routes.rs:5844`). The token routes invalidate by
...
(`os/pkgs/mosd/apid/src/routes.rs:1601`), and for the token id,
*"a token id is 1 to 64 lowercase hex characters"*
(`os/pkgs/mosd/apid/src/routes.rs:1504`) — and that is what this is: apid
...
2. **404 `settings_not_found`** — the path is well formed and names nothing.
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1539`), so the rule is inherited by
...
(`os/pkgs/mosd/apid/src/routes.rs:3069`) after M2. PLAN-023 Amendment 1's
