...
|---|---|---|
| `hostname` | a JSON string accepted by `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3541`) | `204` |
...
(`os/pkgs/mosd/apid/src/routes.rs:6008`) -- because a browser sends whatever
...
(`os/pkgs/mosd/apid/src/routes.rs:6314`). The token routes invalidate by
