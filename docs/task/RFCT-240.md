...
(`os/pkgs/mosd/apid/src/routes.rs:1578`), and for the token id,
*"a token id is 1 to 64 lowercase hex characters"*
(`os/pkgs/mosd/apid/src/routes.rs:1481`) — and that is what this is: apid
...
2. **404 `settings_not_found`** — the path is well formed and names nothing.
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1516`), so the rule is inherited by
