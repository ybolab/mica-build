...
(`os/pkgs/mosd/apid/src/routes.rs:384`) and authenticated by `ApiSession` —
bearer **or** cookie.

| Dot-path | Body | Success |
|---|---|---|
| `hostname` | a JSON string accepted by `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:2538`) | `204` |
...
(`os/pkgs/mosd/apid/src/routes.rs:4592`) -- because a browser sends whatever
...
(`os/pkgs/mosd/apid/src/routes.rs:4898`). The token routes invalidate by
...
(`os/pkgs/mosd/apid/src/routes.rs:1493`), and for the token id,
*"a token id is 1 to 64 lowercase hex characters"*
(`os/pkgs/mosd/apid/src/routes.rs:1396`) — and that is what this is: apid
...
(`os/pkgs/mosd/apid/src/routes.rs:958`) gives one of three, in this order:
...
2. **404 `settings_not_found`** — the path is well formed and names nothing.
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:1431`), so the rule is inherited by
...
(`os/pkgs/mosd/apid/src/routes.rs:2205`) after M2. PLAN-023 Amendment 1's
...
(`os/pkgs/mosd/apid/src/routes.rs:479`), so the gate already handed a `PUT`
