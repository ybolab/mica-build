...
1. **The rules are relational.** `validate_entries`
   (`os/pkgs/mosd/apid/src/routes.rs:3597-3637`) enforces four: a VLAN's parent
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3593-3596`). The pane already runs it over
   *"The candidate tree, not the one entry"*
   (`os/pkgs/mosd/apid/src/routes.rs:5094-5100`).
...
(`os/pkgs/mosd/apid/src/routes.rs:2925-2936`) — and had only ever been handed
...
`unwrap_or_default()` (`os/pkgs/mosd/apid/src/routes.rs:4872-4879`);
`write_peers` writes straight to the peer list's own dot-path,
`.set_settings(&peers_settings_path(iface), &value)`
(`os/pkgs/mosd/apid/src/routes.rs:5185-5188`); `validate_peers` never looks at
the interface, only at
`for (index, peer) in peers.iter().enumerate()`
(`os/pkgs/mosd/apid/src/routes.rs:3560-3583`); and the setter creates them by
...
(`os/pkgs/mosd/apid/src/routes.rs:2117-2124`). The three local constants were
