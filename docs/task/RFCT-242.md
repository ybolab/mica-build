...
1. **The rules are relational.** `validate_entries`
   (`os/pkgs/mosd/apid/src/routes.rs:3701-3741`) enforces four: a VLAN's parent
   ...
   (`os/pkgs/mosd/apid/src/routes.rs:3697-3700`). The pane already runs it over
   *"The candidate tree, not the one entry"*
   (`os/pkgs/mosd/apid/src/routes.rs:5512-5518`).
...
(`os/pkgs/mosd/apid/src/routes.rs:3020-3031`) — and had only ever been handed
...
`unwrap_or_default()` (`os/pkgs/mosd/apid/src/routes.rs:5290-5297`);
`write_peers` writes straight to the peer list's own dot-path,
`.set_settings(&peers_settings_path(iface), &value)`
(`os/pkgs/mosd/apid/src/routes.rs:5603-5606`); `validate_peers` never looks at
the interface, only at
`for (index, peer) in peers.iter().enumerate()`
(`os/pkgs/mosd/apid/src/routes.rs:3664-3687`); and the setter creates them by
