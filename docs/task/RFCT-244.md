...
(`os/pkgs/mosd/apid/src/tests.rs:9899`) asserts the member set exactly, and then
...
(`os/pkgs/mosd/apid/src/tests.rs:10130`) drives both surfaces against **one**
...
(`os/pkgs/mosd/apid/src/tests.rs:10095`) is the case that would be refused by a
...
(`os/pkgs/mosd/apid/src/tests.rs:10161`) drives four rejections and asserts, for
...
(`os/pkgs/mosd/apid/src/tests.rs:9949`), which also asserts the wizard's opposite
...
(`os/pkgs/mosd/apid/src/tests.rs:9991`) drives **one** partial failure through
...
The fixture is `RefusesOnePath` (`os/pkgs/mosd/apid/src/tests.rs:9824`), added
...
(`os/pkgs/mosd/apid/src/tests.rs:10410`) sends the same entry three ways and
...
(`os/pkgs/mosd/apid/src/tests.rs:10248`) holds the absent member, and
`a_rejected_setup_password_is_never_echoed`
(`os/pkgs/mosd/apid/src/tests.rs:10223`) holds that no refusal repeats the
...
(`os/pkgs/mosd/apid/src/tests.rs:10269`) asserts both halves in setup mode, which
...
(`os/pkgs/mosd/apid/src/tests.rs:10318`) checks the event and that neither the
