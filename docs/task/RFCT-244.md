...
`{"token": "..."}` (`docs/design/api.md:1590`). `hostname` and `network` are
both optional; only
...
— but the browser wizard does not"* (`docs/design/api.md:2344-2346`). The token
...
(`docs/design/api.md:1744`). The three M6 call sites pass `Some(...)` and their
