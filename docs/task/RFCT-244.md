...
(`os/pkgs/mosd/apid/src/tests.rs:9849`) asserts the member set exactly, and then
...
(`os/pkgs/mosd/apid/src/tests.rs:10080`) drives both surfaces against **one**
...
(`os/pkgs/mosd/apid/src/tests.rs:10045`) is the case that would be refused by a
...
(`os/pkgs/mosd/apid/src/tests.rs:10111`) drives four rejections and asserts, for
...
(`os/pkgs/mosd/apid/src/tests.rs:9899`), which also asserts the wizard's opposite
...
(`os/pkgs/mosd/apid/src/tests.rs:9941`) drives **one** partial failure through
...
The fixture is `RefusesOnePath` (`os/pkgs/mosd/apid/src/tests.rs:9774`), added
...
(`os/pkgs/mosd/apid/src/tests.rs:10360`) sends the same entry three ways and
...
(`os/pkgs/mosd/apid/src/tests.rs:10198`) holds the absent member, and
`a_rejected_setup_password_is_never_echoed`
(`os/pkgs/mosd/apid/src/tests.rs:10173`) holds that no refusal repeats the
...
(`os/pkgs/mosd/apid/src/tests.rs:10219`) asserts both halves in setup mode, which
...
(`os/pkgs/mosd/apid/src/tests.rs:10268`) checks the event and that neither the
