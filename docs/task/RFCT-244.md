...
`Source(source): Source,` (`os/pkgs/mosd/apid/src/routes.rs:4092-4094`).
...
The route calls `token::mint` (`os/pkgs/mosd/apid/src/routes.rs:4214`), pushes an
`ApiToken` and writes it through `write_tokens`
(`os/pkgs/mosd/apid/src/routes.rs:4287`) — the same three steps
...
(`os/pkgs/mosd/apid/src/routes.rs:4035`) rather than
...
(`os/pkgs/mosd/apid/src/tests.rs:9610`) asserts the member set exactly, and then
...
(`os/pkgs/mosd/apid/src/routes.rs:3258-3263`).
...
(`os/pkgs/mosd/apid/src/tests.rs:9841`) drives both surfaces against **one**
...
|---|---|---|
| password floor | `const MIN_PASSWORD_BYTES: usize = 8;` (`os/pkgs/mosd/apid/src/routes.rs:3438`), read by `fn password_under_floor(password: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3446`) | lifted from two inline `len() < 8` copies; see section 3 |
| hostname | `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3482`) with `const HOSTNAME_RULES: &str =` (`os/pkgs/mosd/apid/src/routes.rs:3423`) | `&& !valid_hostname(hostname)` (`os/pkgs/mosd/apid/src/routes.rs:4142`) |
| interface name | `fn check_iface_name(iface: &str, path: &str) -> Result<(), Box<Response>> {` (`os/pkgs/mosd/apid/src/routes.rs:2487`) | `if let Err(response) = check_iface_name(iface, NETWORK_SETTINGS_PATH) {` (`os/pkgs/mosd/apid/src/routes.rs:4158`), the same helper M6's four routes use |
| static address CIDR | `fn validate_iface(iface: &str, dhcp: bool, address: &str) -> Result<(), &'static str> {` (`os/pkgs/mosd/apid/src/routes.rs:3501`) calling `fn valid_cidr(cidr: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3470`) | `if let Err(message) = validate_iface(iface, cfg.dhcp, address) {` (`os/pkgs/mosd/apid/src/routes.rs:4176`); see section 4 |
| the four relational rules | `fn relational_refusal(entries: &NetworkEntries, path: &str) -> Result<(), Box<Response>> {` (`os/pkgs/mosd/apid/src/routes.rs:2507`) calling `fn validate_entries(entries: &NetworkEntries) -> Result<(), String> {` (`os/pkgs/mosd/apid/src/routes.rs:3748`) | `if let Err(response) = relational_refusal(&candidate, NETWORK_SETTINGS_PATH) {` (`os/pkgs/mosd/apid/src/routes.rs:4191`) on the **candidate** tree |
...
(`os/pkgs/mosd/apid/src/routes.rs:5559-5560`) A bridge port may legitimately name
an interface the device already declares, and
`the_setup_network_tree_is_merged_with_the_stored_one_before_it_is_judged`
(`os/pkgs/mosd/apid/src/tests.rs:9806`) is the case that would be refused by a
...
(`os/pkgs/mosd/apid/src/tests.rs:9872`) drives four rejections and asserts, for
...
nothing is written on error."* (`os/pkgs/mosd/apid/src/routes.rs:3901`). So what
...
(`os/pkgs/mosd/apid/src/routes.rs:3936`) succeeds and the `hostname` write after
it does not.
...
(`os/pkgs/mosd/apid/src/routes.rs:4256`), then
`&& let Err(response) = write_network_map(&state, candidate).await`
(`os/pkgs/mosd/apid/src/routes.rs:4262`), then
`if let Err(err) = state.api.set_settings("access.webAdmin", &value).await {`
(`os/pkgs/mosd/apid/src/routes.rs:4269`), and last
`if let Err(response) = write_tokens(&state, &tokens).await {`
(`os/pkgs/mosd/apid/src/routes.rs:4287`). The password
...
(`os/pkgs/mosd/apid/src/tests.rs:9660`), which also asserts the wizard's opposite
...
(`os/pkgs/mosd/apid/src/tests.rs:9702`) drives **one** partial failure through
...
The fixture is `RefusesOnePath` (`os/pkgs/mosd/apid/src/tests.rs:9535`), added
...
(`os/pkgs/mosd/apid/src/routes.rs:3269`) answers
`StatusCode::SERVICE_UNAVAILABLE,` (`os/pkgs/mosd/apid/src/routes.rs:3272`)
...
`valid_cidr` (`os/pkgs/mosd/apid/src/routes.rs:3470`) has exactly **one** caller,
...
(`os/pkgs/mosd/apid/src/routes.rs:4176`), rather than lifting it or copying it.
...
(`os/pkgs/mosd/apid/src/tests.rs:10121`) sends the same entry three ways and
...
another refuses. So `MIN_PASSWORD_BYTES` (`os/pkgs/mosd/apid/src/routes.rs:3438`)
...
(`os/pkgs/mosd/apid/src/routes.rs:2548`) — which is what section 2.4 already says
...
(`os/pkgs/mosd/apid/src/tests.rs:9959`) holds the absent member, and
`a_rejected_setup_password_is_never_echoed`
(`os/pkgs/mosd/apid/src/tests.rs:9934`) holds that no refusal repeats the
...
(`os/pkgs/mosd/apid/src/routes.rs:559`), and what makes this route open is that
its signature names no `ApiSession` and no `ApiBearer`.

`the_setup_route_is_the_one_api_route_that_takes_no_credential`
(`os/pkgs/mosd/apid/src/tests.rs:9980`) asserts both halves in setup mode, which
...
(`os/pkgs/mosd/apid/src/routes.rs:4279`) — because it is the same event: section
...
(`os/pkgs/mosd/apid/src/tests.rs:10029`) checks the event and that neither the
...
(`os/pkgs/mosd/apid/src/routes.rs:3211`). Both numbers are gate-green, because
