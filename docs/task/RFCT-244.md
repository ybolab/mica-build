...
`{"token": "..."}` (`docs/design/api.md:1596`). `hostname` and `network` are
both optional; only
...
— but the browser wizard does not"* (`docs/design/api.md:2350-2352`). The token
...
(`docs/design/api.md:1750`). The three M6 call sites pass `Some(...)` and their
