...
`{"token": "..."}` (`docs/design/api.md:1581`). `hostname` and `network` are
both optional; only
...
`Source(source): Source,` (`os/pkgs/mosd/apid/src/routes.rs:4142-4144`).
...
The route calls `token::mint` (`os/pkgs/mosd/apid/src/routes.rs:4264`), pushes an
`ApiToken` and writes it through `write_tokens`
(`os/pkgs/mosd/apid/src/routes.rs:4337`) — the same three steps
...
— but the browser wizard does not"* (`docs/design/api.md:2335-2337`). The token
is labelled `const SETUP_TOKEN_NAME: &str = "first-run setup";`
(`os/pkgs/mosd/apid/src/routes.rs:4085`) rather than
...
(`os/pkgs/mosd/apid/src/tests.rs:10152`) asserts the member set exactly, and then
...
(`os/pkgs/mosd/apid/src/tests.rs:10383`) drives both surfaces against **one**
...
|---|---|---|
| password floor | `const MIN_PASSWORD_BYTES: usize = 8;` (`os/pkgs/mosd/apid/src/routes.rs:3471`), read by `fn password_under_floor(password: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3479`) | lifted from two inline `len() < 8` copies; see section 3 |
| hostname | `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3515`) with `const HOSTNAME_RULES: &str =` (`os/pkgs/mosd/apid/src/routes.rs:3456`) | `&& !valid_hostname(hostname)` (`os/pkgs/mosd/apid/src/routes.rs:4192`) |
| interface name | `fn check_iface_name(iface: &str, path: &str) -> Result<(), Box<Response>> {` (`os/pkgs/mosd/apid/src/routes.rs:2462`) | `if let Err(response) = check_iface_name(iface, NETWORK_SETTINGS_PATH) {` (`os/pkgs/mosd/apid/src/routes.rs:4208`), the same helper M6's four routes use |
| static address CIDR | `fn validate_iface(iface: &str, dhcp: bool, address: &str) -> Result<(), &'static str> {` (`os/pkgs/mosd/apid/src/routes.rs:3534`) calling `fn valid_cidr(cidr: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3503`) | `if let Err(message) = validate_iface(iface, cfg.dhcp, address) {` (`os/pkgs/mosd/apid/src/routes.rs:4226`); see section 4 |
| the four relational rules | `fn relational_refusal(entries: &NetworkEntries, path: &str) -> Result<(), Box<Response>> {` (`os/pkgs/mosd/apid/src/routes.rs:2482`) calling `fn validate_entries(entries: &NetworkEntries) -> Result<(), String> {` (`os/pkgs/mosd/apid/src/routes.rs:3798`) | `if let Err(response) = relational_refusal(&candidate, NETWORK_SETTINGS_PATH) {` (`os/pkgs/mosd/apid/src/routes.rs:4241`) on the **candidate** tree |
...
(`os/pkgs/mosd/apid/src/routes.rs:5609-5610`) A bridge port may legitimately name
an interface the device already declares, and
`the_setup_network_tree_is_merged_with_the_stored_one_before_it_is_judged`
(`os/pkgs/mosd/apid/src/tests.rs:10348`) is the case that would be refused by a
...
(`os/pkgs/mosd/apid/src/tests.rs:10414`) drives four rejections and asserts, for
...
nothing is written on error."* (`os/pkgs/mosd/apid/src/routes.rs:3951`). So what
...
(`os/pkgs/mosd/apid/src/routes.rs:3986`) succeeds and the `hostname` write after
it does not.
...
(`os/pkgs/mosd/apid/src/routes.rs:4306`), then
`&& let Err(response) = write_network_map(&state, candidate).await`
(`os/pkgs/mosd/apid/src/routes.rs:4312`), then
`if let Err(err) = state.api.set_settings("access.webAdmin", &value).await {`
(`os/pkgs/mosd/apid/src/routes.rs:4319`), and last
`if let Err(response) = write_tokens(&state, &tokens).await {`
(`os/pkgs/mosd/apid/src/routes.rs:4337`). The password
...
(`os/pkgs/mosd/apid/src/tests.rs:10202`), which also asserts the wizard's opposite
...
(`os/pkgs/mosd/apid/src/tests.rs:10244`) drives **one** partial failure through
...
The fixture is `RefusesOnePath` (`os/pkgs/mosd/apid/src/tests.rs:10077`), added
...
`valid_cidr` (`os/pkgs/mosd/apid/src/routes.rs:3503`) has exactly **one** caller,
`validate_iface` (`:3515`), whose own two callers are both HTML form handlers —
the network pane's entry builder (`:3610`) and `setup_submit` (`:3924`). So at
...
(`os/pkgs/mosd/apid/src/routes.rs:4226`), rather than lifting it or copying it.
...
(`os/pkgs/mosd/apid/src/tests.rs:10665`) sends the same entry three ways and,
...
another refuses. So `MIN_PASSWORD_BYTES` (`os/pkgs/mosd/apid/src/routes.rs:3471`)
and `password_under_floor` (`:3456`) hold the bound once and the three call
...
(`docs/design/api.md:1735`). The three M6 call sites pass `Some(...)` and their
...
(`os/pkgs/mosd/apid/src/tests.rs:10501`) holds the absent member, and
`a_rejected_setup_password_is_never_echoed`
(`os/pkgs/mosd/apid/src/tests.rs:10476`) holds that no refusal repeats the
...
(`os/pkgs/mosd/apid/src/tests.rs:10522`) asserts both halves in setup mode, which
...
(`os/pkgs/mosd/apid/src/routes.rs:4329`) — because it is the same event: section
...
(`os/pkgs/mosd/apid/src/tests.rs:10571`) checks the event and that neither the
