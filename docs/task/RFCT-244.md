...
`Source(source): Source,` (`os/pkgs/mosd/apid/src/routes.rs:4045-4047`).
...
The route calls `token::mint` (`os/pkgs/mosd/apid/src/routes.rs:4167`), pushes an
`ApiToken` and writes it through `write_tokens`
(`os/pkgs/mosd/apid/src/routes.rs:4240`) — the same three steps
...
(`os/pkgs/mosd/apid/src/routes.rs:3988`) rather than
...
(`os/pkgs/mosd/apid/src/tests.rs:9850`) asserts the member set exactly, and then
...
(`os/pkgs/mosd/apid/src/routes.rs:3211-3216`).
...
(`os/pkgs/mosd/apid/src/tests.rs:10081`) drives both surfaces against **one**
...
|---|---|---|
| password floor | `const MIN_PASSWORD_BYTES: usize = 8;` (`os/pkgs/mosd/apid/src/routes.rs:3391`), read by `fn password_under_floor(password: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3399`) | lifted from two inline `len() < 8` copies; see section 3 |
| hostname | `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3435`) with `const HOSTNAME_RULES: &str =` (`os/pkgs/mosd/apid/src/routes.rs:3376`) | `&& !valid_hostname(hostname)` (`os/pkgs/mosd/apid/src/routes.rs:4095`) |
| interface name | `fn check_iface_name(iface: &str, path: &str) -> Result<(), Box<Response>> {` (`os/pkgs/mosd/apid/src/routes.rs:2462`) | `if let Err(response) = check_iface_name(iface, NETWORK_SETTINGS_PATH) {` (`os/pkgs/mosd/apid/src/routes.rs:4111`), the same helper M6's four routes use |
| static address CIDR | `fn validate_iface(iface: &str, dhcp: bool, address: &str) -> Result<(), &'static str> {` (`os/pkgs/mosd/apid/src/routes.rs:3454`) calling `fn valid_cidr(cidr: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3423`) | `if let Err(message) = validate_iface(iface, cfg.dhcp, address) {` (`os/pkgs/mosd/apid/src/routes.rs:4129`); see section 4 |
| the four relational rules | `fn relational_refusal(entries: &NetworkEntries, path: &str) -> Result<(), Box<Response>> {` (`os/pkgs/mosd/apid/src/routes.rs:2482`) calling `fn validate_entries(entries: &NetworkEntries) -> Result<(), String> {` (`os/pkgs/mosd/apid/src/routes.rs:3701`) | `if let Err(response) = relational_refusal(&candidate, NETWORK_SETTINGS_PATH) {` (`os/pkgs/mosd/apid/src/routes.rs:4144`) on the **candidate** tree |
...
(`os/pkgs/mosd/apid/src/routes.rs:5512-5513`) A bridge port may legitimately name
an interface the device already declares, and
`the_setup_network_tree_is_merged_with_the_stored_one_before_it_is_judged`
(`os/pkgs/mosd/apid/src/tests.rs:10046`) is the case that would be refused by a
...
(`os/pkgs/mosd/apid/src/tests.rs:10112`) drives four rejections and asserts, for
...
nothing is written on error."* (`os/pkgs/mosd/apid/src/routes.rs:3854`). So what
...
(`os/pkgs/mosd/apid/src/routes.rs:3889`) succeeds and the `hostname` write after
it does not.
...
(`os/pkgs/mosd/apid/src/routes.rs:4209`), then
`&& let Err(response) = write_network_map(&state, candidate).await`
(`os/pkgs/mosd/apid/src/routes.rs:4215`), then
`if let Err(err) = state.api.set_settings("access.webAdmin", &value).await {`
(`os/pkgs/mosd/apid/src/routes.rs:4222`), and last
`if let Err(response) = write_tokens(&state, &tokens).await {`
(`os/pkgs/mosd/apid/src/routes.rs:4240`). The password
...
(`os/pkgs/mosd/apid/src/tests.rs:9900`), which also asserts the wizard's opposite
...
(`os/pkgs/mosd/apid/src/tests.rs:9942`) drives **one** partial failure through
...
The fixture is `RefusesOnePath` (`os/pkgs/mosd/apid/src/tests.rs:9775`), added
...
(`os/pkgs/mosd/apid/src/routes.rs:3222`) answers
`StatusCode::SERVICE_UNAVAILABLE,` (`os/pkgs/mosd/apid/src/routes.rs:3225`)
...
`valid_cidr` (`os/pkgs/mosd/apid/src/routes.rs:3423`) has exactly **one** caller,
...
(`os/pkgs/mosd/apid/src/routes.rs:4129`), rather than lifting it or copying it.
...
(`os/pkgs/mosd/apid/src/tests.rs:10361`) sends the same entry three ways and
...
another refuses. So `MIN_PASSWORD_BYTES` (`os/pkgs/mosd/apid/src/routes.rs:3391`)
...
(`os/pkgs/mosd/apid/src/routes.rs:2523`) — which is what section 2.4 already says
...
(`os/pkgs/mosd/apid/src/tests.rs:10199`) holds the absent member, and
`a_rejected_setup_password_is_never_echoed`
(`os/pkgs/mosd/apid/src/tests.rs:10174`) holds that no refusal repeats the
...
(`os/pkgs/mosd/apid/src/tests.rs:10220`) asserts both halves in setup mode, which
...
(`os/pkgs/mosd/apid/src/routes.rs:4232`) — because it is the same event: section
...
(`os/pkgs/mosd/apid/src/tests.rs:10269`) checks the event and that neither the
...
(`os/pkgs/mosd/apid/src/routes.rs:3164`). Both numbers are gate-green, because
