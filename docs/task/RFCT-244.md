...
`Source(source): Source,` (`os/pkgs/mosd/apid/src/routes.rs:4107-4109`).
...
The route calls `token::mint` (`os/pkgs/mosd/apid/src/routes.rs:4229`), pushes an
`ApiToken` and writes it through `write_tokens`
(`os/pkgs/mosd/apid/src/routes.rs:4302`) — the same three steps
...
(`os/pkgs/mosd/apid/src/routes.rs:4050`) rather than
...
(`os/pkgs/mosd/apid/src/tests.rs:9914`) asserts the member set exactly, and then
...
(`os/pkgs/mosd/apid/src/tests.rs:10145`) drives both surfaces against **one**
...
|---|---|---|
| password floor | `const MIN_PASSWORD_BYTES: usize = 8;` (`os/pkgs/mosd/apid/src/routes.rs:3453`), read by `fn password_under_floor(password: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3461`) | lifted from two inline `len() < 8` copies; see section 3 |
| hostname | `fn valid_hostname(name: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3497`) with `const HOSTNAME_RULES: &str =` (`os/pkgs/mosd/apid/src/routes.rs:3438`) | `&& !valid_hostname(hostname)` (`os/pkgs/mosd/apid/src/routes.rs:4157`) |
| interface name | `fn check_iface_name(iface: &str, path: &str) -> Result<(), Box<Response>> {` (`os/pkgs/mosd/apid/src/routes.rs:2488`) | `if let Err(response) = check_iface_name(iface, NETWORK_SETTINGS_PATH) {` (`os/pkgs/mosd/apid/src/routes.rs:4173`), the same helper M6's four routes use |
| static address CIDR | `fn validate_iface(iface: &str, dhcp: bool, address: &str) -> Result<(), &'static str> {` (`os/pkgs/mosd/apid/src/routes.rs:3516`) calling `fn valid_cidr(cidr: &str) -> bool {` (`os/pkgs/mosd/apid/src/routes.rs:3485`) | `if let Err(message) = validate_iface(iface, cfg.dhcp, address) {` (`os/pkgs/mosd/apid/src/routes.rs:4191`); see section 4 |
| the four relational rules | `fn relational_refusal(entries: &NetworkEntries, path: &str) -> Result<(), Box<Response>> {` (`os/pkgs/mosd/apid/src/routes.rs:2508`) calling `fn validate_entries(entries: &NetworkEntries) -> Result<(), String> {` (`os/pkgs/mosd/apid/src/routes.rs:3763`) | `if let Err(response) = relational_refusal(&candidate, NETWORK_SETTINGS_PATH) {` (`os/pkgs/mosd/apid/src/routes.rs:4206`) on the **candidate** tree |
...
(`os/pkgs/mosd/apid/src/routes.rs:5574-5575`) A bridge port may legitimately name
an interface the device already declares, and
`the_setup_network_tree_is_merged_with_the_stored_one_before_it_is_judged`
(`os/pkgs/mosd/apid/src/tests.rs:10110`) is the case that would be refused by a
...
(`os/pkgs/mosd/apid/src/tests.rs:10176`) drives four rejections and asserts, for
...
nothing is written on error."* (`os/pkgs/mosd/apid/src/routes.rs:3916`). So what
...
(`os/pkgs/mosd/apid/src/routes.rs:3951`) succeeds and the `hostname` write after
it does not.
...
(`os/pkgs/mosd/apid/src/routes.rs:4271`), then
`&& let Err(response) = write_network_map(&state, candidate).await`
(`os/pkgs/mosd/apid/src/routes.rs:4277`), then
`if let Err(err) = state.api.set_settings("access.webAdmin", &value).await {`
(`os/pkgs/mosd/apid/src/routes.rs:4284`), and last
`if let Err(response) = write_tokens(&state, &tokens).await {`
(`os/pkgs/mosd/apid/src/routes.rs:4302`). The password
...
(`os/pkgs/mosd/apid/src/tests.rs:9964`), which also asserts the wizard's opposite
...
(`os/pkgs/mosd/apid/src/tests.rs:10006`) drives **one** partial failure through
...
The fixture is `RefusesOnePath` (`os/pkgs/mosd/apid/src/tests.rs:9839`), added
...
`valid_cidr` (`os/pkgs/mosd/apid/src/routes.rs:3485`) has exactly **one** caller,
...
(`os/pkgs/mosd/apid/src/routes.rs:4191`), rather than lifting it or copying it.
...
(`os/pkgs/mosd/apid/src/tests.rs:10425`) sends the same entry three ways and
...
another refuses. So `MIN_PASSWORD_BYTES` (`os/pkgs/mosd/apid/src/routes.rs:3453`)
...
(`os/pkgs/mosd/apid/src/tests.rs:10263`) holds the absent member, and
`a_rejected_setup_password_is_never_echoed`
(`os/pkgs/mosd/apid/src/tests.rs:10238`) holds that no refusal repeats the
...
(`os/pkgs/mosd/apid/src/tests.rs:10284`) asserts both halves in setup mode, which
...
(`os/pkgs/mosd/apid/src/routes.rs:4294`) — because it is the same event: section
...
(`os/pkgs/mosd/apid/src/tests.rs:10333`) checks the event and that neither the
