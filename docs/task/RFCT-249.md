...
| `async fn a_psk_the_renderer_cannot_quote_is_refused_by_the_wifi_route` (`os/pkgs/mosd/apid/src/tests.rs:9441`) | The route answers 422 with section 2.4's envelope, `validation_failed`, `source` `apid`, the dot path; the message never echoes the key; and `fake.set_paths()` is empty, so a refused key never reaches mosd |
