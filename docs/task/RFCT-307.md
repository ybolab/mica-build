...
- complete: the marker reaches the image in the disposition that has one and
  ...
  **What it does not catch, so the guarantee is not over-read.** Not the reverse
  ...
  The amd64 pool was rebuilt at this branch's stamp (`git8191d89f4bd9`) before
  any compose, producer by producer, on the private builder `mos-l3-8ud12dym`.
  Each gate ran with its own exit status rather than through a pipe.

  | gate | result |
  | --- | --- |
  | `(cd verify && bun test)` (via `verify/run.sh`) | **green** — 1268/1268, 39 files, 18275 assertions |
  | `(cd build && bun test)` (via `build/run.sh`) | **green** — 889/889, 28 files, 4323 assertions |
  | `make docs-verify` | **green** — 183 index, 448 link, 733 status, 231 zh coverage, 43 board |
  ...
  **Out-of-scope files touched, all gitignored build inputs, none committed:**
  `boards/x64/bsp/out/kernel/` and `pkgs/podman/out-amd64/` copied in from
  `/srv/mos` so the pool could be built without recompiling a kernel and six
  upstream clones; `meta/` generated by the first build; `_out/`. `git status`
  is clean on this branch. `docs/plan/index.md`, `docs/task/index.md` and
