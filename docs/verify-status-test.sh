#!/usr/bin/env bash
# Negative tests for docs/verify-status.sh: each enforcement clause of the
# truth-status grammar is driven against a fixture in which its fact is
# false, and is required to fail with ITS OWN message.
#
#   bash docs/verify-status-test.sh          (or: make docs-verify-test)
#
# HOW. The REAL docs/verify-status.sh is run, once per case, over a fixture
# tree. The verifier resolves its own root from ${BASH_SOURCE}, so a copy at
# ${case}/docs/verify-status.sh reads ${case}/docs and ${case}/Makefile --
...
run_verifier() {
    RC=0
    bash "$1/docs/verify-status.sh" >"${WORK}/out" 2>&1 || RC=$?
}
...
expect_all_pass() {
    ...
    got_checks="$(sed -n 's|^docs/verify-status.sh: \([0-9]*\)/\1 PASS$|\1|p' "${WORK}/out")"
