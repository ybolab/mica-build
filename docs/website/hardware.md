...
by who qualified it and what evidence exists, using the taxonomy defined by
the board-contract plan:
...
The taxonomy itself is planned vocabulary being adopted now; until the
qualification matrix ships, no board on this page may be labelled
mos-qualified.
...
Neither board carries a completed field-reliability qualification matrix yet,
so neither is presented as mos-qualified.
...
procedure are the integrator's path from a blank board to a supported row on
this page; start at [../bsp/porting.md](../bsp/porting.md).
