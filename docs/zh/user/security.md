...
- **已部署设备上没有密钥环轮换渠道**——替换在役设备上的信任锚，
  意味着一个由正被替换的那把密钥签名的镜像。由此留下的两种情形有各自
  不同的答案，且都已成为决定而非悬而未决。**错过轮换重叠窗口**的设备
  靠一次更新恢复，而不是重刷：先前的版本会被重新发布到一个救援仓库，
  仍由外发的那把密钥签名，再用设置其他更新项的同一个已认证调用把设备
  指向它——若设备完全没有网络路径，则用可移动介质把它交过去。移动的是
  设备去哪里看，从不是它信任什么。而从**已被攻陷**
  的 CA 轮换出去没有任何远程答案，只能物理重刷：攻击者握有每一条远程
  路径所依赖的同一份签名权限。
