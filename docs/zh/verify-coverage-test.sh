#!/usr/bin/env bash
# Negative tests for docs/zh/verify-coverage.sh: each enforcement clause of the
# en/zh coverage rule is driven against a fixture in which its fact is false,
# and is required to fail with ITS OWN message.
#
#   bash docs/zh/verify-coverage-test.sh          (or: make docs-verify-test)
#
# HOW. The REAL docs/zh/verify-coverage.sh is run, once per case, over a
# fixture tree. The verifier resolves its own root from ${BASH_SOURCE}, so a
# copy at ${case}/docs/zh/verify-coverage.sh reads ${case}/docs -- the approach
...
| `../bsp/page.md` | db66fc02 | not-translated |
EOF
}
...
run_verifier() {
    RC=0
    bash "$1/docs/zh/verify-coverage.sh" >"${WORK}/out" 2>&1 || RC=$?
}
...
expect_all_pass() {
    ...
    got_checks="$(sed -n 's|^docs/zh/verify-coverage.sh: \([0-9]*\)/\1 PASS$|\1|p' "${WORK}/out")"
...
new_fixture "${FIX}"
replace_line "${FIX}/docs/zh/README.md" '| `../bsp/page.md` | db66fc02 | not-translated |' \
    '| `../bsp/page.md` | db66fc02 |'
expect_fail "a row missing its status cell" 2 \
    "row does not parse" \
    "docs/bsp/page.md has 0 coverage rows"
...
new_fixture "${FIX}"
replace_line "${FIX}/docs/zh/README.md" '| `../bsp/page.md` | db66fc02 | not-translated |' \
    '| `../bsp/page.md` | unknown | not-translated |'
