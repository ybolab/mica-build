#!/usr/bin/env bash
# Cross-build the mosd and apid release binaries for aarch64 (Debian bookworm
...
cargo build --release --locked --target aarch64-unknown-linux-gnu -p mosd -p apid

for name in mosd apid; do
