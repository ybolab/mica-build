...
#   bash os/build-env/deb/preflight.sh --producer board-cx3576
#
...
# os/build-env/deb/producers.sh, the base images from os/build-env/from.sh, and
...
for p in "${REPO_ROOT}/Makefile" "${FROM_SH}" "${PRODUCERS_SH}"; do
    [ -e "${p}" ] || {
        echo "error: ${p} does not exist. os/build-env/deb/preflight.sh derives the repository as three levels above itself; if this file moved, that arithmetic moved with it" >&2
        exit 1
    }
done
...
while [ "$#" -gt 0 ]; do
    ...
    --producer)
        ...
        ;;
    *)
        echo "usage: bash os/build-env/deb/preflight.sh [--producer <name>]" >&2
        exit 1
        ;;
    esac
done
...
# os/build-env/deb/build.sh builds it natively. The base image it will be handed
...
*)
    echo "error: $(uname -m) is not an architecture os/build-env/images.env builds a mos-build-deb for, so there is no container any producer here could pack in" >&2
    exit 1
    ;;
esac
...
the packing step copies, so without it the build stages nothing."
    ...
    # os/build-env/from.sh answers "is it in this host's image store" -- it
