...
#
#   bash os/build-env/deb/producers.sh --dir-for <producer>
#   -> os/pkgs/mosd/deb/mosd
#
...
# ANYWHERE in the tree. Not under os/pkgs/ -- producers live under
# os/rootfs/packages-src/ and os/boards/<board>/deb/ as well, and a glob rooted
...
# and `os-deb-%`, os/build-env/deb/build.sh and os/tests/deb-package-gate.sh all
...
[ -e "${REPO_ROOT}/Makefile" ] || {
    echo "error: ${REPO_ROOT}/Makefile does not exist. os/build-env/deb/producers.sh derives the repository as three levels above itself; if this file moved, that arithmetic moved with it" >&2
    exit 1
}
...
while [ "$#" -gt 0 ]; do
    ...
    --dir-for)
        ...
        ;;
    *)
        echo "usage: bash os/build-env/deb/producers.sh [--dir-for <producer>]" >&2
        exit 1
        ;;
    esac
done
...
for env_file in ${ENVS[@]+"${ENVS[@]}"}; do
    ...
    case "${producer}" in
    *[[:space:]]*)
        echo "error: the producer directory ${rel} has a name containing whitespace. os/build-env/deb/producers.sh emits space-separated fields and every reader splits on that, so such a name would be read as a different producer entirely" >&2
        exit 1
        ;;
    esac
    ...
    # carried through and os/tests/deb-package-gate.sh is what refuses a
    ...
    for a in ${arches}; do
        ...
        amd64 | arm64 | all) ;;
        *)
            echo "error: ${rel}/producer.env declares ARCHES entry '${a}'. The only values are amd64, arm64 and all; amd64 and arm64 are what os/build-env/images.env pins a mos-build-deb for, and 'all' is an architecture-independent package that is a valid member of every pool" >&2
            exit 1
            ;;
        esac
    done
...
if [ -n "${DIR_FOR}" ]; then
    ...
    echo "error: '${DIR_FOR}' is not a producer this repository defines. Discovered: $(printf '%s\n' "${ROWS[@]}" | cut -d' ' -f1 | tr '\n' ' ')-- a producer is a directory holding both a Dockerfile and a producer.env, and its NAME is that directory's basename; see os/build-env/deb/README.md" >&2
    exit 1
fi
