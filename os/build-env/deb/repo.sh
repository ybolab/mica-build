...
# localhost/mos-build-deb -- the same arrangement os/pkgs/mosd/hack/build-target.sh
# uses for cargo.
set -euo pipefail
...
for p in "${REPO_ROOT}/Makefile" "${FROM_SH}"; do
    [ -e "${p}" ] || {
        echo "error: ${p} does not exist. os/build-env/deb/repo.sh derives REPO_ROOT as three levels above itself; if this file moved, that arithmetic moved with it" >&2
        exit 1
    }
done
...
while [ "$#" -gt 0 ]; do
    ...
    --arch)
        ...
        ;;
    *)
        echo "usage: bash os/build-env/deb/repo.sh --arch <amd64|arm64>" >&2
        exit 1
        ;;
    esac
done
...
*)
    echo "error: $(uname -m) is not an architecture os/build-env/images.env builds a mos-build-deb for, so there is no container to run dpkg-scanpackages in" >&2
    exit 1
    ;;
esac
...
docker run --rm \
    ...
    "${IMAGE}" -c '
        ...
        {
            echo "# The local package pool for ${MOS_DEB_ARCH}, read out of the archives by os/build-env/deb/repo.sh."
