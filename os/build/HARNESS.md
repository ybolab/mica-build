...
`os/build/run.sh` is the single entry point. It finds bun — on the host, or
...
`bun test` filter. `bash os/build/run.sh filter --mkimage-x64` exits 1 by name.
...
`os/build` imports the board model from `os/verify/src/` across the two package
...
relative specifier into a sibling package's `src/` directly, and `os/build`'s
...
comparison in `os/verify` was once run from the wrong directory and reported
