...
const VERITY_ENV = [
  ...
  `VERITY_SALT=${SALT}`,
  'VERITY_UUID=5ac35760-0002-4000-8000-000000000005',
...
describe('the boot-attempts range guard, and the empty read that made it vacuous', () => {
  test('POSITIVE CONTROL: the SHIPPED boot.cmd passes, and the guard SAW credits', () => {
    ...
    const seen = requireBootAttempts(cx3576, BOOT_CMD, 'os/boards/cx3576/boot.cmd')
  ...
  test('THE EMPTY READ IS REFUSED, which is the shell\'s hole', () => {
    // os/update/bundle.sh: `done < <(grep -oE ... | awk ...)`. With no match the
...
describe('a slot\'s mos-verity-<slot>.env, and the five cmdlines it refuses', () => {
  ...
  test('no dm-mod.waitfor= is refused with the SAME sentence, which is bundle.sh\'s shape', () => {
    // os/mkimage-v2.sh gives the two their own sentences; os/update/bundle.sh
...
describe('the mount set is derived from the inputs, and nests nothing', () => {
  ...
  test('a BSP and a key directory outside the tree are both mounted', () => {
    ...
    const mounts = bundleMountsFor(inputs, join(REPO_ROOT, 'os/build/.work/bundle-x'))
