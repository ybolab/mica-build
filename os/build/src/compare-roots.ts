...
// and os/tests/dual-build-sanctions.md states it again in its header, because
 ...
 * anything -- the x64 factory root os/tests/factory-root-gate walks is 9,240 --
 ...
 * os/tests/factory-root-gate says the same thing about the same dimension.
 *
...
export function readFileCaps(root: string): Map<string, string> {
  ...
  try {
    ...
    throw new CompareRefusal(
      ...
      + `(Debian: libcap2-bin, Alpine: libcap libcap-setcap), or run this where os/verify's tool `
 ...
 *
 * os/verify already extracts one of these and it is NOT reusable here:
 * `extractLayout` in os/verify/src/smoke.ts unpacks the archive into an OCI
 * LAYOUT directory for buildx to take as a context -- blobs and an index, not a
 * filesystem -- and os/verify is read-only this round in any case. So the
 * unpacking is done here with tar, in the same idiom
 * os/tests/factory-root-gate/inner.sh uses against the same archives:
