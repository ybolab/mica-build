...
describe('every ascent is anchored, and the neighbours miss', () => {
  ...
  test('os/verify: 2 up plus its own manifest, and neither 1 nor 3', () => {
  ...
  test('os/build-env/from.sh: the resolver this package shells out to is where it says', () => {
...
describe('the boards are discovered, not written down', () => {
  test('both shipped boards are found, and by reading the directory', () => {
    ...
    expect(found).toEqual(['cx3576', 'x64'])
  ...
  test('a directory under boards/ with no board.env in it is not a board', () => {
    ...
    try {
      ...
      // The definition file IS the board -- which is also why os/verify takes a
