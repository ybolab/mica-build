...
// out of the ENVIRONMENT. os/mkimage-v2.sh's comment: "without it mkimage
...
export interface BootScriptSpec {
  ...
  /** `-n`. os/mkimage-v2.sh passes "mos boot"; it lands in the legacy header. */
...
export function bootScriptArgs(spec: BootScriptSpec): string[] {
  // -T script -C none: os/mkimage-v2.sh's shape, and no -A. A boot script is
