...
// and os/update/bundle.sh (deleted) compares the version it runs against the one the
...
// and version cross-check are os/update/bundle.sh's (deleted), not this file's.
...
export interface BundleSpec {
   ...
   * thread count. os/update/bundle.sh (deleted) passes exactly this shape.
   */
...
export interface InfoSpec {
   ...
   * passes one, and os/update/bundle.sh's (deleted) verify_bundle does.
   */
  readonly keyring?: string
  /** `--conf`. os/update/bundle.sh (deleted) loads the system.conf the image ships. */
 ...
 * back through rauc with signature verification on is how os/update/bundle.sh (deleted)
...
export async function info(tb: RaucToolbox, spec: InfoSpec): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    throw new ToolError(
      ...
      + `(${(e as Error).message}). Every field os/update/bundle.sh checks -- compatible, version -- `
