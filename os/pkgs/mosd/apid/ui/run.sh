...
if ! command -v bun >/dev/null 2>&1; then
    ...
    image="$(bash "${REPO_ROOT}/os/build-env/from.sh" --ref IMAGE_BUN_1)"
    echo "apid UI: ${image} (no bun on this host)"
    exec docker run --rm \
        -v "${REPO_ROOT}:/workspace" \
        -w /workspace/os/pkgs/mosd/apid/ui \
        "${image}" bash ./run.sh
fi
...
diff -ruN dist "${build_root}/dist" || {
    ...
    echo "       run 'bun run build' in os/pkgs/mosd/apid/ui and commit the result" >&2
    exit 1
}
