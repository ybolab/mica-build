...
#
#   bash os/pkgs/mosd/hack/build-deb.sh --producer mosd --crates "mosd apid" \
#        --arch amd64 --stage <dir>
#
# WHY THIS FILE STILL EXISTS. Every producer in this repository is built by
# os/build-env/deb/build.sh from its producer.env, and this is not a second
# driver: it is the mosd workspace's PREPARE hook implementation, reached
# through os/pkgs/mosd/deb/<producer>/prepare.sh. What it does -- cross-compile
...
# os/build-env/deb/build.sh's half.
set -euo pipefail
...
command -v docker >/dev/null 2>&1 || {
    echo "error: docker is required and not on PATH. The compile runs in a container -- the host carries no cargo -- which is what makes the compiler a value os/build-env/images.env records" >&2
    exit 1
}
...
while [ "$#" -gt 0 ]; do
    ...
    --stage)
        ...
        ;;
    *)
        echo "usage: bash os/pkgs/mosd/hack/build-deb.sh --producer <name> --crates \"<crate> ...\" --arch <amd64|arm64> --stage <dir>" >&2
        exit 1
        ;;
    esac
done
...
[ -n "${STAGE}" ] || { echo "error: --stage is required; it is where os/build-env/deb/build.sh looks for what this hook produced" >&2; exit 1; }
[ -d "${STAGE}" ] || { echo "error: --stage ${STAGE} is not a directory. os/build-env/deb/build.sh creates it before running this hook" >&2; exit 1; }
...
arm64)
    ...
    ;;
*)
    echo "error: --arch ${ARCH} is not amd64 or arm64. Those are the two architectures os/build-env/images.env pins a Rust std for; adding a third is an images.env edit and not an argument to this script" >&2
    exit 1
    ;;
esac
...
# version: that is os/build-env/deb/version.sh's, and it is one rule for the
# whole pool.
