...
/**
 * `os/boards/x64/board.env` as a map of its literal assignments.
 *
 ...
 * `os/boards/x64/grub.cfg` indents both of its linux lines -- the A slot's and
...
/**
 * The container everything here runs in, resolved from os/build-env/images.env
...
function resolveQemuImage(): string {
  const result = spawnSync("bash", [path.join(REPO_ROOT, "os/build-env/from.sh"), "--ref", "IMAGE_DEBIAN_TRIXIE"], {
    encoding: "utf8",
  });
  if (result.status !== 0) {
    die(
      `error: os/build-env/from.sh could not resolve IMAGE_DEBIAN_TRIXIE:\n${(result.stderr ?? "").trim()}`,
    );
  }
...
async function main(): Promise<void> {
  ...
  const board = readBoardEnv(fs.readFileSync(path.join(REPO_ROOT, "os/boards/x64/board.env"), "utf8"));
  ...
  // (see os/build/src/mkimage-x64.ts).
  //
  // MOS_QEMU_REUSE_DISK keeps the disk a previous --prepare-only made, so
  // os/tools/qemu-seed-state.sh's writes survive into the boot. Without it every
  ...
  if (options.prepareOnly) {
    note(`prepared ${disk}; seed it with os/tools/qemu-seed-state.sh, then run with MOS_QEMU_REUSE_DISK=1`);
    return;
  }
