...
#
# It lives here rather than in os/tools/ because it is not a general utility: it
...
#    upstream reads as measured rather than assumed, and os/tests/podman-pins-test.sh
...
#
# The repositories are the ones the src stage clones (os/pkgs/podman/Dockerfile),
...
if behind:
    ...
    print(file=sys.stderr)
    print("  To act on this: in os/pkgs/podman/versions.env set the component's *_VERSION to the", file=sys.stderr)
