...
#
# PRE-FLIGHT MODE. Run with MOS_DEB_PREFLIGHT=1 by os/build-env/deb/preflight.sh
...
[ "${PREFLIGHT}" != 0 ] || [ -n "${MOS_DEB_STAGE:-}" ] || {
    echo "error: MOS_DEB_STAGE is not set. This script is os/pkgs/podman/deb/podman/producer.env's PREPARE hook and is run by os/build-env/deb/build.sh, which sets it; it is not a standalone command" >&2
    exit 1
}
...
VERSIONS_STAMP_SH="${REPO_ROOT}/os/pkgs/podman/versions-stamp.sh"
