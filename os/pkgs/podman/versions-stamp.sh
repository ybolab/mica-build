#!/usr/bin/env bash
# What os/pkgs/podman/out-<arch> was built FROM, written into it and read back
# out of it.
#
#   bash os/pkgs/podman/versions-stamp.sh --lock          # the build's input
#   bash os/pkgs/podman/versions-stamp.sh --digest        # its sha256
#   bash os/pkgs/podman/versions-stamp.sh --stamp <dir>   # write <dir>/VERSIONS.env
#   bash os/pkgs/podman/versions-stamp.sh --check <dir>   # refuse a stale one
#
# THE DEFECT THIS CLOSES is named in os/pkgs/podman/build.sh's own words:
...
# os/pkgs/podman/Dockerfile COPYs -- that file says why, and it is measured: a
...
# which are the other two records os/pkgs/podman/Dockerfile's verify stage
# leaves in the export, and in the KEY=value shape os/pkgs/rauc's
...
--check)
    ...
    [ "${got}" = "${want}" ] || {
        echo "error: ${out} was built from a different os/pkgs/podman/versions.env than the one in this tree.
  stamped:  ${got}
  current:  ${want}
...
executable and the right architecture. Rebuild them -- MOS_ARCH=$(basename "${out}" | sed 's/^out-//') make podman -- or put versions.env back to what they were compiled from." >&2
    ...
    ;;
*)
    echo "usage: bash os/pkgs/podman/versions-stamp.sh --lock | --digest | --stamp <dir> | --check <dir>" >&2
    exit 1
    ;;
esac
