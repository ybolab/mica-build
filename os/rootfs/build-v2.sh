...
if [ -e "$OVERLAY_STAGE/etc/rauc/keyring.pem" ]; then
    ...
    echo "A keyring baked into the signed root makes every flashed device trust that CA's bundles, and the overlay is copied wholesale into every image, so a file left here is a trust root nobody chose. The image's keyring is staged from the repository-root ca/ instead -- delete this file and put the CA you want in ca/ (os/pkgs/rauc/gen-dev-keys.sh writes a development-grade one when ca/ is empty)." >&2
    exit 1
fi
...
CA_DIR="$REPO_ROOT/ca"
bash "$REPO_ROOT/os/pkgs/rauc/gen-dev-keys.sh" --if-absent
if [ ! -s "$CA_DIR/ca.cert.pem" ]; then
    echo "error: $CA_DIR/ca.cert.pem is missing or empty after os/pkgs/rauc/gen-dev-keys.sh --if-absent." >&2
...
# release-grade either; os/verify reads both the same way.
...
# intentions.
#
# EVERY COLUMN IS READ OUT OF THE POOL INDEX, which os/build-env/deb/repo.sh
...
# directory, and it comes from os/build-env/deb/producers.sh, which discovers
