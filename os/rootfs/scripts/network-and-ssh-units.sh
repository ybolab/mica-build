...
# A PRESET PREVENTS; IT DOES NOT RETRACT. That is why the preset is written here
# too AND why the `rm` below stays.
...
#
# The `rm` is still needed on THIS path: stages/10-base installs openssh-server
# before this script runs, so the link already exists by the time the preset
# lands, and preventing is no use against a link that is already there. On the
# package path the ordering is reversed -- dpkg unpacks mos-system's payload
# before it configures openssh-server -- which is why the preset alone is enough
# there. A third ordering reaches neither, and mos-system's postinst asserts
# against it rather than assuming it away.
