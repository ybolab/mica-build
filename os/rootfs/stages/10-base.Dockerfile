...
# libubootenv-tool provides fw_printenv/fw_setenv for RAUC's U-Boot backend and
# the machine-id oneshot; curl is the health gate's apid probe, and without it
# that probe silently SKIPs and the gate covers two of three components while
...
# os/update/rauc/manifest.raucm.in records as DEFERRED.
#
# glib, json-glib and libfdisk are named explicitly below because nothing else
# pulls them in: they are what the self-built rauc links, and every one of them
# is in the image for rauc alone.
#
...
# the verity device from the cmdline without any userspace tool.
#
...
# CONFIG_DM_INIT unset -- it is absent from /boot/config entirely -- so
# `dm-mod.create=` is a parameter it does not implement, and an unknown
...
# is purged from the root afterwards.
#
...
# The TRUST ANCHORS, copied in from the `certs` stage.
#
# libssl3t64 is 8 MB of TLS library, and without a trust store the packed root
# has the code to speak TLS and nothing to decide who to believe. Every
# outbound HTTPS connection then fails the same way -- `podman pull` on the
# container engine, and `curl`, which the package list above installs and the
# package-manager purge explicitly keeps -- with
...
# build time, during the update, or on the next boot. It is precisely because
# that failure is silent that the number is written out here, in
...
# the same 2020-01-01 the layout's FILE_MTIME uses (day 18262), which is what
