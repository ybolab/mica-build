...
# stages/20-install -- the read-only-root wiring: seed units, repart.d,
# network. One link in the chain; stages/README.md says what the chain is.
...
# Networking: DHCP on all eth* (the kernel cmdline has net.ifnames=0).
#
...
# them deliberately. os/verify asserts that ordering, and asserts that no file
# the image ships could be mistaken for a reconciler-owned one -- the network
...
# listening from early boot until mosd's first reconcile shut it down -- the
# window this design exists to close. Turning SSH on is a runtime decision made
# against the settings tree, never a property of the image.
...
# Read-only root wiring: fstab (/srv from DATA, /var from EPHEMERAL, plus the
# STATE and META filesystems and a tmpfs /tmp), the repart.d set that grows
# DATA, the six STATE bind mounts (/var/lib/mos, /var/lib/bluetooth,
# /etc/ssh, /etc/hostname,
...
# first boot), the /var fill-up policies and the fw_env.config pointing at the
# uenv pair.
...
# image cannot inherit a host umask. /root gets its own mkdir/chown/chmod
# rather than joining the 0755 mountpoint list: Debian already ships it at 0700
# root:root, but nothing guarantees that, and a root home that reached the
# image group- or world-readable would be a different defect from the one
# root.mount fixes -- one that /srv, unlike the verity root, is not protected
# against either.
ARG OVERLAY_DIR
