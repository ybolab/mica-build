...
# by whoever assembles the stage list.
#
# It is a decision that is RECORDED. os/build/src/stages.ts writes
...
# nothing are otherwise indistinguishable afterwards.
#
...
# The container engine ships INSTALLED AND INERT.
#
...
# source, and are copied in below. Every package here is derived from podman
...
#                 NFT_EXECUTABLE = "nft"). Without it the first `podman run`
#                 fails with `netavark: nftables error: unable to execute nft`.
#                 A dependency reached by exec is invisible to every
#                 NEEDED-soname check by construction, which is why it is
#                 listed here by name.
#
...
#                 crun 1.21 links libyajl2 instead, so the packaged engine
#                 would need the wrong one of the pair.
#
...
#                 fails to resolve.
#
#                 libsqlite3-0 is deliberately NOT here. podman depends on
#                 mattn/go-sqlite3 (go.mod:44), the cgo driver, but it compiles
#                 the SQLite amalgamation IN unless the `libsqlite3` build tag
#                 is set, so nothing links the system library.
#
...
# implementation; libyajl2 goes with crun.
#
# A SEPARATE apt TRANSACTION from stages/10-base's allowlist. Reaching this
# line already means the engine was asked for, because this file only builds
# when it was.
...
#
# A separate RUN from the engine install, because the files it asserts arrive
