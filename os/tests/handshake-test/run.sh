...
# os/tests/handshake-test/harness.sh inside the builder image against the
...
#
#   make os-uboot-handshake-test        # or: bash os/tests/handshake-test/run.sh
...
docker run --rm \
    ...
    -w /work \
    "${IMAGE_TAG}" \
    bash /repo/os/tests/handshake-test/harness.sh
