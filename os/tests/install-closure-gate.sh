...
#           os/rootfs/packages/resolve.sh            (the package set per board)
#   builds  clean roots from the digest-pinned Debian base, per architecture,
#           and asserts what only an INSTALLED root can answer
#
# os/tests/deb-package-gate.sh says in its own header that it does not do this:
...
#      os/rootfs/packages/resolve.sh, for the board that declares this pool's
...
#      binfmt registration -- the same route os/pkgs/podman/build.sh and
#      os/build-env/deb/build.sh take, and the one os/verify's smoke runner
...
RESOLVE_SH="${REPO_ROOT}/os/rootfs/packages/resolve.sh"
VERSION_SH="${REPO_ROOT}/os/build-env/deb/version.sh"
PODMAN_VERSIONS="${REPO_ROOT}/os/pkgs/podman/versions.env"
RAUC_VERSIONS="${REPO_ROOT}/os/pkgs/rauc/versions.env"
...
# os/tests/deb-package-gate.sh's reason: a discovered list turns a pool that was
...
*)
    echo "error: $(uname -m) is not an architecture os/build-env/images.env pins a base image for" >&2
    exit 1
    ;;
esac

# The base, resolved from images.env by key exactly as os/tests/quadlet-doc-test.sh
...
[ "${#BASE_ARGS[@]}" -eq 2 ] || {
    echo "error: os/build-env/from.sh did not yield IMAGE_DEBIAN_TRIXIE (see its message above); every root below would have been built from an empty FROM" >&2
    exit 1
}
...
# number and from nowhere else. os/verify/src/smoke-pins.ts says why at length:
# a version written down twice is a version that stops matching the binary the
# first time one copy moves.
#
# The four Rust binaries take theirs from os/build-env/deb/version.sh, which is
...
[0-9]*.[0-9]*.[0-9]*) ;;
*)
    echo "error: os/build-env/deb/version.sh printed '${POOL_VERSION}', whose part before the '+' is '${CRATE_VERSION}' and is not <major>.<minor>.<patch>. That prefix is the version the four Rust binaries must report; an unrecognised shape would be compared against every --version output and match none of them" >&2
    exit 1
    ;;
esac
...
# side once rather than per binary, are os/verify/src/smoke-pins.ts's.
pin() {
...
#
# The installed paths are the ones os/verify/src/smoke-register.ts measured, and
...
{
    printf 'mosd\t/usr/bin/mosd\t%s\tos/build-env/deb/version.sh\t-\t-\n' "${CRATE_VERSION}"
    printf 'apid\t/usr/bin/apid\t%s\tos/build-env/deb/version.sh\t-\t-\n' "${CRATE_VERSION}"
    printf 'mos-mqttd\t/usr/bin/mos-mqttd\t%s\tos/build-env/deb/version.sh\t-\t-\n' "${CRATE_VERSION}"
    printf 'mos-mqtt-broker\t/usr/bin/mos-mqtt-broker\t%s\tos/build-env/deb/version.sh\t-\t-\n' "${CRATE_VERSION}"
    ...
    # for os/verify/src/smoke-register.ts's reason: an entry that declares
...
# it, adding an eighth binary under os/pkgs/podman/ -- with its pin, its hash and
# its install line -- would leave this gate reporting a full green over seven of
# eight, which is the drift os/verify/src/smoke-pins.ts exists to refuse in its
# own register.
UNCLAIMED=""
PINS_N=0
...
# alone, which is the distinction os/tests/deb-package-gate.sh's ownership rule
...
# os/tests/deb-package-gate.sh's per-package ENABLEMENT count is correct and
...
while IFS="$(printf '\t')" read -r name path expected origin lim_status lim_stderr; do
    ...
    # os/tests/shell-pipefail-lint.sh refuses by name.
    if [ "${status}" -eq 0 ] &&
...
for o in ${OTHERS}; do
    ...
    # having FAILED because the pattern was found. os/tests/shell-pipefail-lint.sh
...
# shared on purpose, exactly as os/tests/deb-package-gate.sh's ownership rule
# has it.
PAYLOAD_N=0
while IFS= read -r path; do
    case "${path}" in /*) ;; *) continue ;; esac
...
#
# No `# syntax=` line, like os/tests/quadlet-doc-test.sh's generated Dockerfile:
# os/build-env/build.sh checks that pin across the Dockerfiles the tree SHIPS,
...
for arch in "${ARCHES[@]}"; do
    ...
    # os/pkgs/rauc/deb/rauc/prepare.sh finds it: the board files are the one
    # authority on which architecture a board is, and a table here would be a
    # second one.
    boards=()
    for env_file in "${REPO_ROOT}/os/boards"/*/board.env; do
        [ -f "${env_file}" ] || continue
        [ "$(sed -n 's/^MOS_ARCH=//p' "${env_file}")" = "${arch}" ] || continue
        boards+=("$(basename "$(dirname "${env_file}")")")
    done
    [ "${#boards[@]}" -eq 1 ] || {
        echo "error: ${#boards[@]} board(s) under os/boards/ declare MOS_ARCH=${arch} (${boards[*]-none}). This gate installs one board's resolution per pool: with none there is nothing to install, and with two there is no answer to which" >&2
        exit 1
    }
    board="${boards[0]}"
    radios="$(sed -n 's/^BOARD_RADIOS="\(.*\)"$/\1/p' "${REPO_ROOT}/os/boards/${board}/board.env" | head -n1)"
    ...
    [ "${#PKG_SET[@]}" -gt 0 ] || {
        echo "error: os/rootfs/packages/resolve.sh yielded no package for --board ${board} --profile dev --radios '${radios}' (see its message above)" >&2
        exit 1
    }
    ...
    [ "${#PKG_SET_DECLINED[@]}" -gt 0 ] || {
        echo "error: os/rootfs/packages/resolve.sh yielded no package for --board ${board} --profile dev --without rauc (see its message above). That would mean the manifests cannot express a rauc-declined image at all, which is the configuration this root exists to install" >&2
        exit 1
    }
    ...
    # The builder, chosen as os/build-env/deb/build.sh chooses one: a caller who
