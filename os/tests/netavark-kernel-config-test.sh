...
# `for option in ...` loop in os/boards/cx3576/bsp/kernel/Dockerfile, which greps
...
# the rule needing it, read from the tag os/pkgs/podman/versions.env pins.
...
CONFIG="${REPO_ROOT}/os/boards/cx3576/bsp/kernel/config/kernel-cx3576z.config"
DOCKERFILE="${REPO_ROOT}/os/boards/cx3576/bsp/kernel/Dockerfile"
VERSIONS_ENV="${REPO_ROOT}/os/pkgs/podman/versions.env"
...
# =y and not =m: os/boards/common/mos-required.fragment states the rule -- a
...
if [ "${pinned}" = "${CITED_NETAVARK}" ]; then
    pass "os/pkgs/podman/versions.env still pins netavark ${CITED_NETAVARK}"
else
