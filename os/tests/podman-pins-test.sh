#!/usr/bin/env bash
# Drive os/pkgs/podman/check-pins.sh against recorded upstream responses.
#
...
# os/tests/podman-pins/versions-env/. Every assertion below is therefore
# reproducible on a disconnected machine, and none of them writes to
# os/pkgs/podman/versions.env.
#
...
CHECK="${REPO_ROOT}/os/pkgs/podman/check-pins.sh"
FIX="${HERE}/podman-pins"
REAL_ENV="${REPO_ROOT}/os/pkgs/podman/versions.env"
...
expect_rc 0 "the six pins in os/pkgs/podman/versions.env are current"
...
expect_says "os/pkgs/podman/versions.env set the component's \*_VERSION" "the failure text says to edit versions.env"
