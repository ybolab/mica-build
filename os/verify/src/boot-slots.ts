...
//
// PLAN-014 M4g (RFCT-110). This table was written down three times -- in
// `checks-slots.ts` (M4b), in `checks-board.ts` (M4d) and, when batch 4b needed
// it a fourth time, here. A second spelling of "the slots are BOOT-A and BOOT-B
// and their offsets come from the layout" beside the first is exactly the drift
// M4c found twice as a defect and M4f moved `board-scope.ts` out to stop, so
// the definition moved here and the three modules import it. Nothing is copied.
//
...
export interface BootSlot {
  ...
  /** The lowercase trailing letter, which `@SLOT@` expands to (:1837). */
