// Reading a mos disk image without touching the host: os/verify-image-v2.sh's (deleted)
...
export async function readGpt(rt: ToolRuntime, image: string): Promise<GptTable> {
  ...
  // how many partitions there are. os/verify-image-v2.sh:1418 (deleted) counts the same
...
/**
 * `sgdisk --verify`, with the same reading os/verify-image-v2.sh:1394 (deleted) gives it.
 *
 ...
 * os/verify-image-v2.sh:1795 and :1805 (deleted) ask both, separately, and a factory
...
export async function fatVolumeSerial(rt: ToolRuntime, slot: FatSlot): Promise<string> {
  ...
  // Spaces come out, and nothing else does. os/verify-image-v2.sh:1796 (deleted) pipes
 ...
 * mcopy writes them itself, as os/verify-image-v2.sh:1899 and :2003 (deleted) have it write
 ...
 *
 * An absence is an answer here and not a throw: os/verify-image-v2.sh:1923 (deleted)
 ...
 * os/verify-image-v2.sh:1941 (deleted) reads exactly those cells and a helper that threw
...
/**
 * `e2fsck -fn FILE`, read the way os/verify-image-v2.sh:2342 (deleted) reads it.
 *
...
// first four bytes are 0x27051956. os/verify-image-v2.sh:2024 (deleted) reads exactly
 ...
 *
 * NOT the payload from offset 64. os/verify-image-v2.sh:2039 (deleted) strips NULs from
