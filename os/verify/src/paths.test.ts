...
import {
  ascendTo, BOARDS_DIR, boardEnvPath, OS_DIR, PACKAGE_DIR, REPO_ROOT,
...
describe('the shipped boards are DISCOVERED, not written down', () => {
  ...
  // a third definition under os/boards/ was never opened by the lint, which
  ...
  test('a directory with NO board.env is not a board', () => {
    ...
    // os/boards/ must not become a board the lint then fails to read.
  ...
  test('an EMPTY discovery is refused, because an empty loop is a green suite', () => {
    ...
    // when the real os/boards/ was empty is a guard nobody has ever run.
