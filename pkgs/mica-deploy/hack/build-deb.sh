...
for p in "${WORKSPACE}/Cargo.toml" "${FROM_SH}"; do
    [ -e "${p}" ] || {
        echo "error: ${p} does not exist. pkgs/mica-deploy/hack/build-deb.sh derives the workspace as its own directory's parent and the repository as three levels above that; if this file moved, that arithmetic moved with it" >&2
        exit 1
    }
done
...
while [ "$#" -gt 0 ]; do
    ...
    --stage)
        ...
        ;;
    *)
        echo "usage: bash pkgs/mica-deploy/hack/build-deb.sh --producer <name> --bins \"<binary> ...\" --arch <amd64|arm64> --stage <dir>" >&2
        exit 1
        ;;
    esac
done
...
# directory: sharing pkgs/mica-deploy/target/ with an interactive `cargo build`
...
docker run --rm \
    ...
    -w /src/pkgs/mica-deploy \
    -e "TARGET=${TRIPLE}" \
    -e "ELF_ARCH=${ELF_ARCH}" \
    -e "BINS=${BINARIES[*]}" \
    -e "CARGO_TARGET_DIR=/src/pkgs/mica-deploy/target-deb/${PRODUCER}" \
    --entrypoint /bin/bash \
    "${RUST_IMAGE}" -c '
        set -euo pipefail
...
if [ -n "${stray}" ]; then
    echo "error: the ${PRODUCER} producer's target directory holds binaries it does not own:${stray}. This producer ships the DEVICE side of pkgs/mica-deploy and nothing else; mica-deploy is the release-host signing tool. See pkgs/mica-deploy/README.md" >&2
    exit 1
fi
