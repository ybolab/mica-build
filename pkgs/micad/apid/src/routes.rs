...
/// creates `eth1`)"* (`Settings::set` in `pkgs/micad/micad-settings/src/model.rs`), so a
...
/// (`Settings::set` in `pkgs/micad/micad-settings/src/model.rs`) -- so the typed
...
// (`Settings::set` in `pkgs/micad/micad-settings/src/model.rs`), and the reconciler that
// does enforce them runs *after* the save with its verdict deliberately not
// propagated to the caller (`MosdService::write_setting` in `pkgs/micad/micad/src/bus.rs`). A raw
...
/// (`validate_network` in `pkgs/micad/micad/src/reconciler/network.rs`), and it must be a
...
/// (`is_ip_or_cidr` in `pkgs/micad/micad/src/reconciler/network.rs`), for the reason
...
/// (`is_host_port` in `pkgs/micad/micad/src/reconciler/network.rs`).
...
///
/// The echo of `wgkeys::is_key` (`pkgs/micad/micad/src/wgkeys.rs`),
...
/// (`validate_network` in `pkgs/micad/micad/src/reconciler/network.rs`) stays the boundary
