...
(`pkgs/micad/target-deb/<producer>/`, gitignored) rather than shared with
`pkgs/micad/hack/build-target.sh`'s `target/`: a shared one would let a
four-binary build satisfy the assertion with binaries nobody asked for.

## Adding a producer

Create `pkgs/micad/deb/<producer>/` holding `producer.env`, a `Dockerfile`,
