...
#
#   bash pkgs/micad/hack/build-deb.sh --producer micad --crates "micad apid" \
...
# through pkgs/micad/deb/<producer>/prepare.sh. What it does -- cross-compile
...
for p in "${WORKSPACE}/Cargo.toml" "${FROM_SH}"; do
    [ -e "${p}" ] || {
        echo "error: ${p} does not exist. pkgs/micad/hack/build-deb.sh derives the workspace as its own directory's parent and the repository as three levels above that; if this file moved, that arithmetic moved with it" >&2
        exit 1
    }
done
...
while [ "$#" -gt 0 ]; do
    ...
    --stage)
        ...
        ;;
    *)
        echo "usage: bash pkgs/micad/hack/build-deb.sh --producer <name> --crates \"<crate> ...\" --arch <amd64|arm64> --stage <dir>" >&2
        exit 1
        ;;
    esac
done
...
# resolution pkgs/micad/hack/build-target.sh uses -- micad and apid answer
...
# directory: sharing pkgs/micad/target/ with build-target.sh would let a
# four-binary build satisfy the independence assertion below with binaries this
# producer never asked for. Gitignored through pkgs/micad/.gitignore.
...
if [ -n "${stray}" ]; then
    echo "error: the ${PRODUCER} producer's target directory holds binaries it does not own:${stray}. Each producer compiles only its own crates into its own CARGO_TARGET_DIR; see pkgs/micad/deb/README.md" >&2
    exit 1
fi
