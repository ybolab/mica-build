...
# and verify the ELF.
#
#   bash pkgs/micad/hack/build-target.sh <rust-target> <elf-arch-substring>
#   bash pkgs/micad/hack/build-target.sh aarch64-unknown-linux-gnu aarch64
#   bash pkgs/micad/hack/build-target.sh x86_64-unknown-linux-gnu  x86-64
#
...
# rootfs/build.sh, pkgs/micad/hack/build-aarch64.sh and a hand invocation all
...
for p in "${WORKSPACE}/Cargo.toml" "${FROM_SH}"; do
    [ -e "${p}" ] || {
        echo "error: ${p} does not exist. pkgs/micad/hack/build-target.sh derives the workspace as its own directory's parent and the repository as three levels above that; if this file moved, that arithmetic moved with it" >&2
        exit 1
    }
done
...
# The repository is mounted, not pkgs/micad/. That used to be forced: pkgs/micad/Cargo.toml
# listed one workspace member outside this directory, and mounting pkgs/micad/ alone
...
# pkgs/micad/ any more and that reason is gone.
#
...
# next member added outside pkgs/micad/ fails with a sentence rather than with a
...
while IFS= read -r m; do
    ...
    *) continue ;;
    ...
    "${REPO_ROOT}"/*) ;;
    *)
        echo "error: pkgs/micad/Cargo.toml lists the workspace member '${m}', which resolves outside ${REPO_ROOT}. This build mounts the repository into the container and nothing above it, so cargo would report that member as a missing Cargo.toml rather than as a member the container cannot see" >&2
        exit 1
        ;;
    esac
...
# separation mica-podman:build.sh and mica-deploy:build.sh draw: the build
