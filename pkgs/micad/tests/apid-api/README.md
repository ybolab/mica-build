# `pkgs/micad/tests/apid-api` — the apid black-box HTTP suite
...
Nothing here modifies `pkgs/micad/`. Any apid defect this suite finds is **reported,
...
| `07-update-rollback` | the `rollback` verdict micad computes from RAUC's live slot state, and the 409 a refused rollback answers with |
...
`pkgs/micad/apid/openapi.json` ALSO states agrees with the document. It covers
...
```sh
docker run --rm -v "$(git rev-parse --show-toplevel):/w" -w /w/pkgs/micad/tests/apid-api \
