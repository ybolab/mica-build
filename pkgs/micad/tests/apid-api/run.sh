...
#   bash pkgs/micad/tests/apid-api/run.sh --dry-run
#
...
if [ ! -f "${SCRIPT_DIR}/src/main.ts" ]; then
    ...
    # the suite.
    note "pkgs/micad/tests/apid-api/src/main.ts does not exist yet; the suite step will FAIL loudly rather than be skipped quietly"
fi
...
if [ "${DRY_RUN}" -eq 1 ]; then
    ...
    note "would seed     pkgs/micad/tests/apid-api/guest/m7-net-smoke.sh -> DATA:${SMOKE_IN_GUEST} (phase 05c)"
    ...
    note "would run      docker run --network ${NET} -v ${REPO_ROOT}:/w -v ${OUT_REAL}:/w/_out -w /w/pkgs/micad/tests/apid-api ${BUN_IMAGE} bun run src/main.ts"
...
cat > "$ART_DIR/mica-api-console.service" <<'UNIT'
[Unit]
...
UNIT
cat > "$ART_DIR/mica-api-network.service" <<'UNIT'
[Unit]
...
if ! qemu_port --seed \
    ...
    "$ART_DIR/mica-api-console.service" /state/systemd-units/mica-api-console.service \
    "$ART_DIR/mica-api-network.service" /state/systemd-units/mica-api-network.service \
    --enable mica-api-console.service --enable mica-api-network.service > "$ART_DIR/seed.log" 2>&1; then
...
run_suite() {
    ...
    if [ ! -f "${SCRIPT_DIR}/src/main.ts" ]; then
        fail "[${label}] the suite entry point pkgs/micad/tests/apid-api/src/main.ts does not exist, so nothing was asserted about apid"
        SUITE_RC=1
        return 0
    fi
    ...
    set +e
    docker run --rm --label ai-agent=true --network "${NET}" \
        -v "${REPO_ROOT}:/w" -v "${OUT_REAL}:/w/_out" -w /w/pkgs/micad/tests/apid-api \
        -e APID_HOST="${ip}" \
