 ...
 *
 * THE GUEST SIDE IS NOT HERE. `pkgs/micad/tests/apid-api/guest/m7-net-smoke.sh` runs inside
 ...
 */
const REQUIRED = [
  ["keystore-real-readable", "the systemd-network user can read the key micad generated for the managed WireGuard fixture"],
  ...
  ["netuser-exists", "the image carries the systemd-network account micad chowns key files to"],
  [
    "keystore-readable",
...
const phase: Phase = {
  ...
  assumes:
    "the guest booted from a disk that run.sh seeded with pkgs/micad/tests/apid-api/guest/m7-net-smoke.sh " +
    ...
    if (found.size === 0) {
      ...
      report.fail(
        ...
        [
          ...
          "actual:   none. The guest never ran pkgs/micad/tests/apid-api/guest/m7-net-smoke.sh, so NOTHING",
