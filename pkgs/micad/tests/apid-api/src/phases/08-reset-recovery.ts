 ...
 * tier 1 is an intent record micad carries out on the next boot. Nothing here
 ...
 * assertion, and `pkgs/micad/apid/src/tests/reset.rs` proves the gate against a
...
const phase: Phase = {
  ...
  async run({ client, report, state }: PhaseContext): Promise<void> {
    ...
    // saved as TOML by micad and parsed back.
