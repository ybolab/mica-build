#!/usr/bin/env bash
# Compile the binaries the rauc-update producer ships out of the rauc-sign
# crate, prove the producer boundary held, and stage them for packaging.
#
#   bash pkgs/rauc-sign/hack/build-deb.sh --producer rauc-update \
#        --bins "rauc-update rauc-verify" --arch amd64 --stage <dir>
#
# WHY THIS FILE EXISTS. It is this workspace's PREPARE hook implementation,
# reached through pkgs/rauc-sign/deb/<producer>/prepare.sh, and it is the twin
# of pkgs/mosd/hack/build-deb.sh -- deliberately, in the same order and with
# the same refusals, because two hooks that differ only in which workspace they
# compile should read as one hook and any divergence should be visible as a
# diff. What it does -- compile a named set of binaries and then assert that
# NOTHING ELSE was compiled with them -- is a claim about a cargo build, and no
# key in producer.env describes a cargo build.
#
# THE UNIT HERE IS A BINARY, NOT A CRATE, and that is the one place it departs
# from its twin. The mosd workspace has four crates and each is one binary, so
# `cargo build -p <crate>` selects there; this workspace is ONE crate declaring
# three binaries through cargo's auto-discovery, so the selection is `--bin`.
# The independence assertion is the same statement either way, and here it is
# the one that matters most: `rauc-sign` is the RELEASE-side signing tool, it
# runs on a build host and never on a device, and this producer ships it
# nowhere.
#
...
for p in "${WORKSPACE}/Cargo.toml" "${FROM_SH}"; do
    [ -e "${p}" ] || {
        echo "error: ${p} does not exist. pkgs/rauc-sign/hack/build-deb.sh derives the workspace as its own directory's parent and the repository as three levels above that; if this file moved, that arithmetic moved with it" >&2
        exit 1
    }
done
...
while [ "$#" -gt 0 ]; do
    ...
    --stage)
        ...
        ;;
    *)
        echo "usage: bash pkgs/rauc-sign/hack/build-deb.sh --producer <name> --bins \"<binary> ...\" --arch <amd64|arm64> --stage <dir>" >&2
        exit 1
        ;;
    esac
done
...
# Every binary this crate declares, through cargo's auto-discovery: `rauc-sign`
...
ALL_BINARIES=(rauc-sign rauc-verify rauc-update)
...
# directory: sharing pkgs/rauc-sign/target/ with an interactive `cargo build`
...
docker run --rm \
    ...
    -w /src/pkgs/rauc-sign \
    -e "TARGET=${TRIPLE}" \
    -e "ELF_ARCH=${ELF_ARCH}" \
    -e "BINS=${BINARIES[*]}" \
    -e "CARGO_TARGET_DIR=/src/pkgs/rauc-sign/target-deb/${PRODUCER}" \
    --entrypoint /bin/bash \
    "${RUST_IMAGE}" -c '
        set -euo pipefail
...
# INDEPENDENCE, asserted rather than described. `cargo build --bin rauc-update
# --bin rauc-verify` is the intent; this is the evidence, and it is what a later
...
if [ -n "${stray}" ]; then
    echo "error: the ${PRODUCER} producer's target directory holds binaries it does not own:${stray}. This producer ships the DEVICE side of pkgs/rauc-sign and nothing else; rauc-sign is the release-host signing tool. See pkgs/rauc-sign/README.md" >&2
    exit 1
fi
