...
# MOS_DEB_STAGE, for build-env/deb/build.sh to hand to the packing
# Dockerfile as `bin`.
#
# THE COMPILE LIVES IN pkgs/rauc/, not here. pkgs/rauc/Dockerfile builds
# RAUC with -Dnetwork=false -Dstreaming=false and pkgs/rauc/versions.env is
# the file that says why; pkgs/rauc/build.sh drives it for one architecture
...
RAUC_DIR="${MOS_DEB_REPO_ROOT}/pkgs/rauc"

# pkgs/rauc/build.sh spells an architecture as a BOARD -- it reads MOS_ARCH
# out of boards/<board>/board.env -- while the RAUC build itself is
...
[ -n "${BOARD}" ] || {
    echo "error: no board under boards/ declares MOS_ARCH=${MOS_DEB_ARCH}, and pkgs/rauc/build.sh takes its architecture from a board.env. Either that architecture has no board in this tree or MOS_ARCH moved out of board.env" >&2
    exit 1
}

echo "prepare.sh: building rauc for ${MOS_DEB_ARCH} through pkgs/rauc/build.sh (MOS_BOARD=${BOARD})"
...
for f in rauc rauc-service.sh rauc.service de.pengutronix.rauc.conf de.pengutronix.rauc.service; do
    ...
    [ -f "${src}" ] || {
        echo "error: pkgs/rauc/build.sh reported success and left no ${f} in pkgs/rauc/out-${MOS_DEB_ARCH}/. That file is on the activation path -- mos-rauc ships all five -- and a package missing one holds a rauc that answers 'the name de.pengutronix.rauc was not provided by any .service files'" >&2
        exit 1
    }
