...
| `etc/systemd/system/mos-regdb-reload.service` | loads `/lib/firmware/regulatory.db` into cfg80211 with `iw reg reload`, enabled at `multi-user.target`. It exists because a built-in cfg80211 on a board with no initramfs asks for the database before the root is mounted and records that failure in a pointer no later request consults; nl80211's `RELOAD_REGDB` is the only path that clears it |
