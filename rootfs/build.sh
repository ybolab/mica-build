...
if [ -n "${manifest_unknown// /}" ]; then
    ...
    echo "meta.example/updates/manifest.json is the committed shape and pkgs/mosd/mosd/src/baked_meta.rs is the reader; a key in neither would be baked into a read-only root and refused there, where nobody can edit it. Add it to both, or fix the spelling." >&2
    exit 1
fi
