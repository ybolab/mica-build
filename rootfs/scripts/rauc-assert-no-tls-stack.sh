...
#
# Called from pkgs/rauc/deb/rauc/Dockerfile, which packages the binary this
# asserts about, where the reasoning lives.
...
if ldd /usr/bin/rauc | grep -ciE 'curl|gnutls' >/dev/null; then
    ...
    echo "pkgs/rauc/ builds it with -Dnetwork=false -Dstreaming=false precisely so it does not" >&2; exit 1
fi
