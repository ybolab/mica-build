# `tests/apid-api` — the apid black-box HTTP suite
...
Nothing here modifies `micad:`. Any apid defect this suite finds is **reported,
...
`micad:apid/openapi.json` ALSO states agrees with the document. It covers
...
```sh
docker run --rm -v "$(git rev-parse --show-toplevel):/w" -w /w/tests/apid-api \
