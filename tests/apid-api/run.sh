...
if [ ! -f "${SCRIPT_DIR}/src/main.ts" ]; then
    ...
    # the suite.
    note "tests/apid-api/src/main.ts does not exist yet; the suite step will FAIL loudly rather than be skipped quietly"
fi
...
if [ "${DRY_RUN}" -eq 1 ]; then
    ...
    note "would run      docker run --network ${NET} -v ${REPO_ROOT}:/w -v ${OUT_REAL}:/w/_out -w /w/tests/apid-api ${BUN_IMAGE} bun run src/main.ts"
...
run_suite() {
    ...
    if [ ! -f "${SCRIPT_DIR}/src/main.ts" ]; then
        fail "[${label}] the suite entry point tests/apid-api/src/main.ts does not exist, so nothing was asserted about apid"
        SUITE_RC=1
        return 0
    fi
    ...
    set +e
    docker run --rm --label ai-agent=true --network "${NET}" \
        -v "${REPO_ROOT}:/w" -v "${OUT_REAL}:/w/_out" -w /w/tests/apid-api \
        -e APID_HOST="${ip}" \
