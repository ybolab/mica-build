...
# build-env/deb/producers.sh and what each emits out of its own producer.env
...
# localhost/mos-build-deb -- the same arrangement build-env/deb/repo.sh uses,
...
# The two architectures build-env/images.env pins a mos-build-deb for, and
...
*)
    echo "error: $(uname -m) is not an architecture build-env/images.env builds a mos-build-deb for, so there is no container to read the archives in" >&2
    exit 1
    ;;
esac
...
# rather than mounting the repository, for the reason build-env/deb/repo.sh
...
for arch in "${ARCHES[@]}"; do
    ...
    # build-env/deb/repo.sh refuses one: every check below is of the form
    ...
    for d in "${debs[@]}"; do
        ...
        # under pkgs/mosd/ rather than to the pool, because that is the scope
...
# buildx, and a second run of build-env/deb/build.sh normally replays the
