...
mkdir -p _out/rust-gate/mica-deploy _out/cargo/registry _out/cargo/git
...
docker run --rm --label ai-agent=true --network traefik \
    ...
    -v "$PWD/_out/rust-gate/mica-deploy:/target" \
    -v "$PWD/_out/cargo/registry:/usr/local/cargo/registry" \
    -v "$PWD/_out/cargo/git:/usr/local/cargo/git" \
    -w /src/pkgs/mica-deploy -e CARGO_TARGET_DIR=/target --entrypoint /bin/bash "$image" -ceu '
    command -v cc
    cc -std=c11 -Wall -Wextra -Werror -shared -fPIC /src/tests/file-ab-faults/io-fault.c -ldl -o /evidence/io-fault.so
