...
for (const board of ['x64', 'virt-arm64']) {
  for (const port of [undefined, '22345', '0', '65536', 'invalid']) {
    test(`API launcher ${board}: SSH port ${port ?? 'unset'} crosses the Docker boundary unchanged`, async () => {
      ...
      try {
        for (const path of ['pkgs/mosd/tests/apid-api/run.sh', 'pkgs/mosd/tests/apid-api/src/qemu.ts',
          'pkgs/mosd/tests/apid-api/src/main.ts', `boards/${board}/board.env`, 'build-env/from.sh', 'build-env/images.env']) {
        ...
        const child = Bun.spawnSync(['bash', join(work, 'pkgs/mosd/tests/apid-api/run.sh')], { env, timeout: 20000 })
