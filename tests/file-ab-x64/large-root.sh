...
for size in small large; do
    ...
    for iteration in 1 2 3; do
        ...
        timeout -k 10 330 docker run --rm --label ai-agent=true --network traefik \
            -v "$evidence:/w" -v "$PWD/tests/file-ab-x64:/harness:ro" \
        ...
        bash tests/file-ab-x64/shutdown-check.sh "$log"
        python3 tests/file-ab-x64/metrics.py "$log" > "$work/$size-$iteration.json"
    done
done
