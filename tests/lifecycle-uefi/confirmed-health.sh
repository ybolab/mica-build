...
bash /harness/boot.sh "$output/disk.img" writable 300 "$arch" > "$output/refused.log" 2>&1
...
bash /harness/boot.sh "$output/disk.img" writable 300 "$arch" > "$output/fallback.log" 2>&1
