...
# and its siblings take as seven positional arguments, derived from
...
#   bash tests/lifecycle-uefi/runtime-build.sh "$ROOT_IMAGE" "$KERNEL_DIR" "$CERT" "$KEY" "$INIT" "$BOARD" "$SHUTDOWN"
#
...
for f in "${OUT}/root/rootfs.img" "${OUT}/lifecycle/mica-init" "${OUT}/lifecycle/mica-shutdown" "${BOARD_DIR}/kernel/kernel.release" "${SIGNING}/verity/signer.cert.pem" "${SIGNING}/verity/signer.key.pem"; do
...
printf 'ROOT_IMAGE=%q\nKERNEL_DIR=%q\nCERT=%q\nKEY=%q\nINIT=%q\nBOARD=%q\nSHUTDOWN=%q\n' \
    ...
    "$PWD/${OUT}/lifecycle/mica-init" "${BOARD}" "$PWD/${OUT}/lifecycle/mica-shutdown"
