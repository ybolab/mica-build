#!/usr/bin/env bash
# Drive pkgs/podman/check-pins.sh against recorded upstream responses.
#
...
CHECK="${REPO_ROOT}/pkgs/podman/check-pins.sh"
FIX="${HERE}/podman-pins"
REAL_ENV="${REPO_ROOT}/pkgs/podman/versions.env"
...
expect_rc 0 "the six pins in pkgs/podman/versions.env are current"
...
expect_says "pkgs/podman/versions.env set the component's \*_VERSION" "the failure text says to edit versions.env"
