...
# rootfs, which is everything that lands in an image: an overlay drop-in or
...
discard_hits="$(grep -rn -- '--discard=no' "${REPO_ROOT}/rootfs" 2>/dev/null |
    grep -v ':[[:space:]]*#' || true)"
if [ -z "${discard_hits}" ]; then
    pass "no --discard=no override is staged into any image (rootfs); first-boot TRIM is enabled and the loader is protected structurally"
else
...
# The repart container's base, from build-env/images.env, resolved once for
...
REPART_BASE="$(bash "${REPO_ROOT}/build-env/from.sh" --ref IMAGE_DEBIAN_BOOKWORM)"
