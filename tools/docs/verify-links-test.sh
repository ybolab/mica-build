#!/usr/bin/env bash
# Negative tests for tools/docs/verify-links.sh: each assertion is driven against a
...
#
#   bash tools/docs/verify-links-test.sh          (or: make docs-verify-test)
#
# HOW. The REAL tools/docs/verify-links.sh is run, once per case, over a fixture
# tree. Nothing is reimplemented here. The verifier resolves its own root
# from ${BASH_SOURCE}, so a copy at ${case}/tools/docs/verify-links.sh reads
...
run_verifier() {
    RC=0
    bash "$1/tools/docs/verify-links.sh" >"${WORK}/out" 2>&1 || RC=$?
}
...
expect_all_pass() {
    ...
    got_checks="$(sed -n 's|^tools/docs/verify-links.sh: \([0-9]*\)/\1 PASS$|\1|p' "${WORK}/out")"
