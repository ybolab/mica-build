...
[ -f "${REPO_ROOT}/deps/boards/${BOARD}.json" ] || [ -f "${REPO_ROOT}/deps/packages/mica-kernel-${BOARD}.json" ] || die "product ${NAME}: BOARD=${BOARD} is not a pinned board; the pinned boards are: $(bash "${HERE}/board-pool.sh" --list | tr '\n' ' ')"
