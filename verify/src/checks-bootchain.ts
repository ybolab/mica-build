...
import { boardsWhere, hasDramWindow, hasLed, hasRawBlob, isUBoot, SHIPPED } from './board-scope.ts'
      ...
      run: async (ctx: ImageContext): Promise<readonly CheckResult[]> => {
        ...
        if (size === 0) {
          return [verdict(matchesId, false,
            ...
            + `'make -C boards/${ctx.board.name}/bsp ${variant}')`)]
        }
 ...
 * to derive it from: `boards/cx3576/board.env` declares
 ...
 * `boards/cx3576/bsp/kernel/build.sh`, and in the .dts the kernel build
...
/** The register's entries, built at module load from every board boards/ ships. */
