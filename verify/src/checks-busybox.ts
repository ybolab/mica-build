...
const BUSYBOX_PACKAGE = 'mos-busybox'
...
export const BUSYBOX_CHECKS: readonly CheckCase[] = [
    ...
    run: async (ctx): Promise<readonly CheckResult[]> => {
      ...
      if (st === undefined || !st.isFile()) {
        return [verdict('packed-busybox-present', false,
          `the emergency BusyBox binary is not at ${BUSYBOX_PATH} as a regular file. mos-busybox `
...

  {
    ...
    // mos's own /usr/lib/mos scripts, because nothing in that set legitimately
...

  {
    ...
    // /usr/share/mos/manifest.tsv and nothing else, so nothing about them needs
    ...
    // check is green, and it is exactly the shape `mos-ca-trust`'s keyring
