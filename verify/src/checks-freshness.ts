...
// `_out/cx3576/rootfs-verity.img` and `pkgs/podman/out-arm64/podman`, so an x64
...
/**
 * `pkgs` -- where the engine build leaves the binary this check dates.
 *
 ...
 * `pkgs/` would mutate the checkout the suite is run from. The same seam
 ...
 *
 * Both derived, neither written down. A third board dropped into `boards/`
...
export function freshnessInputs(board: Board, outDir: string): readonly FreshnessInput[] {
  ...
  if (arch === '') {
    ...
    throw new ToolOutputError(
      `${board.path} declares no MOS_ARCH, so this check cannot work out which pkgs/podman/out-* `
