...
describe('s905x5m package inventory', () => {
  ...
  test('absolute directory links resolve configuration and inventory inside the packed root', async () => {
    const fx = fixture(true)
    renameSync(join(fx.root, 'etc'), join(fx.root, 'board-config'))
    symlinkSync('/board-config', join(fx.root, 'etc'))
    renameSync(join(fx.root, 'usr/share/mos'), join(fx.root, 'board-inventory'))
    symlinkSync('/board-inventory', join(fx.root, 'usr/share/mos'))
    expect((await result(fx.ctx)).verdict).toBe('pass')
    writeFileSync(join(fx.root, 'board-config/mos/wireless.conf'), 'MODULES="unrelated"\n')
    expect((await result(fx.ctx)).verdict).toBe('fail')
  })
