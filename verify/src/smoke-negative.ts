 ...
 *
 * (5) is the clause's verb. `rootfs/build.sh` runs `run.sh --smoke` under
...
export async function negativeRun(opts: {
  ...
  log(`verify negative: ${loaded.id} is named ${base} for this run; every mutation is built FROM that`)
